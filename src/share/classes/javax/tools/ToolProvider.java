...
public class ToolProvider {
    ...
    try {
      if (System.getProperty(propertyName) != null) {
        ...
        if (st.length > 2) {
          ...
          method = String
              .format((Locale) null, "%s(%s:%s)", frame.getMethodName(),
      ...
      System.err.format((Locale) null, "%s: %s; %s%n",
          ToolProvider.class.getName(), reason, ex.getLocalizedMessage());
    }
    return null;
  }
  ...
  public static JavaCompiler getSystemJavaCompiler() {
    return instance()
        .getSystemTool(JavaCompiler.class, defaultJavaCompilerName);
  }
  ...
  private Class<?> findSystemToolClass(String toolClassName)
    ...
    try {
      ...
      // (tools.jar)
      ClassLoader cl = (refToolClassLoader == null ? null : refToolClassLoader
          .get());
      if (cl == null) {
