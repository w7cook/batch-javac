...
/**
 * A visitor for declarations that scans declarations contained within the given
 * declaration in source code order. For example, when visiting a class, the
 * methods, fields, constructors, and nested types of the class are also
 * visited.
 * 
 * To control the processing done on a declaration, users of this class pass in
 * their own visitors for pre and post processing. The preprocessing visitor is
 * called before the contained declarations are scanned; the postprocessing
 * visitor is called after the contained declarations are scanned.
 * 
...
class SourceOrderDeclScanner extends DeclarationScanner {
  static class SourceOrderComparator implements
      java.util.Comparator<Declaration> {
    ...
    private static class DeclPartialOrder extends
    ...
    private int compareEqualPosition(Declaration d1, Declaration d2) {
      assert (d1.getPosition() == d2.getPosition()) || // Handles two null
                                                       // positions.
          (d1.getPosition().file().compareTo(d2.getPosition().file()) == 0
              && d1.getPosition().line() == d2.getPosition().line() && d1
              .getPosition().column() == d2.getPosition().column());
  ...
  public void visitTypeDeclaration(TypeDeclaration d) {
    ...
    SortedSet<Declaration> decls = new TreeSet<Declaration>(
        SourceOrderDeclScanner.comparator);
