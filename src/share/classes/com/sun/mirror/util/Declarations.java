...
public interface Declarations {
   ...
   * @param sup
   *          the second member
   * @return <tt>true</tt> if and only if the first member hides the second
   */
  boolean hides(MemberDeclaration sub, MemberDeclaration sup);

  /**
   * Tests whether one method overrides another. When a non-abstract method
   * overrides an abstract one, the former is also said to <i>implement</i> the
   * latter.
   * 
   * @param sub
   *          the first method
   * @param sup
   *          the second method
   * @return <tt>true</tt> if and only if the first method overrides the second
   */
