...
public class LocalVariableTableWriter extends InstructionDetailWriter {
  public enum NoteKind {
    ...
    public abstract boolean match(LocalVariableTable_attribute.Entry entry,
        int pc);
  ...
  static LocalVariableTableWriter instance(Context context) {
    LocalVariableTableWriter instance = context
        .get(LocalVariableTableWriter.class);
    if (instance == null)
  ...
  public void reset(Code_attribute attr) {
    ...
    LocalVariableTable_attribute lvt = (LocalVariableTable_attribute) (attr.attributes
        .get(Attribute.LocalVariableTable));
    if (lvt == null)
      return;
  ...
  public void writeLocalVariables(int pc, NoteKind kind) {
    ...
    if (entries != null) {
      for (ListIterator<LocalVariableTable_attribute.Entry> iter = entries
          .listIterator(kind == NoteKind.END ? entries.size() : 0); kind == NoteKind.END ? iter
          .hasPrevious() : iter.hasNext();) {
        LocalVariableTable_attribute.Entry entry = kind == NoteKind.END ? iter
            .previous() : iter.next();
