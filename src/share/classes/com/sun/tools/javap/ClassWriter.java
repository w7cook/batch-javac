...
public class ClassWriter extends BasicWriter {
  ...
  public void write(ClassFile cf) {
    ...
    if (sigAttr == null) {
      ...
      try {
        ...
        // FieldType and a ClassSignatureType that only contains a superclass
        // type.
  ...
  class JavaTypePrinter implements Type.Visitor<StringBuilder, StringBuilder> {
    ...
    private void append(StringBuilder sb, String prefix,
    ...
    private void appendIfNotEmpty(StringBuilder sb, String prefix,
        List<? extends Type> list, String suffix) {
      if (!isEmpty(list))
  ...
  protected void writeField(Field f) {
    ...
    if (options.showConstants && !options.compat) { // BUG 4111861 print static
  ...
  protected void writeMethod(Method m) {
    ...
    if (options.showLineAndLocalVariableTables) {
      if (code != null) {
        attrWriter.write(code, code.attributes.get(Attribute.LineNumberTable),
            constant_pool);
    ...
    setPendingNewline(options.showDisassembled || options.showAllAttrs
        || options.showInternalSignatures
        || options.showLineAndLocalVariableTables || options.verbose);
  }
  ...
  String getJavaParameterTypes(Descriptor d, AccessFlags flags) {
    try {
      return getJavaName(adjustVarargs(flags,
          d.getParameterTypes(constant_pool)));
    } catch (ConstantPoolException e) {
      return report(e);
    } catch (DescriptorException e) {
      return report(e);
   ...
   * Character and string values are processed to escape characters outside the
   * basic printable ASCII set.
   * 
   * @param d
