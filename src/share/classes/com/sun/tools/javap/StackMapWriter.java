...
public class StackMapWriter extends InstructionDetailWriter {
  ...
  public void reset(Code_attribute attr) {
    setStackMap((StackMapTable_attribute) attr.attributes
        .get(Attribute.StackMapTable));
  }
  ...
  void setStackMap(StackMapTable_attribute attr) {
    ...
    verification_type_info[] initialLocals = new verification_type_info[(isStatic ? 0
        : 1)
        + args.length];
    if (!isStatic)
      initialLocals[0] = new CustomVerificationTypeInfo("this");
    for (int i = 0; i < args.length; i++) {
      initialLocals[(isStatic ? 0 : 1) + i] = new CustomVerificationTypeInfo(
          args[i].replace(".", "/"));
    }
  ...
  void print(verification_type_info entry) {
    ...
    case ITEM_Object:
      try {
        ConstantPool cp = classWriter.getClassFile().constant_pool;
        ConstantPool.CONSTANT_Class_info class_info = cp
            .getClassInfo(((Object_variable_info) entry).cpool_index);
  ...
  class StackMapBuilder implements
      StackMapTable_attribute.stack_map_frame.Visitor<Integer, Integer> {
    ...
    public Integer visit_chop_frame(chop_frame frame, Integer pc) {
      ...
      verification_type_info[] new_locals = new verification_type_info[prev.locals.length
          - k];
    ...
    public Integer visit_same_frame_extended(same_frame_extended frame,
        Integer pc) {
    ...
    public Integer visit_append_frame(append_frame frame, Integer pc) {
      ...
      System.arraycopy(frame.locals, 0, new_locals, prev.locals.length,
          frame.locals.length);
      StackMap m = new StackMap(new_locals, empty);
      map.put(new_pc, m);
      return new_pc;
