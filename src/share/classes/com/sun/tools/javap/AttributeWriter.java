 ...
 */
public class AttributeWriter extends BasicWriter implements
    Attribute.Visitor<Void, Void> {
  ...
  public Void visitCharacterRangeTable(CharacterRangeTable_attribute attr,
    ...
    for (int i = 0; i < attr.character_range_table.length; i++) {
      ...
      print(String.format("    %2d, %2d, %6x, %6x, %4x", e.start_pc, e.end_pc,
          e.character_range_start, e.character_range_end, e.flags));
      tab();
      print(String.format("// %2d, %2d, %4d:%02d, %4d:%02d", e.start_pc,
          e.end_pc, (e.character_range_start >> 10),
          (e.character_range_start & 0x3ff), (e.character_range_end >> 10),
  ...
  public Void visitInnerClasses(InnerClasses_attribute attr, Void ignore) {
    ...
    for (int i = 0; i < attr.classes.length; i++) {
      ...
      if (options.compat) {
        ...
        access_flags = access_flags.ignore(ACC_STATIC | ACC_PROTECTED
            | ACC_PRIVATE | ACC_INTERFACE | ACC_SYNTHETIC | ACC_ENUM);
  ...
  String getInnerName(ConstantPool constant_pool,
  ...
  public Void visitLocalVariableTable(LocalVariableTable_attribute attr,
    ...
    for (LocalVariableTable_attribute.Entry entry : attr.local_variable_table) {
      ...
      println(formatter.format("%8d %7d %5d %5s   %s", entry.start_pc,
          entry.length, entry.index,
  ...
  class StackMapTableWriter // also handles CLDC StackMap attributes
    ...
    public Void visit_same_frame(StackMapTable_attribute.same_frame frame,
        Void p) {
      printHeader(frame);
      println(" /* same */");
      return null;
    ...
    public Void visit_same_locals_1_stack_item_frame_extended(
        StackMapTable_attribute.same_locals_1_stack_item_frame_extended frame,
        Void p) {
      printHeader(frame);
    ...
    public Void visit_chop_frame(StackMapTable_attribute.chop_frame frame,
        Void p) {
      printHeader(frame);
      println(" /* chop */");
      indent(+1);
    ...
    public Void visit_append_frame(StackMapTable_attribute.append_frame frame,
        Void p) {
      printHeader(frame);
      println(" /* append */");
      println("     offset_delta = " + frame.offset_delta);
    ...
    public Void visit_full_frame(StackMapTable_attribute.full_frame frame,
        Void p) {
      printHeader(frame);
    ...
    void printMap(String name,
        StackMapTable_attribute.verification_type_info[] map) {
      print(name + " = [");
      for (int i = 0; i < map.length; i++) {
        ...
        case StackMapTable_attribute.verification_type_info.ITEM_Object:
          print(" ");
          constantWriter
              .write(((StackMapTable_attribute.Object_variable_info) info).cpool_index);
          break;
        case StackMapTable_attribute.verification_type_info.ITEM_Uninitialized:
          print(" " + mapTypeName(tag));
          print(" "
              + ((StackMapTable_attribute.Uninitialized_variable_info) info).offset);
          break;
        default:
          print(" " + mapTypeName(tag));
        }
  ...
  String toHex(byte b, int w) {
    if (options.compat) // BUG 6622260: javap prints negative bytes incorrectly
                        // in hex
