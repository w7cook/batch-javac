...
class CodeWriter extends BasicWriter {
  ...
  public void writeVerboseHeader(Code_attribute attr, ConstantPool constant_pool) {
    ...
    println("stack=" + attr.max_stack + ", locals=" + attr.max_locals
        + ", args_size=" + argCount);
  ...
  public void writeInstr(Instruction instr) {
    ...
    // compute the number of indentations for the body of multi-line
    // instructions
    // This is 6 (the width of "%4d: "), divided by the width of each
    // indentation level,
  ...
  Instruction.KindVisitor<Void, Integer> instructionPrinter = new Instruction.KindVisitor<Void, Integer>() {
    ...
    public Void visitConstantPoolRefAndValue(Instruction instr, int index,
        int value, Integer indent) {
      print("#" + index + ",  " + value);
      tab();
      print("// ");
      printConstant(index);
    ...
    public Void visitLookupSwitch(Instruction instr, int default_, int npairs,
        int[] matches, int[] offsets, Integer indent) {
      int pc = instr.getPC();
      print("{ // " + npairs);
      indent(indent);
    ...
    public Void visitTableSwitch(Instruction instr, int default_, int low,
        int high, int[] offsets, Integer indent) {
      int pc = instr.getPC();
      print("{ // " + low + " to " + high);
      indent(indent);
  ...
  public void writeExceptionTable(Code_attribute attr) {
    if (attr.exception_table_langth > 0) {
      ...
      for (int i = 0; i < attr.exception_table.length; i++) {
        ...
        print(String.format(" %5d %5d %5d", handler.start_pc, handler.end_pc,
            handler.handler_pc));
        print("   ");
  ...
  private List<InstructionDetailWriter> getDetailWriters(Code_attribute attr) {
    List<InstructionDetailWriter> detailWriters = new ArrayList<InstructionDetailWriter>();
