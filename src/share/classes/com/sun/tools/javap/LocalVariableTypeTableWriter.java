...
public class LocalVariableTypeTableWriter extends InstructionDetailWriter {
  public enum NoteKind {
    ...
    public abstract boolean match(LocalVariableTypeTable_attribute.Entry entry,
        int pc);
  ...
  static LocalVariableTypeTableWriter instance(Context context) {
    LocalVariableTypeTableWriter instance = context
        .get(LocalVariableTypeTableWriter.class);
    if (instance == null)
  ...
  public void reset(Code_attribute attr) {
    ...
    LocalVariableTypeTable_attribute lvt = (LocalVariableTypeTable_attribute) (attr.attributes
        .get(Attribute.LocalVariableTypeTable));
    if (lvt == null)
      return;
  ...
  public void writeLocalVariables(int pc, NoteKind kind) {
    ...
    if (entries != null) {
      for (ListIterator<LocalVariableTypeTable_attribute.Entry> iter = entries
          .listIterator(kind == NoteKind.END ? entries.size() : 0); kind == NoteKind.END ? iter
          .hasPrevious() : iter.hasNext();) {
        LocalVariableTypeTable_attribute.Entry entry = kind == NoteKind.END ? iter
            .previous() : iter.next();
