...
/**
 * "Main" class for javap, normally accessed from the command line via Main, or
 * from JSR199 via DisassemblerTool.
 * 
 * <p>
...
public class JavapTask implements DisassemblerTool.DisassemblerTask, Messages {
  ...
  static abstract class Option {
    ...
    abstract void process(JavapTask task, String opt, String arg)
        throws BadArgs;
  ...
  new Option(false, "-s") {
    void process(JavapTask task, String opt, String arg) {
      ...
      new Option(false, "-XDdetails") {
        void process(JavapTask task, String opt, String arg) {
          task.options.details = EnumSet
              .allOf(InstructionDetailWriter.Kind.class);
        }
      ...
      new Option(false, "-XDdetails:") {
        ...
        boolean handleArg(JavapTask task, String arg) {
          ...
          for (InstructionDetailWriter.Kind k : InstructionDetailWriter.Kind
              .values()) {
  ...
  public JavapTask(Writer out, JavaFileManager fileManager,
  ...
  protected boolean writeClass(ClassWriter classWriter, String className)
    ...
    if (!className.endsWith(".class")) {
      ...
      if (!cfName.replaceAll("[/$]", ".").equals(
          className.replaceAll("[/$]", ".")))
        reportWarning("warn.unexpected.class", className,
            cfName.replace('/', '.'));
    }
    write(cfInfo);
      ...
      if (a instanceof InnerClasses_attribute) {
        ...
        try {
          ...
          for (int i = 0; i < inners.classes.length; i++) {
            ...
            ConstantPool.CONSTANT_Class_info outerClassInfo = cf.constant_pool
                .getClassInfo(outerIndex);
            ...
            if (outerClassName.equals(cf.getName())) {
              ...
              ConstantPool.CONSTANT_Class_info innerClassInfo = cf.constant_pool
                  .getClassInfo(innerIndex);
              String innerClassName = innerClassInfo.getName();
              classWriter.println("// inner class "
                  + innerClassName.replaceAll("[/$]", "."));
              classWriter.println();
  ...
  protected JavaFileObject open(String className) throws IOException {
    ...
    // see if it is an inner class, by replacing dots to $, starting from the
    // right
    String cn = className;
    int lastDot;
  ...
  private JavaFileObject getClassFileObject(String className)
      throws IOException {
    JavaFileObject fo;
    fo = fileManager.getJavaFileForInput(StandardLocation.PLATFORM_CLASS_PATH,
        className, JavaFileObject.Kind.CLASS);
    if (fo == null)
      fo = fileManager.getJavaFileForInput(StandardLocation.CLASS_PATH,
          className, JavaFileObject.Kind.CLASS);
    return fo;
  }
  ...
  private void showHelp() {
    ...
    for (Option o : recognizedOptions) {
      ...
      if (name.startsWith("X") || name.equals("fullversion")
          || name.equals("h") || name.equals("verify"))
        continue;
  ...
  private void reportError(String key, Object... args) {
    diagnosticListener
        .report(createDiagnostic(Diagnostic.Kind.ERROR, key, args));
  }

  private void reportNote(String key, Object... args) {
    diagnosticListener
        .report(createDiagnostic(Diagnostic.Kind.NOTE, key, args));
  }

  private void reportWarning(String key, Object... args) {
    diagnosticListener.report(createDiagnostic(Diagnostic.Kind.WARNING, key,
        args));
  }
