 ...
 * Subclasses are for specific native interfaces. At the time of its original
 * writing, this interface is rich enough to support JNI and the old 1.0-style
...
public abstract class Gen {
  ...
  /**
   * Override this abstract method, generating content for the named class into
   * the outputstream.
   */
  protected abstract void write(OutputStream o, TypeElement clazz)
      throws Util.Exit;

  /**
   * Override this method to provide a list of #include statements required by
  ...
  /**
   * We explicitly need to write ASCII files because that is what C compilers
   * understand.
   */
   ...
   * processing.
   * 
   * Buffer size chosen as an approximation from a single sampling of: expr `du
   * -sk` / `ls *.h | wc -l`
   */
  ...
  /*
   * Write the contents of byte[] b to a file named file. Writing is done if
   * either the file doesn't exist or if the contents are different.
   */
  ...
  protected String cppGuardBegin() {
    return "#ifdef __cplusplus" + lineSep + "extern \"C\" {" + lineSep
        + "#endif";
  }
  ...
  protected String guardBegin(String cname) {
    return "/* Header for class " + cname + " */" + lineSep + lineSep
        + "#ifndef _Included_" + cname + lineSep + "#define _Included_" + cname;
  }
  ...
  protected FileObject getFileObject(CharSequence className) throws IOException {
    ...
    return fileManager.getFileForOutput(StandardLocation.SOURCE_OUTPUT, "",
        name, null);
  }
