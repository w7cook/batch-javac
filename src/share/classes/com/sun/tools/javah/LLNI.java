...
public class LLNI extends Gen {
  ...
  protected void write(OutputStream o, TypeElement clazz) throws Util.Exit {
    try {
      ...
      // FIXME check if errors occurred on the PrintWriter and throw exception
      // if so
  ...
  protected void generateDeclsForClass(PrintWriter pw, TypeElement clazz,
      String cname) throws TypeSignature.SignatureException, Util.Exit {
    ...
    pw.println("/* LLNI Header for class " + clazz.getQualifiedName() + " */"
        + lineSep);
  ...
  protected String mangleClassName(String s) {
    return s.replace('.', '_').replace('/', '_').replace(innerDelim, '_');
  }
  ...
  protected void forwardDecls(PrintWriter pw, TypeElement clazz)
    ...
    TypeElement superClass = (TypeElement) (types.asElement(clazz
        .getSuperclass()));
  ...
  protected void structSectionForClass(PrintWriter pw, TypeElement jclazz,
      String cname) {
    ...
    if (fieldDef != null) {
      if (!res.printedOne) { /* add separator */
        if (res.bottomMost) {
          ...
          res.s = res.s + "    /* inherited members from " + res.className
              + ": */" + lineSep;
        }
        res.printedOne = true;
      }
  ...
  private int doTwoWordFields(FieldDefsRes res, TypeElement clazz, int offset,
      String cname, boolean padWord) {
    boolean first = true;
    List<VariableElement> fields = ElementFilter.fieldsIn(clazz
        .getEnclosedElements());
  ...
  FieldDefsRes fieldDefs(TypeElement clazz, String cname, boolean bottomMost) {
    ...
    TypeElement superclazz = (TypeElement) types.asElement(clazz
        .getSuperclass());

    if (superclazz != null) {
      String supername = superclazz.getQualifiedName().toString();
      res = new FieldDefsRes(clazz, fieldDefs(superclazz, cname, false),
          bottomMost);
  ...
  protected String addStructMember(VariableElement member, String cname,
    ...
    if (member.getModifiers().contains(Modifier.STATIC)) {
      ...
      if (padWord)
        res = "    java_int padWord" + padFieldNum++ + ";" + lineSep;
      res = "    " + llniType(mt, false, false) + " " + llniFieldName(member);
      if (isLongOrDouble(mt))
        res = res + "[2]";
      res = res + ";" + lineSep;
    }
    return res;
  ...
  static private final boolean isWindows = System.getProperty("os.name")
      .startsWith("Windows");
    ...
    if (exp != null) {
      ...
      if (suffix != null) {
        ...
        if ((suffix.equals("L") && (val == Integer.MIN_VALUE))
          ...
          res = "    #undef  " + cn + lineSep + "    #define " + cn + " ("
              + (val + 1) + suffix + "-1)" + lineSep;
        } else if (suffix.equals("L") || suffix.endsWith("LL")) {
          res = "    #undef  " + cn + lineSep + "    #define " + cn + " " + val
              + suffix + lineSep;
        } else {
          res = "    #undef  " + cn + lineSep + "    #define " + cn + " " + exp
              + suffix + lineSep;
        }
      }
    }
  ...
  protected void methodSectionForClass(PrintWriter pw, TypeElement clazz,
      String cname) throws TypeSignature.SignatureException, Util.Exit {
  ...
  protected String methodDecl(ExecutableElement method, TypeElement clazz,
      String cname) throws TypeSignature.SignatureException, Util.Exit {
    String res = null;
    ...
    res = "JNIEXPORT " + jniType(retType) + " JNICALL" + lineSep
        + jniMethodName(method, cname, longName) + "(JNIEnv *, "
    ...
    /*
     * It would have been nice to include the argument names in the declaration,
     * but there seems to be a bug in the "BinaryField" class, causing the
     * getArguments() method to return "null" for most (non-constructor)
     * methods.
     */
  ...
  protected final String jniMethodName(ExecutableElement method, String cname,
      boolean longName) throws TypeSignature.SignatureException {
  ...
  protected String llniType(TypeMirror t, boolean handleize,
  ...
  protected final String cRcvrDecl(Element field, String cname) {
    return (field.getModifiers().contains(Modifier.STATIC) ? "jclass"
        : "jobject");
  }
  ...
  /*
   * Do unicode to ansi C identifier conversion. %%% This may not be right, but
   * should be called more often.
   */
