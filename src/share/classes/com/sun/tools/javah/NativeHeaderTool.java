 ...
 * 
 *        <p>
 *        <b>This is NOT part of any supported API. If you write code that
 *        depends on this, you do so at your own risk. This code and its
...
public interface NativeHeaderTool extends Tool, OptionChecker {
  ...
  /**
   * Creates a future for a native header task with the given components and
   * arguments. The task might not have completed as described in the
   * NativeHeaderTask interface.
   * 
   * <p>
   ...
   * 
   * @param out
   *          a Writer for additional output from the task; use
   *          {@code System.err} if {@code null}
   * @param fileManager
   *          a file manager; if {@code null} use the task's standard
   *          filemanager
   * @param diagnosticListener
   *          a diagnostic listener; if {@code null} use the compiler's default
   *          method for reporting diagnostics
   * @param options
   *          task options, {@code null} means no options
   * @param classes
   *          class names for which native headers should be generated
   * @return an object representing the task to be done
   * @throws RuntimeException
   *           if an unrecoverable error occurred in a user supplied component.
   *           The {@linkplain Throwable#getCause() cause} will be the error in
   *           user code.
   * @throws IllegalArgumentException
   *           if any of the given compilation units are of other kind than
   *           {@linkplain JavaFileObject.Kind#SOURCE source}
   */
  NativeHeaderTask getTask(Writer out, JavaFileManager fileManager,
   ...
   * any non-fatal diagnostics. Fatal errors will be signalled with the
   * appropriate exceptions.
   * 
   * <p>
   * The standard file manager will be automatically reopened if it is accessed
   * after calls to {@code flush} or {@code close}. The standard file manager
   * must be usable with other tools.
   * 
   * @param diagnosticListener
   *          a diagnostic listener for non-fatal diagnostics; if {@code null}
   ...
   * @param charset
   *          the character set used for decoding bytes; if {@code null} use the
   *          platform default
  ...
  /**
   * Interface representing a future for a native header task. The task has not
   * yet started. To start the task, call the {@linkplain #call call} method.
   * 
   * <p>
  ...
  interface NativeHeaderTask extends Callable<Boolean> {
    ...
    /**
     * Performs this native header task. The task may only be performed once.
     ...
     *           component. The {@linkplain Throwable#getCause() cause} will be
     *           the error in user code.
     * @throws IllegalStateException
