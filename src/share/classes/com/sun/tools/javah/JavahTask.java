...
public class JavahTask implements NativeHeaderTool.NativeHeaderTask {
  ...
  static abstract class Option {
    ...
    abstract void process(JavahTask task, String opt, String arg)
        throws BadArgs;
  ...
  JavahTask(Writer out, JavaFileManager fileManager,
  ...
  private DiagnosticListener<JavaFileObject> getDiagnosticListenerForStream(
      OutputStream s) {
  ...
  int run(String[] args) {
    try {
      ...
      diagnosticListener.report(createDiagnostic("err.internal.error",
          e.getMessage()));
      return 1;
    } catch (Util.Exit e) {
      return e.exitValue;
  ...
  private void handleOptions(Iterable<String> args, boolean allowClasses)
      throws BadArgs {
    if (log == null) {
  ...
  public boolean run() throws Util.Exit {
    ...
    if (ofile != null) {
      if (!(fileManager instanceof StandardJavaFileManager)) {
        diagnosticListener.report(createDiagnostic(
            "err.cant.use.option.for.fm", "-o"));
        return false;
      }
      Iterable<? extends JavaFileObject> iter = ((StandardJavaFileManager) fileManager)
      ...
      if (odir != null) {
        ...
        try {
          ((StandardJavaFileManager) fileManager).setLocation(
              StandardLocation.CLASS_OUTPUT, Collections.singleton(odir));
        } catch (IOException e) {
    ...
    CompilationTask t = c.getTask(log, fileManager, diagnosticListener, opts,
        internalize(classes), null);
  ...
  static StandardJavaFileManager getDefaultFileManager(
  ...
  private void showHelp() {
    ...
    for (Option o : recognizedOptions) {
      if (o.isHidden())
        continue;
      String name = o.aliases[0].substring(1); // there must always be at least
                                               // one name
  ...
  private Diagnostic<JavaFileObject> createDiagnostic(final String key,
      final Object... args) {
  ...
  private String getMessage(Locale locale, String key, Object... args) {
    ...
    if (b == null) {
      try {
        b = ResourceBundle.getBundle("com.sun.tools.javah.resources.l10n",
            locale);
        bundles.put(locale, b);
      } catch (MissingResourceException e) {
        throw new InternalError("Cannot find javah resource bundle for locale "
            + locale, e);
      }
    }
  ...
  class JavahProcessor extends AbstractProcessor {
    ...
    public boolean process(Set<? extends TypeElement> annotations,
      ...
      try {
        Set<TypeElement> classes = getAllClasses(ElementFilter.typesIn(roundEnv
            .getRootElements()));
        if (classes.size() > 0) {
        ...
        messager
            .printMessage(
                ERROR,
                getMessage("class.not.found", cf.sym.getQualifiedName()
                    .toString()));
      } catch (ClassNotFoundException cnfe) {
        messager.printMessage(ERROR,
            getMessage("class.not.found", cnfe.getMessage()));
      } catch (IOException ioe) {
        messager.printMessage(ERROR,
            getMessage("io.exception", ioe.getMessage()));
      } catch (Util.Exit e) {
        exit = e;
      }
    ...
    private void getAllClasses0(Iterable<? extends TypeElement> classes,
        Set<TypeElement> allClasses) {
      for (TypeElement c : classes) {
        allClasses.add(c);
        getAllClasses0(ElementFilter.typesIn(c.getEnclosedElements()),
            allClasses);
      }
    }
    ...
    private void checkMethodParameters(Set<TypeElement> classes) {
      ...
      for (TypeElement te : classes) {
        for (ExecutableElement ee : ElementFilter.methodsIn(te
            .getEnclosedElements())) {
