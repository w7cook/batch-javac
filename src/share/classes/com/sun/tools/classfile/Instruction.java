...
public class Instruction {
  /**
   * The kind of an instruction, as determined by the position, size and types
   * of its operands.
   */
  public static enum Kind {
    ...
    CPREF_W(3),
    /**
     * Opcode is followed by a 2-byte index into the constant pool, an unsigned
     * byte value.
     */
    CPREF_W_UBYTE(4),
    /**
     * Opcode is followed by a 2-byte index into the constant pool., an unsigned
     * byte value, and a zero byte.
     */
    CPREF_W_UBYTE_ZERO(5),
    /**
     * Opcode is followed by variable number of operands, depending on the
     * instruction.
     */
    DYNAMIC(-1),
    ...
    LOCAL(2),
    /**
     * Opcode is followed by a 1-byte reference to a local variable, and a
     * signed byte value.
     */
    LOCAL_BYTE(3),
    ...
    WIDE_CPREF_W(4),
    /**
     * Wide opcode is followed by a 2-byte index into the constant pool, and a
     * signed short value.
     */
    ...
    /**
     * The length, in bytes, of this kind of instruction, or -1 is the length
     * depends on the specific instruction.
     */
  ...
  public interface KindVisitor<R, P> {
    ...
    R visitLookupSwitch(Instruction instr, int default_, int npairs,
        int[] matches, int[] offsets, P p);

    /** See {@link Kind#DYNAMIC}. */
    R visitTableSwitch(Instruction instr, int default_, int low, int high,
        int[] offsets, P p);
  ...
  /**
   * The kind of primitive array type to create. See JVMS chapter 6, newarray.
   */
  public static enum TypeKind {
    T_BOOLEAN(4, "boolean"), T_CHAR(5, "char"), T_FLOAT(6, "float"), T_DOUBLE(
        7, "double"), T_BYTE(8, "byte"), T_SHORT(9, "short"), T_INT(10, "int"), T_LONG(
        11, "long");
  ...
  /**
   * Get the length, in bytes, of this instruction, including the opcode and all
   * its operands.
   */
  public int length() {
    Opcode opcode = getOpcode();
  ...
  /**
   * Invoke a method on the visitor according to the kind of this instruction,
   * passing in the decoded operands for the instruction.
   */
  public <R, P> R accept(KindVisitor<R, P> visitor, P p) {
    ...
    case ATYPE:
      return visitor.visitArrayType(this, TypeKind.get(getUnsignedByte(1)), p);
    ...
    case CPREF_W_UBYTE_ZERO:
      return visitor.visitConstantPoolRefAndValue(this, getUnsignedShort(1),
          getUnsignedByte(3), p);
    ...
    case DYNAMIC: {
      ...
      case TABLESWITCH: {
        ...
        return visitor.visitTableSwitch(this, default_, low, high, values, p);
      }
      case LOOKUPSWITCH: {
        int pad = align(pc + 1) - pc;
        int default_ = getInt(pad);
        ...
        return visitor.visitLookupSwitch(this, default_, npairs, matches,
            offsets, p);
      }
      default:
    ...
    case LOCAL_BYTE:
      return visitor
          .visitLocalAndValue(this, getUnsignedByte(1), getByte(2), p);
    ...
    case WIDE_CPREF_W_SHORT:
      return visitor.visitConstantPoolRefAndValue(this, getUnsignedShort(2),
          getUnsignedByte(4), p);
