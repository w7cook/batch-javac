...
/**
 * A framework for determining {@link Dependency dependencies} between class
 * files.
 * 
 ...
 * further characterize the type of dependency, such as a dependency on a method
 * within a class.
 * 
...
public class Dependencies {
  ...
  public interface ClassFileReader {
     ...
     * @throws Dependencies
     *           #ClassFileNotFoundException if the classfile cannot be found
     */
  ...
  /**
   * Get a finder used to locate the API dependencies for a class. These include
   * the superclass, superinterfaces, and classes referenced in the declarations
   * of fields and methods. The fields and methods that are checked can be
   * limited according to a specified access. The access parameter must be one
   * of {@link AccessFlags#ACC_PUBLIC ACC_PUBLIC},
   * {@link AccessFlags#ACC_PRIVATE ACC_PRIVATE},
   * {@link AccessFlags#ACC_PROTECTED ACC_PROTECTED}, or 0 for package private
   * access. Members with greater than or equal accessibility to that specified
   * will be searched for dependencies.
   * 
   * @param access
   *          the access of members to be checked
  ...
  /**
   * Get the default filter used to determine included when searching the
   * transitive closure of all the dependencies. Unless overridden, the default
   * filter accepts all dependencies.
   * 
  ...
  /**
   * Get a filter which uses a regular expression on the target's class name to
   * determine if a dependency is of interest.
   * 
   * @param pattern
   *          the pattern used to match the target's class name
   * @return a filter for matching the target class name with a regular
   *         expression
   */
  ...
  /**
   * Get a filter which checks the package of a target's class name to determine
   * if a dependency is of interest. The filter checks if the package of the
   * target's class matches any of a set of given package names. The match may
   * optionally match subpackages of the given names as well.
   * 
   * @param packageNames
   *          the package names used to match the target's class name
   * @param matchSubpackages
   *          whether or not to match subpackages as well
   * @return a filter for checking the target package name against a list of
   *         package names
   */
  public static Filter getPackageFilter(Set<String> packageNames,
   ...
   * the transitive closure of all the dependencies. Unless overridden, the
   * default filter accepts all dependencies.
   * 
   * @return the filter
   */
   ...
   * {@link Dependencies#getFinder finder} and {@link Dependencies#getFilter
   * filter}. The search may optionally include the transitive closure of all
   * the filtered dependencies, by also searching in the classes named in those
   * dependencies.
   * 
   * @param classFinder
   *          a finder to locate class files
   * @param rootClassNames
   *          the names of the root classes from which to begin searching
   * @param transitiveClosure
   *          whether or not to also search those classes named in any filtered
   *          dependencies that are found.
   * @return the set of dependencies that were found
   * @throws ClassFileNotFoundException
   *           if a required class file cannot be found
   * @throws ClassFileError
   *           if an error occurs while processing a class file, such as an
   *           error in the internal class file structure.
   */
  public Set<Dependency> findAllDependencies(ClassFileReader classFinder,
      Set<String> rootClassNames, boolean transitiveClosure)
      throws ClassFileNotFoundException {
   ...
   *          dependencies that are found.
   * @param recorder
   *          a recorder for handling the results
   * @throws ClassFileNotFoundException
   *           if a required class file cannot be found
   * @throws ClassFileError
   *           if an error occurs while processing a class file, such as an
   *           error in the internal class file structure.
   */
  public void findAllDependencies(ClassFileReader classFinder,
      Set<String> rootClassNames, boolean transitiveClosure, Recorder recorder)
      throws ClassFileNotFoundException {
  ...
  /**
   * This class identifies class names directly or indirectly in the constant
   * pool.
   */
  ...
  /**
   * This class identifies class names in the signatures of classes, fields, and
   * methods in a class.
   */
  static class APIDependencyFinder extends BasicDependencyFinder {
      ...
      try {
        ...
        for (Method m : classfile.methods) {
          if (checkAccess(m.access_flags)) {
            ...
            Exceptions_attribute e = (Exceptions_attribute) m.attributes
    ...
    boolean checkAccess(AccessFlags flags) {
      ...
      if ((showAccess == AccessFlags.ACC_PUBLIC)
          && (isProtected || isPrivate || isPackage))
        return false;
      else if ((showAccess == AccessFlags.ACC_PROTECTED)
          && (isPrivate || isPackage))
        return false;
  ...
  static abstract class BasicDependencyFinder implements Finder {
    ...
    class Visitor implements ConstantPool.Visitor<Void, Void>,
      ...
      void scan(Descriptor d, Attributes attrs) {
        try {
          ...
          Signature_attribute sa = (Signature_attribute) attrs
      ...
      public Void visitClass(CONSTANT_Class_info info, Void p) {
        try {
          if (info.getName().startsWith("["))
            new Signature(info.name_index).getType(constant_pool).accept(this,
                null);
          else
      ...
      public Void visitInterfaceMethodref(
          CONSTANT_InterfaceMethodref_info info, Void p) {
        return visitRef(info, p);
      }
      ...
      public Void visitNameAndType(CONSTANT_NameAndType_info info, Void p) {
        try {
          new Signature(info.type_index).getType(constant_pool).accept(this,
              null);
          return null;
        } catch (ConstantPoolException e) {
          throw new ClassFileError(e);
