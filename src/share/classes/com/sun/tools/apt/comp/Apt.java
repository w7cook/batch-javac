...
public class Apt extends ListBuffer<Env<AttrContext>> {
  ...
  protected static final Context.Key<Apt> aptKey = new Context.Key<Apt>();
  ...
  /**
   * Used to scan javac trees to build data structures needed for bootstrapping
   * the apt environment. In particular:
   * 
   * <ul>
   * 
   * <li>Generate list of canonical names of annotation types that appear in
   * source files given on the command line
   * 
   * <li>Collect list of javac symbols representing source files given on the
   * command line
   * 
   * </ul>
   */
    ...
    if (symbol != null) {
      if (symbol.getAnnotationMirrors() != null)
        for (Attribute.Compound compound : symbol.getAnnotationMirrors())
          annotationSet.add(compound.type.tsym.toString()); // should fullName
                                                            // be used instead
                                                            // of toString?
  ...
  public void main(
      com.sun.tools.javac.util.List<JCTree.JCCompilationUnit> treeList,
    ...
    AnnotationProcessorEnvironmentImpl trivAPE = new AnnotationProcessorEnvironmentImpl(
        spectypedecls, typedecls, origOptions, context);
    ...
    if (options.get("-print") != null) {
      ...
      AnnotationProcessor proc = (new BootstrapAPF()).getProcessorFor(
          new HashSet<AnnotationTypeDeclaration>(), trivAPE);
      proc.process();
    } else {
      // Discovery process
      ...
      {
        /*
         * If a factory is provided by the user, the "-factory" and
         * "-factorypath" options are not used.
         * 
         * Otherwise, if the "-factory" option is used, search the appropriate
         * path for the named class. Otherwise, use sun.misc.Service to
         * implement the default discovery policy.
         */
        ...
        if (providedFactory != null) {
          ...
          try {
            AnnotationProcessorFactory factory = (AnnotationProcessorFactory) (aptCL
          ...
          Iterator<AnnotationProcessorFactory> iter = sun.misc.Service
              .providers(AnnotationProcessorFactory.class, aptCL);
          providers = iter;
        ...
        while (providers.hasNext()) {
          ...
          try {
            ...
            if (options.get("-XPrintFactoryInfo") != null) {
              out.println("Factory "
                  + apf.getClass().getName()
                  + " matches "
                  + ((matchedStrings.size() == 0) ? "nothing." : matchedStrings));
            }
      ...
      {
        java.util.Set<Class<? extends AnnotationProcessorFactory>> neglectedFactories = new LinkedHashSet<Class<? extends AnnotationProcessorFactory>>(
            productiveFactories);
      ...
      try {
        for (Map.Entry<AnnotationProcessorFactory, Set<AnnotationTypeDeclaration>> entry : factoryToAnnotation
            .entrySet()) {
          AnnotationProcessorFactory apFactory = entry.getKey();
          AnnotationProcessor processor = apFactory.getProcessorFor(
              entry.getValue(), trivAPE);
          if (processor != null)
      ...
      AnnotationProcessor proc = AnnotationProcessors
          .getCompositeAnnotationProcessor(temp);
  ...
  /**
   * Convert import-style string to regex matching that string. If the string is
   * a valid import-style string, return a regex that won't match anything.
   */
  Pattern importStringToPattern(String s) {
    if (com.sun.tools.javac.processing.JavacProcessingEnvironment
        .isValidImportString(s)) {
      return com.sun.tools.javac.processing.JavacProcessingEnvironment
          .validImportStringToPattern(s);
    } else {
      Bark bark = Bark.instance(context);
