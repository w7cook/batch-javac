...
public class Main {
  ...
  // Preserve parameter names from
  // class files if the class was
  ...
  private class Option {
    ...
    String helpSynopsis() {
      return name
          + (argsNameKey == null ? ""
              : ((hasSuffix ? "" : " ") + getLocalizedString(argsNameKey)));
    }
  ...
  private Option[] recognizedOptions = {
      ...
      new Option("-g:{lines,vars,source}", "opt.g.lines.vars.source") {
        ...
        boolean process(String option) {
          ...
          for (StringTokenizer t = new StringTokenizer(suboptions, ","); t
              .hasMoreTokens();) {
      ...
      /*
       * Option to treat both classes and source files as declarations that can
       * be given on the command line and processed as the result of an apt
       * round.
       */
      ...
      // use complex inference from context in the position of a method call
      // argument
      ...
      /*
       * This is a back door to the compiler's option table. -Dx=y sets the
       * option x to the value y. -Dx sets the option x to the value x.
       */
      new HiddenOption("-XD") {
        String s;
  ...
  /**
   * List of top level names of generated source files from most recent apt
   * round.
   */
   ...
   */
  java.util.Set<Class<? extends AnnotationProcessorFactory>> productiveFactories = new java.util.LinkedHashSet<Class<? extends AnnotationProcessorFactory>>();
  ...
  void warning(String key, Object... args) {
    Bark.printLines(out, ownName + ": " + getLocalizedString(key, args));
  }
  ...
  protected java.util.List<String> processArgs(String[] flags) {
    ...
    Source source = (sourceString != null) ? Source.lookup(sourceString)
        : Source.JDK1_5; // JDK 5 is the latest supported source version
    String targetString = options.get("-target");
    Target target = (targetString != null) ? Target.lookup(targetString)
    ...
    if (Character.isDigit(target.name.charAt(0))
      ...
      if (targetString != null) {
        if (sourceString == null) {
          warning("warn.target.default.source.conflict", targetString,
              source.requiredTarget().name);
        } else {
          warning("warn.source.target.conflict", sourceString,
              source.requiredTarget().name);
        }
        return null;
      } else {
  ...
  public int compile(String[] args, AnnotationProcessorFactory factory) {
    ...
    /*
     * Process the command line options to create the intial options data. This
     * processing is at least partially reused by any recursive apt calls.
     */
    ...
    /*
     * A run of apt only gets passed the most recently generated files; the
     * initial run of apt gets passed the files from the command line.
     */
     ...
     * 
     * If any of the prior apt rounds generated any new source files, the n'th
     * apt round (and any javac invocation) has the source destination path
     * ("-s path") as the last element of the "-sourcepath" to the n'th call.
     * 
     * If any of the prior apt rounds generated any new class files, the n'th
     * apt round (and any javac invocation) has the class destination path
     * ("-d path") as the last element of the "-classpath" to the n'th call.
     */
    ...
    try {
      /*
       * Record original options for future annotation processor invocations.
       */
      ...
      JavacFileManager fm = (JavacFileManager) context
          .get(JavaFileManager.class);
      {
        ...
        if (fm.hasLocation(StandardLocation.CLASS_PATH)) {
          ...
          augmentedClassPath = baseClassPath
              + ((classDest == null) ? "." : classDest);
        } else {
          baseClassPath = ".";
          if (classDest != null)
            augmentedClassPath = baseClassPath
                + (File.pathSeparator + classDest);
        }
      ...
      {
        /*
         * Use a url class loader to look for classes on the user-specified
         * class path. Prepend computed bootclass path, which includes extdirs,
         * to the URLClassLoader apt uses.
         */
        String aptclasspath = "";
        String bcp = "";
        Iterable<? extends File> bootclasspath = fm
        ...
        if (providedFactory == null && options.get("-factorypath") != null) // same
                                                                            // CL
                                                                            // even
                                                                            // if
                                                                            // new
          ...
          // written
          augmentedAptCL = new URLClassLoader(
              pathToURLs(augmentedClassPath.substring(baseClassPath.length())),
              aptCL);
        }
      }
      ...
      do {
        ...
        Options newOptions = Options.instance(newContext); // creates a new
                                                           // context
        newOptions.putAll(options);

        // populate with old options... don't bother reparsing command line,
        // etc.
      ...
      } while (((genSourceFileNames.size() != 0) || (classesAsDecls && genClassFileNames
          .size() != 0)) && bark.nerrors == 0);
    ...
    /*
     * Do not compile if a processor has reported an error or if there are no
     * source files to process. A more sophisticated test would also fail for
     * syntax errors caught by javac.
     */
    if (options.get("-nocompile") == null && options.get("-print") == null
        && bark.nerrors == 0
        && (origFilenames.size() > 0 || aggregateGenSourceFileNames.size() > 0)) {
      /*
       * Need to create new argument string for calling javac: 1. apt specific
       * arguments (e.g. -factory) must be stripped out 2. proper settings for
       * sourcepath and classpath must be used 3. generated class names must be
       * added 4. class file names as declarations must be removed
       */

      int newArgsLength = args.length + (needSourcePath ? 1 : 0)
          + (needClassPath ? 1 : 0) + aggregateGenSourceFileNames.size();
  ...
  int compile(String[] args, Context context) {
    ...
    if (!assertionsEnabled) {
      // Bark.printLines(out,
      // "fatal error: assertions must be enabled when running javac");
      // return EXIT_ABNORMAL;
    }
    ...
    try {
      ...
      List<Symbol.ClassSymbol> cs = comp.compile(
          List.from(nameList.toArray(new String[0])), origOptions, aptCL,
          providedFactory, productiveFactories, aggregateGenFiles);

      /*
       * If there aren't new source files, we shouldn't bother running javac if
       * there were errors.
       * 
       * If there are new files, we should try running javac in case there were
       * typing errors.
       */

      if (comp.errorCount() != 0 || options.get("-Werror") != null
          && comp.warningCount() != 0)
        return EXIT_ERROR;
    } catch (IOException ex) {
      ioMessage(ex);
  ...
  void bugMessage(Throwable ex) {
    Bark.printLines(out,
        getLocalizedString("msg.bug", AptJavaCompiler.version()));
  ...
  private static void initResource() {
    try {
      ...
      Error x = new FatalError(
          "Fatal Error: Resource for apt or javac is missing");
      x.initCause(e);
      throw x;
    }
  }
