...
public class AptJavaCompiler extends com.sun.tools.javac.main.JavaCompiler {
  ...
  java.util.Set<java.io.File> aggregateGenFiles = java.util.Collections
      .emptySet();
  ...
  /**
   * Emit class files. This switch is always set, except for the first phase of
   * retrofitting, where signatures are parsed.
   */
  ...
  public List<ClassSymbol> compile(
      ...
      java.util.Set<java.io.File> aggregateGenFiles) throws Throwable {
    ...
    try {
      ...
      for (List<String> l = filenames; l.nonEmpty(); l = l.tail) {
        ...
        JavaFileObject fo = fm.getJavaFileObjectsFromStrings(List.of(l.head))
            .iterator().next();
      ...
      if (errorCount() == 0) {
        apt.main(roots, classes, origOptions, aptCL, providedFactory,
            productiveFactories);
