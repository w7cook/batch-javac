...
public class Bark extends Log {
  ...
  protected static final Context.Key<Bark> barkKey = new Context.Key<Bark>();

  /**
   * Preregisters factories to create and use a Bark object for use as both a
   * Log and a Bark.
   */
  ...
  /**
   * Sets a flag indicating whether or not to ignore all diagnostics. When
   * ignored, they are not reported to the output writers, not are they counted
   * in the various counters.
   * 
   * @param b
   *          If true, subsequent diagnostics will be ignored.
  ...
  /**
   * Report a warning, unless suppressed by the -nowarn option or the maximum
   * number of warnings has been reached.
   * 
   * @param key
  ...
  public void aptWarning(int pos, String key, Object... args) {
    report(aptDiags.warning(source, new SimpleDiagnosticPosition(pos), key,
        args));
  }
   ...
   * 
   * @param key
   *          The key for the localized note message.
   * @param args
   *          Fields of the note message.
   */
