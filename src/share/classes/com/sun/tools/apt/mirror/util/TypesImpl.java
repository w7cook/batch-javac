...
public class TypesImpl implements Types {
  ...
  private static final Context.Key<Types> typesKey = new Context.Key<Types>();
  ...
  public TypeMirror getErasure(TypeMirror t) {
    return env.typeMaker
        .getType(env.jctypes.erasure(((TypeMirrorImpl) t).type));
  }
  ...
  public PrimitiveType getPrimitiveType(PrimitiveType.Kind kind) {
    ...
    case BOOLEAN:
      ...
      break;
    case BYTE:
      prim = env.symtab.byteType;
      break;
    case SHORT:
      prim = env.symtab.shortType;
      break;
    case INT:
      prim = env.symtab.intType;
      break;
    case LONG:
      prim = env.symtab.longType;
      break;
    case CHAR:
      prim = env.symtab.charType;
      break;
    case FLOAT:
      prim = env.symtab.floatType;
      break;
    case DOUBLE:
      prim = env.symtab.doubleType;
      break;
    default:
      assert false;
    }
  ...
  public ArrayType getArrayType(TypeMirror componentType) {
    ...
    return (ArrayType) env.typeMaker.getType(new Type.ArrayType(
        ((TypeMirrorImpl) componentType).type, env.symtab.arrayClass));
  }
  ...
  public TypeVariable getTypeVariable(TypeParameterDeclaration tparam) {
    return (TypeVariable) env.typeMaker
        .getType(((DeclarationImpl) tparam).sym.type);
  }
  ...
  public WildcardType getWildcardType(Collection<ReferenceType> upperBounds,
    ...
    return (WildcardType) env.typeMaker.getType(new Type.WildcardType(bound,
        kind, env.symtab.boundClass));
  }
  ...
  public DeclaredType getDeclaredType(TypeDeclaration decl,
    ...
    if (typeArgs.length == 0)
      return (DeclaredType) env.typeMaker
          .getType(env.jctypes.erasure(sym.type));
  ...
  private DeclaredType getDeclaredType(Type outer, ClassSymbol sym,
      TypeMirror... typeArgs) {
    ...
    return (DeclaredType) env.typeMaker.getType(new Type.ClassType(outer, targs
        .toList(), sym));
  }
}
