...
public class TypeDeclarationImpl extends MemberDeclarationImpl implements
    TypeDeclaration {
  ...
  /**
   * Returns the type's name, with any type parameters (including those of outer
   * classes). Type names are qualified.
   */
  ...
  public Collection<? extends MethodDeclaration> getMethods() {
    ...
    for (Symbol s : getMembers(true)) {
      if (s.kind == Kinds.MTH && !s.isConstructor()
          && !env.names.clinit.equals(s.name)) { // screen out static
                                                 // initializers
        MethodSymbol m = (MethodSymbol) s;
        res.add((MethodDeclaration) env.declMaker.getExecutableDeclaration(m));
      }
    }
    return res;
  }
  ...
  /**
   * Returns a type's name, with any type parameters (including those of outer
   * classes). Type names are qualified.
   */
  static String toString(AptEnv env, ClassSymbol c) {
    ...
    if (c.isInner()) {
      ...
      sb.append(toString(env, enclosing)).append('.').append(c.name);
    } else {
      sb.append(c);
    }
