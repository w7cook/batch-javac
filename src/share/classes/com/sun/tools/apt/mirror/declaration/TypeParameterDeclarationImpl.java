...
public class TypeParameterDeclarationImpl extends DeclarationImpl implements
  ...
  /**
   * Returns the type parameter's name along with any "extends" clause. Class
   * names are qualified. No implicit "extends Object" is added.
   */
  ...
  public Declaration getOwner() {
    ...
    return ((owner.kind & Kinds.TYP) != 0) ? env.declMaker
        .getTypeDeclaration((ClassSymbol) owner) : env.declMaker
        .getExecutableDeclaration((MethodSymbol) owner);
  }
  ...
  /**
   * Returns the type parameter's name along with any "extends" clause. See
  ...
  /**
   * Returns the bounds of a type variable, eliding java.lang.Object if it
   * appears alone.
   */
  private static Iterable<Type> getExtendsBounds(AptEnv env, Type.TypeVar tv) {
    return (tv.getUpperBound().tsym == env.symtab.objectType.tsym) ? com.sun.tools.javac.util.List
        .<Type> nil() : env.jctypes.getBounds(tv);
  }
}
