...
public class FilerImpl implements Filer {
  /*
   * The Filer class must maintain a number of constraints. First, multiple
   * attempts to open the same path within the same invocation of apt results in
   * an IOException being thrown. For example, trying to open the same source
   * file twice:
   * 
   * createSourceFile("foo.Bar") ... createSourceFile("foo.Bar")
   * 
   * is disallowed as is opening a text file that happens to have the same name
   * as a source file:
   * 
   * createSourceFile("foo.Bar") ... createTextFile(SOURCE_TREE, "foo", new
   * File("Bar"), null)
   * 
   * Additionally, creating a source file that corresponds to an already created
   * class file (or vice versa) generates at least a warning. This is an error
   * if -XclassesAsDecls is being used since you can't create the same type
   * twice. However, if the Filer is used to create a text file named *.java
   * that happens to correspond to an existing class file, a warning is *not*
   * generated. Similarly, a warning is not generated for a binary file named
   * *.class and an existing source file.
   * 
   * The reason for this difference is that source files and class files are
   * registered with apt and can get passed on as declarations to the next round
   * of processing. Files that are just named *.java and *.class are not
   * processed in that manner; although having extra source files and class
   * files on the source path and class path can alter the behavior of the tool
  ...
  private enum FileKind {
    ...
    CLASS {
      void register(File file, String name, FilerImpl that) throws IOException {
        if (that.filesCreated.contains(new File(
            that.locations.get(SOURCE_TREE), that.nameToPath(name, ".java")))) {
    ...
    abstract void register(File file, String name, FilerImpl that)
        throws IOException;
  }
  ...
  private final static String DEFAULT_ENCODING = new OutputStreamWriter(
      new ByteArrayOutputStream()).getEncoding();

  private String encoding; // name
                           // of
                           // charset
                           // used
                           // for
  ...
  private final EnumMap<Location, File> locations; // where
                                                   // new
                                                   // files
                                                   // go

  private static final Context.Key<FilerImpl> filerKey = new Context.Key<FilerImpl>();
  ...
  public PrintWriter createSourceFile(String name) throws IOException {
    ...
    File file = new File(locations.get(SOURCE_TREE), pathname);
  ...
  public OutputStream createClassFile(String name) throws IOException {
    ...
    File file = new File(locations.get(CLASS_TREE), pathname);
   ...
   * {@inheritDoc}
   */
  public PrintWriter createTextFile(Location loc, String pkg, File relPath,
      String charsetName) throws IOException {
    File file = (pkg.length() == 0) ? relPath : new File(nameToPath(pkg),
        relPath.getPath());
    if (charsetName == null) {
      charsetName = encoding;
    }
    return getPrintWriter(loc, file.getPath(), charsetName, null,
        FileKind.OTHER);
  }
  ...
  /**
   * Returns a writer for a text file given its location, its pathname relative
   * to that location, and its encoding.
   */
   ...
   */
  private PrintWriter getPrintWriter(File file, String encoding, String name,
  ...
  /**
   * Returns an output stream for a binary file given its location and its
   * pathname relative to that location.
   */
  private OutputStream getOutputStream(Location loc, String pathname,
      String name, FileKind kind) throws IOException {
    File file = new File(locations.get(loc), pathname);
    return getOutputStream(file, name, kind);
  }

  private OutputStream getOutputStream(File file, String name, FileKind kind)
      throws IOException {
    prepareFile(file, name, kind);
   ...
   * If not, creates intermediate directories, and deletes the file if it
   * already exists.
   */
  private void prepareFile(File file, String name, FileKind kind)
      throws IOException {
    if (roundOver) {
