 ...
 * its context. It also uses a prebuilt index if available. It supports
 * invocations where it will serialize an optimized zip index file to disk.
 * 
 ...
 * default location is the value of the "java.io.tmpdir" system property.
 * 
...
public class ZipFileIndex {
  ...
  ZipFileIndex(File zipFile, RelativeDirectory symbolFilePrefix,
   ...
   * timestamp of the file and if its the same as the one at the time the index
   * was build we don't need to reopen anything.
   */
  private void checkIndex() throws IOException {
    ...
    try {
      ...
      ZipDirectory directory = new ZipDirectory(zipRandomFile, 0L, totalLength,
          this);
  ...
  public synchronized long getLastModified(RelativeFile path)
      throws IOException {
  ...
  public synchronized int read(RelativeFile path, byte[] buffer)
      throws IOException {
    Entry entry = getZipIndexEntry(path);
    if (entry == null)
      throw new FileNotFoundException();
  ...
  private byte[] getHeader(Entry entry) throws IOException {
    ...
    if ((get2ByteLittleEndian(header, 6) & 1) != 0)
      throw new ZipException("encrypted zip file"); // offset 6 in the header of
                                                    // the ZipFileEntry
    return header;
  }
  ...
  private class ZipDirectory {
    ...
    public ZipDirectory(RandomAccessFile zipRandomFile, long start, long end,
        ZipFileIndex index) throws IOException {
     ...
     * zip_util.c from the JDK sources. This is a Java port of that function.
     */
    private void findCENRecord(long start, long end) throws IOException {
      ...
      // There is a variable-length field after the dir offset record. We need
      // to do consequential search.
      while (endbufend >= 22) {
        ...
        while (i >= 0
            && !(endbuf[i] == 0x50 && endbuf[i + 1] == 0x4b
                && endbuf[i + 2] == 0x05 && endbuf[i + 3] == 0x06 && endbufpos
                + i + 22 + get2ByteLittleEndian(endbuf, i + 20) == totalLength)) {
          i--;
        }
    ...
    private int readEntry(int pos, List<Entry> entryList,
      ...
      if (directory == null) {
        ...
        while (directories.get(tempDirectory) == null) {
          directories.put(tempDirectory, new DirectoryEntry(tempDirectory,
              zipFileIndex));
          if (tempDirectory.path.indexOf("/") == tempDirectory.path.length() - 1)
            break;
          else {
            // use shared RelativeDirectory objects for parent dirs
            tempDirectory = getRelativeDirectory(tempDirectory.dirname()
                .getPath());
          }
        }
      } else {
        if (directories.get(directory) == null) {
          directories.put(directory,
              new DirectoryEntry(directory, zipFileIndex));
        }
      }

      // For each dir create also a file
      if (fileStart != fileEnd) {
        Entry entry = new Entry(directory, new String(zipDir, fileStart,
            fileEnd - fileStart, "UTF-8"));
  ...
  static class DirectoryEntry {
    ...
    private void initEntries() {
      ...
      if (!zipFileIndex.readFromIndex) {
        int from = -Arrays.binarySearch(zipFileIndex.entries, new Entry(
            dirName, ZipFileIndex.MIN_CHAR)) - 1;
        int to = -Arrays.binarySearch(zipFileIndex.entries, new Entry(dirName,
            MAX_CHAR)) - 1;
