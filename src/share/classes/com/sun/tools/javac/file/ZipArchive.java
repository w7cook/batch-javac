...
public class ZipArchive implements Archive {
  ...
  protected ZipArchive(JavacFileManager fm, ZipFile zfile, boolean initMap)
      throws IOException {
    this.fileManager = fm;
    this.zfile = zfile;
  ...
  protected void initMap() throws IOException {
    for (Enumeration<? extends ZipEntry> e = zfile.entries(); e
        .hasMoreElements();) {
      ZipEntry entry;
      try {
        entry = e.nextElement();
  ...
  public static class ZipFileObject extends BaseFileObject {
    ...
    /**
     * Check if two file objects are equal. Two ZipFileObjects are equal if the
     * absolute paths of the underlying zip files are equal and if the paths
     * within those zip files are equal.
     */
    @Override
