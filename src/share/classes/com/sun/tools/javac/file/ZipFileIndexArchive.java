...
public class ZipFileIndexArchive implements Archive {
  ...
  public JavaFileObject getFileObject(RelativeDirectory subdirectory,
    ...
    JavaFileObject ret = new ZipFileIndexFileObject(fileManager, zfIndex,
        entry, zfIndex.getZipFile());
    return ret;
  }
  ...
  /**
   * A subclass of JavaFileObject representing zip entries using the
   * com.sun.tools.javac.file.ZipFileIndex implementation.
   */
  public static class ZipFileIndexFileObject extends BaseFileObject {
    ...
    ZipFileIndexFileObject(JavacFileManager fileManager, ZipFileIndex zfIndex,
        ZipFileIndex.Entry entry, File zipFileName) {
      super(fileManager);
    ...
    protected CharsetDecoder getDecoder(boolean ignoreEncodingErrors) {
      return fileManager.getDecoder(fileManager.getEncodingName(),
    ...
    /**
     * Check if two file objects are equal. Two ZipFileIndexFileObjects are
     * equal if the absolute paths of the underlying zip files are equal and if
     * the paths within those zip files are equal.
     */
    @Override
