 ...
 * container, such as a directory or zip file. Internally, the file separator is
 * always '/'.
 * 
 * <p>
...
public abstract class RelativePath implements Comparable<RelativePath> {
  /**
   * @param p
   *          must use '/' as an internal separator
   */
  ...
  /**
   * Used to represent a platform-neutral subdirectory within a
   * platform-specific container, such as a directory or zip file. Internally,
   * the file separator is always '/', and if the path is not empty, it always
   * ends in a '/' as well.
   */
  public static class RelativeDirectory extends RelativePath {
    ...
    /**
     * Return true if this subdirectory "contains" the other path. A
     * subdirectory path does not contain itself.
     **/
   ...
   * container, such as a directory or zip file. Internally, the file separator
   * is always '/'. It never ends in '/'.
   */
  public static class RelativeFile extends RelativePath {
    static RelativeFile forClass(CharSequence className,
        JavaFileObject.Kind kind) {
      return new RelativeFile(className.toString().replace('.', '/')
          + kind.extension);
    }
