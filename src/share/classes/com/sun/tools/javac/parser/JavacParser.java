...
public class JavacParser implements Parser {
   ...
   */
  protected JavacParser(ParserFactory fac, Lexer S, boolean keepDocComments,
      boolean keepLineMap) {
    this.S = S;
   ...
   * an expression mode = TYPE : a type mode = NOPARAMS : no parameters allowed
   * for type mode = TYPEARG : type argument
   */
  ...
  /**
   * Report a syntax error at given position using the given argument unless one
   * was already reported at the same position.
   */
  ...
  void checkNoMods(long mods) {
    if (mods != 0) {
      ...
      error(S.pos(), "mod.not.allowed.here", Flags.asFlagSet(lowestMod));
    }
  }
  ...
  /**
   * Make an entry into docComments hashtable, provided flag keepDocComments is
   * set and given doc comment is non-null.
   * 
   * @param tree
  ...
  /**
   * Store ending position for a tree. The ending position should be the ending
  ...
  /**
   * Store ending position for a tree. The ending position should be greater of
   * the ending position of the previous token and errorEndPos.
   * 
   * @param t
   *          The tree.
   */
  ...
  /**
   * Get the start position for a tree node. The start position is defined to be
   * the position of the first character of the first token of the node's source
   * text.
   * 
   * @param tree
   *          The tree node
   ...
   * position of the last character of the last token of the node's source text.
   * Returns Position.NOPOS if end positions are not generated or the position
  ...
  JCExpression literal(Name prefix) {
    ...
    case LONGLITERAL:
      try {
        t = F.at(pos).Literal(TypeTags.LONG,
    ...
    case TRUE:
    case FALSE:
      t = F.at(pos).Literal(TypeTags.BOOLEAN, (S.token() == TRUE ? 1 : 0));
      break;
    case NULL:
   ...
   * BracketsOpt "." CLASS | Expression "]" ) | Arguments | "." ( CLASS | THIS |
   * [TypeArguments] SUPER Arguments | NEW [TypeArguments] InnerCreator ) ] |
   ...
   * BracketsOpt Selector = "." [TypeArguments] Ident [Arguments] | "." THIS |
   * "." [TypeArguments] SUPER SuperSuffix | "." NEW [TypeArguments]
   * InnerCreator | "[" Expression "]" TypeSelector = "." Ident [TypeArguments]
    ...
    case LPAREN:
      if (typeArgs == null && (mode & EXPR) != 0) {
        ...
        if ((mode & TYPE) != 0 && S.token() == LT) {
          ...
          if ((mode & TYPE) != 0 && (S.token() == COMMA || S.token() == GT)) {
            mode = TYPE;
  ...
  JCExpression typeArgument() {
    ...
    if (S.token() == EXTENDS) {
      ...
      // error recovery
      reportSyntaxError(S.prevEndPos(), "expected3", GT, EXTENDS, SUPER);
   ...
   */
  JCNewClass classCreatorRest(int newpos, JCExpression encl,
  ...
  JCBlock block(int pos, long flags) {
    ...
    // the Block node has a field "endpos" for first char of last token, which
    // is
    ...
    while (true) {
      ...
      case FINAL: {
        ...
        if (S.token() == INTERFACE || S.token() == CLASS || allowEnums
          ...
          // A "LocalVariableDeclarationStatement" subsumes the terminating
          // semicolon
      ...
      default:
        ...
        if (S.token() == COLON && t.getTag() == JCTree.IDENT) {
          ...
          // This Exec is an "ExpressionStatement"; it subsumes the terminating
          // semicolon
   ...
   * FormalParameter : Expression ")" Statement | WHILE ParExpression Statement
   * | DO Statement WHILE ParExpression ";" | TRY Block ( Catches | [Catches]
   * FinallyPart ) | TRY "(" ResourceSpecification ";"opt ")" Block [Catches]
   * [FinallyPart] | SWITCH ParExpression "{" SwitchBlockStatementGroups "}" |
   * SYNCHRONIZED ParExpression Block | RETURN [Expression] ";" | THROW
   * Expression ";" | BREAK [Ident] ";" | CONTINUE [Ident] ";" | ASSERT
   * Expression [ ":" Expression ] ";" | ";" | ExpressionStatement | Ident ":"
   * Statement
   */
  @SuppressWarnings("fallthrough")
  public JCStatement parseStatement() {
    ...
    case FOR: {
      S.nextToken();
      accept(LPAREN);
      List<JCStatement> inits = S.token() == SEMI ? List.<JCStatement> nil()
          : forInit();
      if (inits.length() == 1 && inits.head.getTag() == JCTree.VARDEF
          && ((JCVariableDecl) inits.head).init == null && S.token() == COLON) {
        checkForeach();
    ...
    case BREAK: {
      S.nextToken();
      Name label = (S.token() == IDENTIFIER || S.token() == ASSERT || S.token() == ENUM) ? ident()
          : null;
  ...
  JCCatch catchClause() {
    ...
    JCExpression paramType = catchTypes.size() > 1 ? toP(F.at(
        catchTypes.head.getStartPosition()).TypeUnion(catchTypes))
        : catchTypes.head;
   ...
   * SwitchBlockStatementGroup = SwitchLabel BlockStatements SwitchLabel = CASE
  ...
  List<JCExpressionStatement> forUpdate() {
    return moreStatementExpressions(S.pos(), parseExpression(),
   ...
   * @param dc
   *          The documentation comment for the variable declarations, or null.
   */
  <T extends ListBuffer<? super JCVariableDecl>> T variableDeclaratorsRest(
      int pos, JCModifiers mods, JCExpression type, Name name, boolean reqInit,
      String dc, T vdefs) {
  ...
  JCVariableDecl variableDeclaratorId(JCModifiers mods, JCExpression type) {
    int pos = S.pos();
    Name name = ident();
    if ((mods.flags & Flags.VARARGS) != 0 && S.token() == LBRACKET) {
   ...
   */
  JCTree resource() {
    return variableDeclaratorRest(S.pos(), optFinal(Flags.FINAL), parseType(),
        ident(), true, null);
  }
  ...
  public JCTree.JCCompilationUnit parseCompilationUnit() {
    ...
    JCTree.JCCompilationUnit toplevel = F.at(pos).TopLevel(packageAnnotations,
        pid, defs.toList());
    attach(toplevel, dc);
  ...
  JCStatement classOrInterfaceOrEnumDeclaration(JCModifiers mods, String dc) {
    if (S.token() == CLASS) {
      ...
      if (S.token() == ENUM) {
        ...
        return toP(F.Exec(syntaxError(pos, errs, "expected3", CLASS, INTERFACE,
            ENUM)));
      }
    } else {
      if (S.token() == ENUM) {
      ...
      return toP(F.Exec(syntaxError(pos, errs, "expected2", CLASS, INTERFACE)));
    }
  }

  /**
   * ClassDeclaration = CLASS Ident TypeParametersOpt [EXTENDS Type] [IMPLEMENTS
   * TypeList] ClassBody
   * 
   * @param mods
  ...
  /**
   * InterfaceDeclaration = INTERFACE Ident TypeParametersOpt [EXTENDS TypeList]
   * InterfaceBody
   * 
   * @param mods
  ...
  JCClassDecl enumDeclaration(JCModifiers mods, String dc) {
    ...
    JCClassDecl result = toP(F.at(pos).ClassDef(mods, name,
  ...
  List<JCTree> enumBody(Name enumName) {
    ...
    if (S.token() == COMMA) {
      ...
      if (S.token() != SEMI && S.token() != RBRACE) {
        defs.append(syntaxError(S.pos(), "expected3", COMMA, RBRACE, SEMI));
        S.nextToken();
      }
    }
    if (S.token() == SEMI) {
  ...
  JCTree enumeratorDeclaration(Name enumName) {
    ...
    JCNewClass create = F.at(createPos).NewClass(null, typeArgs, ident, args,
        body);
    ...
    if (S.token() == SEMI) {
      ...
      if (S.token() == CLASS || S.token() == INTERFACE || allowEnums
        ...
        if (S.token() == LPAREN && !isInterface
          ...
          return List.of(methodDeclaratorRest(pos, mods, null, names.init,
              typarams, isInterface, true, dc));
        } else {
          pos = S.pos();
          name = ident();
          if (S.token() == LPAREN) {
            return List.of(methodDeclaratorRest(pos, mods, type, name,
                typarams, isInterface, isVoid, dc));
          } else if (!isVoid && typarams.isEmpty()) {
            List<JCTree> defs = variableDeclaratorsRest(pos, mods, type, name,
                isInterface, dc, new ListBuffer<JCTree>()).toList();
            storeEnd(defs.last(), S.endPos());
            accept(SEMI);
            return defs;
          } else {
            pos = S.pos();
            List<JCTree> err = isVoid ? List.<JCTree> of(toP(F.at(pos)
                .MethodDef(mods, name, type, typarams,
                    List.<JCVariableDecl> nil(), List.<JCExpression> nil(),
                    null, null))) : null;
            return List.<JCTree> of(syntaxError(S.pos(), err, "expected",
                LPAREN));
          }
        }
      }
   ...
   * TypeList] ";" VoidInterfaceMethodDeclaratorRest = FormalParameters [THROWS
   * TypeList] ";" ConstructorDeclaratorRest = "(" FormalParameterListOpt ")"
   * [THROWS TypeList] MethodBody
   */
  ...
  /**
   * Return operation tag of unary operator represented by token, -1 if token is
