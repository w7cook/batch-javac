...
public class Scanner implements Lexer {
  ...
  /**
   * The input buffer, index of next chacter to be read, index of one past last
   * character in buffer.
   */
  private char[] buf;
  private int bp;
  ...
  /**
   * Create a scanner from the input array. This method might modify the array.
   * To avoid copying the input array, ensure that
   * {@code inputLength < input.length} or {@code input[input.length -1]} is a
   * white space character.
   * 
   * @param fac
   *          the factory which created this Scanner
   * @param input
   *          the input, might be modified
   * @param inputLength
   *          the size of the input. Must be positive and less than or equal to
   *          input.length.
   */
   ...
   * 
   * @param radix
   *          The radix of the number; one of 2, j8, 10, 16.
   */
      ...
      default:
        if (ch < '\u0080') {
          ...
          if (high != 0) {
            ...
            isJavaIdentifierPart = Character.isJavaIdentifierPart(Character
                .toCodePoint(high, ch));
          } else {
  ...
  /**
   * Scan surrogate pairs. If 'ch' is a high surrogate and the next character is
   * a low surrogate, then put the low surrogate in 'ch', and return the high
   * surrogate. otherwise, just return 0.
   */
   ...
   * (which is treated as the beginning of the first line). Stops positioned at
   * the closing '/'.
   */
        ...
        default:
          if (isSpecial(ch)) {
            ...
            if (ch < '\u0080') {
              ...
              if (high != 0) {
                ...
                isJavaIdentifierStart = Character
                    .isJavaIdentifierStart(Character.toCodePoint(high, ch));
              } else {
            ...
            if (isJavaIdentifierStart) {
              scanIdent();
            } else if (bp == buflen || ch == EOI && bp + 1 == buflen) { // JLS
                                                                        // 3.5
              token = EOF;
              pos = bp = eofPos;
            } else {
  ...
  /**
   * Called when a complete comment has been scanned. pos and endPos will mark
  ...
  /**
   * Called when a complete whitespace run has been scanned. pos and endPos will
   * mark the whitespace boundary.
   */
  ...
  protected void processLineTerminator() {
    if (scannerDebug)
      System.out.println("processTerminator(" + pos + "," + endPos + ")=|"
          + new String(getRawCharacters(pos, endPos)) + "|");
  }
