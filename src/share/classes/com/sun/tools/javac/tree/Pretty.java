...
public class Pretty extends JCTree.Visitor {
   ...
   * detail would not be valid java source.
   */
  ...
  /**
   * Derived visitor method: print list of expression trees, separated by given
   * string.
   * 
   * @param sep
   ...
   * followed by class definition. if class definition == null, print all
   * definitions in toplevel.
   * 
   * @param tree
   *          The toplevel tree
   * @param cdef
   *          The class definition, which is assumed to be part of the toplevel
   *          tree.
   */
  public void printUnit(JCCompilationUnit tree, JCClassDecl cdef)
    ...
    for (List<JCTree> l = tree.defs; l.nonEmpty()
      ...
      if (l.head.getTag() == JCTree.IMPORT) {
        ...
        if (name == name.table.names.asterisk || cdef == null
  ...
  public void visitMethodDef(JCMethodDecl tree) {
    try {
      ...
      if (tree.name == tree.name.table.names.init && enclClassName == null
          && sourceOutput)
        return;
      println();
      align();
    ...
    try {
      ...
      if (tree.def != null) {
        ...
        enclClassName = tree.def.name != null ? tree.def.name
            : tree.type != null
                && tree.type.tsym.name != tree.type.tsym.name.table.names.empty ? tree.type.tsym.name
                : null;
  ...
  public void visitLiteral(JCLiteral tree) {
    try {
      ...
      case TypeTags.CHAR:
        char c;
        if (tree.value instanceof Character) {
          c = ((Character)tree.value);
        }
        else {
          c = (char) ((Number) tree.value).intValue();
        }
        print("\'" + Convert.quote(String.valueOf(c)) + "\'");
        break;
