 ...
 * 
 * <p>
 * PathFileObjects are, for the most part, straightforward wrappers around Path
 * objects. The primary complexity is the support for "inferBinaryName". This is
 * left as an abstract method, implemented by each of a number of different
 * factory methods, which compute the binary name based on information available
 * at the time the file object is created.
 * 
 * <p>
...
abstract class PathFileObject implements JavaFileObject {
  ...
  /**
   * Create a PathFileObject within a directory, such that the binary name can
   * be inferred from the relationship to the parent directory.
   */
  static PathFileObject createDirectoryPathFileObject(
      JavacPathFileManager fileManager, final Path path, final Path dir) {
  ...
  /**
   * Create a PathFileObject in a file system such as a jar file, such that the
   * binary name can be inferred from its position within the filesystem.
   */
  static PathFileObject createJarPathFileObject(
      JavacPathFileManager fileManager, final Path path) {
    return new PathFileObject(fileManager, path) {
      @Override
      String inferBinaryName(Iterable<? extends Path> paths) {
  ...
  /**
   * Create a PathFileObject whose binary name can be inferred from the relative
   * path to a sibling.
   */
  static PathFileObject createSiblingPathFileObject(
      JavacPathFileManager fileManager, final Path path,
   ...
   */
  static PathFileObject createSimplePathFileObject(
      JavacPathFileManager fileManager, final Path path) {
    return new PathFileObject(fileManager, path) {
      @Override
      String inferBinaryName(Iterable<? extends Path> paths) {
  ...
  public boolean isNameCompatible(String simpleName, Kind kind) {
    ...
    if (pn.equalsIgnoreCase(sn)) {
      try {
        // allow for Windows
        return path.toRealPath(LinkOption.NOFOLLOW_LINKS).getFileName()
            .toString().equals(sn);
      } catch (IOException e) {
      }
    }
  ...
  public Reader openReader(boolean ignoreEncodingErrors) throws IOException {
    CharsetDecoder decoder = fileManager.getDecoder(
        fileManager.getEncodingName(), ignoreEncodingErrors);
  ...
  public boolean equals(Object other) {
    return (other instanceof PathFileObject && path
        .equals(((PathFileObject) other).path));
  }
  ...
  protected static String toBinaryName(Path relativePath) {
    return toBinaryName(relativePath.toString(), relativePath.getFileSystem()
        .getSeparator());
  }
