...
public class JavacFiler implements Filer, Closeable {
  ...
  private class FilerInputJavaFileObject extends FilerInputFileObject implements
      JavaFileObject {
    private final JavaFileObject javaFileObject;
  ...
  private class FilerOutputStream extends FilterOutputStream {
    ...
    public synchronized void close() throws IOException {
      if (!closed) {
        closed = true;
        /*
         * If an IOException occurs when closing the underlying stream, still
  ...
  /**
   * Wrap a {@code Writer} returned from the {@code JavaFileManager} to properly
   * register source or class files when they are closed.
   */
  private class FilerWriter extends FilterWriter {
    ...
    public synchronized void close() throws IOException {
      if (!closed) {
        closed = true;
        /*
         * If an IOException occurs when closing the underlying Writer, still
         * try to process the file.
         */
  ...
  /**
   * Names of source files closed in this round. This set must be synchronized.
  ...
  /**
   * Names of all created source files. Its iterators should preserve insertion
   * order.
   */
  ...
  public FileObject getResource(JavaFileManager.Location location,
    ...
    FileObject fileObject = fileManager.getFileForInput(location,
        pkg.toString(), relativeName.toString());
    if (fileObject == null) {
      String name = (pkg.length() == 0) ? relativeName.toString()
          : (pkg + "/" + relativeName);
  ...
  private void closeFileObject(String typeName, FileObject fileObject) {
    /*
     * If typeName is non-null, the file object was opened as a source or class
     * file by the user. If a file was opened as a resource, typeName will be
     * null and the file is *not* subject to annotation processing.
     */
    if ((typeName != null)) {
      if (!(fileObject instanceof JavaFileObject))
