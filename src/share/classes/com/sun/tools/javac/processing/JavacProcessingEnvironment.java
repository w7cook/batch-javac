...
public class JavacProcessingEnvironment implements ProcessingEnvironment,
    ...
    if (options.isSet(XPRINT)) {
      ...
      try {
        ...
        processorClassLoader = fileManager
            ...
            .getClassLoader(ANNOTATION_PROCESSOR_PATH) : fileManager
            .getClassLoader(CLASS_PATH);

        /*
         * If the "-processor" option is used, search the appropriate path for
         * the named class. Otherwise, use a service provider mechanism to
         * create the processor iterator.
         */
         ...
         * (i.e. no processor was specified either explicitly, or implicitly, in
         * service configuration file.) Otherwise, we cannot continue.
         */
  ...
  private Iterator<Processor> handleServiceLoaderUnavailability(String key,
    ...
    if (fileManager instanceof JavacFileManager) {
      ...
      Iterable<? extends File> workingPath = fileManager
          ...
          .getLocation(ANNOTATION_PROCESSOR_PATH) : standardFileManager
          .getLocation(CLASS_PATH);
   ...
   * over annotations processors. If java.util.ServiceLoader is present use it,
   * otherwise, use sun.misc.Service, otherwise fail if a loader is needed.
   */
  private class ServiceIterator implements Iterator<Processor> {
    ...
    ServiceIterator(ClassLoader classLoader, Log log) {
      ...
      try {
        ...
        Method loadMethod = loaderClass.getMethod(loadMethodName, Class.class,
            ClassLoader.class);
    ...
    public boolean hasNext() {
      if (nextProc != null)
        ...
        if (!names.hasNext())
          ...
          try {
            try {
              processor = (Processor) (processorCL.loadClass(processorName)
                  .newInstance());
            } catch (ClassNotFoundException cnfe) {
   ...
   * been used on a prior round, its process method is called on all subsequent
   * rounds, perhaps with an empty set of annotations to process. The
   * {@code annotatedSupported} method caches the supported annotation
  ...
  static class ProcessorState {
    ...
    /**
     * Checks whether or not a processor's source version is compatible with the
     * compilation source version. The processor's source version needs to be
     * greater than or equal to the source version of the compile.
     */
    private void checkSourceVersionCompatibility(Source source, Log log) {
      ...
      if (procSourceVersion.compareTo(Source.toSourceVersion(source)) < 0) {
        log.warning("proc.processor.incompatible.source.version",
            procSourceVersion, processor.getClass().getName(), source.name);
      }
    }
    ...
    private boolean checkOptionName(String optionName, Log log) {
      ...
      if (!valid)
        log.error("proc.processor.bad.option.name", optionName, processor
            .getClass().getName());
      return valid;
    }
  ...
  /**
   * This class holds information about the processors that have been discoverd
   * so far as well as the means to discover more, if necessary. A single
   * iterator should be used per round of annotation processing. The iterator
   * first visits already discovered processors then fails over to the service
   * provider mechanism if additional queries are made.
   */
  class DiscoveredProcessors implements Iterable<ProcessorState> {
    ...
    class ProcessorStateIterator implements Iterator<ProcessorState> {
      ...
      /**
       * Run all remaining processors on the procStateList that have not already
       * run this round with an empty set of annotations.
       */
  ...
  private void discoverAndRunProcs(Context context,
      Set<TypeElement> annotationsPresent, List<ClassSymbol> topLevelClasses,
    ...
    while (unmatchedAnnotations.size() > 0 && psi.hasNext()) {
      ...
      if (matchedNames.size() > 0 || ps.contributed) {
        ...
        if (printProcessorInfo || verbose) {
          log.printNoteLines("x.print.processor.info", ps.processor.getClass()
              .getName(), matchedNames.toString(), processingResult);
        }
  ...
  class Round {
    ...
    private Round(Context context, int number, int priorErrors,
      ...
      // the following is for the benefit of
    ...
    Round(Context context, List<JCCompilationUnit> roots,
      ...
      // compatibility with the previous revision of the code. Strictly
      // speaking,
      // it should not be necessary, but a javah golden file test fails without
      // it.
      topLevelClasses = getTopLevelClasses(roots).prependList(
          classSymbols.reverse());
    ...
    /** Create a new round. */
    private Round(Round prev, Set<JavaFileObject> newSourceFiles,
        Map<String, JavaFileObject> newClassFiles) {
      this(prev.nextContext(), prev.number + 1, prev.nMessagerErrors,
      ...
      topLevelClasses = join(getTopLevelClasses(parsedFiles),
          getTopLevelClassesFromClasses(newClasses));

      packageInfoFiles = join(getPackageInfoFiles(parsedFiles),
    ...
    Round next(Set<JavaFileObject> newSourceFiles,
        Map<String, JavaFileObject> newClassFiles) {
      try {
    ...
    /**
     * Return the number of errors found so far in this round. This may include
     * uncoverable errors, such as parse errors, and transient errors, such as
     * missing symbols.
     */
    int errorCount() {
    ...
    /**
     * Find the set of annotations present in the set of top level classes and
     * package info files to be processed this round.
     */
    ...
    private List<ClassSymbol> enterClassFiles(
    ...
    void run(boolean lastRound, boolean errorStatus) {
      ...
      try {
        if (lastRound) {
          ...
          discoverAndRunProcs(context, annotationsPresent, topLevelClasses,
              packageInfoFiles);
        }
      } finally {
        if (taskListener != null)
    ...
    void showDiagnostics(boolean showAll) {
      ...
      if (!showAll) {
        // suppress errors, which are all presumed to be transient resolve
        // errors
    ...
    private void printRoundInfo(boolean lastRound) {
      if (printRounds || verbose) {
        List<ClassSymbol> tlc = lastRound ? List.<ClassSymbol> nil()
            : topLevelClasses;
        Set<TypeElement> ap = lastRound ? Collections.<TypeElement> emptySet()
            : annotationsPresent;
        log.printNoteLines("x.print.rounds", number, "{" + tlc.toString(", ")
            + "}", ap, lastRound);
      }
    }

    /**
     * Get the context for the next round of processing. Important values are
     * propogated from round to round; other values are implicitly reset.
     */
  ...
  public JavaCompiler doProcessing(Context context,
    ...
    do {
      ...
      round = round
          .next(
              new LinkedHashSet<JavaFileObject>(filer
                  .getGeneratedSourceFileObjects()),
              new LinkedHashMap<String, JavaFileObject>(filer
                  .getGeneratedClasses()));
    ...
    /*
     * If an annotation processor raises an error in a round, that round runs to
     * completion and one last round occurs. The last round may also occur
     * because no more source or class files have been generated. Therefore, if
     * an error was raised on either of the last *two* rounds, the compile
     * should exit with a nonzero exit code. The current value of errorStatus
     * holds whether or not an error was raised on the second to last round;
     * errorRaised() gives the error status of the last round.
     */
    if (messager.errorRaised() || werror && round.warningCount() > 0
        && round.errorCount() > 0)
      errorStatus = true;

    Set<JavaFileObject> newSourceFiles = new LinkedHashSet<JavaFileObject>(
    ...
    if (taskListener != null)
      taskListener
          .finished(new TaskEvent(TaskEvent.Kind.ANNOTATION_PROCESSING));
  ...
  /*
   * Called retroactively to determine if a class loader was required, after we
   * have failed to create one.
   */
  ...
  /**
   * Convert import-style string for supported annotations into a regex matching
   * that string. If the string is a valid import-style string, return a regex
  ...
  /**
   * Return true if the argument string is a valid import-style string
   * specifying claimed annotations; return false otherwise.
   */
