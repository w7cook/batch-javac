...
public class PrintingProcessor extends AbstractProcessor {
  ...
  void print(Element element) {
    new PrintingElementVisitor(writer, processingEnv.getElementUtils()).visit(
        element).flush();
  }
  ...
  public static class PrintingElementVisitor extends
      SimpleElementVisitor7<PrintingElementVisitor, Boolean> {
    ...
    public PrintingElementVisitor visitExecutable(ExecutableElement e, Boolean p) {
      ...
      if (kind != STATIC_INIT && kind != INSTANCE_INIT) {
        ...
        if (kind == CONSTRUCTOR && enclosing != null && NestingKind.ANONYMOUS ==
    ...
    @Override
    public PrintingElementVisitor visitTypeParameter(TypeParameterElement e,
        Boolean p) {
      writer.print(e.getSimpleName());
      return this;
    }
          ...
          if (e.isVarArgs()) {
            ...
            if (tm.getKind() != TypeKind.ARRAY)
              throw new AssertionError(
                  "Var-args parameter is not an array type: " + tm);
