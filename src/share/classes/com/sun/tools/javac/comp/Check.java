 ...
 */
public class Check {
  protected static final Context.Key<Check> checkKey = new Context.Key<Check>();
  ...
  DeferredLintHandler setDeferredLintHandler(
  ...
  void warnDeprecated(DiagnosticPosition pos, Symbol sym) {
    if (!lint.isSuppressed(LintCategory.DEPRECATION))
      deprecationHandler
          .report(pos, "has.been.deprecated", sym, sym.location());
  }
  ...
  public Type completionError(DiagnosticPosition pos, CompletionFailure ex) {
    ...
    if (ex instanceof ClassReader.BadClassFile && !suppressAbortOnBadClassFile)
      throw new Abort();
    else
      return syms.errType;
  }
  ...
  Type typeError(DiagnosticPosition pos, String problem, Type found, Type req,
      Object explanation) {
  ...
  Type typeTagError(DiagnosticPosition pos, Object required, Object found) {
    ...
    return types.createErrorType(found instanceof Type ? (Type) found
        : syms.errType);
  }
    ...
    if (s.next != null) {
      for (Scope.Entry e = s.next.lookup(c.name); e.scope != null
        ...
        if (e.sym.kind == TYP && e.sym.type.tag != TYPEVAR
            && (e.sym.owner.kind & (VAR | MTH)) != 0 && c.name != names.error) {
          duplicateError(pos, e.sym);
          return;
        }
      }
   ...
   * classes, or as a class defined in its enclosing scope. return true if class
   * is unique in its enclosing scope.
   * 
   * @param pos
   *          Position for error reporting.
   * @param name
  ...
  /**
   * Return name of local class. This is of the form <enclClass> $ n <classname>
   * where enclClass is the flat name of the enclosing class, classname is the
  ...
  Type checkType(DiagnosticPosition pos, Type found, Type req, String errKey) {
    if (req.tag == ERROR)
      return req;
    if (found.tag == FORALL)
      return instantiatePoly(pos, (ForAll) found, req,
          convertWarner(pos, found, req));
    if (req.tag == NONE)
      return found;
    if (types.isAssignable(found, req, convertWarner(pos, found, req)))
      return found;
    if (found.tag <= DOUBLE && req.tag <= DOUBLE)
      return typeError(pos, diags.fragment("possible.loss.of.precision"),
          found, req);
  ...
  /**
   * Instantiate polymorphic type to some prototype, unless prototype is
   * `anyPoly' in which case polymorphic type is returned unchanged.
   */
  ...
  Type checkCastable(DiagnosticPosition pos, Type found, Type req) {
    if (found.tag == FORALL) {
      ...
      return typeError(pos, diags.fragment("inconvertible.types"), found, req);
    }
  }

  // where
  /**
   * Is type a type variable, or a (possibly multi-dimensional) array of type
   * variables?
   */
   ...
   *          Position to be used for error reporting.
   * @param a
   *          The type that should be bounded by bs.
   * @param bs
   *          The bound.
   */
  private boolean checkExtends(Type a, TypeVar bs) {
    if (a.isUnbound()) {
      ...
      return types.isCastable(bs.getUpperBound(), types.upperBound(a),
          Warner.noWarnings);
  ...
  Type checkClassType(DiagnosticPosition pos, Type t) {
    if (t.tag != CLASS && t.tag != ERROR)
      return typeTagError(pos, diags.fragment("type.req.class"),
          (t.tag == TYPEVAR) ? diags.fragment("type.parameter", t) : t);
    else
      return t;
  }
   ...
   *          The type to be checked.
   * @param noBounds
   *          True if type bounds are illegal here.
   */
    ...
    if (noBounds && t.isParameterized()) {
      ...
      while (args.nonEmpty()) {
        if (args.head.tag == WILDCARD)
          return typeTagError(pos, diags.fragment("type.req.exact"), args.head);
        args = args.tail;
      }
    }
    return t;
  ...
  Type checkReifiableReferenceType(DiagnosticPosition pos, Type t) {
    if (t.tag != CLASS && t.tag != ARRAY && t.tag != ERROR) {
      return typeTagError(pos, diags.fragment("type.req.class.array"), t);
  ...
  Type checkRefType(DiagnosticPosition pos, Type t) {
    ...
    case ERROR:
      return t;
    default:
      return typeTagError(pos, diags.fragment("type.req.ref"), t);
    }
  }

  /**
   * Check that each type is a reference type, i.e. a class, interface or array
   * type or a type variable.
   * 
   * @param trees
   *          Original trees, used for error reporting.
   * @param types
  ...
  /**
   * Check that usage of diamond operator is correct (i.e. diamond should not be
  ...
  Type checkDiamond(JCNewClass tree, Type t) {
    if (!TreeInfo.isDiamond(tree) || t.isErroneous()) {
      ...
      log.error(tree.clazz.pos(), "cant.apply.diamond.1", t,
          diags.fragment("diamond.and.anon.class", t));
      return types.createErrorType(t);
    } else if (t.tsym.type.getTypeArguments().isEmpty()) {
      log.error(tree.clazz.pos(), "cant.apply.diamond.1", t,
          diags.fragment("diamond.non.generic", t));
      return types.createErrorType(t);
    } else if (tree.typeargs != null && tree.typeargs.nonEmpty()) {
      log.error(tree.clazz.pos(), "cant.apply.diamond.1", t,
          diags.fragment("diamond.and.explicit.params", t));
      return types.createErrorType(t);
    } else {
      return t;
    }
  ...
  void checkVarargsMethodDecl(Env<AttrContext> env, JCMethodDecl tree) {
    ...
    if (hasTrustMeAnno && !isTrustMeAllowedOnMethod(m)) {
      if (varargElemType != null) {
        log.error(tree, "varargs.invalid.trustme.anno", syms.trustMeType.tsym,
      ...
      warnUnsafeVararg(tree, "varargs.redundant.trustme.anno",
          syms.trustMeType.tsym, diags.fragment(
              "varargs.trustme.on.reifiable.varargs", varargElemType));
    } else if (!hasTrustMeAnno && varargElemType != null
        && !types.isReifiable(varargElemType)) {
      warnUnchecked(tree.params.head.pos(),
          "unchecked.varargs.non.reifiable.type", varargElemType);
    }
  }
  ...
  private boolean isTrustMeAllowedOnMethod(Symbol s) {
    return (s.flags() & VARARGS) != 0
        && (s.isConstructor() || (s.flags() & (STATIC | FINAL)) != 0);
  }
   ...
   *          Position to be used for error reporting.
   * @param argtypes
   *          Actual arguments supplied to vararg method.
   */
  void checkVararg(DiagnosticPosition pos, List<Type> argtypes, Symbol msym) {
    Type argtype = argtypes.last();
    if (!types.isReifiable(argtype)
        && (!allowSimplifiedVarargs
            || msym.attribute(syms.trustMeType.tsym) == null || !isTrustMeAllowedOnMethod(msym))) {
      warnUnchecked(pos, "unchecked.generic.array.creation", argtype);
    }
  }
  ...
  private Type firstIncompatibleTypeArg(Type type) {
    ...
    List<Type> tvars_cap = types.substBounds(formals, formals,
  ...
  /**
   * Check that given modifiers are legal for given symbol and return modifiers
   * together with any implicit modififiers for that symbol. Warning: we can't
   * use flags() here since this method is called during class enter, when
   * flags() would cause a premature completion.
   * 
   * @param pos
   *          Position to be used for error reporting.
   * @param flags
  ...
  long checkFlags(DiagnosticPosition pos, long flags, Symbol sym, JCTree tree) {
    ...
    case TYP:
      if (sym.isLocal()) {
        ...
        if ((sym.owner.flags_field & STATIC) == 0 && (flags & ENUM) != 0)
    ...
    checkDisjoint(pos, flags, ABSTRACT, PRIVATE | STATIC))
        && checkDisjoint(pos, flags, ABSTRACT | INTERFACE, FINAL | NATIVE
            | SYNCHRONIZED)
        ...
        && (sym.kind == TYP || checkDisjoint(pos, flags, ABSTRACT | NATIVE,
            STRICTFP))) {
      // skip
    }
   ...
   * 
   * If the enum does have specialized enum contants, it is <i>not</i> final.
   */
   ...
   * possible cycles.
   * 
   * Visitor method: Validate a type expression, if it is not null, catching and
   * reporting any completion failures.
   */
  ...
  class Validator extends JCTree.Visitor {
    ...
    public void validateTrees(List<? extends JCTree> trees, boolean checkRaw,
        boolean isOuter) {
      for (List<? extends JCTree> l = trees; l.nonEmpty(); l = l.tail)
    ...
    void checkRaw(JCTree tree, Env<AttrContext> env) {
      if (lint.isEnabled(LintCategory.RAW) && tree.type.tag == CLASS
          && !TreeInfo.isDiamond(tree) && !env.enclClass.name.isEmpty() && // anonymous
                                                                           // or
                                                                           // intersection
          tree.type.isRaw()) {
        log.warning(LintCategory.RAW, tree.pos(), "raw.class.use", tree.type,
            tree.type.tsym.type);
      }
    }
  }
  ...
  /*
   * The following methods treat classes as sets that contain the class itself
  ...
  boolean intersects(Type t, List<Type> ts) {
    for (List<Type> l = ts; l.nonEmpty(); l = l.tail)
      if (types.isSubtype(t, l.head) || types.isSubtype(l.head, t))
        return true;
    return false;
  }
  ...
  boolean isUnchecked(ClassSymbol exc) {
    return exc.kind == ERR || exc.isSubClass(syms.errorType.tsym, types)
  ...
  boolean isUnchecked(Type exc) {
    return (exc.tag == TYPEVAR) ? isUnchecked(types.supertype(exc))
   ...
   * check overriding of synthetic methods (reason: they might be final). todo:
   * check whether this is still necessary. (2) Admit the case where an
   * interface proxy throws fewer exceptions than the method it implements.
   * Augment the proxy methods with the undeclared exceptions in this case. (3)
   * When generics are enabled, admit the case where an interface proxy has a
   * result type extended by the result type of the method it implements. Change
   * the proxies result type to the smaller type in this case.
   * 
   * @param tree
   ...
   */
  void checkOverride(JCTree tree, MethodSymbol m, MethodSymbol other,
      ClassSymbol origin) {
    // Don't check overriding of synthetic methods or by bridge methods.
    if ((m.flags() & (SYNTHETIC | BRIDGE)) != 0
        || (other.flags() & SYNTHETIC) != 0) {
      return;
    }
    ...
    // method (JLS 8.4.6.1).
    if ((other.flags() & FINAL) != 0 || (m.flags() & STATIC) == 0
    ...
    if ((origin.flags() & INTERFACE) == 0
      ...
      log.error(TreeInfo.diagnosticPositionFor(m, tree),
          "override.weaker.access", cannotOverride(m, other),
          other.flags() == 0 ? Flag.PACKAGE : asFlagSet(other.flags()
              & AccessFlags));
      return;
    }
    ...
    boolean resultTypesOK = types.returnTypeSubstitutable(mt, ot, otres,
        overrideWarner);
    if (!resultTypesOK) {
      if (!allowCovariantReturns && m.owner != origin
          && m.owner.isSubClass(other.owner, types)) {
        // allow limited interoperability with covariant returns
      } else {
        log.error(TreeInfo.diagnosticPositionFor(m, tree),
            "override.incompatible.ret", cannotOverride(m, other), mtres, otres);
        return;
      }
    } else if (overrideWarner.hasNonSilentLint(LintCategory.UNCHECKED)) {
      warnUnchecked(TreeInfo.diagnosticPositionFor(m, tree),
          "override.unchecked.ret", uncheckedOverrides(m, other), mtres, otres);
    }

    // Error if overriding method throws an exception not reported
    // by overridden method.
    List<Type> otthrown = types.subst(ot.getThrownTypes(), otvars, mtvars);
    List<Type> unhandledErased = unhandled(mt.getThrownTypes(),
        types.erasure(otthrown));
    List<Type> unhandledUnerased = unhandled(mt.getThrownTypes(), otthrown);
    if (unhandledErased.nonEmpty()) {
      log.error(TreeInfo.diagnosticPositionFor(m, tree),
          "override.meth.doesnt.throw", cannotOverride(m, other),
          unhandledUnerased.head);
      return;
    } else if (unhandledUnerased.nonEmpty()) {
      warnUnchecked(TreeInfo.diagnosticPositionFor(m, tree),
          "override.unchecked.thrown", cannotOverride(m, other),
          unhandledUnerased.head);
      return;
    }
    ...
    if ((((m.flags() ^ other.flags()) & Flags.VARARGS) != 0)
      ...
      log.warning(TreeInfo.diagnosticPositionFor(m, tree),
          ((m.flags() & Flags.VARARGS) != 0) ? "override.varargs.missing"
              : "override.varargs.extra", varargsOverrides(m, other));
    }
  ...
  // where
  private boolean isDeprecatedOverrideIgnorable(MethodSymbol m,
      ClassSymbol origin) {
    // If the method, m, is defined in an interface, then ignore the issue if
    // the method
    // is only inherited via a supertype and also implemented in the supertype,
    // because in that case, we will rediscover the issue when examining the
    // method
    // in the supertype.
    // If the method, m, is not defined in an interface, then the only time we
    // need to
    // address the issue is when the method is the supertype implemementation:
    // any other
  ...
  /**
   * Check that a class does not inherit two concrete methods with the same
   * signature.
   * 
   * @param pos
   *          Position to be used for error reporting.
  ...
  public void checkCompatibleConcretes(DiagnosticPosition pos, Type site) {
    ...
    for (Type t1 = sup; t1.tsym.type.isParameterized(); t1 = types
        .supertype(t1)) {
      for (Scope.Entry e1 = t1.tsym.members().elems; e1 != null; e1 = e1.sibling) {
        Symbol s1 = e1.sym;
        if (s1.kind != MTH || (s1.flags() & (STATIC | SYNTHETIC | BRIDGE)) != 0
            || !s1.isInheritedIn(site.tsym, types)
            || ((MethodSymbol) s1).implementation(site.tsym, types, true) != s1)
          continue;
        ...
        for (Type t2 = sup; t2.tag == CLASS; t2 = types.supertype(t2)) {
          for (Scope.Entry e2 = t2.tsym.members().lookup(s1.name); e2.scope != null; e2 = e2
              .next()) {
            Symbol s2 = e2.sym;
            if (s2 == s1
                || s2.kind != MTH
                ...
                || ((MethodSymbol) s2).implementation(site.tsym, types, true) != s2)
              continue;
            Type st2 = types.memberType(t2, s2);
            if (types.overrideEquivalent(st1, st2))
              log.error(pos, "concrete.inheritance.conflict", s1, t1, s2, t2,
                  sup);
          }
        }
      }
   ...
   * with same name and arguments but incompatible return types.
   * 
   * @param pos
   *          Position to be used for error reporting.
   * @param t1
   ...
   */
  public boolean checkCompatibleAbstracts(DiagnosticPosition pos, Type t1,
      Type t2) {
    return checkCompatibleAbstracts(pos, t1, t2, types.makeCompoundType(t1, t2));
  }
   ...
   */
  private Symbol firstIncompatibility(DiagnosticPosition pos, Type t1, Type t2,
      Type site) {
  ...
  /**
   * Compute all the supertypes of t, indexed by type symbol (except thise in
   * typesSkip).
   */
  ...
  private Symbol firstDirectIncompatibility(DiagnosticPosition pos, Type t1,
      Type t2, Type site) {
    for (Scope.Entry e1 = t1.tsym.members().elems; e1 != null; e1 = e1.sibling) {
      Symbol s1 = e1.sym;
      Type st1 = null;
      ...
      for (Scope.Entry e2 = t2.tsym.members().lookup(s1.name); e2.scope != null; e2 = e2
        ...
        if (types.overrideEquivalent(st1, st2)) {
          ...
          boolean compat = types.isSameType(rt1, rt2)
              || rt1.tag >= CLASS
              && rt2.tag >= CLASS
              && (types.covariantReturnType(rt1, rt2, Warner.noWarnings) || types
                  .covariantReturnType(rt2, rt1, Warner.noWarnings))
              || checkCommonOverriderIn(s1, s2, site);
          if (!compat) {
            log.error(pos, "types.incompatible.diff.ret", t1, t2, s2.name + "("
                + types.memberType(t2, s2).getParameterTypes() + ")");
            return s2;
          }
        } else if (checkNameClash((ClassSymbol) site.tsym, s1, s2)
            && !checkCommonOverriderIn(s1, s2, site)) {
          log.error(pos, "name.clash.same.erasure.no.override", s1,
              s1.location(), s2, s2.location());
          return s2;
        }
      }
    }
  ...
  boolean checkCommonOverriderIn(Symbol s1, Symbol s2, Type site) {
    ...
    for (Type t : supertypes.values()) {
      for (Scope.Entry e = t.tsym.members().lookup(s1.name); e.scope != null; e = e
          .next()) {
        Symbol s3 = e.sym;
        if (s3 == s1 || s3 == s2 || s3.kind != MTH
            || (s3.flags() & (BRIDGE | SYNTHETIC)) != 0)
          continue;
        ...
        if (types.overrideEquivalent(st3, st1)
          ...
          boolean compat = rt13.tag >= CLASS
              && rt23.tag >= CLASS
              && (types.covariantReturnType(rt13, rt1, Warner.noWarnings) && types
                  .covariantReturnType(rt23, rt2, Warner.noWarnings));
          if (compat)
            return true;
        }
      }
  ...
  private boolean checkNameClash(ClassSymbol origin, Symbol s1, Symbol s2) {
    ...
    return (cf.accepts(s1) && cf.accepts(s2) && types.hasSameArgs(
        s1.erasure(types), s2.erasure(types)));
  }
    ...
    try {
      ...
      if (undef != null) {
        if ((c.flags() & ENUM) != 0
          ...
          log.error(pos, "does.not.override.abstract", c, undef1,
              undef1.location());
        }
      }
    } catch (CompletionFailure ex) {
  ...
  private MethodSymbol firstUndef(ClassSymbol impl, ClassSymbol c) {
    ...
    if (c == impl || (c.flags() & (ABSTRACT | INTERFACE)) != 0) {
      ...
      for (List<Type> l = types.interfaces(c.type); undef == null
          && l.nonEmpty(); l = l.tail) {
  ...
  private void checkNonCyclic1(DiagnosticPosition pos, Type t,
      List<TypeVar> seen) {
    final TypeVar tv;
  ...
  private void noteCyclic(DiagnosticPosition pos, ClassSymbol c) {
    ...
    Type st = types.supertype(c.type);
    if (st.tag == CLASS)
      ((ClassType) c.type).supertype_field = types.createErrorType(
          (ClassSymbol) st.tsym, Type.noType);
      ...
      if ((allowGenerics || origin != lc) && (lc.flags() & ABSTRACT) != 0) {
        for (Scope.Entry e = lc.members().elems; e != null; e = e.sibling) {
          if (e.sym.kind == MTH
            ...
            MethodSymbol absmeth = (MethodSymbol) e.sym;
            MethodSymbol implmeth = absmeth
                .implementation(origin, types, false);
            if (implmeth != null
                && implmeth != absmeth
  ...
  void checkConflicts(DiagnosticPosition pos, Symbol sym, TypeSymbol c) {
    for (Type ct = c.type; ct != Type.noType; ct = types.supertype(ct)) {
      for (Scope.Entry e = ct.tsym.members().lookup(sym.name); e.scope == ct.tsym
          .members(); e = e.next()) {
        // VM allows methods and variables with differing types
        if (sym.kind == e.sym.kind
            && types.isSameType(types.erasure(sym.type),
  ...
  /**
   * Check that all non-override equivalent methods accessible from 'site' are
   * mutually compatible (JLS 8.4.8/9.4.1).
   * 
   * @param pos
   *          Position to be used for error reporting.
   * @param site
   ...
   * @param sym
   *          The method symbol to be checked.
   */
    ...
    for (Symbol s1 : types.membersClosure(site, false).getElementsByName(
      ...
      for (Symbol s2 : types.membersClosure(site, false).getElementsByName(
        ...
        if (!types.isSubSignature(sym.type, types.memberType(site, s1), false)
          ...
          String key = s2 == sym ? "name.clash.same.erasure.no.override"
              : "name.clash.same.erasure.no.override.1";
          log.error(pos, key, sym, sym.location(), s1, s1.location(), s2,
              s2.location());
          return;
        }
      }
  ...
  /**
   * Check that all static methods accessible from 'site' are mutually
   * compatible (JLS 8.4.8).
   * 
   * @param pos
   *          Position to be used for error reporting.
   * @param site
  ...
  void checkHideClashes(DiagnosticPosition pos, Type site, MethodSymbol sym) {
    ...
    for (Symbol s : types.membersClosure(site, true).getElementsByName(
      ...
      if (!types.isSubSignature(sym.type, types.memberType(site, s), false)
        ...
        log.error(pos, "name.clash.same.erasure.no.hide", sym, sym.location(),
            s, s.location());
        return;
      }
    }
  ...
  private class ClashFilter implements Filter<Symbol> {
    ...
    public boolean accepts(Symbol s) {
      return s.kind == MTH && (s.flags() & SYNTHETIC) == 0 && !shouldSkip(s)
          && s.isInheritedIn(site.tsym, types) && !s.isConstructor();
    }
  }
  ...
  /**
   * Check that class c does not implement directly or indirectly the same
   * parameterized interface with two different argument lists.
   * 
   * @param pos
   *          Position to be used for error reporting.
   * @param type
   *          The type whose interfaces are checked.
   */
  ...
  // where
  /**
   * Enter all interfaces of type `type' into the hash table `seensofar' with
   * their class symbol as key and their type as value. Make sure no class is
   * entered with two different types.
   */
  void checkClassBounds(DiagnosticPosition pos,
      Map<TypeSymbol, Type> seensofar, Type type) {
    if (type.isErroneous())
      return;
    for (List<Type> l = types.interfaces(type); l.nonEmpty(); l = l.tail) {
      ...
      if (oldit != null) {
        ...
        if (!types.containsTypeEquivalent(oldparams, newparams))
          log.error(pos, "cant.inherit.diff.arg", it.tsym,
              Type.toString(oldparams), Type.toString(newparams));
      }
  ...
  /**
   * Annotation types are restricted to primitives, String, an enum, an
   * annotation, Class, Class<?>, Class<? extends Anything>, arrays of the
   * preceding.
   */
  ...
  void validateAnnotationType(DiagnosticPosition pos, Type type) {
    if (type.isPrimitive())
      return;
    if (types.isSameType(type, syms.stringType))
      return;
    if ((type.tsym.flags() & Flags.ENUM) != 0)
      return;
    if ((type.tsym.flags() & Flags.ANNOTATION) != 0)
      return;
    if (types.lowerBound(type).tsym == syms.classType.tsym)
      return;
  ...
  void validateAnnotationMethod(DiagnosticPosition pos, MethodSymbol m) {
    for (Type sup = syms.annotationType; sup.tag == CLASS; sup = types
        .supertype(sup)) {
      Scope s = sup.tsym.members();
      for (Scope.Entry e = s.lookup(m.name); e.scope != null; e = e.next()) {
        if (e.sym.kind == MTH && (e.sym.flags() & (PUBLIC | PROTECTED)) != 0
  ...
  boolean annotationApplicable(JCAnnotation a, Symbol s) {
    Attribute.Compound atTarget = a.annotationType.type.tsym
        .attribute(syms.annotationTargetType.tsym);
    if (atTarget == null)
      return true;
    ...
    for (Attribute app : arr.values) {
      ...
      if (e.value.name == names.TYPE) {
        ...
        if (s.kind == TYP
            || s.kind == VAR
            || (s.kind == MTH && !s.isConstructor() && s.type.getReturnType().tag != VOID))
          return true;
      } else
        return true; // recovery
    }
  ...
  public void validateAnnotation(JCAnnotation a) {
    ...
    Set<MethodSymbol> members = new TreeSet<MethodSymbol>(
    ...
    if (missingDefaults.nonEmpty()) {
      String key = (missingDefaults.size() > 1) ? "annotation.missing.default.value.1"
    ...
    if (a.args.head.getTag() != JCTree.ASSIGN)
  ...
  void checkDeprecatedAnnotation(DiagnosticPosition pos, Symbol s) {
    if (allowAnnotations && lint.isEnabled(LintCategory.DEP_ANN)
        && (s.flags() & DEPRECATED) != 0 && !syms.deprecatedType.isErroneous()
        && s.attribute(syms.deprecatedType.tsym) == null) {
      log.warning(LintCategory.DEP_ANN, pos, "missing.deprecated.annotation");
    }
  }

  void checkDeprecated(final DiagnosticPosition pos, final Symbol other,
      final Symbol s) {
    if ((s.flags() & DEPRECATED) != 0 && (other.flags() & DEPRECATED) == 0
   ...
   */
  int checkOperator(DiagnosticPosition pos, OperatorSymbol operator, int tag,
      Type left, Type right) {
    if (operator.opcode == ByteCodes.error) {
      log.error(pos, "operator.cant.be.applied.1", treeinfo.operatorName(tag),
          left, right);
    }
   ...
   * @param operand
   *          The right hand operand for the expression
   */
  void checkDivZero(DiagnosticPosition pos, Symbol operator, Type operand) {
    if (operand.constValue() != null && lint.isEnabled(LintCategory.DIVZERO)
        && operand.tag <= LONG
  ...
  void checkEmptyIf(JCIf tree) {
    if (tree.thenpart.getTag() == JCTree.SKIP && tree.elsepart == null
  ...
  boolean checkUnique(DiagnosticPosition pos, Symbol sym, Scope s) {
    ...
    for (Scope.Entry e = s.lookup(sym.name); e.scope == s; e = e.next()) {
      if (sym != e.sym
          ...
          && (sym.kind != MTH || types.hasSameArgs(types.erasure(sym.type),
              types.erasure(e.sym.type)))) {
        if ((sym.flags() & VARARGS) != (e.sym.flags() & VARARGS)) {
          varargsDuplicateError(pos, sym, e.sym);
          return true;
        } else if (sym.kind == MTH
            && !types.hasSameArgs(sym.type, e.sym.type, false)) {
   ...
   * but make an exception for two single-type imports which denote the same
   * type.
   * 
   * @param pos
   *          Position for error reporting.
  ...
  /**
   * Check that static single-type import is not already imported or top-level
   * defined, but make an exception for two single-type imports which denote the
   * same type.
   * 
   * @param pos
   *          Position for error reporting.
   ...
   *          The scope
   * @param staticImport
   *          Whether or not this was a static import
   */
   ...
   *          Whether or not this was a static import
   */
  private boolean checkUniqueImport(DiagnosticPosition pos, Symbol sym,
      Scope s, boolean staticImport) {
  ...
  public void checkCanonical(JCTree tree) {
    if (!isCanonical(tree))
      log.error(tree.pos(), "import.requires.canonical", TreeInfo.symbol(tree));
  }
  ...
  private class ConversionWarner extends Warner {
    ...
    public ConversionWarner(DiagnosticPosition pos, String uncheckedKey,
    ...
    public void warn(LintCategory lint) {
      ...
      super.warn(lint);
      if (warned)
        return; // suppress redundant diagnostics
      switch (lint) {
      case UNCHECKED:
        Check.this.warnUnchecked(pos(), "prob.found.req",
            diags.fragment(uncheckedKey), found, expected);
        break;
      case VARARGS:
        if (method != null && method.attribute(syms.trustMeType.tsym) != null
            && isTrustMeAllowedOnMethod(method)
            && !types.isReifiable(method.type.getParameterTypes().last())) {
          Check.this.warnUnsafeVararg(pos(),
              "varargs.unsafe.use.varargs.param", method.params.last());
        }
        break;
      default:
