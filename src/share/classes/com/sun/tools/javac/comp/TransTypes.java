...
public class TransTypes extends TreeTranslator {
  ...
  protected static final Context.Key<TransTypes> transTypesKey = new Context.Key<TransTypes>();
  ...
  /**
   * Flag to indicate whether or not to generate bridge methods. For pre-Tiger
   * source there is no need for bridge methods, so it can be skipped to get
   * better performance for -source 1.4 etc.
   */
  ...
  /**
   * A hashtable mapping bridge methods to the methods they override after type
   * erasure.
   */
  ...
  /**
   * Construct an attributed tree to coerce an expression to some erased target
   * type, unless the expression is already assignable to that type. If target
   * type is a constant type, use its base type instead.
   * 
   * @param tree
   *          The expression tree.
   * @param target
  ...
  JCExpression coerce(JCExpression tree, Type target) {
    ...
    if (tree.type.isPrimitive() == target.isPrimitive()) {
      return types.isAssignable(tree.type, btarget, Warner.noWarnings) ? tree
          : cast(tree, btarget);
    }
    return tree;
  }
  ...
  /**
   * Given an erased reference type, assume this type as the tree's type. Then,
   * coerce to some given target type unless target type is null. This operation
   * is used in situations like the following:
   * 
   ...
   * @param target
   *          The target type, which is usually the erasure of the expression's
   *          original type.
   */
  ...
  /**
   * Translate method argument list, casting each argument to its corresponding
   * type in a list of target types.
   * 
   * @param _args
   *          The method argument list.
   * @param parameters
   *          The list of target types.
   * @param varargsElement
   *          The erasure of the varargs element type, or null if translating a
   *          non-varargs invocation
   */
  <T extends JCTree> List<T> translateArgs(List<T> _args,
      List<Type> parameters, Type varargsElement) {
  ...
  /**
   * Add a bridge definition and enter corresponding method symbol in local
   * scope of origin.
   * 
   * @param pos
   *          The source code position to be used for the definition.
   * @param meth
   *          The method for which a bridge needs to be added
   * @param impl
   *          That method's implementation (possibly the method itself)
   * @param origin
   *          The class to which the bridge will be added
   * @param hypothetical
   *          True if the bridge method is not strictly necessary in the binary,
   *          but is represented in the symbol table to detect erasure clashes.
   * @param bridges
   *          The list buffer to which the bridge will be added
   */
  void addBridge(DiagnosticPosition pos, MethodSymbol meth, MethodSymbol impl,
      ClassSymbol origin, boolean hypothetical, ListBuffer<JCTree> bridges) {
    make.at(pos);
    ...
    if (hypothetical)
      flags |= HYPOTHETICAL;
    MethodSymbol bridge = new MethodSymbol(flags, meth.name, bridgeType, origin);
    if (!hypothetical) {
      ...
      JCExpression receiver = (impl.owner == origin) ? make.This(origin
          .erasure(types)) : make.Super(
          types.supertype(origin.type).tsym.erasure(types), origin);
      ...
      JCStatement stat = (origErasure.getReturnType().tag == VOID) ? make
          .Exec(call) : make.Return(coerce(call, bridgeType.getReturnType()));
  ...
  /**
   * Add bridge if given symbol is a non-private, non-static member of the given
   * class, which is either defined in the class or non-final inherited, and one
   * of the two following conditions holds: 1. The method's type changes in the
   * given class, as compared to the class where the symbol was defined, (in
   * this case we have extended a parameterized class with non-trivial
   * parameters). 2. The method has an implementation with a different erased
   * return type. (in this case we have used co-variant returns). If a bridge
   * already exists in some other class, no new bridge is added. Instead, it is
   * checked that the bridge symbol overrides the method symbol. (Spec ???).
   * todo: what about bridges for privates???
   * 
   * @param pos
   *          The source code position to be used for the definition.
   * @param sym
   *          The symbol for which a bridge might have to be added.
   * @param origin
   *          The class in which the bridge would go.
   * @param bridges
   *          The list buffer to which the bridge would be added.
   */
  void addBridgeIfNeeded(DiagnosticPosition pos, Symbol sym,
      ClassSymbol origin, ListBuffer<JCTree> bridges) {
    if (sym.kind == MTH && sym.name != names.init
      ...
      if (bridge == null || bridge == meth
        ...
        } else if (impl == meth && impl.owner != origin
            && (impl.flags() & FINAL) == 0
        ...
        if (other != null && other != meth) {
          if (impl == null || !impl.overrides(other, origin, types, true)) {
            ...
            log.error(pos, "name.clash.same.erasure.no.override", other,
                other.location(origin.type, types), meth,
        ...
        if (bridge.owner == origin
          ...
          log.error(pos, "name.clash.same.erasure.no.override", bridge,
              bridge.location(origin.type, types), meth,
              meth.location(origin.type, types));
      }
    }
  }
   ...
   * @param dest
   *          The type in which the bridge would go
   */
  private boolean isBridgeNeeded(MethodSymbol method, MethodSymbol impl,
      Type dest) {
    if (impl != method) {
   ...
   * @param method
   *          the method to look for in class
   * @param erasure
   *          the erasure of method
   */
  private boolean isSameMemberWhenErased(Type type, MethodSymbol method,
      Type erasure) {
    return types.isSameType(erasure(types.memberType(type, method)), erasure);
  }

  void addBridges(DiagnosticPosition pos, TypeSymbol i, ClassSymbol origin,
   ...
   * 
   * @param pos
   *          The source code position to be used for the bridges.
   * @param origin
   *          The class in which the bridges go.
   * @param bridges
   *          The list buffer to which the bridges are added.
   */
  void addBridges(DiagnosticPosition pos, ClassSymbol origin,
      ListBuffer<JCTree> bridges) {
  ...
  public void visitMethodDef(JCMethodDecl tree) {
    ...
    for (Scope.Entry e = tree.sym.owner.members().lookup(tree.name); e.sym != null; e = e
        .next()) {
      if (e.sym != tree.sym && types.isSameType(erasure(e.sym.type), tree.type)) {
        log.error(tree.pos(), "name.clash.same.erasure", tree.sym, e.sym);
        return;
      }
    }
  }
  ...
  public void visitSwitch(JCSwitch tree) {
    ...
    boolean enumSwitch = selsuper != null && selsuper.tsym == syms.enumSym;
  ...
  public void visitReturn(JCReturn tree) {
    tree.expr = translate(tree.expr, currentMethod.sym.erasure(types)
        .getReturnType());
    result = tree;
  }
  ...
  public void visitNewClass(JCNewClass tree) {
    ...
    if (tree.varargsElement != null)
      tree.varargsElement = types.erasure(tree.varargsElement);
    tree.args = translateArgs(tree.args, tree.constructor.erasure(types)
        .getParameterTypes(), tree.varargsElement);
  ...
  public void visitSelect(JCFieldAccess tree) {
    ...
    if (t.isCompound()) {
      if ((tree.sym.flags() & IPROXY) != 0) {
        tree.sym = ((MethodSymbol) tree.sym).implemented(
            (TypeSymbol) tree.sym.owner, types);
      }
  ...
  private boolean boundsRestricted(ClassSymbol c) {
    ...
    if (st.isParameterized()) {
      ...
      while (!actuals.isEmpty() && !formals.isEmpty()) {
        ...
        if (!types.isSameType(types.erasure(actual), types.erasure(formal)))
          return true;
  ...
  class NeedsOverridBridgeFilter implements Filter<Symbol> {
    ...
    public boolean accepts(Symbol s) {
      return s.kind == MTH && !s.isConstructor() && s.isInheritedIn(c, types)
          && (s.flags() & FINAL) == 0
          && (s.flags() & (SYNTHETIC | OVERRIDE_BRIDGE)) != SYNTHETIC;
    }
  }

  private void addOverrideBridges(DiagnosticPosition pos, MethodSymbol impl,
      MethodSymbol member, ClassSymbol c, ListBuffer<JCTree> bridges) {
    ...
    JCExpression receiver = make.Super(
        types.supertype(c.type).tsym.erasure(types), c);
    Type calltype = erasure(impl.type.getReturnType());
    JCExpression call = make.Apply(
        null,
        make.Select(receiver, impl).setType(calltype),
        translateArgs(make.Idents(md.params), implErasure.getParameterTypes(),
            null)).setType(calltype);
    JCStatement stat = (member.getReturnType().tag == VOID) ? make.Exec(call)
