...
public class Infer {
  ...
  protected Infer(Context context) {
    ...
    invalidInstanceException = new InvalidInstanceException(diags);

  }

  public static class InferenceException extends
  ...
  public static class NoInstanceException extends InferenceException {
    ...
    boolean isAmbiguous; // exist several
                         // incomparable best
                         // instances?
   ...
   * its `inst' field. Throws a NoInstanceException if this not possible because
   * an `inst' field is null. Note: mutually referring undertvars will be left
   * uninstantiated (that is, they will be replaced by the underlying
   * type-variable).
   */
  ...
  Mapping getInstFun = new Mapping("getInstFun") {
    public Type apply(Type t) {
      ...
      case UNDETVAR:
        ...
        return isConstraintCyclic(that) ? that.qtype : apply(that.inst);
      default:
        return t.map(this);
      }
    }
    ...
    private boolean isConstraintCyclic(UndetVar uv) {
      Types.UnaryVisitor<Boolean> constraintScanner = new Types.UnaryVisitor<Boolean>() {
        ...
        public Boolean visitClassType(ClassType t, Void ignored) {
          if (t.isCompound()) {
            return visit(types.supertype(t)) || visit(types.interfaces(t));
          } else {
   ...
   * NoInstanceException if this not possible.
   */
  void maximizeInst(UndetVar that, Warner warn) throws NoInstanceException {
    ...
    if (that.inst == null || that.inst.isErroneous())
      throw ambiguousNoInstanceException.setMessage(
          "no.unique.maximal.instance.exists", that.qtype, hibounds);
  }
  ...
  void minimizeInst(UndetVar that, Warner warn) throws NoInstanceException {
    ...
    if (that.inst == null) {
      ...
      if (that.inst == null || that.inst.tag == ERROR)
        throw ambiguousNoInstanceException.setMessage(
            "no.unique.minimal.instance.exists", that.qtype, lobounds);
      ...
      if (hibounds.tail.isEmpty())
        ...
        for (List<Type> bs = hibounds; bs.nonEmpty() && hb == null; bs = bs.tail) {
          if (isSubClass(bs.head, hibounds))
            hb = types.fromUnknownFun.apply(bs.head);
        }
      if (hb == null || !types.isSubtypeUnchecked(hb, hibounds, warn)
   ...
   * instantiation exists which makes this type a subtype of type `to', return
   * the instantiated type. If no instantiation exists, or if several
   * incomparable best instantiations exist throw a NoInstanceException.
   */
  public Type instantiateExpr(ForAll that, Type to, Warner warn)
    ...
    if (!types.isSubtype(qtype1,
      ...
      throw unambiguousNoInstanceException.setMessage(
          "infer.no.conforming.instance.exists", that.tvars, that.qtype, to);
    }
    for (List<Type> l = undetvars; l.nonEmpty(); l = l.tail)
    ...
    if (Type.containsAny(targs, that.tvars)) {
      ...
      targs = types.subst(targs, that.tvars,
  ...
  // where
  private List<Type> instaniateAsUninferredVars(List<Type> undetvars,
      List<Type> tvars) {
    ListBuffer<Type> new_targs = ListBuffer.lb();
    // step 1 - create syntethic captured vars
    for (Type t : undetvars) {
      UndetVar uv = (UndetVar) t;
      Type newArg = new CapturedType(t.tsym.name, t.tsym, uv.inst,
          syms.botType, null);
    ...
    for (Type t : new_targs.toList()) {
      ...
      WildcardType wt = new WildcardType(ct.bound, BoundKind.EXTENDS,
          syms.boundClass);
      ct.wildcard = wt;
    }
   ...
   */
  public Type instantiateMethod(final Env<AttrContext> env, List<Type> tvars,
      MethodType mt, final Symbol msym, final List<Type> argtypes,
      final boolean allowBoxing, final boolean useVarargs, final Warner warn)
      throws InferenceException {
    ...
    if (varargsFormal == null && actuals.size() != formals.size()) {
    ...
    while (actuals.nonEmpty() && formals.head != varargsFormal) {
      ...
      boolean works = allowBoxing ? types.isConvertible(actual, undetFormal,
          warn) : types.isSubtypeUnchecked(actual, undetFormal, warn);
      if (!works) {
        throw unambiguousNoInstanceException.setMessage(
            "infer.no.conforming.assignment.exists", tvars, actualNoCapture,
            formal);
      }
        ...
        List<Type> getConstraints(TypeVar tv, ConstraintKind ck) {
          for (Type t : restundet.toList()) {
            ...
            if (uv.qtype == tv) {
              switch (ck) {
              case EXTENDS:
                return uv.hibounds.appendList(types.subst(types.getBounds(tv),
                    all_tvars, inferredTypes));
              case SUPER:
                return uv.lobounds;
              case EQUAL:
  ...
  /**
   * A delegated type representing a partially uninferred method type. The
   * return type of a partially uninferred method type is a ForAll type - when
   * the return type is instantiated (see Infer.instantiateExpr) the underlying
   * method type is also updated.
   */
  static abstract class UninferredMethodType extends DelegatedType {
    ...
    void instantiateReturnType(Type restype, List<Type> inferred, Types types)
      ...
      qtype = new MethodType(types.subst(getParameterTypes(), tvars, inferred),
          restype, types.subst(UninferredMethodType.this.getThrownTypes(),
              tvars, inferred), UninferredMethodType.this.qtype.tsym);
      check(inferred, types);
    }

    abstract void check(List<Type> inferred, Types types)
        throws NoInstanceException;
  ...
  private void checkArgumentsAcceptable(Env<AttrContext> env,
      List<Type> actuals, List<Type> formals, boolean allowBoxing,
      boolean useVarargs, Warner warn) {
    try {
      rs.checkRawArgumentsAcceptable(env, actuals, formals, allowBoxing,
          useVarargs, warn);
  ...
  /**
   * Try to instantiate argument type `that' to given type `to'. If this fails,
   * try to insantiate `that' to `to' where every occurrence of a type variable
   * in `tvars' is replaced by an unknown type.
   */
   ...
   */
  void checkWithinBounds(List<Type> tvars, List<Type> arguments, Warner warn)
    ...
    for (List<Type> tvs = tvars, args = arguments; tvs.nonEmpty(); tvs = tvs.tail, args = args.tail) {
      ...
      if (!types.isSubtypeUnchecked(args.head, bounds, warn))
        throw invalidInstanceException.setMessage(
            "inferred.do.not.conform.to.bounds", args.head, bounds);
    }
  }
  ...
  Type instantiatePolymorphicSignatureInstance(Env<AttrContext> env, Type site,
    ...
    case JCTree.TYPECAST:
      ...
      restype = (TreeInfo.skipParens(castTree.expr) == env.tree) ? castTree.clazz.type
          : syms.objectType;
      break;
    case JCTree.EXEC:
      JCTree.JCExpressionStatement execTree = (JCTree.JCExpressionStatement) env.next.tree;
      restype = (TreeInfo.skipParens(execTree.expr) == env.tree) ? syms.voidType
          : syms.objectType;
      break;
    default:
    ...
    List<Type> exType = spMethod != null ? spMethod.getThrownTypes() : List
        .of(syms.throwableType); // make it throw all exceptions

    MethodType mtype = new MethodType(paramtypes, restype, exType,
        syms.methodClass);
    return mtype;
  }
  ...
  Mapping implicitArgType = new Mapping("implicitArgType") {
    public Type apply(Type t) {
      t = types.erasure(t);
      if (t.tag == BOT)
        // nulls type as the marker type Null (which has
        // no instances)
