...
import com.sun.tools.javac.batch.BatchLower;
...
public class Lower extends TreeTranslator {
  ...
  public static Lower instance(Context context) {
    ...
    if (instance == null)
      instance = new BatchLower(context);
    return instance;
  }
  ...
  protected DiagnosticPosition make_pos;
  ...
  protected boolean debugLower;
  protected PkgInfo pkginfoOpt;
   ...
   */
  protected MethodSymbol currentMethodSym;
   ...
   */
  protected JCClassDecl classDef(ClassSymbol c) {
  ...
  /**
   * A hash table mapping class symbols to lists of free variables. accessed by
   * them. Only free variables of the method immediately containing a class are
  ...
  class FreeVarCollector extends TreeScanner {
    ...
    private void visitSymbol(Symbol _sym) {
      ...
      if (sym.kind == VAR || sym.kind == MTH) {
        ...
        if (sym != null && sym.owner == owner) {
          ...
          if (outerThisStack.head != null && outerThisStack.head != _sym)
    ...
    /**
     * If tree refers to a qualified this or super expression for anything but
     * the current class, add the outer this stack as a free variable.
     */
    ...
    /**
     * If tree refers to a superclass constructor call, add all free variables
     * of the superclass.
     */
  ...
  /**
   * Return the variables accessed from within a local class, which are declared
   * in the local class' owner. (in reverse order of first access).
   */
   ...
   * 
   * <p>
   * For each enum that appears as the type of a switch expression, we maintain
   * an EnumMapping to assist in the translation, as exemplified by the
   * following example:
   * 
   * <p>
   * we translate
  ...
  class EnumMapping {
    EnumMapping(DiagnosticPosition pos, TypeSymbol forEnum) {
      ...
      Name varName = names.fromString(target.syntheticNameChar()
          ...
          + writer.xClassName(forEnum.type).toString().replace('/', '.')
              .replace('.', target.syntheticNameChar()));
      ClassSymbol outerCacheClass = outerCacheClass();
      this.mapVar = new VarSymbol(STATIC | SYNTHETIC | FINAL, varName,
          new ArrayType(syms.intType, syms.arrayClass), outerCacheClass);
    ...
    int next = 1; // 0 (unused map elements) go to
                  // the default label
    ...
    void translate() {
      ...
      MethodSymbol valuesMethod = lookupMethod(pos, names.values, forEnum.type,
          List.<Type> nil());
      ...
      Symbol ordinalMethod = lookupMethod(pos, names.ordinal, forEnum.type,
          List.<Type> nil());
      List<JCCatch> catcher = List.<JCCatch> nil().prepend(
          make.Catch(make.VarDef(new VarSymbol(PARAMETER, names.ex,
              syms.noSuchFieldErrorType, syms.noSymbol), null), make.Block(0,
      ...
      for (Map.Entry<VarSymbol, Integer> e : values.entrySet()) {
        ...
        JCStatement _try = make
            .Try(make.Block(0, List.of(exec)), catcher, null);
        stmts.append(_try);
      }
  ...
  /**
   * Make an attributed tree representing a literal. This will be an Ident node
   * in the case of boolean literals, a Literal node in all other cases.
   * 
   * @param type
   *          The literal's type.
   * @param value
   ...
   * obtained by appending '$' chars at the end of the name until no match is
   * found.
   * 
   * @param name
   *          base name
   * @param s
   *          scope in which the name has to be unique
  ...
  /**
   * Check whether synthetic symbols generated during lowering conflict with
   * user-defined symbols.
   * 
   * @param translatedTrees
   *          lowered class trees
   */
   ...
   */
  protected MethodSymbol lookupConstructor(DiagnosticPosition pos, Type qual,
      List<Type> args) {
   ...
   * However, there is a small possibility that an existing class will not be
   * generated as expected if it is inside a conditional with a constant
  ...
  private void checkAccessConstructorTags() {
    for (List<ClassSymbol> l = accessConstrTags; l.nonEmpty(); l = l.tail) {
      ...
      if (isTranslatedClassAvailable(c))
        continue;
      // Create class definition tree.
      JCClassDecl cdef = make.ClassDef(make.Modifiers(STATIC | SYNTHETIC),
  ...
  private boolean isTranslatedClassAvailable(ClassSymbol c) {
    for (JCTree tree : translated) {
      if (tree.getTag() == JCTree.CLASSDEF && ((JCClassDecl) tree).sym == c) {
        return true;
      }
    }
    return false;
   ...
   * increment/decrement. Access codes for assignment operations are determined
   * by method accessCode below.
   * 
   * All access codes for accesses to the current class are even. If a member of
   * the superclass should be accessed instead (because access was via a
   * qualified super), add one to the corresponding code for the current class,
   * making the number odd. This numbering scheme is used by the backend to
   * decide whether to issue an invokevirtual or invokespecial call.
   * 
   * @see Gen.visitSelect(Select tree)
   */
  private static final int DEREFcode = 0, ASSIGNcode = 2, PREINCcode = 4,
      PREDECcode = 6, POSTINCcode = 8, POSTDECcode = 10, FIRSTASGOPcode = 12;
  ...
  /**
   * Map bytecode of binary operation to access code of corresponding assignment
   * operation. This is always an even number.
   */
   ...
   * @param enclOp
   *          The closest enclosing operation node of tree, null if tree is not
   *          a subtree of an operation.
   */
  ...
  MethodSymbol accessSymbol(Symbol sym, JCTree tree, JCTree enclOp,
    ...
    if (accessor == null) {
      accessor = new MethodSymbol(STATIC | SYNTHETIC, accessName(
          anum.intValue(), acode), new MethodType(argtypes, restype, thrown,
  ...
  /**
   * The qualifier to be used for accessing a symbol in an outer class. This is
   * either C.sym or C.this.sym, depending on whether or not sym is static.
   * 
   * @param sym
   *          The accessed symbol.
   */
  JCExpression accessBase(DiagnosticPosition pos, Symbol sym) {
    return (sym.flags() & STATIC) != 0 ? access(make.at(pos.getStartPosition())
        .QualIdent(sym.owner)) : makeOwnerThis(pos, sym, true);
  }
  ...
  boolean needsProtectedAccess(Symbol sym, JCTree tree) {
    if ((sym.flags() & PROTECTED) == 0 || sym.owner.owner == currentClass.owner
        || // fast special case
        sym.packge() == currentClass.packge())
      return false;
    if (!currentClass.isSubClass(sym.owner, types))
      return true;
    if ((sym.flags() & STATIC) != 0 || tree.getTag() != JCTree.SELECT
        || TreeInfo.name(((JCFieldAccess) tree).selected) == names._super)
      return false;
    return !((JCFieldAccess) tree).selected.type.tsym.isSubClass(currentClass,
        types);
  }
      ...
      if (sym.owner.kind == TYP) {
        ...
        if (accReq || baseReq) {
          ...
          if (accReq) {
            ...
            return make.at(tree.pos).Select(accessBase(tree.pos(), sym), sym)
                .setType(tree.type);
          }
        }
      }
  ...
  Symbol accessConstructor(DiagnosticPosition pos, Symbol constr) {
    if (needsPrivateAccess(constr)) {
      ...
      if (aconstr == null) {
        ...
        aconstr = new MethodSymbol(SYNTHETIC, names.init, new MethodType(
  ...
  void makeAccessible(Symbol sym) {
    ...
    if (sym.name == names.init) {
      ...
      for (int i = 0; i < NCODES; i++) {
        if (accessors[i] != null)
          cdef.defs = cdef.defs.prepend(accessDef(cdef.pos, sym, accessors[i],
              i));
      }
    }
  }
   ...
   * class, as well as its this$n symbol (if needed). Proxy scopes are nested in
   * the same way classes are. Inside a constructor, proxies and any this$n
   ...
   * for translated TWR blocks
   */
  Scope twrVars;

  /**
   * A stack containing the this$n field of the currently translated classes (if
   * needed) in innermost first order. Inside a constructor, proxies and any
   * this$n symbol are duplicated in an additional innermost scope, where they
   * represent the constructor parameters.
   */
   ...
   * is
   * 
   * { final VariableModifiers_minus_final R #resource = Expression; Throwable
   * #primaryException = null;
   * 
   * try ResourceSpecificationtail Block catch (Throwable #t) {
   * #primaryException = t; throw #t; } finally { if (#resource != null) { if
   * (#primaryException != null) { try { #resource.close(); } catch(Throwable
   * #suppressedException) {
   * #primaryException.addSuppressed(#suppressedException); } } else {
   * #resource.close(); } } }
   * 
   * @param tree
   *          The try statement to inspect.
   * @return A a desugared try-with-resources tree, or the original try block if
  ...
  private JCBlock makeTwrBlock(List<JCTree> resources, JCBlock block, int depth) {
    ...
    if (resource instanceof JCVariableDecl) {
      ...
      VarSymbol syntheticTwrVar = new VarSymbol(SYNTHETIC | FINAL,
          ...
          (resource.type.tag == TypeTags.BOT) ? syms.autoCloseableType
              : resource.type, currentMethodSym);
      twrVars.enter(syntheticTwrVar);
      JCVariableDecl syntheticTwrVarDecl = make.VarDef(syntheticTwrVar,
          (JCExpression) resource);
      expr = (JCExpression) make.Ident(syntheticTwrVar);
      stats.add(syntheticTwrVarDecl);
    }

    // Add primaryException declaration
    VarSymbol primaryException = new VarSymbol(SYNTHETIC, makeSyntheticName(
        names.fromString("primaryException" + depth), twrVars),
    ...
    VarSymbol param = new VarSymbol(FINAL | SYNTHETIC, names.fromString("t"
        + target.syntheticNameChar()), syms.throwableType, currentMethodSym);
    ...
    JCBlock catchBlock = make.Block(0L,
        List.<JCStatement> of(assign, rethrowStat));
  ...
  private JCBlock makeTwrFinallyClause(Symbol primaryException,
      JCExpression resource) {
    // primaryException.addSuppressed(catchException);
    VarSymbol catchException = new VarSymbol(0, make.paramName(2),
        syms.throwableType, currentMethodSym);
    JCStatement addSuppressionStatement = make.Exec(makeCall(
        make.Ident(primaryException), names.addSuppressed,
        List.<JCExpression> of(make.Ident(catchException))));

    // try { resource.close(); } catch (e) { primaryException.addSuppressed(e);
    // }
    JCBlock tryBlock = make.Block(0L,
        List.<JCStatement> of(makeResourceCloseInvocation(resource)));
    JCVariableDecl catchExceptionDecl = make.VarDef(catchException, null);
    JCBlock catchBlock = make.Block(0L,
        List.<JCStatement> of(addSuppressionStatement));
    List<JCCatch> catchClauses = List.<JCCatch> of(make.Catch(
        catchExceptionDecl, catchBlock));
    JCTry tryTree = make.Try(tryBlock, catchClauses, null);

    // if (primaryException != null) {try...} else resourceClose;
    JCIf closeIfStatement = make.If(
        makeNonNullCheck(make.Ident(primaryException)), tryTree,
        makeResourceCloseInvocation(resource));

    // if (#resource != null) { if (primaryException ... }
    return make.Block(0L, List.<JCStatement> of(make.If(
        makeNonNullCheck(resource), closeIfStatement, null)));
  }

  private JCStatement makeResourceCloseInvocation(JCExpression resource) {
    // create resource.close() method invocation
    JCExpression resourceClose = makeCall(resource, names.close,
   ...
   *          The accessed symbol.
   * @param preciseMatch
   *          should we accept a type that is a subtype of sym's owner, even if
   *          it doesn't contain sym due to hiding, overriding, or
  ...
  /**
   * Return tree simulating the assignment <this.name = name>, where name is the
  ...
  JCStatement initField(int pos, Name name) {
    ...
    return make.Exec(make.Assign(
        make.Select(make.This(lhs.owner.erasure(types)), lhs), make.Ident(rhs))
   ...
   * However, for backward compatibility in 1.4 and earlier we use the top-level
   * class itself.
   */
  ...
  private MethodSymbol classDollarSym(DiagnosticPosition pos) {
    ClassSymbol outerCacheClass = outerCacheClass();
    MethodSymbol classDollarSym = (MethodSymbol) lookupSynthetic(classDollar,
  ...
  JCBlock classDollarSymBody(DiagnosticPosition pos, JCMethodDecl md) {
    ...
    if (target.classLiteralsNoInit()) {
      ...
      VarSymbol clsym = new VarSymbol(STATIC | SYNTHETIC, names.fromString("cl"
          + target.syntheticNameChar()), syms.classLoaderType, outerCacheClass);
      ...
      Symbol forNameSym = lookupMethod(make_pos, names.forName,
          ...
          List.of(syms.stringType, syms.booleanType, syms.classLoaderType));
      // clvalue := "(cl$ == null) ?
              ...
              makeCall(
                  makeCall(
                      makeCall(newcache, names.getClass,
                          List.<JCExpression> nil()), names.getComponentType,
                      List.<JCExpression> nil()), names.getClassLoader, List
                      .<JCExpression> nil())).setType(syms.classLoaderType),
      ...
      returnResult = make.Block(
          0,
          List.of(make.Call(make. // return
              App(make.QualIdent(forNameSym),
                  List.<JCExpression> of(make.Ident(md.params.head.sym))))));
    }
    ...
    if (target.hasInitCause()) {
      ...
      JCTree throwExpr = makeCall(
          makeNewClass(syms.noClassDefFoundErrorType, List.<JCExpression> nil()),
      ...
      rethrow = make.Throw(makeNewClass(
  ...
  /**
   * The variable symbol that caches T.class values. If none exists yet, create
   * a definition.
   * 
   * @param sig
   *          The signature of type T.
  ...
  private JCExpression classOfType(Type type, DiagnosticPosition pos) {
    ...
    case VOID:
      ...
      Symbol typeSym = rs.access(
          rs.findIdentInType(attrEnv, c.type, names.TYPE, VAR), pos, c.type,
          names.TYPE, true);
      if (typeSym.kind == VAR)
    ...
    case ARRAY:
      if (target.hasClassLiterals()) {
        VarSymbol sym = new VarSymbol(STATIC | PUBLIC | FINAL, names._class,
      ...
      return make_at(pos).Conditional(
          ...
          make.Assign(
              ...
              make.App(
                  ...
                  List.<JCExpression> of(make.Literal(CLASS, sig).setType(
                      syms.stringType))))
              .setType(types.erasure(syms.classType)), make.Ident(cs)).setType(
          types.erasure(syms.classType));
    default:
      throw new AssertionError();
    }
  ...
  private JCExpression assertFlagTest(DiagnosticPosition pos) {
    ...
    if (assertDisabledSym == null) {
      ...
      Symbol desiredAssertionStatusSym = lookupMethod(pos,
          names.desiredAssertionStatus, types.erasure(syms.classType),
          List.<Type> nil());
      JCClassDecl containerDef = classDef(container);
      make_at(containerDef.pos());
      JCExpression notStatus = makeUnary(
          JCTree.NOT,
  ...
  JCTree abstractRval(JCTree rval, Type type, TreeBuilder builder) {
    ...
    VarSymbol var = new VarSymbol(FINAL | SYNTHETIC, names.fromString(target
        .syntheticNameChar() + "" + rval.hashCode()), type, currentMethodSym);
  ...
  public void visitClassDef(JCClassDecl tree) {
    ...
    List<JCVariableDecl> fvdefs = freevarDefs(tree.pos, freevars(currentClass),
        currentClass);
  ...
  private void visitEnumDef(JCClassDecl tree) {
    ...
    VarSymbol valuesVar = new VarSymbol(PRIVATE | FINAL | STATIC | SYNTHETIC,
        valuesName, arrayType, tree.type.tsym);
    JCNewArray newArray = make.NewArray(make.Type(types.erasure(tree.type)),
        List.<JCExpression> nil(), values.toList());
    ...
    if (useClone()) {
      ...
      JCTypeCast valuesResult = make.TypeCast(valuesSym.type.getReturnType(),
      ...
      VarSymbol resultVar = new VarSymbol(FINAL | SYNTHETIC, resultName,
          arrayType, valuesSym);
      JCNewArray resultArray = make.NewArray(
          make.Type(types.erasure(tree.type)),
          List.of(make.Select(make.Ident(valuesVar), syms.lengthVar)), null);
      ...
      if (systemArraycopyMethod == null) {
        systemArraycopyMethod = new MethodSymbol(PUBLIC | STATIC,
            names.fromString("arraycopy"), new MethodType(List.<Type> of(
                syms.objectType, syms.intType, syms.objectType, syms.intType,
                syms.intType), syms.voidType, List.<Type> nil(),
                syms.methodClass), syms.systemType.tsym);
      }
      JCStatement copy = make
          .Exec(make.App(make.Select(make.Ident(syms.systemType.tsym),
              systemArraycopyMethod), List.of(make.Ident(valuesVar),
              make.Literal(0), make.Ident(resultVar), make.Literal(0),
     ...
     * 
     * public static E valueOf(String name) { return (E)Enum.valueOf(E.class,
     * name); }
     * 
     * where E is tree.sym
     */
  ...
  public void visitMethodDef(JCMethodDecl tree) {
    if (tree.name == names.init && (currentClass.flags_field & ENUM) != 0) {
      ...
      JCVariableDecl nameParam = make_at(tree.pos())
          .Param(
              names.fromString(target.syntheticNameChar() + "enum"
                  + target.syntheticNameChar() + "name"), syms.stringType,
              tree.sym);
      ...
      JCVariableDecl ordParam = make
          .Param(
              names.fromString(target.syntheticNameChar() + "enum"
                  + target.syntheticNameChar() + "ordinal"), syms.intType,
              tree.sym);
  ...
  // where
  public void visitMethodDefInternal(JCMethodDecl tree) {
  ...
  private JCTree convert(JCTree tree, Type pt) {
    ...
    JCTree result = make_at(tree.pos()).TypeCast(make.Type(pt),
        (JCExpression) tree);
    result.type = (tree.type.constValue() != null) ? cfolder.coerce(tree.type,
        pt) : pt;
    return result;
  }
  ...
  <T extends JCTree> T boxIfNeeded(T tree, Type type) {
    ...
    if (havePrimitive) {
      ...
      if (unboxedTarget.tag != NONE) {
        if (!types.isSubtype(tree.type, unboxedTarget)) // e.g. Character c =
                                                        // 89;
  ...
  JCExpression boxPrimitive(JCExpression tree, Type box) {
    ...
    if (target.boxWithConstructors()) {
      Symbol ctor = lookupConstructor(tree.pos(), box, List.<Type> nil()
          .prepend(tree.type));
      return make.Create(ctor, List.of(tree)).setType(box);
    } else {
      Symbol valueOfSym = lookupMethod(tree.pos(), names.valueOf, box, List
          .<Type> nil().prepend(tree.type));
      return make.App(make.QualIdent(valueOfSym), List.of(tree)).setType(box);
    }
  }
    ...
    if (!tree.lhs.type.isPrimitive()
      ...
      JCTree newTree = abstractLval(tree.lhs, new TreeBuilder() {
        public JCTree build(final JCTree lhs) {
          ...
          Symbol newOperator = rs.resolveBinaryOperator(tree.pos(), newTag,
              attrEnv, tree.type, tree.rhs.type);
  ...
  private void visitArrayForeachLoop(JCEnhancedForLoop tree) {
    ...
    JCVariableDecl loopvardef = (JCVariableDecl) make.VarDef(tree.var.mods,
        tree.var.name, tree.var.vartype, loopvarinit).setType(tree.var.type);
  ...
  protected void patchTargets(JCTree body, final JCTree src, final JCTree dest) {
  ...
  private void visitIterableForeachLoop(JCEnhancedForLoop tree) {
    ...
    Symbol hasNext = lookupMethod(tree.expr.pos(), names.hasNext, itvar.type,
        List.<Type> nil());
    ...
    JCVariableDecl indexDef = (JCVariableDecl) make.VarDef(tree.var.mods,
        tree.var.name, tree.var.vartype, vardefinit).setType(tree.var.type);
  ...
  public void visitVarDef(JCVariableDecl tree) {
    ...
    if (currentMethodSym == null) {
      ...
      currentMethodSym = new MethodSymbol((tree.mods.flags & STATIC) | BLOCK,
  ...
  public void visitBlock(JCBlock tree) {
    ...
    if (currentMethodSym == null) {
      ...
      currentMethodSym = new MethodSymbol(tree.flags | BLOCK, names.empty,
          null, currentClass);
    }
  ...
  public JCTree visitStringSwitch(JCSwitch tree) {
    ...
    if (alternatives == 0) { // Strange but legal possibility
      ...
      /*
       * The general approach used is to translate a single string switch
       * statement into a series of two chained switch statements: the first a
       * synthesized statement switching on the argument string's hash value and
       * computing a string's position in the list of original case labels, if
       * any, followed by a second switch on the computed integer value. The
       * second switch has the same code structure as the original string switch
       * statement except that the string case labels are replaced with
       * positional integer constants starting at 0.
       * 
       * The first switch statement can be thought of as an inlined map from
       * strings to their position in the case label list. An alternate
       * implementation would use an actual Map for this purpose, as done for
       * enum switches.
       * 
       * With some additional effort, it would be possible to use a single
       * switch statement on the hash code of the argument, but care would need
       * to be taken to preserve the proper control flow in the presence of hash
       * collisions and other complications, such as fallthroughs. Switch
       * statements with one or two alternatives could also be specially
       * translated into if-then statements to omit the computation of the hash
       * code.
       * 
       * The generated code assumes that the hashing algorithm of String is the
       * same in the compilation environment as in the environment the code will
       * run in. The string hashing algorithm in the SE JDK has been unchanged
       * since at least JDK 1.2. Since the algorithm has been specified since
       * that release as well, it is very unlikely to be changed in the future.
       * 
       * Different hashing algorithms, such as the length of the strings or a
       * perfect hashing algorithm over the particular set of case labels, could
       * potentially be used instead of String.hashCode.
       */
      ...
      Map<String, Integer> caseLabelToPosition = new LinkedHashMap<String, Integer>(
          alternatives + 1, 1.0f);

      // Map of hash codes to the string case labels having that hashCode.
      Map<Integer, Set<String>> hashToString = new LinkedHashMap<Integer, Set<String>>(
          alternatives + 1, 1.0f);
      ...
      /*
       * s$ = top of stack; tmp$ = -1; switch($s.hashCode()) { case
       * caseLabel.hashCode: if (s$.equals("caseLabel_1") tmp$ =
       * caseLabelToPosition("caseLabel_1"); else if (s$.equals("caseLabel_2"))
       * tmp$ = caseLabelToPosition("caseLabel_2"); ... break; ... }
       */

      VarSymbol dollar_s = new VarSymbol(FINAL | SYNTHETIC,
          names.fromString("s" + tree.pos + target.syntheticNameChar()),
          syms.stringType, currentMethodSym);
      stmtList.append(make.at(tree.pos())
          .VarDef(dollar_s, tree.getExpression()).setType(dollar_s.type));

      VarSymbol dollar_tmp = new VarSymbol(SYNTHETIC, names.fromString("tmp"
          + tree.pos + target.syntheticNameChar()), syms.intType,
          currentMethodSym);
      JCVariableDecl dollar_tmp_def = (JCVariableDecl) make.VarDef(dollar_tmp,
          make.Literal(INT, -1)).setType(dollar_tmp.type);
      ...
      JCMethodInvocation hashCodeCall = makeCall(make.Ident(dollar_s),
          names.hashCode, List.<JCExpression> nil()).setType(syms.intType);
      ...
      for (Map.Entry<Integer, Set<String>> entry : hashToString.entrySet()) {
        ...
        for (String caseLabel : stringsWithHashCode) {
          JCMethodInvocation stringEqualsCall = makeCall(make.Ident(dollar_s),
              names.equals, List.<JCExpression> of(make.Literal(caseLabel)));
          elsepart = make.If(
              stringEqualsCall,
              make.Exec(make.Assign(make.Ident(dollar_tmp),
                  make.Literal(caseLabelToPosition.get(caseLabel))).setType(
                  dollar_tmp.type)), elsepart);
        }
   ...
   * 
   * @param env
   *          The attribution environment current at the class definition. We
   *          need this for resolving some additional symbols.
   * @param cdef
   *          The tree representing the class definition.
   */
  public List<JCTree> translateTopLevelClass(Env<AttrContext> env, JCTree cdef,
      TreeMaker make) {
  ...
  private MethodSymbol addEnumToString(JCClassDecl cdef, VarSymbol nameSymbol) {
    Symbol toStringSym = lookupMethod(cdef.pos(), names.toString, cdef.type,
        List.<Type> nil());
  ...
  private MethodSymbol addEnumCompareTo(JCClassDecl cdef,
      VarSymbol ordinalSymbol) {
    Symbol compareToSym = lookupMethod(cdef.pos(), names.compareTo, cdef.type,
