 ...
 * 
 * <p>
 * The JLS has a number of problems in the specification of these flow analysis
 * problems. This implementation attempts to address those issues.
 * 
 * <p>
 ...
 * 
 * <p>
 * There is no spec in the JLS for when a variable is definitely assigned at the
...
public class Flow extends TreeScanner {
   ...
   * reachable code. To obtain the correct set of variables which are definitely
   * unassigned anywhere in current try block, intersect uninitsTry and uninits.
   */
  Bits uninitsTry;
   ...
   * not known to be caught. This will typically result in an error unless it is
   * within a try-finally whose finally block cannot complete normally.
   */
   ...
   */
  void errorUncaught() {
    for (PendingExit exit = pendingExits.next(); exit != null; exit = pendingExits
        .next()) {
      if (classDef != null && classDef.pos == exit.tree.pos) {
        log.error(exit.tree.pos(), "unreported.exception.default.constructor",
            exit.thrown);
      } else if (exit.tree.getTag() == JCTree.VARDEF
          && ((JCVariableDecl) exit.tree).sym.isResourceVariable()) {
        log.error(exit.tree.pos(), "unreported.exception.implicit.close",
            exit.thrown, ((JCVariableDecl) exit.tree).sym.name);
      } else {
   ...
   * a local or a blank final variable?
   */
  ...
  /**
   * Initialize new trackable variable by setting its address field to the next
   * available sequence number and entering it under that index into the vars
   * array.
   */
    ...
    if (sym.adr >= firstadr && trackable(sym)) {
      if ((sym.flags() & FINAL) != 0) {
        if ((sym.flags() & PARAMETER) != 0) {
          ...
          log.error(pos, loopPassTwo ? "var.might.be.assigned.in.loop"
      ...
      if (tree.name != names.empty) {
        ...
        for (List<JCTree> l = tree.defs; l.nonEmpty(); l = l.tail) {
          if (TreeInfo.isInitialConstructor(l.head)) {
            List<Type> mthrown = ((JCMethodDecl) l.head).sym.type
                .getThrownTypes();
            if (firstConstructor) {
              caught = mthrown;
      ...
      if (tree.name == names.empty) {
        for (List<JCTree> l = tree.defs; l.nonEmpty(); l = l.tail) {
          if (TreeInfo.isInitialConstructor(l.head)) {
            ...
            mdef.sym.type = types.createMethodTypeWithThrown(mdef.sym.type,
                thrown);
          }
        }
  ...
  public void visitSwitch(JCSwitch tree) {
    ...
    for (List<JCCase> l = tree.cases; l.nonEmpty(); l = l.tail) {
      ...
      if (!loopPassTwo && alive
          && lint.isEnabled(Lint.LintCategory.FALLTHROUGH)
          && c.stats.nonEmpty() && l.tail.nonEmpty())
        log.warning(Lint.LintCategory.FALLTHROUGH, l.tail.head.pos(),
  ...
  public void visitTry(JCTry tree) {
    ...
    for (List<JCCatch> l = tree.catchers; l.nonEmpty(); l = l.tail) {
      List<JCExpression> subClauses = TreeInfo.isMultiCatch(l.head) ? ((JCTypeUnion) l.head.param.vartype).alternatives
    ...
    for (JCTree resource : tree.resources) {
      List<Type> closeableSupertypes = resource.type.isCompound() ? types
          .interfaces(resource.type).prepend(types.supertype(resource.type))
          : List.of(resource.type);
      for (Type sup : closeableSupertypes) {
        if (types.asSuper(sup, syms.autoCloseableType.tsym) != null) {
          Symbol closeMethod = rs.resolveQualifiedMethod(tree, attrEnv, sup,
              names.close, List.<Type> nil(), List.<Type> nil());
    ...
    scanStat(tree.body);
    List<Type> thrownInTry = allowImprovedCatchAnalysis ? chk.union(thrown,
        List.of(syms.runtimeExceptionType, syms.errorType)) : thrown;
    thrown = thrownPrev;
    caught = caughtPrev;
      ...
      if (!alive) {
        ...
        if (!loopPassTwo && lint.isEnabled(Lint.LintCategory.FINALLY)) {
          log.warning(Lint.LintCategory.FINALLY,
              TreeInfo.diagEndPos(tree.finalizer), "finally.cannot.complete");
        }
      } else {
  ...
  void checkCaughtType(DiagnosticPosition pos, Type exc,
      List<Type> thrownInTry, List<Type> caughtInTry) {
    if (chk.subset(exc, caughtInTry)) {
      log.error(pos, "except.already.caught", exc);
    } else if (!chk.isUnchecked(pos, exc) && !isExceptionOrThrowable(exc)
        && !chk.intersects(exc, thrownInTry)) {
      log.error(pos, "except.never.thrown.in.try", exc);
    } else if (allowImprovedCatchAnalysis) {
      List<Type> catchableThrownTypes = chk
          .intersect(List.of(exc), thrownInTry);
      // 'catchableThrownTypes' cannnot possibly be empty - if 'exc' was an
      // unchecked exception, the result list would not be empty, as the
      // augmented
      // thrown set includes { RuntimeException, Error }; if 'exc' was a checked
      // exception, that would have been covered in the branch above
      if (chk.diff(catchableThrownTypes, caughtInTry).isEmpty()
          && !isExceptionOrThrowable(exc)) {
        String key = catchableThrownTypes.length() == 1 ? "unreachable.catch"
            : "unreachable.catch.1";
  ...
  public void visitThrow(JCThrow tree) {
    ...
    if (sym != null && sym.kind == VAR
        ...
        && preciseRethrowTypes.get(sym) != null && allowImprovedRethrowAnalysis) {
  ...
  public void visitNewClass(JCNewClass tree) {
    ...
    try {
      ...
      if (tree.def != null)
        for (List<Type> l = tree.constructor.type.getThrownTypes(); l
            .nonEmpty(); l = l.tail) {
  ...
  public void visitTypeCast(JCTypeCast tree) {
    ...
    if (!tree.type.isErroneous() && lint.isEnabled(Lint.LintCategory.CAST)
      ...
      log.warning(Lint.LintCategory.CAST, tree.pos(), "redundant.cast",
          tree.expr.type);
    }
  }
  ...
  private boolean is292targetTypeCast(JCTypeCast tree) {
    ...
    if (expr.getTag() == JCTree.APPLY) {
      ...
      is292targetTypeCast = sym != null && sym.kind == MTH
