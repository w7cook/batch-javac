...
/**
 * This is the second phase of Enter, in which classes are completed by entering
 * their members into the class scope using MemberEnter.complete(). See Enter
 * for an overview.
 * 
 * <p>
...
public class MemberEnter extends JCTree.Visitor implements Completer {
  protected static final Context.Key<MemberEnter> memberEnterKey = new Context.Key<MemberEnter>();
  ...
  /**
   * A queue for classes whose members still need to be entered into the symbol
   * table.
   */
  ListBuffer<Env<AttrContext>> halfcompleted = new ListBuffer<Env<AttrContext>>();

  /**
   * Set to true only when the first of a set of classes is processed from the
   * halfcompleted queue.
   */
  boolean isFirst = true;

  /**
   * A flag to disable completion from time to time during member enter, as we
   * only need to look up types. This avoids unnecessarily deep recursion.
   */
   ...
   */
  private void importAll(int pos, final TypeSymbol tsym, Env<AttrContext> env) {
   ...
   *          The (import) scope in which imported classes are entered.
   */
  private void importStaticAll(int pos, final TypeSymbol tsym,
      Env<AttrContext> env) {
    ...
    new Object() {
      ...
      void importFrom(TypeSymbol tsym) {
        ...
        for (Scope.Entry e = fromScope.elems; e != null; e = e.sibling) {
          Symbol sym = e.sym;
          if (sym.kind == TYP && (sym.flags() & STATIC) != 0
              && staticImportAccessible(sym, packge)
              && sym.isMemberOf(origin, types) && !toScope.includes(sym))
      ...
      void importFrom(TypeSymbol tsym) {
        ...
        for (Scope.Entry e = fromScope.elems; e != null; e = e.sibling) {
          ...
          if (sym.isStatic() && sym.kind != TYP
              && staticImportAccessible(sym, packge) && !toScope.includes(sym)
   ...
   *          Position to be used for error reporting.
   * @param tsym
   *          The class from which the name is imported.
   * @param name
   *          The (simple) name being imported.
   * @param env
   *          The environment containing the named import scope to add to.
   */
  private void importNamedStatic(final DiagnosticPosition pos,
      final TypeSymbol tsym, final Name name, final Env<AttrContext> env) {
    if (tsym.kind != TYP) {
    ...
    new Object() {
      ...
      void importFrom(TypeSymbol tsym) {
        ...
        for (Scope.Entry e = tsym.members().lookup(name); e.scope != null; e = e
            .next()) {
          Symbol sym = e.sym;
          if (sym.isStatic() && sym.kind == TYP
              && staticImportAccessible(sym, packge)
    ...
    annotate.earlier(new Annotate.Annotator() {
      ...
      void importFrom(TypeSymbol tsym) {
        ...
        for (Scope.Entry e = tsym.members().lookup(name); e.scope != null; e = e
            .next()) {
          Symbol sym = e.sym;
          if (sym.isStatic() && staticImportAccessible(sym, packge)
              && sym.isMemberOf(origin, types)) {
            found = true;
            if (sym.kind == MTH || sym.kind != TYP
                && chk.checkUniqueStaticImport(pos, sym, toScope))
              toScope.enter(sym, sym.owner.members(), origin.members());
          }
        }
      ...
      public void enterAnnotation() {
        ...
        try {
          importFrom(tsym);
          if (!found) {
            log.error(pos, "cant.resolve.location", KindName.STATIC, name,
                List.<Type> nil(), List.<Type> nil(),
                Kinds.typeKindName(tsym.type), tsym.type);
          }
        } finally {
          log.useSource(prev);
        }
   ...
   *          The environment containing the named import scope to add to.
   */
  private void importNamed(DiagnosticPosition pos, Symbol tsym,
      Env<AttrContext> env) {
    if (tsym.kind == TYP
  ...
  Type signature(List<JCTypeParameter> typarams, List<JCVariableDecl> params,
      JCTree res, List<JCExpression> thrown, Env<AttrContext> env) {
  ...
  /**
   * Enter field and method definitions and process import clauses, catching any
   * completion failure exceptions.
   */
  ...
  private void addEnumMembers(JCClassDecl tree, Env<AttrContext> env) {
    JCExpression valuesType = make.Type(new ArrayType(tree.sym.type,
        syms.arrayClass));

    // public static T[] values() { return ???; }
    JCMethodDecl values = make.MethodDef(
        make.Modifiers(Flags.PUBLIC | Flags.STATIC), names.values, valuesType,
    ...
    JCMethodDecl valueOf = make.MethodDef(
        make.Modifiers(Flags.PUBLIC | Flags.STATIC),
        names.valueOf,
        make.Type(tree.sym.type),
        List.<JCTypeParameter> nil(),
        List.of(make.VarDef(make.Modifiers(Flags.PARAMETER),
            names.fromString("name"), make.Type(syms.stringType), null)),
        List.<JCExpression> nil(), // thrown
        null, // make.Block(0,
              // Tree.emptyList.prepend(make.Return(make.Ident(names._null)))),
        null);
    ...
    JCMethodDecl ordinal = make.at(tree.pos).MethodDef(
        make.Modifiers(Flags.PUBLIC | Flags.FINAL), names.ordinal,
        make.Type(syms.intType), List.<JCTypeParameter> nil(),
        List.<JCVariableDecl> nil(), List.<JCExpression> nil(), null, null);
    memberEnter(ordinal, env);

    // public final String name() { return ???; }
    JCMethodDecl name = make.MethodDef(
        make.Modifiers(Flags.PUBLIC | Flags.FINAL), names._name,
        make.Type(syms.stringType), List.<JCTypeParameter> nil(),
        List.<JCVariableDecl> nil(), List.<JCExpression> nil(), null, null);
    memberEnter(name, env);

    // public int compareTo(E other) { return ???; }
    MethodSymbol compareTo = new MethodSymbol(Flags.PUBLIC, names.compareTo,
        new MethodType(List.of(tree.sym.type), syms.intType, List.<Type> nil(),
            syms.methodClass), tree.sym);
  ...
  public void visitImport(JCImport tree) {
    ...
    p = attr.attribTree(s.selected, localEnv, tree.staticImport ? TYP
        : (TYP | PCK), Type.noType).tsym;
  ...
  public void visitMethodDef(JCMethodDecl tree) {
    ...
    DeferredLintHandler prevLintHandler = chk
        .setDeferredLintHandler(deferredLintHandler.setPos(tree.pos()));
    try {
      // Compute the method type
      m.type = signature(tree.typarams, tree.params, tree.restype, tree.thrown,
          localEnv);
    } finally {
  ...
  public void visitVarDef(JCVariableDecl tree) {
    ...
    if ((tree.mods.flags & VARARGS) != 0) {
      ...
      // to do it this way because varargs is represented in the tree as a
      // modifier
  ...
  /**
   * Create a fresh environment for a variable's initializer. If the variable is
   * a field, the owner of the environment's scope is be the variable itself,
   * otherwise the owner is the method enclosing the variable definition.
   * 
   * @param tree
  ...
  Env<AttrContext> initEnv(JCVariableDecl tree, Env<AttrContext> env) {
    Env<AttrContext> localEnv = env.dupto(new AttrContextEnv(tree, env.info
        .dup()));
  ...
  public Env<AttrContext> getMethodEnv(JCMethodDecl tree, Env<AttrContext> env) {
    ...
    mEnv.info.lint = mEnv.info.lint.augment(tree.sym.attributes_field,
        tree.sym.flags());
  ...
  void annotateLater(final List<JCAnnotation> annotations,
      final Env<AttrContext> localEnv, final Symbol s) {
    ...
    annotate.later(new Annotate.Annotator() {
      ...
      public void enterAnnotation() {
        ...
        try {
          if (s.attributes_field != null && s.attributes_field.nonEmpty()
  ...
  private void enterAnnotations(List<JCAnnotation> annotations,
      Env<AttrContext> env, Symbol s) {
    ListBuffer<Attribute.Compound> buf = new ListBuffer<Attribute.Compound>();
    Set<TypeSymbol> annotated = new HashSet<TypeSymbol>();
    if (!skipAnnotations)
      for (List<JCAnnotation> al = annotations; al.nonEmpty(); al = al.tail) {
        JCAnnotation a = al.head;
        Attribute.Compound c = annotate.enterAnnotation(a, syms.annotationType,
            env);
        if (c == null)
          continue;
        buf.append(c);
        // Note: @Deprecated has no effect on local variables and parameters
        if (!c.type.isErroneous() && s.owner.kind != MTH
  ...
  void annotateDefaultValueLater(final JCExpression defaultValue,
      final Env<AttrContext> localEnv, final MethodSymbol m) {
    annotate.later(new Annotate.Annotator() {
      public String toString() {
  ...
  public void complete(Symbol sym) throws CompletionFailure {
    ...
    try {
      ...
      // Determine supertype.
      Type supertype = (tree.extending != null) ? attr.attribBase(
          tree.extending, baseEnv, true, false, true)
          : ((tree.mods.flags & Flags.ENUM) != 0 && !target
              .compilerBootstrap(c)) ? attr.attribBase(enumBase(tree.pos, c),
              baseEnv, true, false, false)
      ...
      if ((tree.mods.flags & Flags.ENUM) != 0 && target.compilerBootstrap(c)) {
        ...
        interfaceTrees = interfaceTrees.prepend(make.Type(new ClassType(
            syms.comparableType.getEnclosingType(), List.of(c.type),
            syms.comparableType.tsym)));
        // add interface Serializable
        interfaceTrees = interfaceTrees.prepend(make
            .Type(syms.serializableType));
      }
      ...
      if ((c.flags_field & ANNOTATION) != 0) {
        ...
        ct.all_interfaces_field = (all_interfaces == null) ? ct.interfaces_field
            : all_interfaces.toList();
      }
      ...
      if ((c.flags_field & INTERFACE) == 0) {
        VarSymbol thisSym = new VarSymbol(FINAL | HASINIT, names._this, c.type,
            c);
        thisSym.pos = Position.FIRSTPOS;
        env.info.scope.enter(thisSym);
        if (ct.supertype_field.tag == CLASS) {
          VarSymbol superSym = new VarSymbol(FINAL | HASINIT, names._super,
      ...
      if (checkClash && c.owner.kind == PCK && c.owner != syms.unnamedPackage
          && reader.packageExists(c.fullname)) {
        log.error(tree.pos, "clash.with.pkg.of.same.name", Kinds.kindName(sym),
            c);
      }
  ...
  private Env<AttrContext> baseEnv(JCClassDecl tree, Env<AttrContext> env) {
    ...
    Env<AttrContext> outer = env.outer; // the base clause can't see members of
                                        // this class
   ...
   * 
   * @param pos
   *          The position for trees and diagnostics, if any
   * @param c
   *          The class symbol of the enum
   */
  private JCExpression enumBase(int pos, ClassSymbol c) {
    JCExpression result = make.at(pos).TypeApply(make.QualIdent(syms.enumSym),
        List.<JCExpression> of(make.Type(c.type)));
    return result;
  }

  Type modelMissingTypes(Type t, final JCExpression tree,
      final boolean interfaceExpected) {
    if (t.tag != ERROR)
      return t;
    ...
    return new ErrorType(((ErrorType) t).getOriginalType(), t.tsym) {
      ...
      public Type getModelType() {
        if (modelType == null)
          modelType = new Synthesizer(getOriginalType(), interfaceExpected)
              .visit(tree);
        return modelType;
      }
    };
    ...
    public void visitTypeApply(JCTypeApply tree) {
      if (tree.type.tag != ERROR) {
        ...
        if (synthesizedSymbols.contains(clazzType.tsym))
          synthesizeTyparams((ClassSymbol) clazzType.tsym,
              tree.arguments.size());
   ...
   */
  JCTree DefaultConstructor(TreeMaker make, ClassSymbol c, List<Type> typarams,
      List<Type> argtypes, List<Type> thrown, long flags, boolean based) {
    List<JCVariableDecl> params = make.Params(argtypes, syms.noSymbol);
    List<JCStatement> stats = List.nil();
    if (c.type != syms.objectType)
      stats = stats.prepend(SuperCall(make, typarams, params, based));
    if ((c.flags() & ENUM) != 0
        && (types.supertype(c.type).tsym == syms.enumSym || target
            .compilerBootstrap(c))) {
    ...
    if (c.name.isEmpty())
      flags |= ANONCONSTR;
    JCTree result = make.MethodDef(make.Modifiers(flags), names.init, null,
        make.TypeParams(typarams), params, make.Types(thrown),
        make.Block(0, stats), null);
    return result;
  }
   ...
   * @param typarams
   *          The type parameters that need to be passed to super
   * @param based
   *          Is first parameter a this$n?
   */
  JCExpressionStatement SuperCall(TreeMaker make, List<Type> typarams,
      List<JCVariableDecl> params, boolean based) {
    JCExpression meth;
    if (based) {
    ...
    List<JCExpression> typeargs = typarams.nonEmpty() ? make.Types(typarams)
        : null;
