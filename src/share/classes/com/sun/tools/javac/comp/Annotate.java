...
/**
 * Enter annotations on symbols. Annotations accumulate in a queue, which is
 * processed at the top level of any set of recursive calls requesting it be
 * processed.
 * 
 * <p>
...
public class Annotate {
  protected static final Context.Key<Annotate> annotateKey = new Context.Key<Annotate>();
  ...
  /**
   * A client that has annotations to add registers an annotator, the method it
   * will use to add the annotation. There are no parameters; any needed data
   * should be captured by the Annotator.
   */
  ...
  /**
   * Process a single compound annotation, returning its Attribute. Used from
   * MemberEnter for attaching the attributes to the annotated symbol.
   */
  Attribute.Compound enterAnnotation(JCAnnotation a, Type expected,
      Env<AttrContext> env) {
    // The annotation might have had its type attributed (but not checked)
    // by attr.attribAnnotationTypes during MemberEnter, in which case we do not
    // need to do it again.
    Type at = (a.annotationType.type != null ? a.annotationType.type : attr
        .attribType(a.annotationType, env));
    a.type = chk.checkType(a.annotationType.pos(), at, expected);
    if (a.type.isErroneous())
      return new Attribute.Compound(a.type,
          List.<Pair<MethodSymbol, Attribute>> nil());
    if ((a.type.tsym.flags() & Flags.ANNOTATION) == 0) {
      log.error(a.annotationType.pos(), "not.annotation.type",
          a.type.toString());
      return new Attribute.Compound(a.type,
          List.<Pair<MethodSymbol, Attribute>> nil());
    }
    ...
    if (args.length() == 1 && args.head.getTag() != JCTree.ASSIGN) {
      ...
      args.head = make.at(args.head.pos).Assign(make.Ident(names.value),
          args.head);
    }
    ...
    for (List<JCExpression> tl = args; tl.nonEmpty(); tl = tl.tail) {
      ...
      Symbol method = rs.resolveQualifiedMethod(left.pos(), env, a.type,
      ...
      if (!method.type.isErroneous())
        buf.append(new Pair<MethodSymbol, Attribute>((MethodSymbol) method,
            value));
      t.type = result;
    }
  ...
  Attribute enterAttributeValue(Type expected, JCExpression tree,
      Env<AttrContext> env) {
    ...
    if (expected.tag == TypeTags.ARRAY) { // should really be isArray()
      if (tree.getTag() != JCTree.NEWARRAY) {
        tree = make.at(tree.pos).NewArray(null, List.<JCExpression> nil(),
            List.of(tree));
      }
    ...
    if (expected.tag == TypeTags.CLASS
      ...
      if (sym == null || TreeInfo.nonstaticSelect(tree)
          || sym.kind != Kinds.VAR || (sym.flags() & Flags.ENUM) == 0) {
