...
/**
 * Helper class for constant folding, used by the attribution phase. This class
 * is marked strictfp as mandated by JLS 15.4.
 * 
 * <p>
...
strictfp class ConstFold {
  protected static final Context.Key<ConstFold> constFoldKey = new Context.Key<ConstFold>();
   ...
   * @param argtypes
   *          The operation's argument types (a list of length 1 or 2). Argument
   *          types are assumed to have non-null constValue's.
   */
   ...
   * @param operand
   *          The operation's operand type. Argument types are assumed to have
   *          non-null constValue's.
   */
    ...
    try {
      if (opcode > ByteCodes.preMask) {
        ...
        case imod:
          return syms.intType.constType(intValue(l) % intValue(r));
        case iand:
          return (left.tag == BOOLEAN ? syms.booleanType : syms.intType)
        ...
        case ladd:
          return syms.longType.constType(new Long(longValue(l) + longValue(r)));
        case lsub:
          return syms.longType.constType(new Long(longValue(l) - longValue(r)));
        case lmul:
          return syms.longType.constType(new Long(longValue(l) * longValue(r)));
        ...
        case lshl:
        case lshll:
          return syms.longType.constType(new Long(longValue(l) << intValue(r)));
        case lshr:
        case lshrl:
          return syms.longType.constType(new Long(longValue(l) >> intValue(r)));
        case lushr:
          return syms.longType
              .constType(new Long(longValue(l) >>> intValue(r)));
        case lcmp:
   ...
   * 
   * @param etype
   *          The source type of the coercion, which is assumed to be a constant
   *          type compatble with ttype.
   * @param ttype
