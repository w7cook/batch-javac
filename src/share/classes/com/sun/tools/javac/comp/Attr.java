...
public class Attr extends JCTree.Visitor {
  ...
  protected Attr(Context context) {
    ...
    relax = (options.isSet("-retrofit") || options.isSet("-relax"));
    findDiamonds = options.get("findDiamond") != null && source.allowDiamond();
  ...
  /**
   * Switch: generates a warning if diamond can be safely applied to a given new
   * expression
   */
  ...
  /**
   * Check kind and type of given tree against protokind and prototype. If check
   * succeeds, store type in tree and return it. If check fails, store errType
   * in tree and return it. No checks are performed if the prototype is a method
   * type. It is not necessary in this case since we know that kind and type are
   * correct.
   * 
   * @param tree
  ...
  Type check(JCTree tree, Type owntype, int ownkind, int pkind, Type pt) {
    if (owntype.tag != ERROR && pt.tag != METHOD && pt.tag != FORALL) {
      if ((ownkind & ~pkind) == 0) {
        ...
        log.error(tree.pos(), "unexpected.type", kindNames(pkind),
            kindName(ownkind));
   ...
   * @param base
   *          If the variable is referred to in a Select, the part to the left
   *          of the `.', null otherwise.
   * @param env
   *          The current environment.
   */
  void checkAssignable(DiagnosticPosition pos, VarSymbol v, JCTree base,
      Env<AttrContext> env) {
    if ((v.flags() & FINAL) != 0
        && ((v.flags() & HASINIT) != 0 || !((base == null || (base.getTag() == JCTree.IDENT && TreeInfo
            .name(base) == names._this)) && isAssignableAsBlankFinal(v, env)))) {
  ...
  /**
   * Does tree represent a static reference to an identifier? It is assumed that
   * tree is either a SELECT or an IDENT. We have to weed out selects from
   * non-type names here.
   * 
   * @param tree
   *          The candidate tree.
  ...
  public Symbol attribIdent(JCTree tree, JCCompilationUnit topLevel) {
    ...
    localEnv.enclClass = make.ClassDef(make.Modifiers(0), syms.errSymbol.name,
        null, null, null, null);
  ...
  /**
   * Derived visitor method: attribute an expression tree with no constraints on
   * the computed type.
   */
  ...
  List<Type> attribArgs(List<JCExpression> trees, Env<AttrContext> env) {
    ...
    for (List<JCExpression> l = trees; l.nonEmpty(); l = l.tail)
      argtypes.append(chk.checkNonVoid(l.head.pos(),
          types.upperBound(attribTree(l.head, env, VAL, Infer.anyPoly))));
    return argtypes.toList();
  }

  /**
   * Attribute a type argument list, returning a list of types. Caller is
   * responsible for calling checkRefTypes.
   */
  ...
  /**
   * Attribute a type argument list, returning a list of types. Check that all
   * the types are references.
   */
   ...
   * 
   * @param env
   *          The env for the const value
   * @param initializer
   *          The initializer for the const value
   * @param type
   *          The expected type, or null
   * @see VarSymbol#setlazyConstValue
   */
  public Object attribLazyConstantValue(Env<AttrContext> env,
      JCTree.JCExpression initializer, Type type) {
    ...
    // Having found the enclosing lint value, we can initialize the lint value
    // for this class
    env.info.lint = lintEnv.info.lint.augment(
        env.info.enclVar.attributes_field, env.info.enclVar.flags());
  ...
  /**
   * Attribute type reference in an `extends' or `implements' clause. Supertypes
   * of anonymous inner classes are usually already attributed.
   * 
   * @param tree
  ...
  Type checkBase(Type t, JCTree tree, Env<AttrContext> env,
      boolean classExpected, boolean interfaceExpected, boolean checkExtensible) {
    if (t.isErroneous())
      return t;
  ...
  public void visitMethodDef(JCMethodDecl tree) {
    ...
    try {
      ...
      if ((owner.flags() & ANNOTATION) != 0) {
        ...
        // validate annotation method's return type (could be an annotation
        // type)
        chk.validateAnnotationType(tree.restype);
        // ensure that annotation method does not clash with members of
        // Object/Annotation
        chk.validateAnnotationMethod(tree.pos(), m);

        if (tree.defaultValue != null) {
          // if default value is an annotation, check it is a well-formed
          // annotation value (e.g. no duplicate values, no missing values,
          // etc.)
      ...
      if (tree.body == null) {
        ...
        if ((owner.flags() & INTERFACE) == 0
            && (tree.mods.flags & (ABSTRACT | NATIVE)) == 0 && !relax)
        ...
        if (tree.name == names.init && owner.type != syms.objectType) {
          ...
          if (body.stats.isEmpty() || !TreeInfo.isSelfCall(body.stats.head)) {
            ...
            log.error(tree.body.stats.head.pos(),
                "call.to.super.not.allowed.in.enum.ctor", env.enclClass.sym);
          }
        }
  ...
  public void visitForeachLoop(JCEnhancedForLoop tree) {
    ...
    if (elemtype == null) {
      ...
      if (base == null) {
        log.error(tree.expr.pos(), "foreach.not.applicable.to.type", exprType,
  ...
  public void visitSwitch(JCSwitch tree) {
    ...
    if (types.isSameType(seltype, syms.stringType)) {
      if (allowStringsInSwitch) {
        ...
        log.error(tree.selector.pos(), "string.switch.not.supported.in.source",
            sourceName);
      }
    }
        ...
        if (enumSwitch) {
          ...
          if (pattype.tag != ERROR) {
            if (pattype.constValue() == null) {
              log.error(c.pat.pos(), (stringSwitch ? "string.const.req"
                  : "const.expr.req"));
  ...
  public void visitTry(JCTry tree) {
    ...
    Env<AttrContext> tryEnv = isTryWithResource ? env.dup(tree,
        localEnv.info.dup(localEnv.info.scope.dup())) : localEnv;
    // Attribute resource declarations
    for (JCTree resource : tree.resources) {
      if (resource.getTag() == JCTree.VARDEF) {
        attribStat(resource, tryEnv);
        chk.checkType(resource, resource.type, syms.autoCloseableType,
        ...
        attribExpr(resource, tryEnv, syms.autoCloseableType,
            "try.not.applicable.to.type");
      }
    }
    // Attribute body
    ...
    for (List<JCCatch> l = tree.catchers; l.nonEmpty(); l = l.tail) {
      ...
      Env<AttrContext> catchEnv = localEnv.dup(c,
          localEnv.info.dup(localEnv.info.scope.dup()));
      ...
      chk.checkType(c.param.vartype.pos(),
          chk.checkClassType(c.param.vartype.pos(), ctype), syms.throwableType);
  ...
  void checkAutoCloseable(DiagnosticPosition pos, Env<AttrContext> env,
    ...
    if (!resource.isErroneous()
      ...
      try {
        ...
        close = rs.resolveQualifiedMethod(pos, env, resource, names.close,
      ...
      if (close.kind == MTH
          && close.overrides(syms.autoCloseableClose, resource.tsym, types,
              true)
          && chk.isHandled(syms.interruptedExceptionType,
              types.memberType(resource, close).getThrownTypes())
          && env.info.lint.isEnabled(LintCategory.TRY)) {
        log.warning(LintCategory.TRY, pos,
            "try.resource.throws.interrupted.exc", resource);
      }
    }
  }
  ...
  public void visitConditional(JCConditional tree) {
    ...
    result = check(
        tree,
        capture(condType(tree.pos(), tree.cond.type, tree.truepart.type,
  ...
  private Type condType(DiagnosticPosition pos, Type condtype, Type thentype,
    ...
    return ((condtype.constValue() != null) && (thentype.constValue() != null) && (elsetype
        .constValue() != null)) ? cfolder.coerce(condtype.isTrue() ? thentype
        : elsetype, ctype) : ctype;
  }
   ...
   *          The type of the expression's else-part.
   */
  private Type condType1(DiagnosticPosition pos, Type condtype, Type thentype,
      Type elsetype) {
  ...
  // where
  /**
   * Return the target of a break or continue statement, if it exists, report an
   * error if not. Note: The target of a labelled break or continue is the
   ...
   * application will have in its type field the return type of the method, not
   * the method's type itself!
   */
          ...
          if (encl.tag == CLASS) {
            ...
            if (tree.meth.getTag() == JCTree.SELECT) {
              ...
              chk.checkRefType(qualifier.pos(),
                  attribExpr(qualifier, localEnv, encl));
      ...
      if (allowGenerics && methName == names.getClass && tree.args.isEmpty()) {
        ...
        restype = new ClassType(restype.getEnclosingType(),
            List.<Type> of(new WildcardType(types.erasure(qualifier),
                BoundKind.EXTENDS, syms.boundClass)), restype.tsym);
      }
  ...
  public void visitNewClass(JCNewClass tree) {
    ...
    (clazz.getTag() == JCTree.TYPEAPPLY) ? ((JCTypeApply) clazz).clazz : clazz;
    ...
    if (TreeInfo.isDiamond(tree) && !clazztype.isErroneous()) {
    ...
    } else if (allowDiamondFinder && tree.def == null
        && !clazztype.isErroneous() && clazztype.getTypeArguments().nonEmpty()
        && findDiamonds) {
      boolean prevDeferDiags = log.deferDiagnostics;
      Queue<JCDiagnostic> prevDeferredDiags = log.deferredDiagnostics;
      Type inferred = null;
      try {
        ...
        log.deferDiagnostics = true;
        log.deferredDiagnostics = ListBuffer.lb();
        inferred = attribDiamond(localEnv, tree, clazztype, mapping, argtypes,
            typeargtypes);
      } finally {
        log.deferDiagnostics = prevDeferDiags;
        log.deferredDiagnostics = prevDeferredDiags;
      ...
      if (inferred != null
          ...
          && inferred.tag == CLASS
          && types.isAssignable(inferred, pt.tag == NONE ? clazztype : pt,
              Warner.noWarnings)) {
        String key = types.isSameType(clazztype, inferred) ? "diamond.redundant.args"
    ...
    if (clazztype.tag == CLASS) {
      ...
      // current instance (JLS ???).
        ...
        tree.constructor = rs.resolveConstructor(tree.pos(), rsEnv, clazztype,
            argtypes, typeargtypes);
        tree.constructorType = tree.constructor.type.isErroneous() ? syms.errType
            : checkMethod(clazztype, tree.constructor, rsEnv, tree.args,
                argtypes, typeargtypes, rsEnv.info.varArgs);
        if (rsEnv.info.varArgs)
          Assert.check(tree.constructorType.isErroneous()
              || tree.varargsElement != null);
      }
      ...
      if (cdef != null) {
        ...
        Symbol sym = rs.resolveConstructor(tree.pos(), localEnv, clazztype,
            argtypes, typeargtypes, true, useVarargs);
        ...
        if (tree.constructor.kind > ERRONEOUS) {
          ...
          tree.constructorType = checkMethod(clazztype, tree.constructor,
              localEnv, tree.args, argtypes, typeargtypes, useVarargs);
        }
      }
  ...
  Type attribDiamond(Env<AttrContext> env, JCNewClass tree, Type clazztype,
      Pair<Scope, Scope> mapping, List<Type> argtypes, List<Type> typeargtypes) {
    if (clazztype.isErroneous() || clazztype.isInterface()
    ...
    Symbol constructor;
    try {
      constructor = rs.resolveDiamond(tree.pos(), localEnv,
          clazztype.tsym.type, argtypes, typeargtypes);
    } finally {
    ...
    if (constructor.kind == MTH) {
      ClassType ct = new ClassType(clazztype.getEnclosingType(),
          clazztype.tsym.type.getTypeArguments(), clazztype.tsym);
      clazztype = checkMethod(ct, constructor, localEnv, tree.args, argtypes,
          typeargtypes, localEnv.info.varArgs).getReturnType();
    } else {
  ...
  /**
   * Creates a synthetic scope containing fake generic constructors. Assuming
   * that the original scope contains a constructor of the kind: Foo(X x, Y y),
   * where X,Y are class type-variables declared in Foo, the synthetic scope is
   * added a generic constructor of the kind: <X,Y>Foo<X,Y>(X x, Y y). This is
   * crucial in order to enable diamond inference. The inferred return type of
   * the synthetic constructor IS the inferred type for the diamond operator.
   */
  private Pair<Scope, Scope> getSyntheticScopeMapping(Type ctype) {
    ...
    Pair<Scope, Scope> mapping = new Pair<Scope, Scope>(ctype.tsym.members(),
        new Scope(ctype.tsym));

    // for each constructor in the original scope, create a synthetic
    // constructor
    // whose return type is the type of the class in which the constructor is
    // declared, and insert it into the new scope.
    for (Scope.Entry e = mapping.fst.lookup(names.init); e.scope != null; e = e
        .next()) {
      Type synthRestype = new ClassType(ctype.getEnclosingType(),
          ctype.tsym.type.getTypeArguments(), ctype.tsym);
      MethodSymbol synhConstr = new MethodSymbol(e.sym.flags(), names.init,
  ...
  private final Pair<Scope, Scope> erroneousMapping = new Pair<Scope, Scope>(
      null, null);
  ...
  public void visitAssignop(JCAssignOp tree) {
    ...
    if (operator.kind == MTH && !owntype.isErroneous()
        && !operand.isErroneous()) {
      chk.checkOperator(tree.pos(), (OperatorSymbol) operator, tree.getTag()
          - JCTree.ASGOffset, owntype, operand);
  ...
  public void visitUnary(JCUnary tree) {
    ...
    Type argtype = (JCTree.PREINC <= tree.getTag() && tree.getTag() <= JCTree.POSTDEC) ? attribTree(
        tree.arg, env, VAR, Type.noType) : chk.checkNonVoid(tree.arg.pos(),
  ...
  public void visitBinary(JCBinary tree) {
    ...
    if (operator.kind == MTH && !left.isErroneous() && !right.isErroneous()) {
      ...
      int opc = chk.checkOperator(tree.lhs.pos(), (OperatorSymbol) operator,
  ...
  public void visitSelect(JCFieldAccess tree) {
    ...
    if (sitesym != null
        ...
        && sym.overrides(syms.autoCloseableClose, sitesym.type.tsym, types,
            true) && env.info.lint.isEnabled(LintCategory.TRY)) {
   ...
   */
  private Symbol selectSym(JCFieldAccess tree, Type site, Env<AttrContext> env,
      Type pt, int pkind) {
  ...
  private Symbol selectSym(JCFieldAccess tree, Symbol location, Type site,
    ...
    switch (site.tag) {
    case PACKAGE:
      return rs.access(rs.findIdentInPackage(env, site.tsym, name, pkind), pos,
          location, site, name, true);
    case ARRAY:
    case CLASS:
      if (pt.tag == METHOD || pt.tag == FORALL) {
        return rs.resolveQualifiedMethod(pos, env, location, site, name,
            pt.getParameterTypes(), pt.getTypeArguments());
        ...
        List<Type> typeargs = allowGenerics ? List.of(types.erasure(site))
            : List.<Type> nil();
    ...
    case TYPEVAR:
      ...
      Symbol sym = (site.getUpperBound() != null) ? selectSym(tree, location,
          capture(site.getUpperBound()), env, pt, pkind) : null;
      if (sym == null) {
        log.error(pos, "type.var.cant.be.deref");
        return syms.errSymbol;
      } else {
        Symbol sym2 = (sym.flags() & Flags.PRIVATE) != 0 ? rs.new AccessError(
            env, site, sym) : sym;
   ...
   * field of a raw type, which is being assigned to, issue an unchecked warning
   * if its type changes under erasure. (5) if symbol is an instance method of a
   * raw type, issue an unchecked warning if its argument types change under
   * erasure. If checks succeed: If symbol is a constant, return its constant
   * type else if symbol is a method, return its result type otherwise return
   * its type. Otherwise return errType.
   * 
   * @param tree
  ...
  Type checkId(JCTree tree, Type site, Symbol sym, Env<AttrContext> env,
    ...
    case MTH: {
      ...
      owntype = checkMethod(site, sym, env, app.args, pt.getParameterTypes(),
          pt.getTypeArguments(), env.info.varArgs);
      break;
    }
    case PCK:
    case ERR:
      ...
      break;
    default:
      throw new AssertionError("unexpected kind: " + sym.kind + " in tree "
          + tree);
    }
  ...
  /**
   * Check that variable is initialized and evaluate the variable's initializer,
   * if not yet done. Also check that variable is not referenced before it is
   * defined.
   * 
   * @param tree
  ...
  public Type checkMethod(Type site, Symbol sym, Env<AttrContext> env,
    ...
    if (allowGenerics && (sym.flags() & STATIC) == 0
      ...
      if (s != null
        ...
        chk.warnUnchecked(env.tree.pos(), "unchecked.call.mbr.of.raw.type",
            sym, s);
      }
    }
    ...
    if (owntype == null) {
      if (!pt.isErroneous())
        log.error(env.tree.pos(), "internal.error.cant.instantiate", sym, site,
      ...
      if (warned && sym.type.tag == FORALL) {
        chk.warnUnchecked(env.tree.pos(), "unchecked.meth.invocation.applied",
            kindName(sym), sym.name,
            rs.methodArguments(sym.type.getParameterTypes()),
            rs.methodArguments(argtypes), kindName(sym.location()),
            sym.location());
        owntype = new MethodType(owntype.getParameterTypes(),
            types.erasure(owntype.getReturnType()), types.erasure(owntype
                .getThrownTypes()), syms.methodClass);
      }
      if (useVarargs) {
  ...
  private void assertConvertible(JCTree tree, Type actual, Type formal,
    ...
    if (formal.isCompound() && types.isSubtype(actual, types.supertype(formal))
        && types.isSubtypeUnchecked(actual, types.interfaces(formal), warn))
      return;

    if (false) {
      // TODO: make assertConvertible work
      chk.typeError(tree.pos(), diags.fragment("incompatible.types"), actual,
          formal);
  ...
  /**
   * Visitor method for parameterized types. Bound checking is left until later,
   * since types are attributed before supertype structure is completely known
   */
  ...
  public void visitTypeUnion(JCTypeUnion tree) {
    ...
    for (JCExpression typeTree : tree.alternatives) {
      ...
      ctype = chk.checkType(typeTree.pos(),
          chk.checkClassType(typeTree.pos(), ctype), syms.throwableType);
        ...
        if (chk.intersects(ctype, multicatchTypes.toList())) {
          for (Type t : multicatchTypes) {
            ...
            if (sub || sup) {
              ...
              Type a = sub ? ctype : t;
              Type b = sub ? t : ctype;
              log.error(typeTree.pos(), "multicatch.types.must.be.disjoint", a,
                  b);
            }
          }
        }
    ...
    if (t.tag == CLASS) {
      List<Type> alternatives = ((all_multicatchTypes == null) ? multicatchTypes
          : all_multicatchTypes).toList();
  ...
  /**
   * Attribute a top level tree. These trees are encountered when the package
   * declaration has annotations.
   */
   ...
   * 
   * @param pos
   *          The source position at which completion errors are to be reported.
   * @param c
  ...
  void attribClass(ClassSymbol c) throws CompletionFailure {
    ...
    if ((c.flags_field & UNATTRIBUTED) != 0) {
      ...
      // because the annotations were not available at the time the env was
      // created. Therefore,
      // we look up the environment chain for the first enclosing environment
      // for which the
      // lint value is set. Typically, this is the parent env, but might be
      // further if there
      ...
      try {
        ...
        if (st.tsym != null && ((st.tsym.flags_field & Flags.ENUM) != 0)
            && ((c.flags_field & (Flags.ENUM | Flags.COMPOUND)) == 0)
  ...
  private void attribClassBody(Env<AttrContext> env, ClassSymbol c) {
    ...
    // check that a resource implementing AutoCloseable cannot throw
    ...
    if (env.info.lint.isEnabled(LintCategory.SERIAL) && isSerializable(c)
  ...
  /**
   * Handle missing types/symbols in an AST. This routine is useful when the
   * compiler has encountered some errors (which might have ended up terminating
   * attribution abruptly); if the compiler is used in fail-over mode (e.g. by
   * an IDE) and the AST contains semantic errors, this routine prevents NPE to
   * be progagated during subsequent compilation steps.
   */
  ...
  class PostAttrAnalyzer extends TreeScanner {
    ...
    public void visitNewClass(JCNewClass that) {
      if (that.constructor == null) {
        that.constructor = new MethodSymbol(0, names.init, syms.unknownType,
            syms.noSymbol);
      }
    ...
    public void visitBinary(JCBinary that) {
      if (that.operator == null)
        that.operator = new OperatorSymbol(names.empty, syms.unknownType, -1,
            syms.noSymbol);
