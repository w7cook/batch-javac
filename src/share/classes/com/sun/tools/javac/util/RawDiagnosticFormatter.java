...
/**
 * A raw formatter for diagnostic messages. The raw formatter will format a
 * diagnostic according to one of two format patterns, depending on whether or
 * not the source name and position are set. This formatter provides a
 * standardized, localize-independent implementation of a diagnostic formatter;
 * as such, this formatter is best suited for testing purposes.
 * 
 * <p>
...
public final class RawDiagnosticFormatter extends AbstractDiagnosticFormatter {
  ...
  public RawDiagnosticFormatter(Options options) {
    super(null, new SimpleConfiguration(options, EnumSet.of(
        DiagnosticPart.SUMMARY, DiagnosticPart.DETAILS,
    ...
    try {
      ...
      if (d.getPosition() != Position.NOPOS) {
        ...
        buf.append(':');
          ...
          && d.getSource().getKind() == JavaFileObject.Kind.CLASS) {
        buf.append(formatSource(d, false, null));
        buf.append(":-:-:");
      } else
        buf.append('-');
