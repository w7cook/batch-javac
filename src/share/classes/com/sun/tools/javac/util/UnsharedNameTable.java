...
/**
 * Implementation of Name.Table that stores names in individual arrays using
 * weak references. It is recommended for use when a single shared byte array is
 * unsuitable.
 * 
 * <p>
...
public class UnsharedNameTable extends Name.Table {
   ...
   *          The main name table
   * @param hashSize
   *          the (constant) size to be used for the hash table needs to be a
   *          power of two.
   */
  ...
  public Name fromUtf(byte[] cs, int start, int len) {
    ...
    while (element != null) {
      ...
      if (n == null) {
        if (firstTableEntry == element) {
          ...
          Assert.checkNonNull(previousNonNullTableEntry,
              "previousNonNullTableEntry cannot be null here.");
    ...
    if (previousNonNullTableEntry == null) { // We are not the first name with
                                             // that hashCode.
      hashes[h] = newEntry;
    } else {
      Assert.checkNull(previousNonNullTableEntry.next,
          "previousNonNullTableEntry.next must be null.");
