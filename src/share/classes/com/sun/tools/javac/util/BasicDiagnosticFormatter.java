...
/**
 * A basic formatter for diagnostic messages. The basic formatter will format a
 * diagnostic according to one of three format patterns, depending on whether or
 * not the source name and position are set. The formatter supports a
 * printf-like string for patterns with the following special characters:
 * <ul>
...
public class BasicDiagnosticFormatter extends AbstractDiagnosticFormatter {
   ...
   * @param msgs
   *          JavacMessages object used for i18n
   */
  ...
  public String formatMessage(JCDiagnostic d, Locale l) {
    ...
    if (getConfiguration().getVisible().contains(DiagnosticPart.SUMMARY)) {
      currentIndentation += getConfiguration().getIndentation(
          DiagnosticPart.SUMMARY);
      buf.append(indent(lines[0], currentIndentation)); // summary
    }
    if (lines.length > 1
        && getConfiguration().getVisible().contains(DiagnosticPart.DETAILS)) {
      currentIndentation += getConfiguration().getIndentation(
    ...
    if (d.isMultiline()
        && getConfiguration().getVisible().contains(
  ...
  protected String addSourceLineIfNeeded(JCDiagnostic d, String msg) {
    if (!displaySource(d))
      ...
      if (singleLine
          || getConfiguration().getSourcePosition() == SourcePosition.BOTTOM)
  ...
  static public class BasicConfiguration extends SimpleConfiguration {
    ...
    public BasicConfiguration(Options options) {
      super(options, EnumSet.of(DiagnosticPart.SUMMARY, DiagnosticPart.DETAILS,
          DiagnosticPart.SUBDIAGNOSTICS, DiagnosticPart.SOURCE));
      initFormat();
      initIndentation();
      ...
      if (indent != null) {
        ...
        try {
          ...
          case 5:
            setIndentation(DiagnosticPart.JLS, Integer.parseInt(levels[4]));
          case 4:
            setIndentation(DiagnosticPart.SUBDIAGNOSTICS,
                Integer.parseInt(levels[3]));
          case 3:
            setIndentation(DiagnosticPart.SOURCE, Integer.parseInt(levels[2]));
          case 2:
            setIndentation(DiagnosticPart.DETAILS, Integer.parseInt(levels[1]));
          default:
            setIndentation(DiagnosticPart.SUMMARY, Integer.parseInt(levels[0]));
          }
    ...
    public BasicConfiguration() {
      super(EnumSet.of(DiagnosticPart.SUMMARY, DiagnosticPart.DETAILS,
          DiagnosticPart.SUBDIAGNOSTICS, DiagnosticPart.SOURCE));
      initFormat();
      initIndentation();
    }
    ...
    private void initFormats(String pos, String nopos, String clazz) {
      availableFormats = new EnumMap<BasicFormatKind, String>(
          BasicFormatKind.class);
     ...
     * 
     * @param diagPart
     *          the diagnostic part for which the indentation is to be retrieved
     ...
     * 
     * @param indentationKind
     *          kind of indentation to be set
     * @param nSpaces
     *          amount of spaces for the specified diagnostic part
     */
     ...
     * 
     * @param sourcePos
     *          a positioning value for source line
     */
    ...
    public enum SourcePosition {
      ...
      BOTTOM,
      /**
       * Source line is displayed after the first line of the diagnostic message
       */
      AFTER_SUMMARY;
    }
     ...
     * 
     * @param kind
     *          the format kind to be set
     * @param s
     *          the metachar string specifying the format
     */
    ...
    // where
    /**
     * This enum contains all the kinds of formatting patterns supported by a
     * basic diagnostic formatter.
     */
