 ...
 * the diagnostic is rendered using a standard formatter, whose type/symbol
 * printer has been replaced by a more refined version provided by this rich
 * formatter. The rich formatter currently enables three different features: (i)
 * simple class names - that is class names are displayed used a non qualified
 * name (thus omitting package info) whenever possible - (ii) where clause list
 * - a list of additional subdiagnostics that provide specific info about
 * type-variables, captured types, intersection types that occur in the
 * diagnostic that is to be formatted and (iii) type-variable disambiguation -
 * when the diagnostic refers to two different type-variables with the same
 * name, their representation is disambiguated by appending an index to the type
 * variable name.
 * 
 * <p>
...
public class RichDiagnosticFormatter extends
  ...
  public static RichDiagnosticFormatter instance(Context context) {
    RichDiagnosticFormatter instance = context
        .get(RichDiagnosticFormatter.class);
    if (instance == null)
  ...
  protected RichDiagnosticFormatter(Context context) {
    super((AbstractDiagnosticFormatter) Log.instance(context)
  ...
  public String format(JCDiagnostic diag, Locale l) {
    ...
    if (getConfiguration().isEnabled(RichFormatterFeature.WHERE_CLAUSES)) {
      ...
      String indent = formatter.isRaw() ? "" : formatter
          .indentString(DetailsInc);
   ...
   * 
   * @param diag
   *          the diagnostic to be preprocessed
   */
  ...
  /**
   * Preprocess a diagnostic argument. A type/symbol argument is preprocessed by
   * specialized type/symbol preprocessors.
   * 
   * @param arg
   *          the argument to be translated
   */
  ...
  protected List<JCDiagnostic> getWhereClauses() {
    ...
    for (WhereClauseKind kind : WhereClauseKind.values()) {
      ...
      for (Map.Entry<Type, JCDiagnostic> entry : whereClauses.get(kind)
          .entrySet()) {
  ...
  private boolean unique(TypeVar typevar) {
    ...
    if (found < 1)
      throw new AssertionError("Missing type variable in where clause "
          + typevar);
    return found == 1;
  }
  ...
  // where
  /**
   * This enum defines all posssible kinds of where clauses that can be attached
   * by a rich diagnostic formatter to a given diagnostic
   */
   ...
   * whether a class name can be compacted or not. Short names are not used if a
   * conflict is detected, e.g. when two classes with the same simple name
   * belong to different packages - in this case the formatter reverts to
   * fullnames as compact names might lead to a confusing diagnostic.
   */
  protected class ClassNameSimplifier {
    ...
    public String simplify(Symbol s) {
      ...
      if (!s.type.isCompound()) {
        ...
        if (conflicts == null
            || (conflicts.size() == 1 && conflicts.contains(s))) {
  ...
  /**
   * Enhanced type/symbol printer that provides support for features like simple
   * names and type variable disambiguation. This enriched printer exploits the
   * info discovered during type/symbol preprocessing. This printer is set on
   * the delegate formatter so that rich type/symbol info can be properly
   * rendered.
   */
  protected class RichPrinter extends Printer {
    ...
    public String visitCapturedType(CapturedType t, Locale locale) {
      if (getConfiguration().isEnabled(RichFormatterFeature.WHERE_CLAUSES)) {
        return localize(locale, "compiler.misc.captured.type",
    ...
    public String visitClassType(ClassType t, Locale locale) {
      if (t.isCompound()
        ...
        return localize(locale, "compiler.misc.intersection.type",
    ...
    public String visitTypeVar(TypeVar t, Locale locale) {
      if (unique(t)
        ...
        return localize(locale, "compiler.misc.type.var", t.toString(),
            indexOf(t, WhereClauseKind.TYPEVAR));
      }
    }

    @Override
    protected String printMethodArgs(List<Type> args, boolean varArgs,
        Locale locale) {
    ...
    public String visitMethodSymbol(MethodSymbol s, Locale locale) {
      ...
      if ((s.flags() & BLOCK) != 0) {
        ...
        String ms = (s.name == s.name.table.names.init) ? ownerName : s.name
            .toString();
        if (s.type != null) {
          ...
          ms += "("
              + printMethodArgs(s.type.getParameterTypes(),
                  (s.flags() & VARARGS) != 0, locale) + ")";
        }
        return ms;
      }
    }
  ...
  /**
   * Preprocess a given type looking for (i) additional info (where clauses) to
   * be added to the main diagnostic (ii) names to be compacted.
   */
  ...
  protected Types.UnaryVisitor<Void> typePreprocessor = new Types.UnaryVisitor<Void>() {
    ...
    public Void visitCapturedType(CapturedType t, Void ignored) {
      if (indexOf(t, WhereClauseKind.CAPTURED) == -1) {
        ...
        JCDiagnostic d = diags.fragment("where.captured" + suffix, t, t.bound,
            t.lower, t.wildcard);
    ...
    public Void visitClassType(ClassType t, Void ignored) {
      if (t.isCompound()) {
        if (indexOf(t, WhereClauseKind.INTERSECTION) == -1) {
          ...
          JCDiagnostic d = diags.fragment("where.intersection", t,
              interfaces.prepend(supertype));
    ...
    public Void visitTypeVar(TypeVar t, Void ignored) {
      if (indexOf(t, WhereClauseKind.TYPEVAR) == -1) {
        ...
        // attributed the
        // bound is not set
        List<Type> bounds = bound != null ? types.getBounds(t) : List
            .<Type> nil();

        nameSimplifier.addUsage(t.tsym);

        boolean boundErroneous = bounds.head == null || bounds.head.tag == NONE
  ...
  /**
   * Preprocess a given symbol looking for (i) additional info (where clauses)
   * to be asdded to the main diagnostic (ii) names to be compacted
   */
   ...
   */
  public static class RichConfiguration extends
    ...
    public RichConfiguration(Options options,
