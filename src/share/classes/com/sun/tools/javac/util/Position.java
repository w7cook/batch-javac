...
/**
 * A class that defines source code positions as simple character offsets from
 * the beginning of the file. The first character is at position 0.
 * 
 * Support is also provided for (line,column) coordinates, but tab expansion is
 * optional and no Unicode excape translation is considered. The first character
 * is at location (1,1).
 * 
 * <p>
...
public class Position {
  ...
  /**
   * A two-way map between line/column numbers and positions, derived from a
   * scan done at creation time. Tab expansion is optionally supported via a
   * character map. Text content is not retained.
   * <p>
   * Notes: The first character position FIRSTPOS is at (FIRSTLINE,FIRSTCOLUMN).
   * No account is taken of Unicode escapes.
   * 
   * @param src
   *          Source characters
   * @param max
   ...
   * @param expandTabs
   *          If true, expand tabs when calculating columns
   */
  public static LineMap makeLineMap(char[] src, int max, boolean expandTabs) {
    LineMapImpl lineMap = expandTabs ? new LineTabMapImpl(max)
        : new LineMapImpl();
...
/** Encode line and column numbers in an integer as:
  ...
  public static interface LineMap extends com.sun.source.tree.LineMap {
     ...
     * 
     * @param line
     *          number of line (first is 1)
     * @return position of first character in line
     * @throws ArrayIndexOutOfBoundsException
     *           if <tt>lineNumber < 1</tt> if
     ...
     *          number of line (first is 1)
     * @param column
     *          number of character on line (first is 1)
     * 
     * @return position of character
     * @throws ArrayIndexOutOfBoundsException
     *           if <tt>line < 1</tt> if <tt>line > no. of lines</tt>
     */
    ...
    /**
     * Find the column for a character position. Note: this method does not
     * handle tab expansion. If tab expansion is needed, use a LineTabMap
     * instead.
     * 
     * @param pos
     *          character offset of the position
  ...
  /**
   * A LineMap that handles tab expansion correctly. The cost is an additional
   * bit per character in the source array.
   */
