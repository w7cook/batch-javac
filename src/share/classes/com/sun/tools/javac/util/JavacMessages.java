...
public class JavacMessages implements Messages {
   ...
   *          the name to identify the resource buundle of localized messages.
   */
  public JavacMessages(String bundleName, Locale locale)
      ...
      for (String bundleName : bundleNames) {
        try {
          ...
          throw new InternalError(
              "Cannot find javac resource bundle for locale " + locale);
        }
      }
      bundleCache.put(locale, new SoftReference<List<ResourceBundle>>(
          bundleList));
    }
    return bundleList;
  }
  ...
  /**
   * Gets the localized string corresponding to a key, formatted with a set of
   * args.
   */
  ...
  /*
   * Static access: javac has a firmly entrenched notion of a default message
   * bundle which it can access from any static context. This is used to get
