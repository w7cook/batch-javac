...
/**
 * This abstract class provides a basic implementation of the functionalities
 * that should be provided by any formatter used by javac. Among the main
 * features provided by AbstractDiagnosticFormatter are:
 * 
 * <ul>
 * <li>Provides a standard implementation of the visitor-like methods defined in
 * the interface DiagnisticFormatter. Those implementations are specifically
 * targeting JCDiagnostic objects.
 * <li>Provides basic support for i18n and a method for executing all
 * locale-dependent conversions
 * <li>Provides the formatting logic for rendering the arguments of a
 * JCDiagnostic object.
 * <ul>
 * 
 * <p>
 ...
 */
public abstract class AbstractDiagnosticFormatter implements
  ...
  /**
   * Current depth level of the disgnostic being formatted (!= 0 for
   * subdiagnostics)
   */
  ...
  /**
   * Initialize an AbstractDiagnosticFormatter by setting its JavacMessages
   * object.
   * 
   * @param messages
   */
   ...
   * @return a Collection whose elements are the formatted arguments of the
   *         diagnostic
   */
        ...
        for (JCDiagnostic d2 : d.getSubdiagnostics()) {
          if (maxCount == -1 || count < maxCount) {
            subdiagnostics = subdiagnostics
                .append(formatSubdiagnostic(d, d2, l));
            count++;
          } else
            break;
        }
  ...
  /**
   * Converts a String into a locale-dependent representation accordingly to a
   * given locale.
   * 
   * @param l
   *          locale object to be used for i18n
  ...
  public boolean displaySource(JCDiagnostic d) {
    return config.getVisible().contains(DiagnosticPart.SOURCE)
        && d.getType() != FRAGMENT && d.getIntPosition() != Position.NOPOS;
  }
  ...
  /**
   * Creates a string with a given amount of empty spaces. Useful for indenting
   ...
   * @param nSpaces
   *          the amount of spaces that should be prepended to each line of the
   *          string
  ...
  static public class SimpleConfiguration implements Configuration {
    ...
    /**
     * Shows a '^' sign under the source line displayed by the formatter (if
     * applicable).
     * 
     * @param caretEnabled
   ...
   * AbstractDiagnosticFormatter. Provides alternate numbering of captured types
   * (they are numbered starting from 1 on each new diagnostic, instead of
   * relying on the underlying hashcode() method which generates unstable
