...
public class Log extends AbstractLog {
  ...
  public static final Context.Key<PrintWriter> outKey = new Context.Key<PrintWriter>();
  ...
  /**
   * Diagnostic listener, if provided through programmatic interface to javac
   * (JSR 199).
   */
  ...
  protected Log(Context context, PrintWriter errWriter, PrintWriter warnWriter,
    ...
    // FIXME
    DiagnosticListener<? super JavaFileObject> dl = context
        .get(DiagnosticListener.class);
  ...
  // where
  private int getIntOption(Options options, OptionName optionName,
      int defaultValue) {
   ...
   */
  public void setDiagnosticFormatter(
      DiagnosticFormatter<JCDiagnostic> diagFormatter) {
  ...
  /**
   * Returns true if an error needs to be reported for a given source name and
   * pos.
   */
  protected boolean shouldReport(JavaFileObject file, int pos) {
    ...
    Pair<JavaFileObject, Integer> coords = new Pair<JavaFileObject, Integer>(
        file, pos);
   ...
   * 
   * @param pos
   *          Buffer index of the error position, must be on current line
   */
  ...
  /**
   * Print the text of a message, translating newlines appropriately for the
   * platform.
   */
  ...
  /**
   * Print the text of a message to the errWriter stream, translating newlines
  ...
  /**
   * Print the text of a message to the noticeWriter stream, translating
   * newlines appropriately for the platform.
   */
  ...
  /**
   * Common diagnostic handling. The diagnostic is counted, and depending on the
   * options and how many diagnostics have been reported so far, the diagnostic
   * may be handed off to writeDiagnostic.
   */
  public void report(JCDiagnostic diagnostic) {
    ...
    case NOTE:
      ...
      // Notes are only generated at the end of a compilation, so should be
      // small
      // in number.
  ...
  /**
   * Find a localized string in the resource bundle. Because this method is
   * static, it ignores the locale. Use localize(key, args) when possible.
   * 
   * @param key
   *          The key for the localized string.
   * @param args
   *          Fields to substitute into the string.
   */
  public static String getLocalizedString(String key, Object... args) {
    return JavacMessages
        .getDefaultLocalizedString("compiler.misc." + key, args);
  }
  ...
  private void printRawError(int pos, String msg) {
    if (source == null || pos == Position.NOPOS) {
      ...
      if (file != null)
        printLines(errWriter, file.getName() + ":" + line + ": " + msg);
