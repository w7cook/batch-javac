...
public class Main {
   ...
   * Result codes.
   */
  static final int EXIT_OK = 0, // Compilation completed with no
                                // errors.
  ...
  private Option[] recognizedOptions = RecognizedOptions
      .getJavaCompilerOptions(new OptionHelper() {
        ...
        public void printVersion() {
          Log.printLines(out,
              getLocalizedString("version", ownName, JavaCompiler.version()));
        }

        public void printFullVersion() {
          Log.printLines(
              out,
              getLocalizedString("fullVersion", ownName,
  ...
  public List<File> processArgs(String[] flags) { // XXX sb protected
    ...
    while (ac < flags.length) {
      ...
      if (flag.length() > 0) {
        ...
        int firstOptionToCheck = flag.charAt(0) == '-' ? 0
            : recognizedOptions.length - 1;
  ...
  public int compile(String[] args) {
    ...
    JavacFileManager.preRegister(context); // can't create it until Log has been
                                           // set up
   ...
   */
  public int compile(String[] args, Context context,
      List<JavaFileObject> fileObjects, Iterable<? extends Processor> processors) {
    if (options == null)
    ...
    try {
      ...
      try {
        ...
        if (files == null) {
          ...
          return EXIT_CMDERR;
        } else if (files.isEmpty() && fileObjects.isEmpty()
            && classnames.isEmpty()) {
          // it is allowed to compile nothing if just asking for help or version
          // info
          if (options.isSet(HELP) || options.isSet(X) || options.isSet(VERSION)
      ...
      boolean batchMode = (options.isUnset("nonBatchMode") && System
          .getProperty("nonBatchMode") == null);
      if (batchMode)
      ...
      if (log.expectDiagKeys != null) {
        if (log.expectDiagKeys.isEmpty()) {
          ...
          Log.printLines(log.noticeWriter,
              "expected diagnostic keys not found: " + log.expectDiagKeys);
          return EXIT_ERROR;
        }
      }
      ...
      // exceptions.
      if (comp == null || comp.errorCount() == 0 || options == null
          || options.isSet("dev"))
        bugMessage(ex);
