...
public class Gen extends JCTree.Visitor {
  ...
  protected Gen(Context context) {
    ...
    varDebugInfo = options.isUnset(G_CUSTOM) ? options.isSet(G) : options
        .isSet(G_CUSTOM, "vars");
    ...
    generateIproxies = target.requiresIproxy() || options.isSet("miranda");
  ...
  /**
   * Construct a symbol to reflect the qualifying type that should appear in the
   * byte code as per JLS 13.1.
   * 
  ...
  LocalItem makeTemp(Type type) {
    VarSymbol v = new VarSymbol(Flags.SYNTHETIC, names.empty, type,
        env.enclMethod.sym);
    code.newLocal(v);
   ...
   * @param isStatic
   *          A flag that indicates whether we call a static or instance method.
   */
  void callMethod(DiagnosticPosition pos, Type site, Name name,
      List<Type> argtypes, boolean isStatic) {
  ...
  /**
   * Is the given method definition an access method resulting from a qualified
   * super? This is signified by an odd access code.
   */
  private boolean isAccessSuper(JCMethodDecl enclMethod) {
    return (enclMethod.mods.flags & SYNTHETIC) != 0
  ...
  private boolean isOddAccessName(Name name) {
    return name.startsWith(accessDollar)
   ...
   * Any calls to finalizers are appended to the environments `cont' chain. Mark
   * beginning of gap in catch all range for finalizer.
   */
  ...
  List<JCTree> normalizeDefs(List<JCTree> defs, ClassSymbol c) {
    ...
    if (clinitCode.length() != 0) {
      MethodSymbol clinit = new MethodSymbol(STATIC, names.clinit,
          new MethodType(List.<Type> nil(), syms.voidType, List.<Type> nil(),
              syms.methodClass), c);
  ...
  private void checkStringConstant(DiagnosticPosition pos, Object constValue) {
    if (nerrs != 0
        || // only complain about a long string once
        constValue == null || !(constValue instanceof String)
  ...
  /**
   * Add abstract methods for all methods defined in one of the interfaces of a
   * given class, provided they are not already implemented in the class.
   * 
   * @param c
   ...
   * called "Miranda methods". Enter the newly created method into its enclosing
   * class scope. Note that it is not entered into the class tree, as the
   * emitter doesn't need to see it there to emit an abstract method.
   * 
   * @param c
   *          The class to which the Miranda method is added.
   * @param m
   *          The interface method symbol for which a Miranda method is added.
   */
  private void addAbstractMethod(ClassSymbol c, MethodSymbol m) {
    MethodSymbol absMeth = new MethodSymbol(m.flags() | IPROXY | SYNTHETIC,
        m.name, m.type, // was c.type.memberType(m), but now only !generics
                        // supported
        c);
  ...
  /**
   * Visitor method: generate code for a definition, catching and reporting any
   * completion failures.
   * 
   * @param tree
   ...
   *          The CharacterRangeTable flags indicating type of the entry.
   */
  public void genStats(List<JCStatement> trees, Env<GenContext> env,
      int crtFlags) {
    if (!genCrt) {
      genStats(trees, env);
      return;
   ...
   * @param markBranches
   *          The flag to indicate that the condition is a flow controller so
   *          produced conditions should contain a proper tree to generate
  ...
  /**
   * Visitor method: generate code for an expression, catching and reporting any
   * completion failures.
   * 
   * @param tree
   *          The expression to be visited.
   ...
   * @param pts
   *          The expression's expected types (i.e. the formal parameter types
   ...
   * @param fatcode
   *          A flag that indicates whether all jumps are within 32K. We first
   *          invoke this method under the assumption that fatcode == false,
   *          i.e. all jumps are within 32K. If this fails, fatcode is set to
   *          true and we try again.
   */
  void genMethod(JCMethodDecl tree, Env<GenContext> env, boolean fatcode) {
    ...
    if (Code.width(types.erasure(env.enclMethod.sym.type).getParameterTypes())
        + (((tree.mods.flags & STATIC) == 0 || meth.isConstructor()) ? 1 : 0) > ClassFile.MAX_PARAMETERS) {
  ...
  private int initCode(JCMethodDecl tree, Env<GenContext> env, boolean fatcode) {
    ...
    meth.code = code = new Code(meth, fatcode, lineDebugInfo ? toplevel.lineMap
        : null, varDebugInfo, stackMap, debugCode, genCrt ? new CRTable(tree,
        env.toplevel.endPositions) : null, syms, types, pool);
    ...
    if ((tree.mods.flags & STATIC) == 0) {
      ...
      code.setDefined(code.newLocal(new VarSymbol(FINAL, names._this, selfType,
          meth.owner)));
    }
  ...
  public void visitDoLoop(JCDoWhileLoop tree) {
    genLoop(tree, tree.body, tree.cond, List.<JCExpressionStatement> nil(),
        false);
  }
  ...
  public void visitSwitch(JCSwitch tree) {
    ...
    if (cases.isEmpty()) {
      ...
      sel.load().drop();
      if (genCrt)
        code.crt.put(TreeInfo.skipParens(tree.selector), CRT_FLOW_CONTROLLER,
            startpcCrt, code.curPc());
    } else {
      ...
      int opcode = nlabels > 0
          && table_space_cost + 3 * table_time_cost <= lookup_space_cost + 3
              * lookup_time_cost ? tableswitch : lookupswitch;
  ...
  public void visitTry(final JCTry tree) {
    ...
    if (!useJsrLocally) {
      useJsrLocally = (stackMap == StackMapFormat.NONE)
          && (jsrlimit <= 0 || jsrlimit < 100
      ...
      void gen() {
        if (useJsrLocally) {
          if (tree.finalizer != null) {
            ...
            tryEnv.info.cont = new Chain(code.emitJump(jsr), tryEnv.info.cont,
                jsrState);
          }
  ...
  void genTry(JCTree body, List<JCCatch> catchers, Env<GenContext> env) {
    ...
    boolean hasFinalizer = env.info.finalize != null
    ...
    if (hasFinalizer) {
      ...
      genFinalizer(env);
      excVar.load();
      registerCatch(body.pos(), startseg, env.info.gaps.next().intValue(),
          catchallpc, 0);
      code.emitop0(athrow);
      code.markDead();
  ...
  void genCatch(JCCatch tree, Env<GenContext> env, int startpc, int endpc,
      List<Integer> gaps) {
    if (startpc != endpc) {
      List<JCExpression> subClauses = TreeInfo.isMultiCatch(tree) ? ((JCTypeUnion) tree.param.vartype).alternatives
   ...
   */
  void registerCatch(DiagnosticPosition pos, int startpc, int endpc,
  ...
  int estimateCodeComplexity(JCTree tree) {
    ...
    class ComplexityScanner extends TreeScanner {
      ...
      // note: for break, continue, and return we don't take unwind() into
      // account.
  ...
  public void visitApply(JCMethodInvocation tree) {
    ...
    genArgs(tree.args, TreeInfo.symbol(tree.meth).externalType(types)
        .getParameterTypes());
    result = m.invoke();
  }
  ...
  public void visitAssignop(JCAssignOp tree) {
    ...
    if (operator.opcode == string_add) {
      ...
      if ((tree.getTag() == JCTree.PLUS_ASG || tree.getTag() == JCTree.MINUS_ASG)
        ...
        if (tree.getTag() == JCTree.MINUS_ASG)
          ival = -ival;
    ...
    if (tree.getTag() == JCTree.NOT) {
      ...
      case JCTree.PREDEC:
        ...
        if (od instanceof LocalItem
          ...
          if (od.typecode != INTcode && Code.truncate(od.typecode) == INTcode)
    ...
    if (operator.opcode == string_add) {
      ...
      if (!lcond.isTrue()) {
        ...
        result = items.makeCondItem(rcond.opcode,
            Code.mergeChains(trueJumps, rcond.trueJumps), rcond.falseJumps);
      } else {
        result = lcond;
      }
    } else {
  ...
  Symbol getStringBufferAppend(JCTree tree, Type t) {
    ...
    if (method == null) {
      method = rs.resolveInternalMethod(tree.pos(), attrEnv, stringBufferType,
          names.append, List.of(t), null);
  ...
  public void visitSelect(JCFieldAccess tree) {
    ...
    Item base = (selectSuper) ? items.makeSuperItem() : genExpr(tree.selected,
        tree.selected.type);
   ...
   * 
   * @param env
   *          The attribution environment that belongs to the outermost class
   *          containing this class definition. We need this for resolving some
   *          additional symbols.
   * @param cdef
  ...
  static class GenContext {
    ...
    /**
     * A closure that generates the finalizer of the current environment. Only
     * set for Synchronized and Try contexts.
     */
    ...
    /**
     * A list buffer containing all gaps in the finalizer range, where a catch
     * all exception should not apply.
     */
