...
public class CRTable implements CRTFlags {
  ...
  /**
   * The tree of the method this table is intended for. We should traverse this
   * tree to get source ranges.
   */
   ...
   * 
   * @param tree
   *          The tree or the list of trees for which we are storing the code
   *          pointers.
   * @param flags
  ...
  public int writeCRT(ByteBuffer databuf, Position.LineMap lineMap, Log log) {
    ...
    for (List<CRTEntry> l = entries.toList(); l.nonEmpty(); l = l.tail) {
      ...
      if (crtDebug) {
        ...
        System.out.print("Start: pos = " + pos.startPos + ", pc = "
            + entry.startPc);
      }
      ...
      if (crtDebug) {
        System.out.print("End:   pos = " + pos.endPos + ", pc = "
            + (entry.endPc - 1));
      }
  ...
  /**
   * Source file positions in CRT are integers in the format: line-number <<
   * LINESHIFT + column-number
   */
  ...
  /**
   * This class contains methods to compute source positions for trees. Extends
   * Tree.Visitor to traverse the abstract syntax tree.
   */
  class SourceComputer extends JCTree.Visitor {
    ...
    /**
     * Visitor method: compute source positions for a list of catch clauses in
     * try statements.
     */
    ...
    public void visitBlock(JCBlock tree) {
      SourceRange sr = new SourceRange(startPos(tree), endPos(tree));
      csp(tree.stats); // doesn't compare because block's ending position is
                       // defined
      result = sr;
    }
