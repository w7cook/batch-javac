...
public class ClassWriter extends ClassFile {
   ...
   * innerclass attributes 'm' -- methods For example, to dump everything: javac
   * -XDdumpmodifiers=cifm MyProg.java
   */
  ...
  // where
  private final static String[] flagName = { "PUBLIC", "PRIVATE", "PROTECTED",
      "STATIC", "FINAL", "SUPER", "VOLATILE", "TRANSIENT", "NATIVE",
  ...
  void writeCode(Code code) {
    ...
    if (code.varBufferSize > 0) {
      ...
      for (int i = 0; i < code.varBufferSize; i++) {
        ...
        Assert.check(var.length >= 0 && (var.start_pc + var.length) <= code.cp);
    ...
    case CLDC:
      for (int i = 0; i < nframes; i++) {
        ...
        for (int j = 0; j < frame.locals.length; j += (target
            .generateEmptyAfterBig() ? 1 : Code.width(frame.locals[j]))) {
          localCount++;
        }
        if (debugstackmap)
        ...
        for (int j = 0; j < frame.stack.length; j += (target
            .generateEmptyAfterBig() ? 1 : Code.width(frame.stack[j]))) {
          stackCount++;
        }
        if (debugstackmap)
  ...
  long getLastModified(FileObject filename) {
    ...
    try {
      ...
      throw new AssertionError(
          "CRT: couldn't get source file modification date: " + e.getMessage());
    }
    return mod;
  }
}
