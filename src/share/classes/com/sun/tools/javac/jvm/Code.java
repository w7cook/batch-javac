...
public class Code {
  ...
  /**
   * A buffer for expression catch data. Each enter is a vector of four unsigned
   * shorts.
   */
   ...
   * unsigned shorts.
   */
  List<char[]> lineInfo = List.nil(); // handled in
                                      // stack
                                      // fashion
   ...
   * statement, NOPOS otherwise. We need this to emit line numbers lazily, which
   * we need to do because of jump-to-jump optimization.
   */
  ...
  /**
   * Construct a code object, given the settings of the fatcode, debugging info
   * switches and the CharacterRangeTable.
   */
  public Code(MethodSymbol meth, boolean fatcode, Position.LineMap lineMap,
      boolean varDebugInfo, StackMapFormat stackMap, boolean debugCode,
      CRTable crt, Symtab syms, Types types, Pool pool) {
    this.meth = meth;
  ...
  private void emitop(int op) {
    ...
    if (alive) {
      ...
      if (debugCode)
        System.err.println("emit@" + cp + " stack=" + state.stacksize + ": "
            + mnem(op));
      emit1(op);
    }
  }
  ...
  public void emitop0(int op) {
    ...
    case aaload: {
      ...
      Type stackType = a.tag == BOT ? syms.objectType : types.erasure(types
          .elemtype(a));
  ...
  /**
   * Emit an opcode with two one-byte operand fields; widen if either field does
   * not fit in a byte.
   */
  ...
  public void emitop2(int op, int od) {
    ...
    case putstatic:
      ...
      break;
    case new_:
      state.push(uninitializedObject(((Symbol) (pool.pool[od])).erasure(types),
          cp - 3));
      break;
    case sipush:
      state.push(syms.intType);
    ...
    case checkcast: {
      ...
      Type t = (o instanceof Symbol) ? ((Symbol) o).erasure(types) : types
          .erasure(((Type) o));
      state.push(t);
      break;
    }
  ...
  /**
   * Declare an entry point with initial state plus a pushed value; return
  ...
  void emitCLDCStackMap(int pc, int localsSize) {
    ...
    if (stackMapBuffer == null) {
      ...
      StackMapFrame[] newStackMapBuffer = new StackMapFrame[stackMapBufferSize << 1];
      System.arraycopy(stackMapBuffer, 0, newStackMapBuffer, 0,
          stackMapBufferSize);
  ...
  void emitStackMapFrame(int pc, int localsSize) {
    ...
    if (stackMapTableBuffer == null) {
      ...
      StackMapTableFrame[] newStackMapTableBuffer = new StackMapTableFrame[stackMapBufferSize << 1];
      System.arraycopy(stackMapTableBuffer, 0, newStackMapTableBuffer, 0,
          stackMapBufferSize);
      stackMapTableBuffer = newStackMapTableBuffer;
    }
    stackMapTableBuffer[stackMapBufferSize++] = StackMapTableFrame.getInstance(
        frame, lastFrame.pc, lastFrame.locals, types);
  ...
  /**
   * A chain represents a list of unresolved jumps. Jump locations are sorted in
   * decreasing order.
   */
  public static class Chain {
    ...
    /**
     * The machine state after the jump instruction. Invariant: all elements of
     * a chain list have the same stacksize and compatible stack and register
     * contents.
     */
    Code.State state;
    ...
    /**
     * Construct a chain from its jump position, stacksize, previous chain, and
     * machine state.
     */
  ...
  /**
   * Emit a branch with given opcode; return its chain. branch differs from jump
   * in that jsr is treated as no-op.
   */
  ...
  public void resolve(Chain chain, int target) {
    ...
    for (; chain != null; chain = chain.next) {
      ...
      if (get1(chain.pc) == goto_ && chain.pc + 3 == target && target == cp
        ...
        Assert.check(!alive || chain.state.stacksize == newState.stacksize
  ...
  public static Chain mergeChains(Chain chain1, Chain chain2) {
    ...
    if (chain1 == null)
      return chain2;
    Assert.check(chain1.state.stacksize == chain2.state.stacksize
        && chain1.state.nlocks == chain2.state.nlocks);
    if (chain1.pc < chain2.pc)
      return new Chain(chain2.pc, mergeChains(chain1, chain2.next),
          chain2.state);
  ...
  class State implements Cloneable {
    ...
    Type pop2() {
      ...
      Type result = stack[stacksize];
      stack[stacksize] = null;
      Assert.check(stack[stacksize + 1] == null && result != null
          && width(result) == 2);
      return result;
    }
    ...
    /**
     * Force the top of the stack to be treated as this supertype of its current
     * type.
     */
    ...
    State join(State other) {
      ...
      for (int i = 0; i < stacksize;) {
        ...
        Type result = t == tother ? t : types.isSubtype(t, tother) ? tother
            : types.isSubtype(tother, t) ? t : error();
        int w = width(result);
        stack[i] = result;
        if (w == 2)
  ...
  static class LocalVar {
    ...
    public String toString() {
      return "" + sym + " in register " + ((int) reg) + " starts at pc="
          + ((int) start_pc) + " length=" + ((int) length);
    }
  };
  ...
  void putVar(LocalVar var) {
    if (!varDebugInfo)
      return;
    if ((var.sym.flags() & Flags.SYNTHETIC) != 0)
      return;
    if (varBuffer == null)
  ...
  public void endScopes(int first) {
    ...
    nextreg = first;
    for (int i = nextreg; i < prevNextReg; i++)
      endScope(i);
  }
