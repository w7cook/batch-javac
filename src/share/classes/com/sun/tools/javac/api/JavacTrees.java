...
public class JavacTrees extends Trees {
  ...
  public JCTree getTree(Element e, AnnotationMirror a, AnnotationValue v) {
    Pair<JCTree, JCCompilationUnit> treeTopLevel = elements.getTreeAndTopLevel(
        e, a, v);
  ...
  public TreePath getPath(Element e, AnnotationMirror a, AnnotationValue v) {
    final Pair<JCTree, JCCompilationUnit> treeTopLevel = elements
        .getTreeAndTopLevel(e, a, v);
    if (treeTopLevel == null)
      return null;
  ...
  public boolean isAccessible(Scope scope, Element member, DeclaredType type) {
    if (scope instanceof JavacScope && member instanceof Symbol
        && type instanceof com.sun.tools.javac.code.Type) {
      Env<AttrContext> env = ((JavacScope) scope).env;
      return resolve.isAccessible(env, (com.sun.tools.javac.code.Type) type,
          (Symbol) member, true);
    } else
      return false;
  }
    ...
    for (; l.nonEmpty(); l = l.tail) {
      ...
      default:
        ...
        if (field != null && field.getInitializer() == tree) {
          ...
          JCExpression expr = copier.copy((JCExpression) tree,
              (JCTree) path.getLeaf());
  ...
  private Env<AttrContext> attribExprToTree(JCExpression expr,
      Env<AttrContext> env, JCTree tree) {
    JavaFileObject prev = log.useSource(env.toplevel.sourcefile);
    try {
  ...
  /**
   * Makes a copy of a tree, noting the value resulting from copying a
   * particular leaf.
   **/
   ...
   * @returns TypeMirror corresponding to the original type, replaced by the
   *          ErrorType. noType (type.tag == NONE) is returned if there is no
   *          original type.
   */
  public TypeMirror getOriginalType(javax.lang.model.type.ErrorType errorType) {
    if (errorType instanceof com.sun.tools.javac.code.Type.ErrorType) {
      return ((com.sun.tools.javac.code.Type.ErrorType) errorType)
          .getOriginalType();
    }
