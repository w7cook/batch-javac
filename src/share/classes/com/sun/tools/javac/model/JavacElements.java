...
public class JavacElements implements Elements {
  ...
  /**
   * Use a new context. May be called from outside to update internal state for
   * a new annotation-processing round.
   */
  ...
  /**
   * An internal-use utility that creates a reified annotation. This overloaded
   * version take annotation inheritance into account.
   */
  ...
  /**
   * Returns a symbol given the type's or packages's canonical name, or null if
  ...
  private <S extends Symbol> S nameToSymbol(String nameStr, Class<S> clazz) {
    ...
    try {
      ...
      return (sym.kind != Kinds.ERR && sym.exists() && clazz.isInstance(sym) && name
          .equals(sym.getQualifiedName())) ? clazz.cast(sym) : null;
  ...
  /**
   * Returns the tree for an annotation given a list of annotations in which to
   * search (recursively) and their corresponding trees. Returns null if the
  ...
  /**
   * Returns the tree node corresponding to this element, or null if none can be
   * found.
   */
        ...
        if (overrider.sym.kind == e.sym.kind
          ...
          if (overrider.sym.getKind() == ElementKind.METHOD
              && overrides((ExecutableElement) overrider.sym,
                  (ExecutableElement) e.sym, (TypeElement) type.asElement())) {
            continue members;
          }
        }
  ...
  /**
   * Print a representation of the elements to the given writer in the specified
   * order. The main purpose of this method is for diagnostics. The exact format
   * of the output is <em>not</em> specified and is subject to change.
   * 
   * @param w
  ...
  public void printElements(java.io.Writer w, Element... elements) {
    for (Element element : elements)
      (new PrintingProcessor.PrintingElementVisitor(w, this)).visit(element)
          .flush();
  }
   ...
   * corresponding to the given element, annotation and value. If the element is
   * null, null is returned. If the annotation is null or cannot be found, the
   * tree node and compilation unit for the element is returned. If the
   * annotation value is null or cannot be found, the tree node and compilation
   * unit for the annotation is returned.
   */
