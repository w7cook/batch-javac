...
public class Symtab {
  ...
  public final Type dateType;
  public final Type timeType;  
   ...
   * indexed by flat names. The table does not contain local classes. It should
   * be updated from the outside to reflect classes defined by compiled source
   * files.
   */
   ...
   * updated from outside to reflect packages defined by compiled source files.
   */
  ...
  private void enterBinop(String name, Type left, Type right, Type res,
    ...
    predefClass.members().enter(
        new OperatorSymbol(names.fromString(name), new MethodType(List.of(left,
            right), res, List.<Type> nil(), methodClass), opcode, predefClass));
  }

  /**
   * Enter a binary operation, as above but with two opcodes, which get encoded
   * as (opcode1 << ByteCodeTags.preShift) + opcode2.
   * 
   * @param opcode1
   *          First opcode.
   * @param opcode2
      ...
      sym.completer = new Completer() {
        public void complete(Symbol sym) throws CompletionFailure {
          try {
            ...
            MethodSymbol boxMethod = new MethodSymbol(PUBLIC | STATIC, n,
                new MethodType(List.of(type), sym.type, List.<Type> nil(),
                    methodClass), sym);
            sym.members().enter(boxMethod);
            MethodSymbol unboxMethod = new MethodSymbol(PUBLIC,
                type.tsym.name.append(names.Value), // x.intValue()
                new MethodType(List.<Type> nil(), type, List.<Type> nil(),
                    methodClass), sym);
  ...
  protected Symtab(Context context) throws CompletionFailure {
    ...
    unknownSymbol = new ClassSymbol(PUBLIC | STATIC | ACYCLIC,
        names.fromString("<any?>"), null, rootPackage);
    ...
    autoCloseableClose = new MethodSymbol(PUBLIC, names.close, new MethodType(
        List.<Type> nil(), voidType, List.of(exceptionType), methodClass),
        autoCloseableType.tsym);
    trustMeType = enterClass("java.lang.SafeVarargs");
    dateType = enterClass("java.sql.Date");
    timeType = enterClass("java.sql.Time");
    ...
    arrayCloneMethod = new MethodSymbol(PUBLIC, names.clone, new MethodType(
        List.<Type> nil(), objectType, List.<Type> nil(), methodClass),
        arrayClass);
