...
public abstract class Symbol implements Element {
   ...
   * should be accessed through the accessor method to make sure that the class
   * symbol is loaded.
   */
  public long flags() {
    return flags_field;
  ...
  /**
   * The Java source which this symbol represents. A description of this symbol;
   * overrides Object.
   */
   ...
   * reporting.
   * 
   * @return null if the symbol is a package or a toplevel class defined in the
   *         default package; otherwise, the owner symbol is returned
   */
  public Symbol location() {
    if (owner.name == null
        || (owner.name.isEmpty() && owner.kind != PCK && owner.kind != TYP)) {
      return null;
    }
    return owner;
  }
  ...
  /**
   * The external type of a symbol. This is the symbol's erased type except for
   * constructors of inner classes which get the enclosing instance class added
   * as first argument.
   */
  ...
  /**
   * Recognize if this symbol was split from a @PolymorphicSignature symbol in
   * the source.
   */
  ...
  /**
   * Fully check membership: hierarchy, protection, and hiding. Does not exclude
   * methods not inherited due to overriding.
   */
  public boolean isMemberOf(TypeSymbol clazz, Types types) {
    return owner == clazz || clazz.isSubClass(owner, types)
        && isInheritedIn(clazz, types) && !hiddenIn((ClassSymbol) clazz, types);
  }
   ...
   * interface, it is already assumed that the interface is a superinterface of
   * given class.
   * 
   * @param clazz
   *          The class for which we want to establish membership. This must be
   *          a subclass of the member's owner.
   */
  ...
  /**
   * The (variable or method) symbol seen as a member of given class type`site'
   * (this might change the symbol's type). This is used exclusively for
   ...
   * members of class `origin'? It is assumed that _other is a member of origin.
   * 
   * It is assumed that both symbols have the same name. The static modifier is
  ...
  public static class TypeSymbol extends Symbol implements TypeParameterElement {
    ...
    /**
     * form a fully qualified name from a name and an owner, after converting to
     * flat representation
     */
    ...
    /**
     * A total ordering between type symbols that refines the class inheritance
     * graph.
     * 
  ...
  public static class PackageSymbol extends TypeSymbol implements
    ...
    /**
     * A package "exists" if a type or package that exists has been seen within
     * it.
     */
    public boolean exists() {
  ...
  public static class ClassSymbol extends TypeSymbol implements TypeElement {
    ...
    /**
     * a scope for all class members; variables, methods and inner classes type
     * parameters are not part of this scope
     */
    public Scope members_field;
  ...
  public static class VarSymbol extends Symbol implements VariableElement {
    ...
    /**
     * The variable's address. Used for different purposes during flow analysis,
     * translation and code generation. Flow analysis: If this is a blank final
     * or local variable, its sequence number. Translation: If this is a private
     * field, its access number. Code generation: If this is a local variable,
     * its logical slot number.
     */
    public int adr = -1;
    ...
    public Symbol asMemberOf(Type site, Types types) {
      return new VarSymbol(flags_field, name, types.memberType(site, this),
          owner);
    }
    ...
    public void setLazyConstValue(final Env<AttrContext> env, final Attr attr,
    ...
    /**
     * The variable's constant value, if this is a constant. Before the constant
     * value is evaluated, it points to an initalizer environment. If this is
     * not a constant, it can be used for other stuff.
     */
    private Object data;
  ...
  public static class MethodSymbol extends Symbol implements ExecutableElement {
    ...
    /**
     * For an attribute field accessor, its default value if any. The value is
     * null if none appeared in the method declaration.
     */
    ...
    public String toString() {
      if ((flags() & BLOCK) != 0) {
        ...
        String s = (name == name.table.names.init) ? owner.name.toString()
            : name.toString();
        if (type != null) {
          if (type.tag == FORALL)
    ...
    public Symbol implementedIn(TypeSymbol c, Types types) {
      Symbol impl = null;
      for (Scope.Entry e = c.members().lookup(name); impl == null
    ...
    /**
     * Will the erasure of this method be considered by the VM to override the
     * erasure of the other when seen from class `origin'?
     */
    ...
    /**
     * The implementation of this (abstract) symbol in class origin, from the
     * VM's point of view, null if method does not have an implementation in
     * class.
     * 
     * @param origin
    ...
    /**
     * Does this symbol override `other' symbol, when both are seen as members
     * of class `origin'? It is assumed that _other is a member of origin.
     * 
    ...
    public boolean overrides(Symbol _other, TypeSymbol origin, Types types,
      ...
      if ((flags() & ABSTRACT) != 0 || (other.flags() & ABSTRACT) == 0
          || !other.isOverridableIn(origin) || !this.isMemberOf(origin, types))
        return false;
    ...
    private static final Filter<Symbol> implementation_filter = new Filter<Symbol>() {
      public boolean accepts(Symbol s) {
        return s.kind == Kinds.MTH && (s.flags() & SYNTHETIC) == 0;
      }
    };

    public MethodSymbol implementation(TypeSymbol origin, Types types,
        boolean checkResult, Filter<Symbol> implFilter) {
      MethodSymbol res = types.implementation(this, origin, checkResult,
          implFilter);
      if (res != null)
        return res;
    ...
    public Symbol asMemberOf(Type site, Types types) {
      return new MethodSymbol(flags_field, name, types.memberType(site, this),
          owner);
    }
  ...
  /**
   * A visitor for symbols. A visitor is used to implement operations (or
   * relations) on symbols. Most common operations on types are binary relations
   * and this interface is designed for binary relations, that is, operations on
   * the form Symbol&nbsp;&times;&nbsp;P&nbsp;&rarr;&nbsp;R. <!-- In plain text:
   * Type x P -> R -->
   * 
   * @param <R>
