 ...
 */
public abstract class Printer implements Type.Visitor<String, Locale>,
   ...
   * 
   * @param locale
   *          the locale in which the string is to be rendered
   * @param key
   *          the key corresponding to the message to be displayed
   * @param args
   ...
   * 
   * @param t
   *          the captured type for which an id is to be retrieved
   * @param locale
   *          locale settings
  ...
  /**
   * Create a printer with default i18n support provided by Messages. By
   * default, captured types ids are generated using hashcode.
   * 
   * @param messages
   *          Messages class to be used for i18n
  ...
  /**
   * * Get a localized string representation for all the symbols in the input
   * list.
   * 
   * @param ts
    ...
    if (seenCaptured.contains(t))
      ...
      try {
        ...
        return localize(locale, "compiler.misc.type.captureof",
            capturedVarId(t, locale), visit(t.wildcard, locale));
      } finally {
  ...
  public String visitType(Type t, Locale locale) {
    String s = (t.tsym == null || t.tsym.name == null) ? localize(locale,
        "compiler.misc.type.none") : t.tsym.name.toString();
    return s;
  }

  /**
   * Converts a class name into a (possibly localized) string. Anonymous inner
   * classes gets converted into a localized string.
   * 
   * @param t
   *          the type of the class whose name is to be rendered
   * @param longform
   *          if set, the class' fullname is displayed - if unset the short name
   *          is chosen (w/o package)
   * @param locale
   *          the locale in which the string is to be rendered
   * @return localized string representation
   */
  ...
  /**
   * Converts a set of method argument types into their corresponding localized
   * string representation.
   * 
   * @param args
   *          arguments to be rendered
   * @param varArgs
   *          if true, the last method argument is regarded as a vararg
   * @param locale
   *          the locale in which the string is to be rendered
   * @return localized string representation
   */
  ...
  public String visitClassSymbol(ClassSymbol sym, Locale locale) {
    return sym.name.isEmpty() ? localize(locale,
        "compiler.misc.anonymous.class", sym.flatname) : sym.fullname
        .toString();
  }
  ...
  public String visitMethodSymbol(MethodSymbol s, Locale locale) {
    if ((s.flags() & BLOCK) != 0) {
      ...
      String ms = (s.name == s.name.table.names.init) ? s.owner.name.toString()
          : s.name.toString();
      if (s.type != null) {
  ...
  public String visitPackageSymbol(PackageSymbol s, Locale locale) {
    return s.isUnnamed() ? localize(locale, "compiler.misc.unnamed.package")
        : s.fullname.toString();
  }
