...
public class Flags {
  ...
  /**
   * Flag is set for package symbols if a package has a member or directory and
   * therefore exists.
   */
  ...
  /**
   * Flag for class symbols is set and later re-set as a lock in Enter to detect
   * cycles in the superclass/superinterface relations. Similarly for
   * constructor call cycle detection in Attr.
   */
  public static final int LOCKED = 1 << 27;

  /**
   * Flag for class symbols is set and later re-set to indicate that a class has
   * been entered but has not yet been attributed.
   */
  ...
  /**
   * Flag for annotation type symbols to indicate it has been checked and found
   * acyclic.
   */
  ...
  /**
   * Flag that marks a hypothetical method that need not really be generated in
   * the binary, but is present in the symbol table to simplify checking for
   * erasure clashes.
   */
  ...
  /**
   * Flag that marks a signature-polymorphic invoke method. (These occur inside
   * java.lang.invoke.MethodHandle.)
   */
  public static final long POLYMORPHIC_SIGNATURE = 1L << 40;

  /**
   * Flag that marks a special kind of bridge methods (the ones that come from
   * restricted supertype bounds)
   */
   ...
   * Modifier masks.
   */
  public static final int AccessFlags = PUBLIC | PROTECTED | PRIVATE,
      LocalClassFlags = FINAL | ABSTRACT | STRICTFP | ENUM | SYNTHETIC,
      MemberClassFlags = LocalClassFlags | INTERFACE | AccessFlags,
      ClassFlags = LocalClassFlags | INTERFACE | PUBLIC | ANNOTATION,
      InterfaceVarFlags = FINAL | STATIC | PUBLIC, VarFlags = AccessFlags
          | FINAL | STATIC | VOLATILE | TRANSIENT | ENUM,
      ConstructorFlags = AccessFlags, InterfaceMethodFlags = ABSTRACT | PUBLIC,
      MethodFlags = AccessFlags | ABSTRACT | STATIC | NATIVE | SYNCHRONIZED
          | FINAL | STRICTFP;
  ...
  private static Map<Long, Set<Modifier>> modifierSets = new java.util.concurrent.ConcurrentHashMap<Long, Set<Modifier>>(
      64);
  ...
  public enum Flag {
    ...
    PUBLIC("public"), PRIVATE("private"), PROTECTED("protected"), STATIC(
        ...
        "volatile"), TRANSIENT("transient"), NATIVE("native"), INTERFACE(
        "interface"), ABSTRACT("abstract"), STRICTFP("strictfp"), BRIDGE(
        "bridge"), SYNTHETIC("synthetic"), DEPRECATED("deprecated"), HASINIT(
        "hasinit"), ENUM("enum"), IPROXY("iproxy"), NOOUTERTHIS("noouterthis"), EXISTS(
        "exists"), COMPOUND("compound"), CLASS_SEEN("class_seen"), SOURCE_SEEN(
        "source_seen"), LOCKED("locked"), UNATTRIBUTED("unattributed"), ANONCONSTR(
        "anonconstr"), ACYCLIC("acyclic"), PARAMETER("parameter"), VARARGS(
        "varargs"), PACKAGE("package");
