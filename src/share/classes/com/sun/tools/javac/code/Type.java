...
public class Type implements PrimitiveType {
  ...
  /**
   * The constant value of this type, null if this type does not have a constant
   * value attribute. Only primitive types and strings (ClassType) can have a
  ...
  /**
   * Get the representation of this type used for modelling purposes. By
   * default, this is itself. For ErrorType, a different value may be provided,
   */
  ...
  /**
   * The Java source which this type list represents. A List is represented as a
   * comma-spearated listing of the elements in that list.
   */
  ...
  /**
   * This method is analogous to isSameType, but weaker, since we never complete
   * classes. Where isSameType would complete a class, equals assumes that the
   ...
   * its parameters. An array type is a raw type if its element type is raw. All
   * other types are not raw. Type validation will ensure that the only raw
   * types in a program are types that miss all their type variables.
   */
  ...
  public static class ClassType extends Type implements DeclaredType {
    ...
    /**
     * The enclosing type of this type. If this is the type of an inner class,
     * outer_field refers to the type of its enclosing instance class, in all
     * other cases it referes to noType.
     */
    ...
    /**
     * A cache variable for the type parameters of this type, appended to all
     * parameters of its enclosing class.
     * 
     * @see #allparams
     */
    ...
    public ClassType(Type outer, List<Type> typarams, TypeSymbol tsym) {
      ...
      /*
       * // this can happen during error recovery assert outer.isParameterized()
       * ? typarams.length() == tsym.type.typarams().length() : outer.isRaw() ?
       * typarams.length() == 0 : true;
       */
    }
     ...
     */
    public boolean isRaw() {
      return this != tsym.type && // necessary, but not sufficient condition
    ...
    public boolean contains(Type elem) {
      return elem == this
          ...
          || (isCompound() && (supertype_field.contains(elem) || contains(
              interfaces_field, elem)));
    }
  ...
  public static class MethodType extends Type implements ExecutableType {
     ...
     * The Java source which this type represents.
     * 
     * XXX 06/09/99 iris This isn't correct Java syntax, but it probably should
     * be.
     */
    public String toString() {
    ...
    public boolean isErroneous() {
      return isErroneous(argtypes) || restype != null && restype.isErroneous();
    }
  ...
  public static class TypeVar extends Type implements TypeVariable {
    ...
    /**
     * The upper bound of this type variable; set from outside. Must be nonempty
     * once it is set. For a bound, `bound' is the bound type itself. Multiple
     * bounds are expressed as a single class type which has the individual
     * bounds as superclass, respectively interfaces. The class type then has as
     * `tsym' a compiler generated class `c', which has a flag COMPOUND and
     * whose owner is the type variable itself. Furthermore, the erasure_field
     * of the class points to the first class or interface bound.
     */
    public Type bound = null;

    /**
     * The lower bound of this type variable. TypeVars don't normally have a
     * lower bound, so it is normally set to syms.botType. Subtypes, such as
     * CapturedType, may provide a different value.
     */
    public Type lower;
  ...
  /**
   * A captured type variable comes from wildcards which can have both upper and
   * lower bound. CapturedType extends TypeVar with a lower bound.
   */
  public static class CapturedType extends TypeVar {
    ...
    @Override
    public String toString() {
      return "capture#" + (hashCode() & 0xFFFFFFFFL) % Printer.PRIME + " of "
          + wildcard;
    }
  }
  ...
  public static class ForAll extends DelegatedType implements ExecutableType {
    ...
    /**
     * Replaces this ForAll's typevars with a set of concrete Java types and
     * returns the instantiated generic type. Subclasses should override in
     * order to check that the list of types is a valid instantiation of the
     * ForAll's typevars.
     * 
     * @param actuals
     *          list of actual types
     * @param types
     *          types instance
     * @return qtype where all occurrences of tvars are replaced by types in
     *         actuals
     */
    ...
    public enum ConstraintKind {
      /**
       * upper bound constraint (a type variable must be instantiated with a
       * type T, where T is a subtype of all the types specified by its EXTENDS
       * constraints).
       */
      EXTENDS,
      /**
       * lower bound constraint (a type variable must be instantiated with a
       * type T, where T is a supertype of all the types specified by its SUPER
       * constraints).
       */
      SUPER,
      /**
     ...
     * 
     * @param tv
     *          the type-variable for which the constraint is to be retrieved
     * @param ck
     *          the constraint kind to be retrieved
   ...
   * the form Type&nbsp;&times;&nbsp;S&nbsp;&rarr;&nbsp;R. <!-- In plain text:
   * Type x S -> R -->
   * 
   * @param <R>
