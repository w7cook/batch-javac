...
public class HtmlDocletWriter extends HtmlDocWriter {
   ...
   * For example, if the file getting generated is "java/lang/Object.html", then
   * the relative path string is "../../". This string can be empty if the file
   * getting generated is in the destination directory.
   */
   ...
   * example, if the file getting generated is "java/lang/Object.html", then the
   * path string is "java/lang".
   */
  public String path = "";
   ...
   */
  public HtmlDocletWriter(ConfigurationImpl configuration, String path,
  ...
  /**
   * Replace {&#064;docRoot} tag used in options that accept HTML text, such as
   * -header, -footer, -top and -bottom, and when converting a relative HREF
   * where commentTagsToString inserts a {&#064;docRoot} where one was missing.
   ...
   * <p>
   * For example, for "-d docs" and -header containing {&#064;docRoot}, when the
   * HTML page for source file p/C1.java is being generated, the {&#064;docRoot}
   * tag would be inserted into the header as "../", the relative path from
   * docs/p/ to docs/ (the document root).
   * <p>
  ...
  public String replaceDocRootDir(String htmlstr) {
    ...
    while (true) {
      if (configuration.docrootparent.length() > 0) {
        ...
        // If next {@docRoot}/.. pattern not found, append rest of htmlstr and
        // exit loop
        if (index < 0) {
  ...
  public Content getAllClassesLinkScript(String id) {
    ...
    String scriptCode = "<!--" + DocletConstants.NL
        ...
        + DocletConstants.NL + "  if(window==top) {" + DocletConstants.NL
        + "    allClassesLink.style.display = \"block\";" + DocletConstants.NL
        + "  }" + DocletConstants.NL + "  else {" + DocletConstants.NL
  ...
  private void addMethodInfo(MethodDoc method, Content dl) {
    ...
    if ((intfacs.length > 0 && new ImplementedMethods(method,
        this.configuration).build().length > 0) || overriddenMethod != null) {
      MethodWriterImpl.addImplementsInfo(this, method, dl);
      if (overriddenMethod != null) {
        MethodWriterImpl.addOverridden(this, method.overriddenType(),
            overriddenMethod, dl);
      }
    }
  }
   ...
   * 
   * @param doc
   *          the doc for which the tags will be generated
   * @param htmltree
  ...
  /**
   * Check whether there are any tags for Serialization Overview section to be
   * printed.
   * 
   * @param field
   *          the FieldDoc object to check for tags.
  ...
  public Content getTargetPackageLink(PackageDoc pd, String target,
      Content label) {
    return getHyperLink(pathString(pd, "package-summary.html"), "", label, "",
        target);
  }
   ...
   * 
   * @param title
   *          String window title to go in the &lt;TITLE&gt; tag
   * @param metakeywords
   *          Array of String keywords for META tag. Each element of the array
   *          is assigned to a separate META tag. Pass in null for no array.
   * @param includeScript
   *          boolean true if printing windowtitle script. False for files that
  ...
  public void printHtmlHeader(String title, String[] metakeywords,
    ...
    if (!configuration.notimestamp) {
      print("<!-- Generated by javadoc (build " + ConfigurationImpl.BUILD_DATE
          + ") on ");
      print(today());
      println(" -->");
    }
    ...
    if (!configuration.notimestamp) {
      ...
      println("<META NAME=\"date\" " + "CONTENT=\""
          + dateFormat.format(new Date()) + "\">");
    }
    if (metakeywords != null) {
      for (int i = 0; i < metakeywords.length; i++) {
        println("<META NAME=\"keywords\" " + "CONTENT=\"" + metakeywords[i]
            + "\">");
      }
    }
    println("");
   ...
   *          is assigned to a separate META tag. Pass in null for no array
   * @param includeScript
   *          true if printing windowtitle script false for files that appear in
   ...
   * 
   * @param header
   *          if true print the user provided header else print the user
   *          provided footer.
   */
   ...
   * 
   * @param header
   *          If true print navigation bar at the top of the page else print the
   *          nevigation bar at the bottom.
   */
  ...
  protected void addNavLinks(boolean header, Content body) {
    if (!configuration.nonavbar) {
      ...
      Content aboutDiv = HtmlTree.DIV(HtmlStyle.aboutLanguage,
          getUserHeaderFooter(header));
  ...
  /**
   * Print the word "NEXT" to indicate that no link is available. Override this
  ...
  /**
   * Print the word "PREV" to indicate that no link is available. Override this
  ...
  protected Content getNavLinkContents() {
    Content linkContent = getHyperLink(relativePath + "overview-summary.html",
        "", overviewLabel, "", "");
  ...
  /**
   * Description for a cell in the navigation bar, but with reverse high-light
   * effect.
   */
   ...
   * 
   * @param pkg
   *          Package to which link will be generated.
   */
  ...
  /**
   * Print the word "Package" in the navigation bar cell, to indicate that link
  ...
  /**
   * Print the word "Use" in the navigation bar cell, to indicate that link is
   * not available.
   */
  ...
  /**
   * Print link for next file. If next is null, just print the label without
   * linking it anywhere.
   * 
   * @param next
  ...
  protected void navHideLists(String link) {
    print(getHyperLinkString(link, "",
        configuration.getText("doclet.NO_FRAMES"), true, "", "", "_top"));
  }
  ...
  protected void navLinkTree() {
    ...
    if (packages.length == 1
        && configuration.root.specifiedClasses().length == 0) {
  ...
  protected Content getNavLinkTree() {
    ...
    if (packages.length == 1
        && configuration.root.specifiedClasses().length == 0) {
      treeLinkContent = getHyperLink(
          pathString(packages[0], "package-tree.html"), "", treeLabel, "", "");
    } else {
  ...
  /**
   * Print the word "Class" in the navigation bar cell, to indicate that class
  ...
  protected Content getNavLinkDeprecated() {
    Content linkContent = getHyperLink(relativePath + "deprecated-list.html",
        "", deprecatedLabel, "", "");
    Content li = HtmlTree.LI(linkContent);
    return li;
  }
  ...
  /**
   * Print link for generated index. If the user has used "-splitindex" command
  ...
  protected Content getNavLinkClassIndex() {
    Content allClassesContent = getHyperLink(relativePath
        + AllClassesFrameWriter.OUTPUT_FILE_NAME_NOFRAMES, "", allclassesLabel,
        "", "");
  ...
  /**
   * Get help file link. If user has provided a help file, then generate a link
   * to the user given file, which is already copied to current or destination
   * directory.
   * 
   * @return a content tree for the link
   */
   ...
   * printed is &lt;TABLE BORDER="1" CELLPADDING="3" CELLSPACING="0"
   * WIDTH="100%">
   */
   ...
   * 
   * @param summary
   *          the summary for the table tag summary attribute.
   */
  ...
  /**
   * Print Html tag for table elements. The tag printed is &lt;TD ALIGN="right"
   * VALIGN="top" WIDTH="1%"&gt;.
   */
  public void tdIndex() {
  ...
  public Content getPackageName(PackageDoc packageDoc) {
    return packageDoc == null || packageDoc.name().length() == 0 ? defaultPackageLabel
   ...
   * @param headingKey
   *          the caption for the deprecated package table
   * @param tableSummary
   *          the summary for the deprecated package table
   * @param tableHeader
   *          table headers for the deprecated package table
   * @param contentTree
   *          the content tree to which the deprecated package table will be
   *          added
   */
   ...
   * passed is "Object.html" and the name of the package is "java.lang", and if
   * the relative path is "../.." then returned string will be
   * "../../java/lang/Object.html"
   * 
   * @param pd
  ...
  public Content getPackageLink(PackageDoc pkg, Content label) {
    ...
    if (included || pkg == null) {
      return getHyperLink(pathString(pkg, "package-summary.html"), "", label);
    } else {
      String crossPkgLink = getCrossPackageLink(Util.getPackageName(pkg));
      if (crossPkgLink != null) {
  ...
  public void printSrcLink(ProgramElementDoc d, String label) {
    ...
    String href = relativePath + DocletConstants.SOURCE_OUTPUT_DIR_NAME
        + DirectoryManager.getDirectoryPath(cd.containingPackage()) + cd.name()
        + ".html#" + SourceToHTMLConverter.getAnchorName(d);
   ...
   * 
   * @param doc
   *          program element doc for which the link will be added
   * @param label
   *          label for the link
   * @param htmltree
  ...
  public void addSrcLink(ProgramElementDoc doc, Content label, Content htmltree) {
    ...
    String href = relativePath + DocletConstants.SOURCE_OUTPUT_DIR_NAME
        + DirectoryManager.getDirectoryPath(cd.containingPackage()) + cd.name()
        + ".html#" + SourceToHTMLConverter.getAnchorName(doc);
  ...
  public String getTypeParameterLinks(LinkInfoImpl linkInfo) {
    LinkFactoryImpl factory = new LinkFactoryImpl(this);
    return ((LinkOutputImpl) factory.getTypeParameterLinks(linkInfo, false))
        .toString();
  }
  ...
  /*************************************************************
   * Return a class cross link to external class documentation. The name must be
   * fully qualified to determine which package the class is in. The -link
   ...
   * @param refMemName
   *          the name of the member being referenced. This should be null or
   *          empty string if no member is being referenced.
   * @param label
  ...
  public String getCrossClassLink(String qualifiedClassName, String refMemName,
      String label, boolean strong, String style, boolean code) {
    ...
    while ((periodIndex = packageName.lastIndexOf('.')) != -1) {
      ...
      if (getCrossPackageLink(packageName) != null) {
        ...
        return getHyperLinkString(configuration.extern.getExternalLink(
            packageName, relativePath, className + ".html?is-external=true"),
            refMemName == null ? "" : refMemName,
            label == null || label.length() == 0 ? defaultLabel : label,
            strong, style, configuration.getText(
                "doclet.Href_Class_Or_Interface_Title", packageName), "");
      }
    }
    return null;
  }
   ...
   */
  public String getPreQualifiedClassLink(int context, ClassDoc cd,
      boolean isStrong) {
    String classlink = "";
  ...
  /**
   * Add the class link with the package portion of the label in plain text. If
   * the qualifier is excluded, it will not be included in the link label.
   * 
   * @param context
   *          the id of the context where the link will be added
   * @param cd
  ...
  public void addPreQualifiedClassLink(int context, ClassDoc cd,
    ...
    contentTree.addContent(new RawHtml(getLink(new LinkInfoImpl(context, cd, cd
        .name(), isStrong))));
  }
   ...
   * @param classDoc
   *          the classDoc that we should link to. This is not necessarily equal
   *          to doc.containingClass(). We may be inheriting comments.
   * @param doc
   *          the member being linked to.
   * @param label
   *          the label for the link.
  ...
  public String getDocLink(int context, ClassDoc classDoc, MemberDoc doc,
    ...
    if (!(doc.isIncluded() || Util.isLinkable(classDoc, configuration()))) {
      ...
      return getLink(new LinkInfoImpl(context, classDoc, getAnchor(emd), label,
          strong));
    } else if (doc instanceof MemberDoc) {
      return getLink(new LinkInfoImpl(context, classDoc, doc.name(), label,
          strong));
    } else {
      return label;
    }
  ...
  public String seeTagToString(SeeTag see) {
    ...
    label = (label.length() > 0) ? ((isplaintext) ? label : getCode() + label
        + getCodeEnd()) : "";
    ...
    // The text from the @see tag. We will output this text when a label is not
    // specified.
    ...
    if (refClass == null) {
      ...
      if (refPackage != null && refPackage.isIncluded()) {
        ...
        String packageName = isplaintext ? refPackage.name() : getCode()
            + refPackage.name() + getCodeEnd();
        result.append(getPackageLinkString(refPackage,
            label.length() == 0 ? packageName : label, false));
      } else {
        // @see is not referencing an included class or package. Check for cross
        // links.
        ...
        if (packageCrossLink != null) {
          ...
          configuration.getDocletSpecificMsg().warning(see.position(),
              "doclet.see.class_or_package_not_found", tagName, seetext);
          result.append((label.length() == 0) ? text : label);
        }
      }
    } else if (refMemName == null) {
      // Must be a class reference since refClass is not null and refMemName is
      // null.
      if (label.length() == 0) {
        label = (isplaintext) ? refClass.name() : getCode() + refClass.name()
            + getCodeEnd();
      ...
      // Must be a member reference since refClass is not null and refMemName is
      // not null.
      ...
      if (see.text().trim().startsWith("#")
        ...
        if (this instanceof ClassWriterImpl) {
          ...
          configuration.getDocletSpecificMsg().warning(see.position(),
              "doclet.see.class_or_package_not_accessible", tagName,
      ...
      text = (isplaintext) ? refMemName : getCode()
          + Util.escapeHtmlChars(refMemName) + getCodeEnd();
   ...
   * 
   * @param doc
   *          the doc for which the inline comment will be added
   * @param tag
   *          the inline tag to be added
   * @param htmltree
   ...
   * 
   * @param doc
   *          the doc for which the inline deprecated comment will be added
   * @param tag
   *          the inline tag to be added
   * @param htmltree
   *          the content tree to which the comment will be added
   ...
   *          the documentation tree to which the summary will be added
   */
  public void addSummaryComment(Doc doc, Tag[] firstSentenceTags,
      Content htmltree) {
   ...
   */
  private void addCommentTags(Doc doc, Tag[] tags, boolean depr, boolean first,
      Content htmltree) {
    if (configuration.nocomment) {
      return;
    }
   ...
   * 
   * @param holderTag
   *          specific tag where comment resides
   * @param doc
   *          specific doc where comment resides
   * @param tags
   *          array of text tags and inline tags (often alternating) present in
   *          the text of interest for this doc
   * @param isFirstSentence
    ...
    for (int i = 0; i < tags.length; i++) {
      ...
      if (tagelem instanceof SeeTag) {
        ...
        TagletOutput output = TagletWriter.getInlineTagOuput(
            configuration.tagletManager, holderTag, tagelem,
        ...
        if (originalLength == 0 && isFirstSentence
          ...
          // If Xdocrootparent switch ON, set the flag to remove the /..
          // occurance after
        ...
        // If Xdocrootparent switch ON, remove the /.. occurance after
        // {@docRoot} tag.
        if (textTagChange) {
          text = text.replaceFirst("/..", "");
          textTagChange = false;
        }
        // This is just a regular text tag. The text may contain html links
        // (<a>)
        ...
        // above in this method). [It might also happen when passed in on the
        // command
  ...
  private boolean shouldNotRedirectRelativeLinks() {
    return this instanceof AnnotationTypeWriter || this instanceof ClassWriter
        || this instanceof PackageSummaryWriter;
  }

  /**
   * Suppose a piece of documentation has a relative link. When you copy that
   * documetation to another place such as the index or class-use page, that
   ...
   * 
   * @param doc
   *          the Doc object whose documentation is being written.
   * @param text
  ...
  private String redirectRelativeLinks(Doc doc, String text) {
    ...
    if (doc instanceof ClassDoc) {
      redirectPathFromRoot = DirectoryManager.getDirectoryPath(((ClassDoc) doc)
          .containingPackage());
    } else if (doc instanceof MemberDoc) {
      redirectPathFromRoot = DirectoryManager
          .getDirectoryPath(((MemberDoc) doc).containingPackage());
    } else if (doc instanceof PackageDoc) {
      redirectPathFromRoot = DirectoryManager
          .getDirectoryPath((PackageDoc) doc);
    } else {
      return text;
    }
    ...
    if (begin >= 0) {
      ...
      while (begin >= 0) {
        if (textBuff.length() > begin + 2
            && !Character.isWhitespace(textBuff.charAt(begin + 2))) {
        ...
        if (begin == 0) {
          ...
          configuration.root.printWarning(doc.position(),
              configuration.getText("doclet.malformed_html_link_tag", text));
          break;
        }
        if (end == -1) {
          // Break without warning. This <a> tag is not necessarily malformed.
          // The text
        ...
        if (!(relativeLink.toLowerCase().startsWith("mailto:")
            ...
            || relativeLink.toLowerCase().startsWith("https:") || relativeLink
  ...
  public String removeNonInlineHtmlTags(String text) {
    ...
    String noninlinetags[] = { "<ul>", "</ul>", "<ol>", "</ol>", "<dl>",
        "</dl>", "<table>", "</table>", "<tr>", "</tr>", "<td>", "</td>",
        "<th>", "</th>", "<p>", "</p>", "<li>", "</li>", "<dd>", "</dd>",
        "<dir>", "</dir>", "<dt>", "</dt>", "<h1>", "</h1>", "<h2>", "</h2>",
        "<h3>", "</h3>", "<h4>", "</h4>", "<h5>", "</h5>", "<h6>", "</h6>",
        "<pre>", "</pre>", "<menu>", "</menu>", "<listing>", "</listing>",
        "<hr>", "<blockquote>", "</blockquote>", "<center>", "</center>",
        "<UL>", "</UL>", "<OL>", "</OL>", "<DL>", "</DL>", "<TABLE>",
        "</TABLE>", "<TR>", "</TR>", "<TD>", "</TD>", "<TH>", "</TH>", "<P>",
        "</P>", "<LI>", "</LI>", "<DD>", "</DD>", "<DIR>", "</DIR>", "<DT>",
        "</DT>", "<H1>", "</H1>", "<H2>", "</H2>", "<H3>", "</H3>", "<H4>",
        "</H4>", "<H5>", "</H5>", "<H6>", "</H6>", "<PRE>", "</PRE>", "<MENU>",
   ...
   * @return an array of strings representing the annotations being documented.
   */
  private List<String> getAnnotations(int indent, AnnotationDesc[] descList,
      boolean linkBreak) {
    ...
    for (int i = 0; i < descList.length; i++) {
      ...
      LinkInfoImpl linkInfo = new LinkInfoImpl(LinkInfoImpl.CONTEXT_ANNOTATION,
          annotationDoc);
      ...
      if (pairs.length > 0) {
        ...
        for (int j = 0; j < pairs.length; j++) {
          ...
          for (Iterator<AnnotationValue> iter = annotationTypeValues.iterator(); iter
              .hasNext();) {
  ...
  private String annotationValueToString(AnnotationValue annotationValue) {
    if (annotationValue.value() instanceof Type) {
      ...
      if (type.asClassDoc() != null) {
        ...
        linkInfo.label = (type.asClassDoc().isIncluded() ? type.typeName()
