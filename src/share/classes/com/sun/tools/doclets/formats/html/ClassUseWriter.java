...
public class ClassUseWriter extends SubWriterHolderWriter {
   ...
   */
  public ClassUseWriter(ConfigurationImpl configuration, ClassUseMapper mapper,
      String path, String filename, String relpath, ClassDoc classdoc)
      throws IOException {
    super(configuration, path, filename, relpath);
    this.classdoc = classdoc;
    if (mapper.classToPackageAnnotations.containsKey(classdoc.qualifiedName()))
      pkgToPackageAnnotations = new HashSet<PackageDoc>(
          mapper.classToPackageAnnotations.get(classdoc.qualifiedName()));
    ...
    if (pkgSet.size() > 0
      ...
      configuration.root
          .printWarning("Internal error: package sets don't match: " + pkgSet
              + " with: " + mapper.classToPackage.get(classdoc.qualifiedName()));
    }
    ...
    subinterfaceUseTableSummary = configuration.getText(
        "doclet.Use_Table_Summary",
    ...
    constructorUseTableSummary = configuration.getText(
        "doclet.Use_Table_Summary",
  ...
  public static void generate(ConfigurationImpl configuration,
    ...
    for (int i = 0; i < classes.length; i++) {
      ...
      // containing
      // package is not since it could still be linked from that package-use
      // page.
      if (!(configuration.nodeprecated && Util.isDeprecated(classes[i]
          .containingPackage())))
  ...
  private Map<String, List<ProgramElementDoc>> pkgDivide(
    ...
    List<? extends ProgramElementDoc> list = classMap.get(classdoc
        .qualifiedName());
    if (list != null) {
  ...
  public static void generate(ConfigurationImpl configuration,
    ...
    try {
      clsgen = new ClassUseWriter(configuration, mapper, path, filename,
          relpath, classdoc);
   ...
   * 
   * @param contentTree
   *          the content tree to which the class use information will be added
   */
   ...
   * 
   * @param contentTree
   *          the content tree to which the packages list will be added
   */
  protected void addPackageList(Content contentTree) throws IOException {
    Content table = HtmlTree.TABLE(
        0,
        3,
        0,
        useTableSummary,
   ...
   * 
   * @param contentTree
   *          the content tree to which the package annotation list will be
   *          added
   */
  protected void addPackageAnnotationList(Content contentTree)
      throws IOException {
    if ((!classdoc.isAnnotationType()) || pkgToPackageAnnotations == null
   ...
   * 
   * @param contentTree
   *          the content tree to which the class list will be added
   */
  protected void addClassList(Content contentTree) throws IOException {
    ...
    for (Iterator<PackageDoc> it = pkgSet.iterator(); it.hasNext();) {
      PackageDoc pkg = it.next();
      Content li = HtmlTree
          .LI(HtmlStyle.blockList, getMarkerAnchor(pkg.name()));
      Content link = new RawHtml(configuration.getText(
          "doclet.ClassUse_Uses.of.0.in.1", getLink(new LinkInfoImpl(
   ...
   * 
   * @param pkg
   *          the package that uses the given class
   * @param contentTree
   *          the content tree to which the package use information will be
   *          added
   */
  protected void addPackageUse(PackageDoc pkg, Content contentTree)
      throws IOException {
    Content tdFirst = HtmlTree.TD(
        HtmlStyle.colFirst,
        getHyperLink("", pkg.name(),
   ...
   *          the content tree to which the class use information will be added
   */
  protected void addClassUse(PackageDoc pkg, Content contentTree)
      throws IOException {
    String classLink = getLink(new LinkInfoImpl(
        LinkInfoImpl.CONTEXT_CLASS_USE_HEADER, classdoc, false));
    String pkgLink = getPackageLinkString(pkg, Util.getPackageName(pkg), false);
    classSubWriter
        .addUseInfo(pkgToClassAnnotations.get(pkg.name()), configuration
            .getText("doclet.ClassUse_Annotation", classLink, pkgLink),
            classUseTableSummary, contentTree);
    ...
    classSubWriter.addUseInfo(pkgToSubclass.get(pkg.name()),
        configuration.getText("doclet.ClassUse_Subclass", classLink, pkgLink),
        subclassUseTableSummary, contentTree);
    classSubWriter.addUseInfo(pkgToSubinterface.get(pkg.name()), configuration
        .getText("doclet.ClassUse_Subinterface", classLink, pkgLink),
        subinterfaceUseTableSummary, contentTree);
    ...
    fieldSubWriter.addUseInfo(pkgToField.get(pkg.name()),
        configuration.getText("doclet.ClassUse_Field", classLink, pkgLink),
        fieldUseTableSummary, contentTree);
    ...
    methodSubWriter.addUseInfo(pkgToMethodParameterAnnotations.get(pkg.name()),
        configuration.getText("doclet.ClassUse_MethodParameterAnnotations",
            classLink, pkgLink), methodUseTableSummary, contentTree);
    methodSubWriter.addUseInfo(pkgToMethodTypeParameter.get(pkg.name()),
        configuration.getText("doclet.ClassUse_MethodTypeParameter", classLink,
            pkgLink), methodUseTableSummary, contentTree);
    methodSubWriter.addUseInfo(pkgToMethodReturn.get(pkg.name()), configuration
        .getText("doclet.ClassUse_MethodReturn", classLink, pkgLink),
        methodUseTableSummary, contentTree);
    methodSubWriter.addUseInfo(pkgToMethodReturnTypeParameter.get(pkg.name()),
        configuration.getText("doclet.ClassUse_MethodReturnTypeParameter",
            classLink, pkgLink), methodUseTableSummary, contentTree);
    methodSubWriter
        .addUseInfo(pkgToMethodArgs.get(pkg.name()), configuration.getText(
            "doclet.ClassUse_MethodArgs", classLink, pkgLink),
            methodUseTableSummary, contentTree);
    methodSubWriter.addUseInfo(pkgToMethodArgTypeParameter.get(pkg.name()),
        configuration.getText("doclet.ClassUse_MethodArgsTypeParameters",
            classLink, pkgLink), methodUseTableSummary, contentTree);
    methodSubWriter.addUseInfo(pkgToMethodThrows.get(pkg.name()), configuration
        .getText("doclet.ClassUse_MethodThrows", classLink, pkgLink),
        methodUseTableSummary, contentTree);
    ...
    constrSubWriter.addUseInfo(
        pkgToConstructorArgTypeParameter.get(pkg.name()), configuration
            .getText("doclet.ClassUse_ConstructorArgsTypeParameters",
                classLink, pkgLink), constructorUseTableSummary, contentTree);
  ...
  protected Content getClassUseHeader() {
    String cltype = configuration
        .getText(classdoc.isInterface() ? "doclet.Interface" : "doclet.Class");
  ...
  protected Content getNavLinkTree() {
    Content linkContent = classdoc.containingPackage().isIncluded() ? getHyperLink(
        "../package-tree.html", "", treeLabel) : getHyperLink(relativePath
        + "overview-tree.html", "", treeLabel);
    Content li = HtmlTree.LI(linkContent);
    return li;
  }
}
