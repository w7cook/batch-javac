...
/**
 * Generate Index for all the Member Names with Indexing in Unicode Order. This
 * class is a base class for {@link SingleIndexWriter} and
...
public class AbstractIndexWriter extends HtmlDocletWriter {
  ...
  /**
   * This constructor will be used by {@link SplitIndexWriter}. Initialises path
   * to this file and relative path from this file.
   * 
   * @param path
   *          Path to the file which is getting generated.
   * @param filename
   *          Name of the file which is getting genrated.
   * @param relpath
   *          Relative path from this file to the current directory.
   * @param indexbuilder
   *          Unicode based Index from {@link IndexBuilder}
   */
  protected AbstractIndexWriter(ConfigurationImpl configuration, String path,
      String filename, String relpath, IndexBuilder indexbuilder)
      throws IOException {
   ...
   *          Name of the file which is getting genrated.
   * @param indexbuilder
   *          Unicode based Index form {@link IndexBuilder}
   */
  protected AbstractIndexWriter(ConfigurationImpl configuration,
      String filename, IndexBuilder indexbuilder) throws IOException {
  ...
  /**
   * Add the member information for the unicode character along with the list of
   * the members.
   * 
   * @param unicode
   *          Unicode for which member list information to be generated
   * @param memberlist
   *          List of members for the unicode character
   * @param contentTree
   ...
   * @param dlTree
   *          the content tree to which the description will be added
   */
  protected void addDescription(PackageDoc pkg, Content dlTree) {
    Content link = getPackageLink(pkg,
   ...
   *          the class being documented
   * @param contentTree
   *          the content tree to which the class info will be added
   */
  protected void addClassInfo(ClassDoc cd, Content contentTree) {
    contentTree.addContent(getResource(
        "doclet.in",
   ...
   * 
   * @param member
   *          MemberDoc for the member of the Class Kind
   * @param dlTree
   *          the content tree to which the description will be added
   */
  protected void addDescription(MemberDoc member, Content dlTree) {
    String name = (member instanceof ExecutableMemberDoc) ? member.name()
        + ((ExecutableMemberDoc) member).flatSignature() : member.name();
  ...
  /**
   * Add comment for each element in the index. If the element is deprecated and
   * it has a @deprecated tag, use that comment. Else if the containing class
   * for this element is deprecated, then add the word "Deprecated." at the
   * start and then print the normal comment.
   * 
   * @param element
   *          Index element
   * @param contentTree
   ...
   * 
   * @param member
   *          MemberDoc for the member within the Class Kind
   * @param contentTree
   *          the content tree to which the member description will be added
   */
  protected void addMemberDesc(MemberDoc member, Content contentTree) {
    ...
    if (member.isField()) {
      if (member.isStatic()) {
        contentTree.addContent(getResource("doclet.Static_variable_in",
            classdesc));
      } else {
      ...
      if (member.isStatic()) {
        contentTree
            .addContent(getResource("doclet.Static_method_in", classdesc));
      } else {
        contentTree.addContent(getResource("doclet.Method_in", classdesc));
      }
    }
    addPreQualifiedClassLink(LinkInfoImpl.CONTEXT_INDEX, containing, false,
        contentTree);
  }
}
