 ...
 */
public class FieldWriterImpl extends AbstractMemberWriter implements
  ...
  public Content getSignature(FieldDoc field) {
    ...
    Content fieldlink = new RawHtml(writer.getLink(new LinkInfoImpl(
        LinkInfoImpl.CONTEXT_MEMBER, field.type())));
    ...
    if (field.inlineTags().length > 0) {
      if (holder.equals(classdoc)
        ...
        Content link = new RawHtml(writer.getDocLink(
            LinkInfoImpl.CONTEXT_FIELD_DOC_COPY, holder, field, holder
                .isIncluded() ? holder.typeName() : holder.qualifiedTypeName(),
            false));
        Content codeLink = HtmlTree.CODE(link);
        Content strong = HtmlTree
            .STRONG(holder.isClass() ? writer.descfrmClassLabel
  ...
  public void addInheritedSummaryAnchor(ClassDoc cd, Content inheritedTree) {
    inheritedTree.addContent(writer
        .getMarkerAnchor("fields_inherited_from_class_"
  ...
  public void addInheritedSummaryLabel(ClassDoc cd, Content inheritedTree) {
    ...
    Content label = new StringContent(cd.isClass() ? configuration().getText(
        "doclet.Fields_Inherited_From_Class") : configuration().getText(
  ...
  protected void addSummaryLink(int context, ClassDoc cd,
    ...
    Content strong = HtmlTree.STRONG(new RawHtml(writer.getDocLink(context, cd,
        (MemberDoc) member, member.name(), false)));
  ...
  protected Content getDeprecatedLink(ProgramElementDoc member) {
    return writer.getDocLink(LinkInfoImpl.CONTEXT_MEMBER, (MemberDoc) member,
  ...
  protected Content getNavSummaryLink(ClassDoc cd, boolean link) {
    if (link) {
      return writer.getHyperLink("", (cd == null) ? "field_summary"
