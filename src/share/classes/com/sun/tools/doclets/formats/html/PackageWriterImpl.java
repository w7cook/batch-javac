 ...
 */
public class PackageWriterImpl extends HtmlDocletWriter implements
   ...
   * create "java/lang" directory in the current or the destination directory if
   * it doesen't exist.
   * 
   * @param configuration
  ...
  public PackageWriterImpl(ConfigurationImpl configuration,
    ...
    super(configuration, DirectoryManager.getDirectoryPath(packageDoc),
        OUTPUT_FILE_NAME, DirectoryManager.getRelativePath(packageDoc.name()));
    this.prev = prev;
    this.next = next;
   ...
   * 
   * @param div
   *          the content tree to which the deprecation information will be
   *          added
   */
  ...
  public void addPackageDescription(Content packageContentTree) {
    if (packageDoc.inlineTags().length > 0) {
      ...
      Content h2Content = new StringContent(configuration.getText(
          "doclet.Package_Description", packageDoc.name()));
  ...
  protected Content getNavLinkClassUse() {
    Content useLink = getHyperLink("package-use.html", "", useLabel, "", "");
