...
public class MethodWriterImpl extends AbstractExecutableMemberWriter implements
  ...
  public Content getMethodDocTreeHeader(MethodDoc method,
    ...
    methodDetailsTree.addContent(writer.getMarkerAnchor(writer
        .getAnchor(method)));
  ...
  public void addComments(Type holder, MethodDoc method, Content methodDocTree) {
    ...
    if (method.inlineTags().length > 0) {
      if (holder.asClassDoc().equals(classdoc)
          || (!(holderClassDoc.isPublic() || Util.isLinkable(holderClassDoc,
              configuration())))) {
        writer.addInlineComment(method, methodDocTree);
      } else {
        Content link = new RawHtml(writer.getDocLink(
            LinkInfoImpl.CONTEXT_METHOD_DOC_COPY,
            holder.asClassDoc(),
            method,
            holder.asClassDoc().isIncluded() ? holder.typeName() : holder
                .qualifiedTypeName(), false));
        Content codelLink = HtmlTree.CODE(link);
        Content strong = HtmlTree
            .STRONG(holder.asClassDoc().isClass() ? writer.descfrmClassLabel
  ...
  public void addInheritedSummaryAnchor(ClassDoc cd, Content inheritedTree) {
    inheritedTree.addContent(writer
        .getMarkerAnchor("methods_inherited_from_class_"
  ...
  public void addInheritedSummaryLabel(ClassDoc cd, Content inheritedTree) {
    ...
    Content label = new StringContent(cd.isClass() ? configuration().getText(
        "doclet.Methods_Inherited_From_Class") : configuration().getText(
  ...
  protected static void addOverridden(HtmlDocletWriter writer,
    ...
    if (method != null) {
      ...
      dl.addContent(dt);
      Content overriddenTypeLink = new RawHtml(writer.getLink(new LinkInfoImpl(
          context, overriddenType)));
      Content codeOverridenTypeLink = HtmlTree.CODE(overriddenTypeLink);
      String name = method.name();
      Content methlink = new RawHtml(writer.getLink(new LinkInfoImpl(
          LinkInfoImpl.CONTEXT_MEMBER, overriddenType.asClassDoc(), writer
              .getAnchor(method), name, false)));
  ...
  protected static void addImplementsInfo(HtmlDocletWriter writer,
    ...
    for (int i = 0; i < implementedMethods.length; i++) {
      ...
      Content methlink = new RawHtml(writer.getDocLink(
          LinkInfoImpl.CONTEXT_MEMBER, implementedMeth, implementedMeth.name(),
          false));
      Content codeMethLink = HtmlTree.CODE(methlink);
      Content dd = HtmlTree.DD(codeMethLink);
      dd.addContent(writer.getSpace());
  ...
  protected void addReturnType(MethodDoc method, Content htmltree) {
    ...
    if (type != null) {
      Content linkContent = new RawHtml(writer.getLink(new LinkInfoImpl(
          LinkInfoImpl.CONTEXT_RETURN_TYPE, type)));
  ...
  protected Content getNavSummaryLink(ClassDoc cd, boolean link) {
    if (link) {
      return writer.getHyperLink("", (cd == null) ? "method_summary"
          : "methods_inherited_from_class_" + configuration().getClassName(cd),
