...
/**
 * Generate the package index for the left-hand frame in the generated output. A
 * click on the package name in this frame will update the page in the bottom
...
public class PackageIndexFrameWriter extends AbstractPackageIndexWriter {
  ...
  protected Content getPackage(PackageDoc pd) {
    ...
    if (pd.name().length() > 0) {
      ...
      packageLinkContent = getHyperLink(pathString(pd, "package-frame.html"),
          "", packageLabel, "", "packageFrame");
    } else {
      packageLabel = new RawHtml("&lt;unnamed package&gt;");
      packageLinkContent = getHyperLink("package-frame.html", "", packageLabel,
          "", "packageFrame");
    }
  ...
  protected void addNavigationBarHeader(Content body) {
    ...
    if (configuration.packagesheader.length() > 0) {
      headerContent = new RawHtml(
          replaceDocRootDir(configuration.packagesheader));
    } else {
