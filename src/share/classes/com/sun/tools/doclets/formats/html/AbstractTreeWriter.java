...
/**
 * Abstract class to print the class hierarchy page for all the Classes. This is
 * sub-classed by {@link PackageTreeWriter} and {@link TreeWriter} to generate
 * the Package Tree and global Tree(for all the classes and packages) pages.
 * 
...
public abstract class AbstractTreeWriter extends HtmlDocletWriter {
   ...
   * @param classtree
   *          Tree built by {@link ClassTree}.
   * @throws IOException
   * @throws DocletAbortException
   */
  protected AbstractTreeWriter(ConfigurationImpl configuration,
      String filename, ClassTree classtree) throws IOException {
   ...
   * relative path from this generated file to the current or the destination
   * directory. This constructor will be used while generating "package tree"
   * file.
   * 
   * @param path
   *          Directories in this path will be created if they are not already
   *          there.
   * @param filename
   *          Name of the package tree file to be generated.
   * @param classtree
   *          The tree built using {@link ClassTree}. for the package pkg.
   * @param pkg
   *          PackageDoc for which tree file will be generated.
   * @throws IOException
   * @throws DocletAbortException
   */
  protected AbstractTreeWriter(ConfigurationImpl configuration, String path,
      String filename, ClassTree classtree, PackageDoc pkg) throws IOException {
    super(configuration, path, filename, DirectoryManager.getRelativePath(pkg
        .name()));
    this.classtree = classtree;
  }

  /**
   * Add each level of the class tree. For each sub-class or sub-interface
   * indents the next level information. Recurses itself to add subclasses info.
   * 
   * @param parent
   *          the superclass or superinterface of the list
   * @param list
   *          list of the sub-classes at this level
   * @param isEnum
   *          true if we are generating a tree for enums
   * @param contentTree
  ...
  /**
   * Add the heading for the tree depending upon tree type if it's a Class Tree
   * or Interface tree.
   * 
   * @param list
   *          List of classes which are at the most base level, all the other
   *          classes in this run will derive from these classes
   * @param heading
  ...
  protected void addTree(List<ClassDoc> list, String heading, Content div) {
    if (list.size() > 0) {
      ...
      div.addContent(HtmlTree.HEADING(HtmlConstants.CONTENT_HEADING, true,
          headingContent));
      addLevelInfo(!firstClassDoc.isInterface() ? firstClassDoc : null, list,
          list == classtree.baseEnums(), div);
    }
  }
   ...
   * 
   * @param parent
   *          the parent class of the class being documented
   * @param cd
    ...
    if (interfaces.length > (cd.isInterface() ? 1 : 0)) {
      ...
      for (int i = 0; i < interfaces.length; i++) {
        if (parent != interfaces[i]) {
          if (!(interfaces[i].isPublic() || Util.isLinkable(interfaces[i],
              configuration()))) {
            continue;
          }
          if (counter == 0) {
          ...
          addPreQualifiedClassLink(LinkInfoImpl.CONTEXT_TREE, interfaces[i],
              contentTree);
          counter++;
        }
      }
