...
public class ClassWriterImpl extends SubWriterHolderWriter implements
  ...
  public ClassWriterImpl(ClassDoc classDoc, ClassDoc prevClass,
    ...
    super(ConfigurationImpl.getInstance(), DirectoryManager
        .getDirectoryPath(classDoc.containingPackage()), classDoc.name()
  ...
  public Content getHeader(String header) {
    String pkgname = (classDoc.containingPackage() != null) ? classDoc
        .containingPackage().name() : "";
  ...
  public void addClassSignature(String modifiers, Content classInfoTree) {
    ...
    if (!isInterface) {
      ...
      if (superclass != null) {
        ...
        Content link = new RawHtml(getLink(new LinkInfoImpl(
            LinkInfoImpl.CONTEXT_CLASS_SIGNATURE_PARENT_NAME, superclass)));
        pre.addContent(link);
      }
    }
    ...
    if (implIntfacs != null && implIntfacs.length > 0) {
      ...
      for (int i = 0; i < implIntfacs.length; i++) {
        ...
        Content link = new RawHtml(getLink(new LinkInfoImpl(
            LinkInfoImpl.CONTEXT_CLASS_SIGNATURE_PARENT_NAME, implIntfacs[i])));
        pre.addContent(link);
        counter++;
      }
    }
  ...
  private Content getTreeForClassHelper(Type type) {
    ...
    if (type.equals(classDoc)) {
      ...
      if (configuration.shouldExcludeQualifier(classDoc.containingPackage()
          .name())) {
  ...
  public void addSubClassInfo(Content classInfoTree) {
    if (classDoc.isClass()) {
      ...
      if (subclasses.size() > 0) {
        ...
        dl.addContent(getClassLinks(LinkInfoImpl.CONTEXT_SUBCLASSES, subclasses));
        classInfoTree.addContent(dl);
      }
    }
  }
  ...
  public void addNestedClassInfo(Content classInfoTree) {
    ...
    if (outerClass != null) {
      ...
      dd.addContent(new RawHtml(getLink(new LinkInfoImpl(
  ...
  private Content getClassLinks(int context, List<?> list) {
    ...
    for (int i = 0; i < list.size(); i++) {
      ...
      if (typeList[i] instanceof ClassDoc) {
        Content link = new RawHtml(getLink(new LinkInfoImpl(context,
            (ClassDoc) (typeList[i]))));
        dd.addContent(link);
      } else {
        Content link = new RawHtml(getLink(new LinkInfoImpl(context,
  ...
  protected Content getNavSummaryLinks() throws Exception {
    ...
    for (int i = 0; i < navLinkLabels.length; i++) {
      ...
      if (writer == null) {
        ...
        writer.addNavSummaryLink(memberSummaryBuilder.members(i),
