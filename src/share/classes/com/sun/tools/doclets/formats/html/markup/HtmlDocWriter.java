 ...
 * contains methods related to the Html Code Generation which are used by the
 * Sub-Classes in the package com.sun.tools.doclets.standard and
 * com.sun.tools.doclets.oneone.
 * 
 * @since 1.2
...
public abstract class HtmlDocWriter extends HtmlWriter {
  ...
  /**
   * Constructor. Initializes the destination file name through the super class
   * HtmlWriter.
   * 
   * @param filename
   *          String file name.
   */
  public HtmlDocWriter(Configuration configuration, String filename)
      throws IOException {
    super(configuration, null, configuration.destDirName + filename,
        configuration.docencoding);
    // use File to normalize file separators
    configuration.message.notice("doclet.Generating_0", new File(
        configuration.destDirName, filename));
  }

  public HtmlDocWriter(Configuration configuration, String path, String filename)
      throws IOException {
    super(configuration, configuration.destDirName + path, filename,
        configuration.docencoding);
    // use File to normalize file separators
    configuration.message.notice("doclet.Generating_0", new File(
        configuration.destDirName, ((path.length() > 0) ? path + File.separator
            : "") + filename));
  }
   ...
   * @param where
   *          Position of the link in the file. Character '#' is not needed.
   * @param label
   *          Tag for the link.
   * @param strong
   *          Boolean that sets label to strong.
   */
   ...
   */
  public void printHyperLink(String link, String where, String label,
      boolean strong, String stylename) {
   ...
   */
  public String getHyperLinkString(String link, String where, String label,
      boolean strong) {
   ...
   *          String style of text defined in style sheet.
   * @param title
   *          String that describes the link's content for accessibility.
   * @param target
   *          Target frame.
   * @return String Hyper Link.
   */
  public String getHyperLinkString(String link, String where, String label,
      boolean strong, String stylename, String title, String target) {
  ...
  /**
   * Keep track of member details list. Print the definition list start tag if
  ...
  /**
   * Print the frameset version of the Html file header. Called only when
   * generating an HTML frameset file.
   * 
   * @param title
   *          Title of this HTML document
   * @param noTimeStamp
   *          If true, don't print time stamp in header
   * @param frameset
   *          the frameset to be added to the HTML document
   */
