...
public class ConfigurationImpl extends Configuration {
   ...
   */
  public ClassDoc currentcd = null; // Set
                                    // this
                                    // classdoc
                                    // in
                                    // the
  ...
  /**
   * Depending upon the command line options provided by the user, set configure
   ...
   * its length is one. If it takes one argument, it's length is two, and so on.
   * This method is called by JavaDoc to parse the options it does not
   ...
   * case-sensitive, use toLowerCase() on the option string before comparing it.
   * </blockquote>
   * 
   * @return number of arguments + 1 for a option. Zero return means option not
  ...
  public int optionLength(String option) {
    ...
    if (option.equals("-nodeprecatedlist") || option.equals("-noindex")
      ...
      return 1;
        ...
        || option.equals("-packagesheader") || option.equals("-doctitle")
        || option.equals("-windowtitle") || option.equals("-top")
        || option.equals("-bottom") || option.equals("-helpfile")
        || option.equals("-stylesheetfile") || option.equals("-charset")
        || option.equals("-overview") || option.equals("-xdocrootparent")) {
      return 2;
    } else {
      return 0;
    }
  ...
  public boolean validOptions(String options[][], DocErrorReporter reporter) {
    ...
    for (int oi = 0; oi < options.length; ++oi) {
      ...
      if (opt.equals("-helpfile")) {
        if (nohelp == true) {
          reporter.printError(getText("doclet.Option_conflict", "-helpfile",
              "-nohelp"));
          return false;
        }
        if (helpfile == true) {
        ...
        if (helpfile == true) {
          reporter.printError(getText("doclet.Option_conflict", "-nohelp",
              "-helpfile"));
          return false;
        }
        nohelp = true;
        ...
        if (nooverview == true) {
          reporter.printError(getText("doclet.Option_conflict", "-overview",
              "-nooverview"));
          return false;
        }
        if (overview == true) {
        ...
        if (overview == true) {
          reporter.printError(getText("doclet.Option_conflict", "-nooverview",
              "-overview"));
          return false;
        }
        nooverview = true;
        ...
        if (noindex == true) {
          reporter.printError(getText("doclet.Option_conflict", "-splitindex",
              "-noindex"));
          return false;
        }
        splitindex = true;
        ...
        if (splitindex == true) {
          reporter.printError(getText("doclet.Option_conflict", "-noindex",
              "-splitindex"));
          return false;
        }
        noindex = true;
  ...
  /**
   * Decide the page which will appear first in the right-hand frame. It will be
   * "overview-summary.html" if "-overview" option is used or no "-overview" but
   * the number of packages is more than one. It will be "package-summary.html"
   * of the respective package if there is only one package to document. It will
   * be a class page(first in the sorted order), if only classes are provided on
   * the command line.
   * 
   * @param root
