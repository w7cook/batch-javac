...
public class SourceToHTMLConverter {
  ...
  /**
   * The number of trailing blank lines at the end of the page. This is inserted
   * so that anchors at the bottom of small pages can be reached.
   */
  ...
  public static void convertRoot(ConfigurationImpl configuration, RootDoc rd,
    ...
    for (int i = 0; i < cds.length; i++) {
      ...
      if (!(configuration.nodeprecated && (Util.isDeprecated(cds[i]) || Util
          .isDeprecated(cds[i].containingPackage()))))
   ...
   *          the name of the directory to output to.
   */
  public static void convertPackage(ConfigurationImpl configuration,
      PackageDoc pd, String outputdir) {
  ...
  public static void convertClass(ConfigurationImpl configuration, ClassDoc cd,
    ...
    try {
      ...
      if (sp instanceof com.sun.tools.javadoc.SourcePositionImpl) {
        FileObject fo = ((com.sun.tools.javadoc.SourcePositionImpl) sp)
            .fileObject();
        if (fo == null)
          return;
      ...
      int lineno = 1;
      String line;
      relativePath = DirectoryManager
          .getRelativePath(DocletConstants.SOURCE_OUTPUT_DIR_NAME)
   ...
   * 
   * @param body
   *          the documentation content to be written to the file.
   * @param outputDir
   *          the directory to output to.
   * @param className
   *          the name of the class that I am converting to HTML.
   * @param configuration
   *          the Doclet configuration to pass notices to.
   */
  private static void writeToFile(Content body, String outputDir,
    ...
    head.addContent(HtmlTree.TITLE(new StringContent(configuration
        .getText("doclet.Window_Source_title"))));
   ...
   * 
   * @param configuration
   *          the doclet configuration for the current run of javadoc
   ...
   * 
   * @param pre
   *          the content tree to which the line number will be added
   * @param lineno
   *          The line number
   */
   ...
   * 
   * @param pre
   *          the content tree to which the line will be added.
   * @param line
  ...
  private static void addLine(Content pre, String line, int tabLength,
    ...
    if (line != null) {
      ...
      Content anchor = HtmlTree.A_NAME("line."
          + Integer.toString(currentLineNo));
