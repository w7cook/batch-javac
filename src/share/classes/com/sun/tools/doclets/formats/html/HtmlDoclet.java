...
public class HtmlDoclet extends AbstractDoclet {
  ...
  /**
   * Create the configuration instance. Override this method to use a different
   * configuration.
   */
  ...
  protected void generateOtherFiles(RootDoc root, ClassTree classtree)
    ...
    if (configuration.linksource) {
      if (configuration.destDirName.length() > 0) {
        SourceToHTMLConverter.convertRoot(configuration, root,
            configuration.destDirName + File.separator
    ...
    if (configuration.stylesheetfile.length() == 0) {
      Util.copyFile(configuration, "stylesheet.css", Util.RESOURCESDIR,
          (configdestdir.isEmpty()) ? System.getProperty("user.dir")
    ...
    for (int i = 0; i < arr.length; i++) {
      ...
      try {
        if (curr.isAnnotationType()) {
          ...
          AbstractBuilder classBuilder = configuration.getBuilderFactory()
    ...
    for (int i = 0; i < packages.length; i++) {
      ...
      if (!(configuration.nodeprecated && Util.isDeprecated(packages[i]))) {
        ...
        AbstractBuilder packageSummaryBuilder = configuration
            .getBuilderFactory().getPackageSummaryBuilder(packages[i], prev,
                next);
        packageSummaryBuilder.build();
        if (configuration.createtree) {
          PackageTreeWriter.generate(configuration, packages[i], prev, next,
   ...
   * 
   * @return number of arguments to option. Zero return means option not known.
  ...
  private void performCopy(String configdestdir, String filename) {
    try {
      ...
      if (filename.length() > 0) {
        ...
        if (!desthelpfile.getCanonicalPath().equals(
          ...
          configuration.message.notice((SourcePosition) null,
              "doclet.Copying_File_0_To_File_1", helpstylefile.toString(),
      ...
      configuration.message.error((SourcePosition) null,
          "doclet.perform_copy_exception_encountered", exc.toString());
      throw new DocletAbortException();
    }
  }
}
