...
public class LinkFactoryImpl extends LinkFactory {
  ...
  protected LinkOutput getClassLink(LinkInfo linkInfo) {
    ...
    String title = (classLinkInfo.where == null || classLinkInfo.where.length() == 0) ? getClassToolTip(
        classDoc,
        classLinkInfo.type != null
            && !classDoc.qualifiedTypeName().equals(
                classLinkInfo.type.qualifiedTypeName())) : "";
    ...
    if (classDoc.isIncluded()) {
      if (configuration.isGeneratedDoc(classDoc)) {
        ...
        if (linkInfo.linkToSelf
          ...
          linkOutput.append(m_writer.getHyperLinkString(filename,
              classLinkInfo.where, label.toString(), classLinkInfo.isStrong,
              classLinkInfo.styleName, title, classLinkInfo.target));
      ...
      String crossLink = m_writer.getCrossClassLink(classDoc.qualifiedName(),
          classLinkInfo.where, label.toString(), classLinkInfo.isStrong,
          classLinkInfo.styleName, true);
      if (crossLink != null) {
  ...
  private String pathString(LinkInfoImpl linkInfo) {
    ...
    buf.append(DirectoryManager.getPathToPackage(
        linkInfo.classDoc.containingPackage(), linkInfo.classDoc.name()
            + ".html"));
    return buf.toString();
  }
}
