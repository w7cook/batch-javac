...
public class PackageUseWriter extends SubWriterHolderWriter {
  ...
  public PackageUseWriter(ConfigurationImpl configuration,
      ClassUseMapper mapper, String filename, PackageDoc pkgdoc)
      throws IOException {
    super(configuration, DirectoryManager.getDirectoryPath(pkgdoc), filename,
    ...
    for (int i = 0; i < content.length; ++i) {
      ...
      Set<ClassDoc> usingClasses = mapper.classToClass.get(usedClass
          .qualifiedName());
   ...
   * @param mapper
   *          the mapping of the class usage.
   * @param pkgdoc
   ...
   * 
   * @param contentTree
   *          the content tree to which the package list will be added
   */
  ...
  protected void addClassList(Content contentTree) throws IOException {
    String[] classTableHeader = new String[] { configuration.getText(
        "doclet.0_and_1", configuration.getText("doclet.Class"),
    ...
    while (itp.hasNext()) {
      ...
      Content table = HtmlTree.TABLE(
          ...
          getTableCaption(configuration().getText(
              ...
              getPackageLinkString(pkgdoc, Util.getPackageName(pkgdoc), false),
              getPackageLinkString(usingPackage,
                  Util.getPackageName(usingPackage), false))));
      table.addContent(getSummaryTableHeader(classTableHeader, "col"));
      Content tbody = new HtmlTree(HtmlTag.TBODY);
      Iterator<ClassDoc> itc = usingPackageToUsedClasses.get(packageName)
          .iterator();
   ...
   * 
   * @param usedClass
   *          the class that uses the given package
   * @param packageName
  ...
  protected void addClassRow(ClassDoc usedClass, String packageName,
      Content contentTree) {
    String path = pathString(usedClass, "class-use/" + usedClass.name()
        + ".html");
   ...
   * 
   * @param pkg
   *          the package that used the given package
   * @param contentTree
  ...
  protected Content getPackageUseHeader() {
    ...
    Content headContent = getResource("doclet.ClassUse_Title", packageText,
        name);
  ...
  protected Content getNavLinkTree() {
    Content linkContent = getHyperLink("package-tree.html", "", treeLabel);
    Content li = HtmlTree.LI(linkContent);
    return li;
  }
}
