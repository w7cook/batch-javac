...
public abstract class AbstractMemberWriter {
   ...
   * 
   * @param cd
   *          the class doc to which to link to
   * @param inheritedTree
   *          the content tree to which the inherited summary label will be
   *          added
   */
   ...
   *          the class doc to be documented
   * @param inheritedTree
   *          the content tree to which the inherited summary anchor will be
   *          added
   */
   ...
   */
  protected abstract void addSummaryLink(int context, ClassDoc cd,
   ...
   *          the class doc to be documented
   * @param link
   *          true if its a link else the label to be printed
   ...
   * 
   * @param name
   *          the member name to be added to the content tree.
   * @param htmltree
   *          the content tree to which the name will be added.
   */
  ...
  /**
   * Return a string describing the access modifier flags. Don't include native
   * or synchronized.
   * 
   ...
   * 
   * @param member
   *          the member for which teh modifier will be added.
   * @param htmltree
   *          the content tree to which the modifier information will be added.
   */
   ...
   * 
   * @param member
   *          the member to add the type for
   * @param type
   *          the type to add
   * @param tdSummaryType
   *          the content tree to which the modified and type will be added
   */
    ...
    if (type == null) {
      ...
      if (member instanceof ExecutableMemberDoc
        ...
        int displayLength = ((AbstractExecutableMemberWriter) this)
            .addTypeParameters((ExecutableMemberDoc) member, code);
        if (displayLength > 10) {
   ...
   *          the member to add the type for
   * @param code
   *          the content tree to which the modified will be added
   */
   ...
   * @param contentTree
   *          the content tree to which the deprecated information will be
   *          added.
   */
  protected void addDeprecatedInfo(ProgramElementDoc member, Content contentTree) {
    String output = (new DeprecatedTaglet())
        .getTagletOutput(member, writer.getTagletWriterInstance(false))
        .toString().trim();
    if (!output.isEmpty()) {
  ...
  protected Content getHead(MemberDoc member) {
    ...
    Content heading = HtmlTree.HEADING(HtmlConstants.MEMBER_HEADING,
        memberContent);
    return heading;
  }

  /**
   * Return true if the given <code>ProgramElement</code> is inherited by the
   * class that is being documented.
   * 
   * @param ped
   *          The <code>ProgramElement</code> being checked. return true if the
   *          <code>ProgramElement</code> is being inherited and false
   *          otherwise.
   */
   ...
   * @param headingKey
   *          the caption for the deprecated members table
   * @param tableSummary
   *          the summary for the deprecated members table
   * @param tableHeader
   *          table headers for the deprecated members table
   * @param contentTree
   *          the content tree to which the deprecated members table will be
   *          added
   */
   ...
   * 
   * @param mems
   *          list of program elements for which the use information will be
   *          added
   * @param heading
   *          the section heading
   * @param tableSummary
   *          the summary for the use table
   * @param contentTree
   *          the content tree to which the use information will be added
   */
  protected void addUseInfo(List<? extends ProgramElementDoc> mems,
    ...
    if (members.size() > 0) {
      ...
      for (int i = 0; it.hasNext(); i++) {
        ...
        addSummaryLink(
            pgmdoc instanceof ClassDoc ? LinkInfoImpl.CONTEXT_CLASS_USE
                : LinkInfoImpl.CONTEXT_MEMBER, cd, pgmdoc, tdLast);
   ...
   *          the visible inherited members map
   * @param liNav
   *          the content tree to which the navigation summary link will be
   *          added
   */
  ...
  protected void serialWarning(SourcePosition pos, String key, String a1,
    ...
    if (configuration().serialwarn) {
      ConfigurationImpl.getInstance().getDocletSpecificMsg()
          .warning(pos, key, a1, a2);
    }
  }
   ...
   * 
   * @param classDoc
   *          the class the inherited member belongs to
   * @param nestedClass
   *          the inherited member that is summarized
   * @param isFirst
   *          true if this is the first member in the list
   * @param isLast
   *          true if this is the last member in the list
   * @param linksTree
   ...
   * 
   * @param classDoc
   *          the class for which the summary table is generated
   ...
   * 
   * @param memberTree
   *          the content tree of member to be documented
   ...
   *          the content tree of member to be documented
   * @param isLastContent
   *          true if the content to be added is the last content
   * @return a content tree that will be added to the class documentation
   */
