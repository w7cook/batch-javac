 ...
 * @since 1.4
 */
public class ConstantsSummaryWriterImpl extends HtmlDocletWriter implements
  ...
  public ConstantsSummaryWriterImpl(ConfigurationImpl configuration)
    ...
    constantsTableSummary = configuration.getText(
        "doclet.Constants_Table_Summary",
        configuration.getText("doclet.Constants_Summary"));
    constantsTableHeader = new String[] { getModifierTypeHeader(),
  ...
  public void addLinkToPackageContent(PackageDoc pkg, String parsedPackageName,
    ...
    if (packageName.length() == 0) {
      ...
      link = getHyperLink("#" + parsedPackageName, "", packageNameContent, "",
          "");
  ...
  public void addPackageName(PackageDoc pkg, String parsedPackageName,
    ...
    if (parsedPackageName.length() == 0) {
      summariesTree
          .addContent(getMarkerAnchor(DocletConstants.UNNAMED_PACKAGE_ANCHOR));
   ...
   * 
   * @param cd
   *          classdoc to be documented
   * @return constant members header content
   */
  public Content getConstantMembersHeader(ClassDoc cd) {
    // generate links backward only to public classes.
    String classlink = (cd.isPublic() || cd.isProtected()) ? getLink(new LinkInfoImpl(
        LinkInfoImpl.CONTEXT_CONSTANT_SUMMARY, cd, false)) : cd.qualifiedName();
