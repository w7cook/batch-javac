...
public class FrameOutputWriter extends HtmlDocletWriter {
   ...
   */
  public FrameOutputWriter(ConfigurationImpl configuration, String filename)
      throws IOException {
   ...
   * documentation. The name of the generated file is "index.html" which is the
   * default first file for Html documents.
   * 
  ...
  /**
   * Generate the contants in the "index.html" file. Print the frame details as
   * well as warning if browser is not supporting the Html frames.
   */
  ...
  /**
   * Add the code for issueing the warning for a non-frame capable web client.
   * Also provide links to the non-frame version documentation.
   * 
   * @param contentTree
   *          the content tree to which the non-frames information will be added
   */
  protected void addFrameWarning(Content contentTree) {
    ...
    Content p = HtmlTree.P(getResource(
  ...
  protected Content getFrameDetails() {
    HtmlTree frameset = HtmlTree.FRAMESET("20%,80%", null,
        "Documentation frame", "top.loadFrames()");
    if (noOfPackages <= 1) {
