...
public class DeprecatedListWriter extends SubWriterHolderWriter {
  ...
  private static final String[] ANCHORS = new String[] { "package",
  ...
  private static final String[] HEADING_KEYS = new String[] {
      ...
      "doclet.Deprecated_Annotation_Types", "doclet.Deprecated_Fields",
  ...
  private static final String[] HEADER_KEYS = new String[] { "doclet.Package",
      "doclet.Interface", "doclet.Class", "doclet.Enum", "doclet.Exceptions",
      "doclet.Errors", "doclet.AnnotationType", "doclet.Field",
      "doclet.Method", "doclet.Constructor", "doclet.Enum_Constant",
   ...
   */
  public DeprecatedListWriter(ConfigurationImpl configuration, String filename)
      throws IOException {
    super(configuration, filename);
    this.configuration = configuration;
    NestedClassWriterImpl classW = new NestedClassWriterImpl(this);
    writers = new AbstractMemberWriter[] { classW, classW, classW, classW,
        classW, classW, new FieldWriterImpl(this), new MethodWriterImpl(this),
        new ConstructorWriterImpl(this), new EnumConstantWriterImpl(this),
   ...
   * specified on the Command Line. Then instantiate DeprecatedListWriter and
   * generate File.
   * 
   * @param configuration
  ...
  public static void generate(ConfigurationImpl configuration) {
    ...
    try {
      DeprecatedListWriter depr = new DeprecatedListWriter(configuration,
          filename);
      depr.generateDeprecatedListFile(new DeprecatedAPIListBuilder(
          configuration));
      depr.close();
  ...
  protected void generateDeprecatedListFile(DeprecatedAPIListBuilder deprapi)
    ...
    for (int i = 0; i < DeprecatedAPIListBuilder.NUM_TYPES; i++) {
      if (deprapi.hasDocumentation(i)) {
        ...
        memberTableSummary = configuration.getText(
        ...
        if (i == DeprecatedAPIListBuilder.PACKAGE)
          addPackageDeprecatedAPI(deprapi.getList(i), HEADING_KEYS[i],
              memberTableSummary, memberTableHeader, div);
        else
          writers[i - 1].addDeprecatedAPI(deprapi.getList(i), HEADING_KEYS[i],
              memberTableSummary, memberTableHeader, div);
      }
    }
    body.addContent(div);
   ...
   * 
   * @param builder
   *          the deprecated list builder
   * @param type
   *          the type of list being documented
   * @param contentTree
   *          the content tree to which the index link will be added
   */
  private void addIndexLink(DeprecatedAPIListBuilder builder, int type,
      Content contentTree) {
   ...
   *          the type of list being documented
   * @param contentTree
   *          the content tree to which the anchor will be added
   */
  private void addAnchor(DeprecatedAPIListBuilder builder, int type,
      Content htmlTree) {
    if (builder.hasDocumentation(type)) {
