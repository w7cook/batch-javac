...
public class PackageFrameWriter extends HtmlDocletWriter {
  ...
  /**
   * The classes to be documented. Use this to filter out classes that will not
   * be documented.
   */
   ...
   * "package-frame.html" file in the respective package directory. For example
   * for package "java.lang" this will generate file "package-frame.html" file
   * in the "java/lang" directory. It will also create "java/lang" directory in
   * the current or the destination directory if it doesen't exist.
   * 
   * @param configuration
  ...
  public PackageFrameWriter(ConfigurationImpl configuration,
    ...
    if (configuration.root.specifiedPackages().length == 0) {
      documentedClasses = new HashSet<ClassDoc>(
          Arrays.asList(configuration.root.classes()));
    }
  }
  ...
  public static void generate(ConfigurationImpl configuration,
    ...
    try {
      ...
      Content heading = HtmlTree.HEADING(HtmlConstants.TITLE_HEADING,
          HtmlStyle.bar, packgen.getTargetPackageLink(packageDoc, "classFrame",
              pkgNameContent));
  ...
  /**
   * Add class listing for all the classes in this package. Divide class listing
   * as per the class kind and generate separate listing for Classes,
  ...
  protected void addClassListing(Content contentTree) {
    ...
    if (packageDoc.isIncluded()) {
      ...
      addClassKindListing(packageDoc.enums(), getResource("doclet.Enums"),
          contentTree);
      addClassKindListing(packageDoc.exceptions(),
          getResource("doclet.Exceptions"), contentTree);
      addClassKindListing(packageDoc.errors(), getResource("doclet.Errors"),
          contentTree);
    ...
    if (arr.length > 0) {
      ...
      for (int i = 0; i < arr.length; i++) {
        ...
        Content link = new RawHtml(
            getLink(new LinkInfoImpl(LinkInfoImpl.PACKAGE_FRAME, arr[i],
