 ...
 * @since 1.4
 */
public class ThrowsTaglet extends BaseExecutableMemberTaglet implements
    InheritableTaglet {
  ...
  public void inherit(DocFinder.Input input, DocFinder.Output output) {
    ...
    if (input.tagId == null) {
      ...
      input.tagId = exception == null ? throwsTag.exceptionName() : throwsTag
    ...
    for (int i = 0; i < tags.length; i++) {
      if (input.tagId.equals(tags[i].exceptionName())
          || (tags[i].exception() != null && (input.tagId.equals(tags[i]
              .exception().qualifiedName())))) {
  ...
  private TagletOutput linkToUndocumentedDeclaredExceptions(
    ...
    for (int i = 0; i < declaredExceptionTypes.length; i++) {
      if (declaredExceptionTypes[i].asClassDoc() != null
          && !alreadyDocumented.contains(declaredExceptionTypes[i].asClassDoc()
              .name())
          && !alreadyDocumented.contains(declaredExceptionTypes[i].asClassDoc()
              .qualifiedName())) {
      ...
      for (int j = 0; j < declaredExceptionTypes.length; j++) {
        ...
        if (inheritedDoc.tagList.size() == 0) {
          inheritedDoc = DocFinder.search(new DocFinder.Input(
              (MethodDoc) holder, this, declaredExceptionTypes[j]
                  .qualifiedTypeName()));
        }
        declaredExceptionTags.addAll(inheritedDoc.tagList);
      }
      result.appendOutput(throwsTagsOutput(
          declaredExceptionTags.toArray(new ThrowsTag[] {}), writer,
  ...
  public TagletOutput getTagletOutput(Doc holder, TagletWriter writer) {
    ...
    if (tags.length > 0) {
      result.appendOutput(throwsTagsOutput(execHolder.throwsTags(), writer,
   ...
   * 
   * @param throwTags
   *          the array of <code>ThrowsTag</code>s to convert.
   * @param writer
  ...
  protected TagletOutput throwsTagsOutput(ThrowsTag[] throwTags,
    ...
    if (throwTags.length > 0) {
      for (int i = 0; i < throwTags.length; ++i) {
        ...
        if ((!allowDups)
            && (alreadyDocumented.contains(tt.exceptionName()) || (cd != null && alreadyDocumented
                .contains(cd.qualifiedName())))) {
          continue;
        }
        if (alreadyDocumented.size() == 0) {
          result.appendOutput(writer.getThrowsHeader());
        ...
        alreadyDocumented.add(cd != null ? cd.qualifiedName() : tt
            .exceptionName());
      }
    }
    return result;
