 ...
 * user given groups, are grouped in default group, named as either
 * "Other Packages" or "Packages" depending upon if "-group" option used or not
 * at all used respectively.
 * </p>
 * <p>
...
public class Group {
  ...
  /**
   * Since we need to sort the keys in the reverse order(longest key first), the
   * compare method in the implementing class is doing the reverse comparison.
   */
   ...
   * option, generate two separate maps. There will be a map for mapping regular
   * expression(only meta character allowed is '*' and that is at the end of the
   * regular expression) on to the group name. And another map for mapping
   * (possible) package names(if the name format doesen't contain meta character
   * '*', then it is assumed to be a package name) on to the group name. This
   * will also sort all the regular expressions found in the reverse order of
   * their lengths, i.e. longest regular expression will be first in the sorted
   * list.
   * 
   * @param groupname
   *          The name of the group from -group option.
   * @param pkgNameFormList
  ...
  public boolean checkPackageGroups(String groupname, String pkgNameFormList) {
    ...
    while (strtok.hasMoreTokens()) {
      ...
      if (id.length() == 0) {
        configuration.message.warning("doclet.Error_in_packagelist", groupname,
            pkgNameFormList);
        return false;
      }
      if (id.endsWith("*")) {
   ...
   * command line. Given a list of packages, search each package name in regular
   * expression map as well as package name map to get the corresponding group
   * name. Create another map with mapping of group name to the package list,
   * which will fall under the specified group. If any package doesen't belong
   * to any specified group on the comamnd line, then a new group named
   * "Other Packages" will be created for it. If there are no groups found, in
   * other words if "-group" option is not at all used, then all the packages
   * will be grouped under group "Packages".
   * 
   * @param packages
  ...
  public Map<String, List<PackageDoc>> groupPackages(PackageDoc[] packages) {
    ...
    String defaultGroupName = (pkgNameGroupMap.isEmpty() && regExpGroupMap
        .isEmpty()) ? configuration.message.getText("doclet.Packages")
   ...
   * 
   * @param map
   *          Map to be searched for gorup name.
   * @param groupname
   *          Group name to search.
   */
  List<PackageDoc> getPkgList(Map<String, List<PackageDoc>> map,
      String groupname) {
