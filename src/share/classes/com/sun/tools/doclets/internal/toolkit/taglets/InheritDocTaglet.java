 ...
 * be used with a method. It is used to inherit documentation from overriden and
 * implemented methods.
 * 
...
public class InheritDocTaglet extends BaseInlineTaglet {
  ...
  /**
   * The inline tag that would appear in the documentation if the writer wanted
   ...
   * <code>MethodDoc</code> item and a String, replace all occurances of @inheritDoc
   * with documentation from it's superclass or superinterface.
   * 
   * @param writer
   *          the writer that is writing the output.
   * @param md
   *          the {@link MethodDoc} that we are documenting.
   * @param holderTag
   *          the tag that holds the inheritDoc tag.
   * @param isFirstSentence
   *          true if we only want to inherit the first sentence.
   */
  private TagletOutput retrieveInheritedDocumentation(TagletWriter writer,
    ...
    Taglet inheritableTaglet = holderTag == null ? null : writer
        .configuration().tagletManager.getTaglet(holderTag.name());
    ...
    DocFinder.Output inheritedDoc = DocFinder
        .search(new DocFinder.Input(md, (InheritableTaglet) inheritableTaglet,
  ...
  public TagletOutput getTagletOutput(Tag tag, TagletWriter tagletWriter) {
    ...
    return tag.name().equals("@inheritDoc") ? retrieveInheritedDocumentation(
        tagletWriter, (MethodDoc) tag.holder(), null,
        tagletWriter.isFirstSentence) : retrieveInheritedDocumentation(
        tagletWriter, (MethodDoc) tag.holder(), tag,
        tagletWriter.isFirstSentence);
  }
}
