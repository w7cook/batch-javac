...
public abstract class Configuration {
  ...
  /**
   * Generate author specific information for all the classes if @author tag is
   * used in the doc comment and if -author option is used.
   ...
   * <code>showversion</code> is set to true if -version option is used.Default
   * is don't show version information.
   */
   ...
   * is used. <code>nodepracted</code> is set to true if -nodeprecated option is
   * used. Default is generate deprected API information.
   */
   ...
   * file for this Configuration, which is common for 1.1 and standard doclets.
   * 
  ...
  /**
   * This method should be defined in all those doclets(configurations), which
   * want to derive themselves from this Configuration. This method can be used
   * to set its own command line options.
   * 
   * @param options
  ...
  public Configuration() {
    message = new MessageRetriever(this,
  ...
  /**
   * This method should be defined in all those doclets which want to inherit
   * from this Configuration. This method should return the number of arguments
   * to the command line option (including the option name). For example,
   * -notimestamp is a single-argument option, so this method would return 1.
   * 
   * @param option
   *          Command line option under consideration.
   * @return number of arguments to option (including the option name). Zero
   *         return means option not known. Negative value means error occurred.
   */
  public int optionLength(String option) {
    option = option.toLowerCase();
    if (option.equals("-author") || option.equals("-docfilessubdirs")
        || option.equals("-keywords") || option.equals("-linksource")
        || option.equals("-nocomment") || option.equals("-nodeprecated")
        || option.equals("-nosince") || option.equals("-notimestamp")
        || option.equals("-quiet") || option.equals("-xnodate")
        || option.equals("-version")) {
      return 1;
    } else if (option.equals("-d") || option.equals("-docencoding")
        || option.equals("-encoding")
        || option.equals("-excludedocfilessubdir") || option.equals("-link")
        || option.equals("-sourcetab") || option.equals("-noqualifier")
        || option.equals("-output") || option.equals("-sourcepath")
        || option.equals("-tag") || option.equals("-taglet")
  ...
  private void initPackageArray() {
    Set<PackageDoc> set = new HashSet<PackageDoc>(Arrays.asList(root
        .specifiedPackages()));
  ...
  public void setOptions(String[][] options) {
    ...
    if (sourcepath.length() == 0) {
      sourcepath = System.getProperty("env.class.path") == null ? "" : System
          .getProperty("env.class.path");
    }
    if (docencoding == null) {
  ...
  /**
   * Initialize the taglet manager. The strings to initialize the simple custom
  ...
  private void initTagletManager(Set<String[]> customTagStrs) {
    tagletManager = tagletManager == null ? new TagletManager(nosince,
        showversion, showauthor, message) : tagletManager;
    String[] args;
    for (Iterator<String[]> it = customTagStrs.iterator(); it.hasNext();) {
      ...
      if (tokens.length == 1) {
        ...
        if (tagletManager.isKnownCustomTag(tagName)) {
          ...
          // Create a simple tag with the heading that has the same name as the
          // tag.
        ...
        // Add simple taglet without heading, probably to excluding it in the
        // output.
  ...
  private boolean checkOutputFileEncoding(String docencoding,
    ...
    try {
      ...
      reporter
          .printError(getText("doclet.Encoding_not_supported", docencoding));
      return false;
    } finally {
      try {
        if (osw != null) {
  ...
  /**
   * Return true if the given doc-file subdirectory should be excluded and false
   * otherwise.
   * 
   * @param docfilesubdir
  ...
  /**
   * Return the qualified name of the <code>ClassDoc</code> if it's qualifier is
   * not excluded. Otherwise, return the unqualified <code>ClassDoc</code> name.
   * 
   * @param cd
   ...
   *           when the given XML file cannot be found.
   */
  public InputStream getBuilderXML() throws FileNotFoundException {
    return builderXMLPath == null ? Configuration.class
        .getResourceAsStream(DEFAULT_BUILDER_XML) : new FileInputStream(
        new File(builderXMLPath));
  }
  ...
  /**
   * Return the comparator that will be used to sort member documentation. To no
   * do any sorting, return null.
   * 
