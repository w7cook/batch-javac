...
/**
 * Build Class Hierarchy for all the Classes. This class builds the Class Tree
 * and the Interface Tree separately.
 * 
...
public class ClassTree {
  ...
  /**
   * List of baseclasses. Contains only java.lang.Object. Can be used to get the
   * mapped listing of sub-classes.
   */
   ...
   * @param noDeprecated
   *          Don't add deprecated classes in the class tree, if true.
   */
   ...
   * @param configuration
   *          The curren configuration of the doclet.
   */
   ...
   * the sub-class list for java.lang.Object which will be having sub-class
   * listing for itself and also for each sub-class itself will have their own
   * sub-class lists.
   * 
   * @param classes
   *          all the classes in this run.
   * @param configuration
  ...
  private void buildTree(ClassDoc[] classes, Configuration configuration) {
    for (int i = 0; i < classes.length; i++) {
      ...
      if (configuration.nodeprecated
          && (Util.isDeprecated(classes[i]) || Util.isDeprecated(classes[i]
              .containingPackage()))) {
        continue;
      }
      if (classes[i].isEnum()) {
    ...
    for (Iterator<List<ClassDoc>> it = subinterfaces.values().iterator(); it
        .hasNext();) {
      Collections.sort(it.next());
    }
    for (Iterator<List<ClassDoc>> it = subclasses.values().iterator(); it
        .hasNext();) {
      Collections.sort(it.next());
    }
  }
  ...
  /**
   * For the class passed map it to it's own sub-class listing. For the Class
   * passed, get the super class, if superclass is non null, (it is not
   * "java.lang.Object") get the "value" from the hashmap for this key Class if
   * entry not found create one and get that. add this Class as a sub class in
   * the list Recurse till hits java.lang.Object Null SuperClass.
   * 
   * @param cd
   *          class for which sub-class mapping to be generated.
   * @param configuration
   *          the current configurtation of the doclet.
   */
  ...
  /**
   * For the interface passed get the interfaces which it extends, and then put
   * this interface in the sub-interface list of those interfaces. Do it
   * recursively. If a interface doesn't have super-interface just attach that
   * interface in the list of all the baseinterfaces.
   * 
   * @param cd
  ...
  /**
   * Adjust the Class Tree. Add the class interface in to it's super-class' or
   * super-interface's sub-interface list.
   * 
   * @param map
   *          the entire map.
   * @param superclass
   *          java.lang.Object or the super-interface.
   * @param cd
   *          sub-interface to be mapped.
   * @returns boolean true if class added, false if class already processed.
   */
  private boolean add(Map<ClassDoc, List<ClassDoc>> map, ClassDoc superclass,
      ClassDoc cd) {
  ...
  /**
   * From the map return the list of sub-classes or sub-interfaces. If list is
   * null create a new one and return it.
   * 
   * @param map
   *          The entire map.
   * @param cd
   *          class for which the sub-class list is requested.
   ...
   * 
   * @param cd
   *          class whose sub-class list is required.
   */
   ...
   * 
   * @param cd
   *          interface whose sub-interface list is required.
   */
   ...
   * 
   * @param cd
   *          interface whose implementing-classes list is required.
   */
  public List<ClassDoc> implementingclasses(ClassDoc cd) {
    ...
    Iterator<ClassDoc> implementingClassesIter, subInterfacesIter = subinterfaces
        .listIterator();
    ClassDoc c;
    while (subInterfacesIter.hasNext()) {
      implementingClassesIter = implementingclasses(subInterfacesIter.next())
          .listIterator();
   ...
   * 
   * @param cd
   *          class/interface whose sub-class/interface list is required.
   * @param isEnum
   *          true if the subclasses should be forced to come from the enum
   *          tree.
   */
  ...
  /**
   * Return a list of all direct or indirect, sub-classes and subinterfaces of
   * the ClassDoc argument.
   * 
   * @param cd
   *          ClassDoc whose sub-classes or sub-interfaces are requested.
   * @param isEnum
   *          true if the subclasses should be forced to come from the enum
   *          tree.
   */
  ...
  /**
   * Return the base-classes list. This will have only one element namely thw
   * classdoc for java.lang.Object, since this is the base class for all
   * classes.
   */
  ...
  /**
   * Return the list of base interfaces. This is the list of interfaces which do
   * not have super-interface.
   */
  ...
  /**
   * Return the list of base enums. This is the list of enums which do not have
   * super-enums.
   */
  ...
  /**
   * Return the list of base annotation types. This is the list of annotation
   * types which do not have super-annotation types.
   */
