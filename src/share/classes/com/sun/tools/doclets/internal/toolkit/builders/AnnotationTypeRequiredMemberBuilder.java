...
public class AnnotationTypeRequiredMemberBuilder extends AbstractMemberBuilder {
  ...
  /**
   * The index of the current member that is being documented at this point in
   * time.
   */
  ...
  public static AnnotationTypeRequiredMemberBuilder getInstance(
    ...
    AnnotationTypeRequiredMemberBuilder builder = new AnnotationTypeRequiredMemberBuilder(
        configuration);
  ...
  /**
   * Returns a list of members that will be documented for the given class. This
  ...
  public void buildAnnotationTypeMember(XMLNode node, Content memberDetailsTree) {
    ...
    if (size > 0) {
      writer.addAnnotationDetailsTreeHeader(classDoc, memberDetailsTree);
      for (currentMemberIndex = 0; currentMemberIndex < size; currentMemberIndex++) {
        Content annotationDocTree = writer.getAnnotationDocTreeHeader(
            (MemberDoc) members.get(currentMemberIndex), memberDetailsTree);
        buildChildren(node, annotationDocTree);
        memberDetailsTree.addContent(writer.getAnnotationDoc(annotationDocTree,
            (currentMemberIndex == size - 1)));
      }
    }
  }
   ...
   * @param node
   *          the XML element that specifies which components to document
   * @param annotationDocTree
   *          the content tree to which the documentation will be added
   */
  public void buildSignature(XMLNode node, Content annotationDocTree) {
    annotationDocTree.addContent(writer.getSignature((MemberDoc) members
