...
public class SimpleTaglet extends BaseTaglet {
   ...
   *          the header to output.
   * @param locations
   *          the possible locations that this tag can appear in. The
   *          <code>String</code> can contain 'p' for package, 't' for type, 'm'
   *          for method, 'c' for constructor and 'f' for field.
   */
  ...
  public boolean inOverview() {
    return locations.indexOf(OVERVIEW) != -1
        && locations.indexOf(EXCLUDED) == -1;
  }
  ...
  public boolean inPackage() {
    return locations.indexOf(PACKAGE) != -1
        && locations.indexOf(EXCLUDED) == -1;
  }

  /**
   * Return true if this <code>SimpleTaglet</code> is used in type documentation
  ...
  public TagletOutput getTagletOutput(Tag tag, TagletWriter writer) {
    return header == null || tag == null ? null : writer.simpleTagOutput(tag,
        header);
  }
