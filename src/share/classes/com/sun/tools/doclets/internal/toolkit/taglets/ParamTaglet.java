...
public class ParamTaglet extends BaseTaglet implements InheritableTaglet {
   ...
   * 
   * @param params
   *          The array of parmeters (from type or executable member) to check.
  ...
  private static Map<String, String> getRankMap(Object[] params) {
    ...
    for (int i = 0; i < params.length; i++) {
      String name = params[i] instanceof Parameter ? ((Parameter) params[i])
          .name() : ((TypeVariable) params[i]).typeName();
  ...
  public void inherit(DocFinder.Input input, DocFinder.Output output) {
    if (input.tagId == null) {
      ...
      Object[] parameters = input.isTypeVariableParamTag ? (Object[]) ((MethodDoc) input.tag
          .holder()).typeParameters() : (Object[]) ((MethodDoc) input.tag
          .holder()).parameters();
      String target = ((ParamTag) input.tag).parameterName();
      int i;
      for (i = 0; i < parameters.length; i++) {
        String name = parameters[i] instanceof Parameter ? ((Parameter) parameters[i])
            .name() : ((TypeVariable) parameters[i]).typeName();
    ...
    ParamTag[] tags = input.isTypeVariableParamTag ? input.method
        .typeParamTags() : input.method.paramTags();
    Map<String, String> rankMap = getRankMap(input.isTypeVariableParamTag ? (Object[]) input.method
        .typeParameters() : (Object[]) input.method.parameters());
    for (int i = 0; i < tags.length; i++) {
      if (rankMap.containsKey(tags[i].parameterName())
        ...
        output.inlineTags = input.isFirstSentence ? tags[i].firstSentenceTags()
            : tags[i].inlineTags();
        return;
      }
    }
   ...
   * 
   * @param holder
   *          the member that holds the param tags.
   * @param writer
   *          the TagletWriter that will write this tag.
   ...
   * 
   * @param doc
   *          the doc that holds the param tags.
   * @param writer
   *          the TagletWriter that will write this tag.
  ...
  private TagletOutput getTagletOutput(boolean isNonTypeParams, Doc holder,
    ...
    if (paramTags.length > 0) {
      result.appendOutput(processParamTags(isNonTypeParams, paramTags,
  ...
  /**
   * Loop through each indivitual parameter. It it does not have a corresponding
   * param tag, try to inherit it.
   */
  private TagletOutput getInheritedTagletOutput(boolean isNonTypeParams,
      Doc holder, TagletWriter writer, Object[] formalParameters,
    ...
    if ((!alreadyDocumented.contains(null)) && holder instanceof MethodDoc) {
      for (int i = 0; i < formalParameters.length; i++) {
        ...
        // Try to inherit it.
        DocFinder.Output inheritedDoc = DocFinder.search(new DocFinder.Input(
            (MethodDoc) holder, this, String.valueOf(i), !isNonTypeParams));
        if (inheritedDoc.inlineTags != null
            && inheritedDoc.inlineTags.length > 0) {
          result.appendOutput(processParamTag(isNonTypeParams, writer,
              (ParamTag) inheritedDoc.holderTag,
              isNonTypeParams ? ((Parameter) formalParameters[i]).name()
   ...
   * 
   * @param paramTags
   *          the array of <code>ParamTag</code>s to convert.
   * @param writer
   *          the TagletWriter that will write this tag.
   * @param alreadyDocumented
   *          the set of exceptions that have already been documented.
   * @param rankMap
   *          a {@link java.util.Map} which holds ordering information about the
   *          parameters.
   * @param nameMap
   *          a {@link java.util.Map} which holds a mapping of a rank of a
   *          parameter to its name. This is used to ensure that the right name
   *          is used when parameter documentation is inherited.
  ...
  private TagletOutput processParamTags(boolean isNonTypeParams,
    ...
    if (paramTags.length > 0) {
      for (int i = 0; i < paramTags.length; ++i) {
        ...
        String paramName = isNonTypeParams ? pt.parameterName() : "<"
            + pt.parameterName() + ">";
        if (!rankMap.containsKey(pt.parameterName())) {
          writer.getMsgRetriever().warning(
              pt.position(),
              isNonTypeParams ? "doclet.Parameters_warn"
                  : "doclet.Type_Parameters_warn", paramName);
        }
        String rank = rankMap.get(pt.parameterName());
        if (rank != null && alreadyDocumented.contains(rank)) {
          writer.getMsgRetriever().warning(
              pt.position(),
              isNonTypeParams ? "doclet.Parameters_dup_warn"
                  : "doclet.Type_Parameters_dup_warn", paramName);
        }
   ...
   * 
   * @param isNonTypeParams
   *          true if this is just a regular param tag. False if this is a type
   *          param tag.
   * @param writer
   *          the taglet writer for output writing.
   * @param paramTag
   *          the tag whose inline tags will be printed.
   * @param name
   *          the name of the parameter. We can't rely on the name in the param
   *          tag because we might be inheriting documentation.
   * @param isFirstParam
   *          true if this is the first param tag being printed.
   * 
   */
  private TagletOutput processParamTag(boolean isNonTypeParams,
      TagletWriter writer, ParamTag paramTag, String name, boolean isFirstParam) {
    TagletOutput result = writer.getOutputInstance();
