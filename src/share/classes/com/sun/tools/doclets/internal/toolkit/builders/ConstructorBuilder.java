...
public class ConstructorBuilder extends AbstractMemberBuilder {
  ...
  public static ConstructorBuilder getInstance(Configuration configuration,
    ...
    builder.constructors = new ArrayList<ProgramElementDoc>(
    ...
    if (configuration.getMemberComparator() != null) {
      Collections.sort(builder.constructors,
  ...
  public void buildConstructorDoc(XMLNode node, Content memberDetailsTree) {
    ...
    if (size > 0) {
      ...
      memberDetailsTree.addContent(writer
          .getConstructorDetails(constructorDetailsTree));
    }
  }
   ...
   * @param node
   *          the XML element that specifies which components to document
   * @param constructorDocTree
   *          the content tree to which the documentation will be added
   */
  public void buildSignature(XMLNode node, Content constructorDocTree) {
    constructorDocTree.addContent(writer
        .getSignature((ConstructorDoc) constructors
            .get(currentConstructorIndex)));
  }
