...
public abstract class Content {
   ...
   * 
   * @param t
   *          reference type to check for null values
   * @return the reference type if not null or else throws a null pointer
   *         exception
   */
  ...
  /**
   * Returns true if the content ends with a newline character. Empty content is
   * considered as ending with new line.
   * 
   * @param contentBuilder
   *          content to test for newline character at the end
  ...
  protected boolean endsWithNewLine(StringBuilder contentBuilder) {
    ...
    while (nlIndex >= 0) {
      if (contentBuilder.charAt(contentIndex) != DocletConstants.NL
          .charAt(nlIndex)) {
        return false;
      }
      contentIndex--;
