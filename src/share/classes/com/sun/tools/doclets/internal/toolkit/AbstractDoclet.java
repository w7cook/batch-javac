...
public abstract class AbstractDoclet {
   ...
   */
  private static final String TOOLKIT_DOCLET_NAME = new com.sun.tools.doclets.formats.html.HtmlDoclet()
      .getClass().getName();
   ...
   * @param root
   *          the {@link RootDoc} that points to the source to document.
  ...
  private void startGeneration(RootDoc root) throws Exception {
    ...
    if (configuration.sourcepath != null
      ...
      StringTokenizer pathTokens = new StringTokenizer(
          configuration.sourcepath, String.valueOf(File.pathSeparatorChar));
      boolean first = true;
      while (pathTokens.hasMoreTokens()) {
        Util.copyDocFiles(configuration, pathTokens.nextToken()
            + File.separator, DocletConstants.DOC_FILES_DIR_NAME, first);
        first = false;
      }
    }
