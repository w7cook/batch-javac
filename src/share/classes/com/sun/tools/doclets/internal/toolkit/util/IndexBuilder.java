...
public class IndexBuilder {
  ...
  private class DocComparator implements Comparator<Doc> {
    public int compare(Doc d1, Doc d2) {
      ...
      } else if (d1 instanceof ProgramElementDoc
          && d2 instanceof ProgramElementDoc) {
  ...
  /**
   * Put all the members(fields, methods and constructors) in the classdoc to
   * the indexmap.
   * 
   * @param classdoc
   ...
   * in a member name, and then add the member to a list of members for that
  ...
  protected boolean shouldAddToIndexMap(Doc element) {
    if (element instanceof PackageDoc)
      ...
      // deprecated.
      return !(noDeprecated && (Util.isDeprecated(element) || Util
          .isDeprecated(((ProgramElementDoc) element).containingPackage())));
  }
