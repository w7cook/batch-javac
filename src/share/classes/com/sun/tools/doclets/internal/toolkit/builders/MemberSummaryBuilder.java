...
public class MemberSummaryBuilder extends AbstractMemberBuilder {
  ...
  private void init(Object writer) throws Exception {
    ...
    for (int i = 0; i < VisibleMemberMap.NUM_MEMBER_TYPES; i++) {
      visibleMemberMaps[i] = new VisibleMemberMap(classDoc, i,
          configuration.nodeprecated);
    }
    memberSummaryWriters = new MemberSummaryWriter[VisibleMemberMap.NUM_MEMBER_TYPES];
    for (int i = 0; i < VisibleMemberMap.NUM_MEMBER_TYPES; i++) {
      if (classDoc.isAnnotationType()) {
        memberSummaryWriters[i] = visibleMemberMaps[i].noVisibleMembers() ? null
  ...
  /**
   * Returns a list of methods that will be documented for the given class. This
   * information can be used for doclet specific documentation generation.
   * 
   * @param classDoc
  ...
  public void buildAnnotationTypeOptionalMemberSummary(XMLNode node,
    ...
    MemberSummaryWriter writer = memberSummaryWriters[VisibleMemberMap.ANNOTATION_TYPE_MEMBER_OPTIONAL];
    VisibleMemberMap visibleMemberMap = visibleMemberMaps[VisibleMemberMap.ANNOTATION_TYPE_MEMBER_OPTIONAL];
    addSummary(writer, visibleMemberMap, false, memberSummaryTree);
  }
  ...
  public void buildAnnotationTypeRequiredMemberSummary(XMLNode node,
      Content memberSummaryTree) {
    MemberSummaryWriter writer = memberSummaryWriters[VisibleMemberMap.ANNOTATION_TYPE_MEMBER_REQUIRED];
    VisibleMemberMap visibleMemberMap = visibleMemberMaps[VisibleMemberMap.ANNOTATION_TYPE_MEMBER_REQUIRED];
    addSummary(writer, visibleMemberMap, false, memberSummaryTree);
  }
  ...
  private void buildSummary(MemberSummaryWriter writer,
    ...
    List<ProgramElementDoc> members = new ArrayList<ProgramElementDoc>(
        visibleMemberMap.getLeafClassMembers(configuration));
    if (members.size() > 0) {
      ...
      for (int i = 0; i < members.size(); i++) {
        ...
        writer.addMemberSummary(classDoc, member, firstSentenceTags, tableTree,
            i);
      }
  ...
  private void buildInheritedSummary(MemberSummaryWriter writer,
      VisibleMemberMap visibleMemberMap, LinkedList<Content> summaryTreeList) {
    for (Iterator<ClassDoc> iter = visibleMemberMap.getVisibleClassesList()
      ...
      List<ProgramElementDoc> inhmembers = visibleMemberMap
          .getMembersFor(inhclass);
        ...
        for (int j = 0; j < inhmembers.size(); ++j) {
          writer.addInheritedMemberSummary(
              inhclass.isPackagePrivate()
                  && !Util.isLinkable(inhclass, configuration) ? classDoc
                  : inhclass, inhmembers.get(j), j == 0,
   ...
   * @param visibleMemberMap
   *          the map for the members to document.
   * @param showInheritedSummary
   *          true if inherited summary should be documented
   * @param memberSummaryTree
   *          the content tree to which the documentation will be added
   */
  private void addSummary(MemberSummaryWriter writer,
    ...
    if (!summaryTreeList.isEmpty()) {
      Content memberTree = writer.getMemberSummaryHeader(classDoc,
          memberSummaryTree);
