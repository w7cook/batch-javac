...
public class Util {
  ...
  /**
   * Return array of class members whose documentation is to be generated. If
   * the member is deprecated do not include such a member in the returned
   * array.
   * 
   * @param members
   *          Array of members to choose from.
   ...
   * @param members
   *          Array of members to choose from.
   * @return List List of eligible members for whom documentation is getting
   *         generated.
   */
  ...
  public static boolean executableMembersEqual(ExecutableMemberDoc member1,
    ...
    if (method1.isStatic() && method2.isStatic()) {
      ...
      if (method1.name().equals(method2.name())
        ...
        for (j = 0; j < targetParams.length; j++) {
          if (!(targetParams[j].typeName().equals(currentParams[j].typeName())
              || currentParams[j].type() instanceof TypeVariable || targetParams[j]
      ...
      return method1.overrides(method2) || method2.overrides(method1)
          || member1 == member2;
    }
  }
   ...
   * outer classes and static inner classes are core classes.
   */
  ...
  /**
   * Copy the given directory contents from the source package directory to the
   * generated documentation directory. For example for a package java.lang this
   * method find out the source location of the package using {@link SourcePath}
   * and if given directory is found in the source directory structure, copy the
   * entire directory, to the generated documentation hierarchy.
   * 
   * @param configuration
   *          The configuration of the current doclet.
   * @param path
   *          The relative path to the directory to be copied.
   * @param dir
   *          The original directory name to copy from.
   * @param overwrite
  ...
  public static void copyDocFiles(Configuration configuration, String path,
    ...
    if (destname.length() > 0
        && !destname.endsWith(DirectoryManager.URL_FILE_SEPARATOR)) {
        ...
        if (srcfile.isFile()) {
          if (destfile.exists() && !overwrite) {
            configuration.message.warning((SourcePosition) null,
                "doclet.Copy_Overwrite_warning", srcfile.toString(),
                destdir.toString());
          } else {
            configuration.message.notice("doclet.Copying_File_0_To_Dir_1",
  ...
  private static boolean checkCopyDocFilesErrors(Configuration configuration,
    ...
    if ((configuration.sourcepath == null || configuration.sourcepath.length() == 0)
        && (configuration.destDirName == null || configuration.destDirName
            .length() == 0)) {
  ...
  /**
   * Copy a file in the resources directory to the destination directory (if it
   * is not there already). If <code>overwrite</code> is true and the
   ...
   * @param overwrite
   *          A flag to indicate whether the file in the destination directory
   ...
   *          true if the newline needs to be replaced with platform- specific
   *          newline.
   */
  public static void copyFile(Configuration configuration, String file,
    ...
    if (destfile.exists() && (!overwrite))
      return;
    try {
      InputStream in = Configuration.class.getResourceAsStream(source
   ...
   * 
   * @param configuration
   *          The Configuration for the current Doclet.
   * @param pkgDoc
  ...
  private static class TypeComparator implements Comparator<Type> {
    public int compare(Type type1, Type type2) {
      return type1.qualifiedTypeName().toLowerCase()
   ...
   * superinterfaces of the implementing interfaces, also iterate over for all
   * the superclasses. For interface return all the extended interfaces as well
   * as superinterfaces for those extended interfaces.
   * 
   * @param type
   *          type whose implemented or super interfaces are sought.
   * @param configuration
   *          the current configuration of the doclet.
   * @param sort
   *          if true, return list of interfaces sorted alphabetically.
   * @return List of all the required interfaces.
   */
  public static List<Type> getAllInterfaces(Type type,
      Configuration configuration, boolean sort) {
    Map<ClassDoc, Type> results = sort ? new TreeMap<ClassDoc, Type>()
    ...
    addAllInterfaceTypes(results, superType,
        superType instanceof ClassDoc ? ((ClassDoc) superType).interfaceTypes()
            : ((ParameterizedType) superType).interfaceTypes(), false,
        configuration);
  ...
  private static void findAllInterfaceTypes(Map<ClassDoc, Type> results,
      ClassDoc c, boolean raw, Configuration configuration) {
    Type superType = c.superclassType();
    if (superType == null)
      return;
    addAllInterfaceTypes(results, superType,
        superType instanceof ClassDoc ? ((ClassDoc) superType).interfaceTypes()
            : ((ParameterizedType) superType).interfaceTypes(), raw,
        configuration);
  }

  private static void findAllInterfaceTypes(Map<ClassDoc, Type> results,
      ParameterizedType p, Configuration configuration) {
  ...
  private static void addAllInterfaceTypes(Map<ClassDoc, Type> results,
      Type type, Type[] interfaceTypes, boolean raw, Configuration configuration) {
    for (int i = 0; i < interfaceTypes.length; i++) {
      Type interfaceType = interfaceTypes[i];
      ClassDoc interfaceClassDoc = interfaceType.asClassDoc();
      if (!(interfaceClassDoc.isPublic() || (configuration != null && isLinkable(
  ...
  public static String getPackageName(PackageDoc packageDoc) {
    return packageDoc == null || packageDoc.name().length() == 0 ? DocletConstants.DEFAULT_PACKAGE_NAME
        : packageDoc.name();
  }
  ...
  public static String getPackageFileHeadName(PackageDoc packageDoc) {
    return packageDoc == null || packageDoc.name().length() == 0 ? DocletConstants.DEFAULT_PACKAGE_FILE_NAME
        : packageDoc.name();
  }
   ...
   *              Exception raised by the FileWriter is passed on to next level.
  ...
  public static Writer genWriter(Configuration configuration, String path,
    ...
    if (path != null) {
      ...
      fos = new FileOutputStream(((path.length() > 0) ? path + File.separator
          : "") + filename);
    } else {
   ...
   *          the separator char.
   * @param maxTokens
   *          the maxmimum number of tokens returned. If the max is reached, the
   *          remaining part of s is appended to the end of the last token.
   * 
  ...
  public static boolean isLinkable(ClassDoc classDoc,
    ...
    return ((classDoc.isIncluded() && configuration.isGeneratedDoc(classDoc)))
        || (configuration.extern.isExternal(classDoc) && (classDoc.isPublic() || classDoc
            .isProtected()));
  }
   ...
   * 
   * @param classDoc
   *          the class we are searching the parent for.
   * @param configuration
   *          the current configuration of the doclet.
   * @return the closest visible super class. Return null if it cannot be found
   *         (i.e. classDoc is java.lang.Object).
   */
   ...
   * 
   * @param cd
   *          the ClassDoc to check.
   * @param lowerCaseOnly
   *          true if you want the name returned in lower case. If false, the
   *          first letter of the name is capatilized.
   * @return
   */
    ...
    for (int j = 0; j < methods.length; j++) {
      ...
      if (currentMethod.name().equals("values")
        ...
        currentMethod.setRawCommentText(configuration.getText(
            "doclet.enum_values_doc", classDoc.name()));
        ...
        if (paramType != null
          ...
          currentMethod.setRawCommentText(configuration
