...
public class ConstantsSummaryBuilder extends AbstractBuilder {
   ...
   */
  public void buildConstantSummary(XMLNode node, Content contentTree)
      throws Exception {
    ...
    for (int i = 0; i < packages.length; i++) {
      if (hasConstantField(packages[i])
        ...
        writer.addLinkToPackageContent(packages[i],
            parsePackageName(packages[i].name()), printedPackageHeaders,
            contentListTree);
      }
    }
  ...
  public void buildClassConstantSummary(XMLNode node, Content summariesTree) {
    ClassDoc[] classes = currentPackage.name().length() > 0 ? currentPackage
        .allClasses() : configuration.classDocCatalog
        .allClasses(DocletConstants.DEFAULT_PACKAGE_NAME);
    Arrays.sort(classes);
  ...
  private boolean hasConstantField(PackageDoc pkg) {
    ...
    if (pkg.name().length() > 0) {
      ...
      classes = configuration.classDocCatalog
          .allClasses(DocletConstants.DEFAULT_PACKAGE_NAME);
    }
    boolean found = false;
  ...
  /**
   * Return true if the given package name has been printed. Also return true if
   * the root of this package has been printed.
   * 
   * @param pkgname
  ...
  private class ConstantFieldBuilder {
     ...
     *          the XML element that specifies which components to document
     * @param classConstantTree
     *          the tree to which the class constants table will be added
     */
    ...
    protected List<FieldDoc> members() {
      List<ProgramElementDoc> l = visibleMemberMapFields
