...
/**
 * Provides methods for creating an array of class, method and field names to be
 * included as meta keywords in the HTML header of class pages. These keywords
 * improve search results on browsers that look for keywords.
 * 
...
public class MetaKeywords {
  ...
  /**
   * Returns an array of strings where each element is a class, method or field
   * name. This array is used to create one meta keyword tag for each element.
   * Method parameter lists are converted to "()" and overloads are combined.
   * 
   * Constructors are not included because they have the same name as the class,
   * which is already included. Nested class members are not included because
   * their definitions are on separate pages.
   */
  ...
  /**
   * Get the current class for a meta tag keyword, as the first and only element
   * of an array list.
   */
  ...
  /**
   * Get members for meta tag keywords as an array, where each member name is a
   * string element of the array. The parameter lists are not included in the
   * keywords; therefore all overloaded methods are combined.<br>
