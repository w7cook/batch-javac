...
public abstract class TagletWriter {
   ...
   * 
   * @param doc
   *          the doc to write deprecated documentation for.
   ...
   * @param includeLink
   *          true if we should link the constant text to the constant field
   *          itself.
  ...
  /**
   * Given an output object, append to it the tag documentation for the given
   * member.
   * 
   * @param tagletManager
   *          the manager that manages the taglets.
   * @param doc
   *          the Doc that we are print tags for.
   * @param taglets
   *          the taglets to print.
   * @param writer
   *          the writer that will generate the output strings.
   * @param output
   ...
   * 
   * @param tagletManager
   *          The taglet manager for the current doclet.
   * @param holderTag
   *          The tag this holds this inline tag. Null if there is no tag that
   *          holds it.
   * @param inlineTag
   *          The inline tag to be documented.
   * @param tagletWriter
   *          The taglet writer to write the output.
  ...
  public static TagletOutput getInlineTagOuput(TagletManager tagletManager,
    ...
    for (int j = 0; j < definedTags.length; j++) {
      if (("@" + definedTags[j].getName()).equals(inlineTag.name())) {
        ...
        TagletOutput output = definedTags[j].getTagletOutput(holderTag != null
            && definedTags[j].getName().equals("inheritDoc") ? holderTag
            : inlineTag, tagletWriter);
        return output;
      }
    }
    return null;
  ...
  /**
   * Converts inline tags and text to TagOutput, expanding the inline tags along
   * the way. Called wherever text can contain an inline tag, such as in
   ...
   *          true if this is the first sentence.
   * @return the {@link TagletOutput} representing the comments.
   */
