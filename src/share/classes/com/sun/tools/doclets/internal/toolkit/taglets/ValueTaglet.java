...
/**
 * An inline Taglet representing the value tag. This tag should only be used
 * with constant fields that have a value. It is used to access the value of
 * constant fields. This inline tag has an optional field name parameter. If the
 * name is specified, the constant value is retrieved from the specified field.
 * A link is also created to the specified field. If a name is not specified,
 * the value is retrieved for the field that the inline tag appears on. The name
 * is specifed in the following format: [fully qualified class name]#[constant
 * field name].
 * 
...
public class ValueTaglet extends BaseInlineTaglet {
   ...
   *          the value tag.
   * @param name
   *          the name of the field to search for. The name should be in
   *          <qualified class name>#<field name> format. If the class name is
   *          omitted, it is assumed that the field is in the current class.
   * 
   * @return the corresponding FieldDoc. If the name is null or empty string,
   *         return field that the value tag was used in.
   * 
   * @throws DocletAbortException
   *           if the value tag does not specify a name to a value field and it
   *           is not used within the comments of a valid field.
   */
