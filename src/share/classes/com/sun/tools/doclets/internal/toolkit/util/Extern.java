 ...
 * generates "package-list"(lists all the packages which are getting documented)
 * file in the current or the destination directory, while generating the
 * documentation.
 * 
...
public class Extern {
  ...
  private class Item {
     ...
     * name. If the same package name is found in the map, then the first mapped
     * Item object or offline location will be retained.
     * 
     * @param packagename
     *          Package name found in the "package-list" file.
     * @param path
     *          URL or Directory path from where the "package-list" file is
     *          picked.
     * @param relative
   ...
   * @param pkglisturl
   *          This can be another URL for "package-list" or ordinary file.
   * @param reporter
   *          The <code>DocErrorReporter</code> used to report errors.
   * @param linkoffline
   *          True if -linkoffline isused and false if -link is used.
   */
  public boolean url(String url, String pkglisturl, DocErrorReporter reporter,
      boolean linkoffline) {
   ...
   * 
   * @param urlOrDirPath
   *          URL or the directory path.
   * @param pkgListUrlOrDirPath
   *          URL or directory path for the "package-list" file or the
   *          "package-list" file itself.
   */
  private String composeExternPackageList(String urlOrDirPath,
    ...
    return isUrl(pkgListUrlOrDirPath) ? fetchURLComposeExternPackageList(
        urlOrDirPath, pkgListUrlOrDirPath) : readFileComposeExternPackageList(
        urlOrDirPath, pkgListUrlOrDirPath);
  }
   ...
   * 
   * @param path
   *          URL or directory path to the packages.
   * @param pkgListPath
   *          Path to the local "package-list" file.
   */
  ...
  /**
   * Read the file "package-list" and for each package name found, create Extern
   * object and associate it with the package name in the map.
   * 
   * @param input
   *          InputStream from the "package-list" file.
   * @param path
   *          URL or the directory path to the packages.
   * @param relative
   *          Is path relative?
   */
