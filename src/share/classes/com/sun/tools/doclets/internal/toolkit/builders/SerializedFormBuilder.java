...
public class SerializedFormBuilder extends AbstractBuilder {
  ...
  /**
   * The header for the serial version UID. Save the string here instead of the
   * properties file because we do not want this string to be localized.
   */
   ...
   * 
   * @param node
   *          the XML element that specifies which components to document
   * @param serializedTree
   *          content tree to which the documentation will be added
   */
  public void buildSerializedForm(XMLNode node, Content serializedTree)
      throws Exception {
    serializedTree = writer.getHeader(configuration
  ...
  public void buildSerializedFormSummaries(XMLNode node, Content serializedTree) {
    ...
    serializedTree.addContent(writer
        .getSerializedContent(serializedSummariesTree));
  }
   ...
   * @param node
   *          the XML element that specifies which components to document
   * @param packageSerializedTree
   *          content tree to which the documentation will be added
   */
  public void buildPackageHeader(XMLNode node, Content packageSerializedTree) {
    packageSerializedTree.addContent(writer.getPackageHeader(Util
  ...
  public void buildSerializableMethods(XMLNode node, Content classContentTree) {
    ...
    if (membersLength > 0) {
      for (int i = 0; i < membersLength; i++) {
        ...
        Content methodsContentTree = methodWriter
            .getMethodsContentHeader((i == membersLength - 1));
    ...
    if (currentClass.serializationMethods().length > 0) {
      ...
      if (currentClass.isSerializable() && !currentClass.isExternalizable()) {
        if (currentClass.serializationMethods().length == 0) {
          Content noCustomizationMsg = methodWriter
              .getNoCustomizationMsg(configuration
                  .getText("doclet.Serializable_no_customization"));
          classContentTree.addContent(methodWriter.getSerializableMethods(
              configuration.getText("doclet.Serialized_Form_methods"),
              noCustomizationMsg));
        }
  ...
  public void buildDeprecatedMethodInfo(XMLNode node, Content methodsContentTree) {
    methodWriter.addDeprecatedMemberInfo((MethodDoc) currentMember,
        methodsContentTree);
  }
  ...
  public void buildMethodDescription(XMLNode node, Content methodsContentTree) {
    methodWriter.addMemberDescription((MethodDoc) currentMember,
        methodsContentTree);
  }
  ...
  public void buildMethodTags(XMLNode node, Content methodsContentTree) {
    ...
    if (method.name().compareTo("writeExternal") == 0
      ...
      if (configuration.serialwarn) {
        configuration.getDocletSpecificMsg().warning(currentMember.position(),
   ...
   * 
   * @param classDoc
   *          the class to print the overview for.
   * @param classContentTree
   *          content tree to which the documentation will be added
   */
  public void buildFieldSerializationOverview(ClassDoc classDoc,
    ...
    if (classDoc.definesSerializableFields()) {
      ...
      if (fieldWriter.shouldPrintOverview(serialPersistentField)) {
        Content serializableFieldsTree = fieldWriter
            .getSerializableFieldsHeader();
        Content fieldsOverviewContentTree = fieldWriter
            .getFieldsContentHeader(true);
    ...
    if (membersLength > 0) {
      ...
      for (int i = 0; i < membersLength; i++) {
        ...
        if (!currentClass.definesSerializableFields()) {
          Content fieldsContentTree = fieldWriter
              .getFieldsContentHeader((i == membersLength - 1));
   ...
   * @param node
   *          the XML element that specifies which components to document
   * @param fieldsContentTree
   *          content tree to which the documentation will be added
   */
  public void buildFieldSubHeader(XMLNode node, Content fieldsContentTree) {
    if (!currentClass.definesSerializableFields()) {
      FieldDoc field = (FieldDoc) currentMember;
      fieldWriter.addMemberHeader(field.type().asClassDoc(), field.type()
          .typeName(), field.type().dimension(), field.name(),
          fieldsContentTree);
    }
  }
  ...
  public void buildSerialFieldTagsInfo(Content serializableFieldsTree) {
    ...
    for (int i = 0; i < tagsLength; i++) {
      Content fieldsContentTree = fieldWriter
          .getFieldsContentHeader((i == tagsLength - 1));
      fieldWriter.addMemberHeader(tags[i].fieldTypeDoc(), tags[i].fieldType(),
          "", tags[i].fieldName(), fieldsContentTree);
  ...
  public void buildFieldInfo(XMLNode node, Content fieldsContentTree) {
    ...
    if ((field.tags("serial").length == 0) && !field.isSynthetic()
      ...
      configuration.message.warning(field.position(),
          "doclet.MissingSerialTag", cd.qualifiedName(), field.name());
    }
  ...
  public static boolean serialInclude(Doc doc) {
    ...
    return doc.isClass() ? serialClassInclude((ClassDoc) doc)
        : serialDocInclude(doc);
  }

  /**
   * Return true if the given ClassDoc should be included in the serialized
   * form.
   * 
   * @param cd
