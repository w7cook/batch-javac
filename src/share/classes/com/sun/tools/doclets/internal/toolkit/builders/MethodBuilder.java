...
public class MethodBuilder extends AbstractMemberBuilder {
  ...
  /**
   * The index of the current field that is being documented at this point in
   * time.
   */
  ...
  public static MethodBuilder getInstance(Configuration configuration,
    ...
    builder.methods = new ArrayList<ProgramElementDoc>(
  ...
  public void buildMethodDoc(XMLNode node, Content memberDetailsTree) {
    ...
    if (size > 0) {
      Content methodDetailsTree = writer.getMethodDetailsTreeHeader(classDoc,
          memberDetailsTree);
      for (currentMethodIndex = 0; currentMethodIndex < size; currentMethodIndex++) {
        ...
        methodDetailsTree.addContent(writer.getMethodDoc(methodDocTree,
            (currentMethodIndex == size - 1)));
      }
  ...
  public void buildSignature(XMLNode node, Content methodDocTree) {
    methodDocTree.addContent(writer.getSignature((MethodDoc) methods
  ...
  public void buildDeprecationInfo(XMLNode node, Content methodDocTree) {
    writer.addDeprecated((MethodDoc) methods.get(currentMethodIndex),
        methodDocTree);
  }
  ...
  public void buildMethodComments(XMLNode node, Content methodDocTree) {
    if (!configuration.nocomment) {
      ...
      if (method.inlineTags().length == 0) {
        ...
        method = docs.inlineTags != null && docs.inlineTags.length > 0 ? (MethodDoc) docs.holder
            : method;
      }
