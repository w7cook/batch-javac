...
public class ClassUseMapper {
  ...
  /**
   * Mapping of ClassDocs to set of PackageDoc used by that class. Entries may
   * be null.
   */
  ...
  /**
   * Mapping of ClassDocs to set of ClassDoc used by that class. Entries may be
   * null.
   */
  public Map<String, Set<ClassDoc>> classToClass = new HashMap<String, Set<ClassDoc>>();

  /**
   * Mapping of ClassDocs to list of ClassDoc which are direct or indirect
   * subclasses of that class. Entries may be null.
   */
  public Map<String, List<ClassDoc>> classToSubclass = new HashMap<String, List<ClassDoc>>();

  /**
   * Mapping of ClassDocs to list of ClassDoc which are direct or indirect
   * subinterfaces of that interface. Entries may be null.
   */
  ...
  /**
   * Mapping of ClassDocs to list of FieldDoc declared as that class. Entries
   * may be null.
   */
  public Map<String, List<FieldDoc>> classToField = new HashMap<String, List<FieldDoc>>();

  /**
   * Mapping of ClassDocs to list of MethodDoc returning that class. Entries may
   * be null.
   */
  ...
  /**
   * Mapping of ClassDocs to list of MethodDoc which throws that class. Entries
   * may be null.
   */
  ...
  /**
   * The mapping of ClassDocs to Constructor arguments that use them as type
   * parameters.
   */
  ...
  /**
   * The mapping of ClassDocs to ExecutableMemberDocs that use them as type
   * parameters.
   */
  public Map<String, List<MethodDoc>> classToExecMemberDocTypeParam = new HashMap<String, List<MethodDoc>>();

  /**
   * The mapping of ClassDocs to ExecutableMemberDocs arguments that use them as
   * type parameters.
   */
  ...
  /**
   * The mapping of ClassDocs to ExecutableMemberDocs that have return type with
   * type parameters of that class.
   */
  ...
  public ClassUseMapper(RootDoc root, ClassTree classtree) {
    ...
    for (Iterator<ClassDoc> it = classtree.baseclasses().iterator(); it
        .hasNext();) {
      subclasses(it.next());
    }
    for (Iterator<ClassDoc> it = classtree.baseinterfaces().iterator(); it
        .hasNext();) {
  ...
  /**
   * Return all subinterfaces of an interface AND fill-in classToSubinterface
   * map.
   */
  ...
  /**
   * Return all implementing classes of an interface (including all subclasses
   * of implementing classes and all classes implementing subinterfaces) AND
   * fill-in both classToImplementingClass and classToSubinterface maps.
   */
  ...
  /**
   * Determine classes used by a method or constructor, so they can be inverse
   * mapped.
   */
  private void mapExecutable(ExecutableMemberDoc em) {
    ...
    for (int k = 0; k < params.length; k++) {
      ...
      if ((!params[k].type().isPrimitive()) && !classArgs.contains(pcd)
          && !(pcd instanceof TypeVariable)) {
        add(isConstructor ? classToConstructorArgs : classToMethodArgs,
            pcd.asClassDoc(), em);
        classArgs.add(pcd);
        mapTypeParameters(isConstructor ? classToConstructorDocArgTypeParam
            : classToExecMemberDocArgTypeParam, pcd, em);
      }
      mapAnnotations(isConstructor ? classToConstructorParamAnnotation
          : classToExecMemberDocParamAnnotation, params[k], em);
    }
  ...
  private <T extends ProgramElementDoc> void add(Map<String, List<T>> map,
      ClassDoc cd, T ref) {
    ...
    classSet(cd).add(
        ref instanceof MemberDoc ? ((MemberDoc) ref).containingClass()
            : (ClassDoc) ref);
  }

  private void addAll(Map<String, List<ClassDoc>> map, ClassDoc cd,
      Collection<ClassDoc> refs) {
    if (refs == null) {
      return;
    }
    // add to specified map
  ...
  /**
   * Map the ClassDocs to the ProgramElementDocs that use them as type
   * parameters.
   * 
   * @param map
   *          the map the insert the information into.
   * @param doc
   *          the doc whose type parameters are being checked.
   * @param holder
  ...
  /**
   * Map the AnnotationType to the ProgramElementDocs that use them as type
   * parameters.
   * 
   * @param map
   *          the map the insert the information into.
   ...
   *          the holder that owns the type parameters.
   */
  private <T extends PackageDoc> void mapAnnotations(Map<String, List<T>> map,
      PackageDoc doc, T holder) {
    AnnotationDesc[] annotations;
