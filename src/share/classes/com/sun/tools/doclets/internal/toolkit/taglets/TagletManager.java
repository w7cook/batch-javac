...
public class TagletManager {
  ...
  /**
   * The alternate seperator for simple tag options. Use this with you want the
   * default seperator to be in the name of the custom tag.
   */
  ...
  /**
   * Keep track of standard tags in lowercase to compare for better error
   * messages when a tag like @docRoot is mistakenly spelled lowercase @docroot.
   */
  ...
  /**
   * Keep track of the tags that may conflict with standard tags in the future
   * (any custom tag without a period in its name).
   */
   ...
   * registered because these Taglets are typically added for every execution of
   * the Doclet. We don't want to see this type of error message every time.
   * 
   * @param customTag
   ...
   * 
   * @param classname
   *          the name of the class representing the custom tag.
   * @param tagletPath
  ...
  public void addCustomTag(String classname, String tagletPath) {
    try {
      ...
      Taglet lastTag = (list != null && list.length > 0) ? (Taglet) list[list.length - 1]
          : null;
      meth.invoke(null, new Object[] { customTags });
      list = customTags.values().toArray();
      Object newLastTag = (list != null && list.length > 0) ? list[list.length - 1]
          : null;
      ...
      message.error("doclet.Error_taglet_not_registered", exc.getClass()
          .getName(), classname);
    }
  ...
  /**
   * Add a new <code>SimpleTaglet</code>. If this tag already exists and the
   * header passed as an argument is null, move tag to the back of the list. If
   * this tag already exists and the header passed as an argument is not null,
   * overwrite previous tag with new one. Otherwise, add new SimpleTaglet to
   * list.
   * 
   * @param tagName
  ...
  /**
   * Check the taglet to see if it is a legacy taglet. Also check its name for
   * errors.
   */
  ...
  /**
   * Given a name of a seen custom tag, remove it from the set of unseen custom
   * tags.
   * 
   * @param name
   ...
   * 
   * @param doc
   *          the Doc object that holds the tags.
   * @param tags
   *          the list of <code>Tag</code>s to check.
   * @param areInlineTags
    ...
    for (int i = 0; i < tags.length; i++) {
      ...
      if (!(standardTags.contains(name) || customTags.containsKey(name))) {
        if (standardTagsLowercase.contains(name.toLowerCase())) {
          message.warning(tags[i].position(), "doclet.UnknownTagLowercase",
              tags[i].name());
          continue;
        } else {
          message.warning(tags[i].position(), "doclet.UnknownTag",
              tags[i].name());
          continue;
        }
      }
  ...
  /**
   * Given the taglet, the tag and the type of documentation that the tag was
   * found in, print a tag misuse warning.
   * 
   * @param taglet
   *          the taglet representing the misused tag.
   * @param tag
   *          the misused tag.
   * @param holderType
  ...
  /**
   * Return the array of <code>Taglet</code>s that can appear in packages.
   * 
   * @return the array of <code>Taglet</code>s that can appear in packages.
   */
  ...
  /**
   * Return the array of <code>Taglet</code>s that can appear in classes or
   * interfaces.
   * 
   * @return the array of <code>Taglet</code>s that can appear in classes or
   *         interfaces.
  ...
  /**
   * Return the array of inline <code>Taglet</code>s that can appear in
   * comments.
   * 
   * @return the array of <code>Taglet</code>s that can appear in comments.
   */
  ...
  /**
   * Return the array of <code>Taglet</code>s that can appear in fields.
   * 
   * @return the array of <code>Taglet</code>s that can appear in field.
   */
  ...
  /**
   * Return the array of <code>Taglet</code>s that can appear in the serialized
   * form.
   * 
   * @return the array of <code>Taglet</code>s that can appear in the serialized
   *         form.
  ...
  /**
   * @return the array of <code>Taglet</code>s that can appear in the given Doc.
   */
  ...
  /**
   * Return the array of <code>Taglet</code>s that can appear in constructors.
   * 
   * @return the array of <code>Taglet</code>s that can appear in constructors.
   */
  ...
  /**
   * Return the array of <code>Taglet</code>s that can appear in methods.
   * 
   * @return the array of <code>Taglet</code>s that can appear in methods.
   */
  ...
  /**
   * Return the array of <code>Taglet</code>s that can appear in an overview.
   * 
   * @return the array of <code>Taglet</code>s that can appear in overview.
   */
  ...
  private void initStandardTags() {
    ...
    customTags.put((temp = new SimpleTaglet("exception", null,
        SimpleTaglet.METHOD + SimpleTaglet.CONSTRUCTOR)).getName(), temp);
    if (!nosince) {
      customTags.put(
          (temp = new SimpleTaglet("since", message.getText("doclet.Since"),
              SimpleTaglet.ALL)).getName(), temp);
    }
    if (showversion) {
      customTags
          .put(
              (temp = new SimpleTaglet("version", message
                  .getText("doclet.Version"), SimpleTaglet.PACKAGE
                  + SimpleTaglet.TYPE + SimpleTaglet.OVERVIEW)).getName(), temp);
    }
    if (showauthor) {
      customTags
          .put(
              (temp = new SimpleTaglet("author", message
                  .getText("doclet.Author"), SimpleTaglet.PACKAGE
                  + SimpleTaglet.TYPE + SimpleTaglet.OVERVIEW)).getName(), temp);
    }
    customTags.put(
        (temp = new SimpleTaglet("serialData", message
            .getText("doclet.SerialData"), SimpleTaglet.EXCLUDED)).getName(),
        temp);
    customTags.put(
        (temp = new SimpleTaglet("factory", message.getText("doclet.Factory"),
    ...
    customTags.put((temp = new LegacyTaglet(new LiteralTaglet())).getName(),
        temp);
    customTags.put((temp = new LegacyTaglet(new CodeTaglet())).getName(), temp);
  ...
  /**
   * Print a list of {@link Taglet}s that might conflict with standard tags in
   * the future and a list of standard tags that have been overriden.
   */
  ...
  /**
   * Given the name of a tag, return the corresponding taglet. Return null if
   * the tag is unknown.
   * 
   * @param name
   *          the name of the taglet to retrieve.
   * @return return the corresponding taglet. Return null if the tag is unknown.
   */
