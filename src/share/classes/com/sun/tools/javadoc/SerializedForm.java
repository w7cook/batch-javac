...
class SerializedForm {
  ...
  /*
   * List of FieldDocImpl - Serializable fields. Singleton list if class defines
   * Serializable fields explicitly. Otherwise, list of default serializable
   * fields. 0 length list for Externalizable.
   */
  private final ListBuffer<FieldDocImpl> fields = new ListBuffer<FieldDocImpl>();

  /*
   * True if class specifies serializable fields explicitly. using special
   * static member, serialPersistentFields.
   */
   ...
   * Constructor.
   * 
   * Catalog Serializable fields for Serializable class. Catalog serialization
   * methods for Serializable and Externalizable classes.
   */
  SerializedForm(DocEnv env, ClassSymbol def, ClassDocImpl cd) {
    if (cd.isExternalizable()) {
      /*
       * look up required public accessible methods, writeExternal and
       * readExternal.
       */
      ...
      if (dsf != null) {
        ...
        /*
         * Define serializable fields with array of ObjectStreamField. Each
         * ObjectStreamField should be documented by a serialField tag.
         */
        ...
        /*
         * Calculate default Serializable fields as all non-transient,
         * non-static fields. Fields should be documented by serial tag.
         */
        computeDefaultSerializableFields(env, def, cd);
      }

      /*
       * Check for optional customized readObject, writeObject, readResolve and
       * writeReplace, which can all contain the serialData tag.
       */
  ...
  /*
   * Check for explicit Serializable fields. Check for a private static array of
   * ObjectStreamField with name SERIALIZABLE_FIELDS.
   */
  private VarSymbol getDefinedSerializableFields(ClassSymbol def) {
    Names names = def.name.table.names;

    /*
     * SERIALIZABLE_FIELDS can be private, so must lookup by ClassSymbol, not by
     * ClassDocImpl.
     */
    for (Scope.Entry e = def.members().lookup(
        names.fromString(SERIALIZABLE_FIELDS)); e.scope != null; e = e.next()) {
   ...
   * 
   * Since the fields of ClassDocImpl might not contain private or package
   * accessible fields, must walk over all members of ClassSymbol.
   */
  ...
  /*
   * Catalog Serializable method if it exists in current ClassSymbol. Do not
   * look for method in superclasses.
   * 
   * Serialization requires these methods to be non-static.
   * 
   * @param method should be an unqualified Serializable method name either
   * READOBJECT, WRITEOBJECT, READRESOLVE or WRITEREPLACE.
   * 
  ...
  private void addMethodIfExist(DocEnv env, ClassSymbol def, String methodName) {
    ...
    for (Scope.Entry e = def.members().lookup(names.fromString(methodName)); e.scope != null; e = e
        .next()) {
      if (e.sym.kind == Kinds.MTH) {
        MethodSymbol md = (MethodSymbol) e.sym;
        if ((md.flags() & Flags.STATIC) == 0) {
          /*
           * WARNING: not robust if unqualifiedMethodName is overloaded method.
           * Signature checking could make more robust. READOBJECT takes a
           * single parameter, java.io.ObjectInputStream. WRITEOBJECT takes a
           * single parameter, java.io.ObjectOutputStream.
           */
  ...
  /*
   * Associate serialField tag fieldName with FieldDocImpl member. Note: A
   * serialField tag does not have to map an existing field of a class.
   */
  private void mapSerialFieldTagImplsToFieldDocImpls(FieldDocImpl spfDoc,
    ...
    for (int i = 0; i < sfTag.length; i++) {
      ...
      for (Scope.Entry e = def.members().lookup(fieldName); e.scope != null; e = e
          .next()) {
        if (e.sym.kind == Kinds.VAR) {
          VarSymbol f = (VarSymbol) e.sym;
   ...
   * serialField tag for each serializable field.
   * <p>
   * 
   * @return an array of FieldDocImpl for representing the visible fields in
   *         this class.
   */
  FieldDoc[] fields() {
  ...
  /**
   * Returns true if Serializable fields are defined explicitly using member,
