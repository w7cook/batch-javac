...
/**
 * This class could be the main entry point for Javadoc when Javadoc is used as
 * a component in a larger software system. It provides operations to construct
 * a new javadoc processor, and to run it on a set of source files.
 * 
 * @author Neal Gafter
 */
public class JavadocTool extends com.sun.tools.javac.main.JavaCompiler {
  ...
  /**
   * Construct a new JavaCompiler processor, using appropriately extended phases
  ...
  /**
   * For javadoc, the parser needs to keep comments. Overrides method from
   * JavaCompiler.
   */
  ...
  public RootDocImpl getRootDocImpl(String doclocale, String encoding,
      ModifierFilter filter, List<String> javaNames, List<String[]> options,
      boolean breakiterator, List<String> subPackages,
      List<String> excludedPackages, boolean docClasses, boolean legacyDoclet,
    ...
    try {
      ...
      if (!docClasses) {
        ...
        Map<String, List<JavaFileObject>> packageFiles = searchSubPackages(
            subPackages, names, excludedPackages);
        ...
        for (List<String> packs = names.toList(); packs.nonEmpty(); packs = packs.tail) {
          ...
          parsePackageClasses(packageName, packageFiles.get(packageName),
              packTrees, excludedPackages);
        }
  ...
  /**
   * search all directories in path for subdirectory name. Add all .java files
   * found in such a directory to args.
   */
  private void parsePackageClasses(String name, Iterable<JavaFileObject> files,
      ListBuffer<JCCompilationUnit> trees, List<String> excludedPackages)
      throws IOException {
    ...
    if (files == null) {
      Location location = docenv.fileManager
      ...
      for (JavaFileObject fo : docenv.fileManager.list(location, name,
          EnumSet.of(JavaFileObject.Kind.SOURCE), false)) {
  ...
  /**
   * Recursively search all directories in path for subdirectory name. Add all
   * packages found in such a directory to packages list.
   */
  private Map<String, List<JavaFileObject>> searchSubPackages(
      List<String> subPackages, ListBuffer<String> packages,
      List<String> excludedPackages) throws IOException {
    Map<String, List<JavaFileObject>> packageFiles = new HashMap<String, List<JavaFileObject>>();
    ...
    if (docenv.fileManager.hasLocation(StandardLocation.SOURCE_PATH)) {
      searchSubPackages(subPackages, includedPackages, packages, packageFiles,
          StandardLocation.SOURCE_PATH, EnumSet.of(JavaFileObject.Kind.SOURCE));
      searchSubPackages(subPackages, includedPackages, packages, packageFiles,
          StandardLocation.CLASS_PATH, EnumSet.of(JavaFileObject.Kind.CLASS));
    } else {
      searchSubPackages(subPackages, includedPackages, packages, packageFiles,
          StandardLocation.CLASS_PATH,
  ...
  private void searchSubPackages(List<String> subPackages,
      Map<String, Boolean> includedPackages, ListBuffer<String> packages,
    ...
    for (String subPackage : subPackages) {
      ...
      for (JavaFileObject fo : docenv.fileManager.list(location, subPackage,
          kinds, true)) {
        String binaryName = docenv.fileManager.inferBinaryName(location, fo);
        String packageName = getPackageName(binaryName);
        String simpleName = getSimpleName(binaryName);
        if (isIncluded(packageName, includedPackages)
            && isValidClassName(simpleName)) {
  ...
  private void searchSubPackage(String packageName,
      ListBuffer<String> packages, List<String> excludedPackages,
    ...
    for (File pathname : pathnames) {
      ...
      if (filenames != null) {
        for (String filename : filenames) {
          if (!addedPackage
              && (isValidJavaSourceFile(filename) || isValidJavaClassFile(filename))
              && !packages.contains(packageName)) {
            packages.append(packageName);
            addedPackage = true;
          } else if (isValidClassName(filename)
              && (new File(f, filename)).isDirectory()) {
            searchSubPackage(packageName + "." + filename, packages,
                excludedPackages, pathnames);
          }
        }
      }
    }
   ...
   * @return true if given file name is a valid class file name and false
   *         otherwise.
   */
   ...
   * @param file
   *          the name of the file to check.
   * @return true if given file name is a valid Java source file name and false
   *         otherwise.
   */
  ...
  /**
   * Return true if given file name is a valid class name (including
   * "package-info").
   * 
   * @param clazzname
  ...
  public static boolean isValidClassName(String s) {
    ...
    if (surrogatesSupported) {
      ...
      if (!Character.isJavaIdentifierStart(cp))
        return false;
      for (int j = Character.charCount(cp); j < s.length(); j += Character
          .charCount(cp)) {
        cp = s.codePointAt(j);
  ...
  /**
   * From a list of top level trees, return the list of contained class
   * definitions
   */
