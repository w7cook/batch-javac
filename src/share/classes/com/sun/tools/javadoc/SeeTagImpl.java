...
/**
 * Represents a see also documentation tag. The @see tag can be plain text, or
 * reference a class or member.
 * 
...
class SeeTagImpl extends TagImpl implements SeeTag, LayoutCharacters {
  ...
  SeeTagImpl(DocImpl holder, String name, String text) {
    ...
    if (where != null) {
      ...
      if (holder instanceof MemberDoc) {
        container = (ClassDocImpl) ((ProgramElementDoc) holder)
            .containingClass();
  ...
  /**
   * get the class name part of @see, For instance, if the comment is @see
   * String#startsWith(java.lang.String) . This function returns String. Returns
   * null if format was not that of java reference. Return empty string if class
  ...
  /**
   * get the package referenced by @see. For instance, if the comment is @see
   * java.lang This function returns a PackageDocImpl for java.lang Returns null
   * if no known package found.
   */
  ...
  /**
   * get the class referenced by the class name part of @see, For instance, if
   * the comment is @see String#startsWith(java.lang.String) . This function
   * returns a ClassDocImpl for java.lang.String. Returns null if class is not a
   * class specified on the javadoc command line..
   */
  ...
  /**
   * get the name of the member referenced by the prototype part of @see, For
   * instance, if the comment is @see String#startsWith(java.lang.String) . This
   * function returns "startsWith(java.lang.String)" Returns null if format was
   * not that of java reference. Return empty string if member name was not
   * specified..
   */
  ...
  /**
   * get the member referenced by the prototype part of @see, For instance, if
   * the comment is @see String#startsWith(java.lang.String) . This function
   * returns a MethodDocImpl for startsWith. Returns null if member could not be
   * determined.
   */
  ...
  private void parseSeeString() {
    ...
    case '<':
      if (text.charAt(len - 1) != '>') {
        docenv().warning(holder, "tag.see.no_close_bracket_on_url", name, text);
      }
      return;
    case '"':
    ...
    for (int i = start; i < len; i += Character.charCount(cp)) {
      ...
      default:
        if (!Character.isJavaIdentifierPart(cp)) {
          docenv().warning(holder, "tag.see.illegal_character", name, "" + cp,
              text);
        }
        break;
      }
  ...
  /**
   * Find what is referenced by the see also. If possible, sets referencedClass
   * and referencedMember.
   * 
   * @param containingClass
   *          the class containing the comment containing the tag. May be null,
   *          if, for example, it is a package comment.
   */
  private void findReferenced(ClassDocImpl containingClass) {
    if (where.length() > 0) {
      ...
      if (referencedClass == null && holder() instanceof ProgramElementDoc) {
        referencedClass = docenv().lookupClass(
            ((ProgramElementDoc) holder()).containingPackage().name() + "."
                + where);
      }
    ...
    if (what == null) {
      ...
      if (paren > 0) {
        ...
        paramarr = new ParameterParseMachine(what.substring(paren,
            what.length())).parseParameters();
        if (paramarr != null) {
        ...
        if (referencedMember == null
            || (fd != null && fd.containingClass().subclassOf(
                referencedMember.containingClass()))) {
          referencedMember = fd;
        }
      }
      if (referencedMember == null) {
        docenv().warning(holder, "tag.see.can_not_find_member", name, what,
            where);
      }
    }
  }
  ...
  private MemberDoc findExecutableMember(String memName, String[] paramarr,
    ...
    if (memName.equals(referencedClass.name())) {
      return ((ClassDocImpl) referencedClass)
          .findConstructor(memName, paramarr);
    } else { // it's a method.
  ...
  class ParameterParseMachine {
    ...
    public String[] parseParameters() {
      ...
      int cp;
      for (int index = 0; index < parameters.length(); index += Character
          .charCount(cp)) {
        ...
        case TNSPACE:
          if (Character.isJavaIdentifierStart(cp)) { // name
            if (prevstate == ARRAYDECORATION) {
              docenv().warning(holder, "tag.missing_comma_space", name,
                  "(" + parameters + ")");
              return (String[]) null;
            }
            addTypeToParamList();
        ...
        case ARRAYDECORATION:
          if (cp == ']') {
            ...
            docenv().warning(holder, "tag.illegal_char_in_arr_dim", name,
                "(" + parameters + ")");
            return (String[]) null;
          }
      ...
      if (state == ARRAYDECORATION || (state == START && prevstate == TNSPACE)) {
        docenv().warning(holder, "tag.illegal_see_tag", "(" + parameters + ")");
      }
      if (typeId.length() > 0) {
