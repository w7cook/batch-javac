...
public class TypeMaker {
  ...
  /**
   * @param errToClassDoc
   *          if true, ERROR type results in a ClassDoc; false preserves legacy
   *          behavior
   */
  ...
  /**
   * Return the string representation of a type use. Bounds of type variables
   * are not included; bounds of wildcard types are. Class names are qualified
   * if "full" is true.
   */
  static String getTypeString(DocEnv env, Type t, boolean full) {
    ...
    case ARRAY:
      ...
      return s.toString();
    case CLASS:
      return ParameterizedTypeImpl.parameterizedTypeToString(env,
          (ClassType) t, full);
    case WILDCARD:
      Type.WildcardType a = (Type.WildcardType) t;
   ...
   * angle-bracketed string. Each parameter is a type variable with optional
   * bounds. Class names are qualified if "full" is true. Return "" if there are
   * no type parameters or we're hiding generics.
   */
   ...
   * angle-bracketed string. Class name are qualified if "full" is true. Return
   * "" if there are no type arguments or we're hiding generics.
   */
  ...
  private static class ArrayTypeImpl implements com.sun.javadoc.Type {
     ...
     * 
     * @return a ClassDocImpl if the type is a Class. Return null if it is a
     *         primitive type..
     */
    ...
    /**
     * Return this type as a <code>ParameterizedType</code> if it represents a
     * parameterized type. Array dimensions are ignored.
     */
