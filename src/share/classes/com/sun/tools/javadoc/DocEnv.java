...
/**
 * Holds the environment for a run of javadoc. Holds only the information needed
 * throughout the run and not the compiler info that could be GC'ed or ported.
 * 
 * @since 1.4
 * @author Robert Field
...
public class DocEnv {
  ...
  private DocEnv(Context context) {
    ...
    externalizableSym = reader.enterClass(names
        .fromString("java.io.Externalizable"));
  ...
  public boolean shouldDocument(ClassSymbol sym) {
    return (sym.flags_field & Flags.SYNTHETIC) == 0 && // no synthetics
        (docClasses || getClassDoc(sym).tree != null) && isVisible(sym);
  }

  // ### Comment below is inaccurate wrt modifier filter testing
  /**
   * Check the visibility if this is an nested class. if this is not a nested
   * class, return true. if this is an static visible nested class, return true.
   * if this is an visible nested class if the outer class is visible return
   * true. else return false. IMPORTANT: This also allows, static nested classes
   * to be defined inside an nested class, which is not allowed by the compiler.
   * So such an test case will not reach upto this method itself, but if
   * compiler allows it, then that will go through.
   */
  ...
  private Map<PackageSymbol, PackageDocImpl> packageMap = new HashMap<PackageSymbol, PackageDocImpl>();
  ...
  private Map<ClassSymbol, ClassDocImpl> classMap = new HashMap<ClassSymbol, ClassDocImpl>();
   ...
   */
  void makeClassDoc(ClassSymbol clazz, String docComment, JCClassDecl tree,
  ...
  private Map<VarSymbol, FieldDocImpl> fieldMap = new HashMap<VarSymbol, FieldDocImpl>();
   ...
   */
  void makeFieldDoc(VarSymbol var, String docComment, JCVariableDecl tree,
      Position.LineMap lineMap) {
    FieldDocImpl result = fieldMap.get(var);
    if (result != null) {
      if (docComment != null)
  ...
  private Map<MethodSymbol, ExecutableMemberDocImpl> methodMap = new HashMap<MethodSymbol, ExecutableMemberDocImpl>();

  /**
   * Create a MethodDoc for this MethodSymbol. Should be called only on symbols
   * representing methods.
   */
  void makeMethodDoc(MethodSymbol meth, String docComment, JCMethodDecl tree,
      Position.LineMap lineMap) {
  ...
  /**
   * Return the MethodDoc for a MethodSymbol. Should be called only on symbols
   * representing methods.
   */
  ...
  /**
   * Create the ConstructorDoc for a MethodSymbol. Should be called only on
   * symbols representing constructors.
   */
  ...
  /**
   * Return the ConstructorDoc for a MethodSymbol. Should be called only on
   * symbols representing constructors.
   */
  ...
  /**
   * Create the AnnotationTypeElementDoc for a MethodSymbol. Should be called
   * only on symbols representing annotation type elements.
   */
  void makeAnnotationTypeElementDoc(MethodSymbol meth, String docComment,
      JCMethodDecl tree, Position.LineMap lineMap) {
    AnnotationTypeElementDocImpl result = (AnnotationTypeElementDocImpl) methodMap
        .get(meth);
    if (result != null) {
      if (docComment != null)
        result.setRawCommentText(docComment);
      ...
      result = new AnnotationTypeElementDocImpl(this, meth, docComment, tree,
          lineMap);
      methodMap.put(meth, result);
    }
  }
  ...
  /**
   * Return the AnnotationTypeElementDoc for a MethodSymbol. Should be called
   * only on symbols representing annotation type elements.
   */
  ...
  /**
   * Return the ParameterizedType of this instantiation. // * ### Could use
   * Type.sameTypeAs() instead of equality matching in hashmap // * ### to avoid
   * some duplication.
   */
  ...
  /**
   * Convert modifier bits from private coding used by the compiler to that of
