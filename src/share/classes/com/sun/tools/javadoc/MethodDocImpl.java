...
public class MethodDocImpl extends ExecutableMemberDocImpl implements MethodDoc {
  ...
  /**
   * Return true if it is a method, which it is. Note: constructors are not
   * methods. This method is overridden by AnnotationTypeElementDocImpl.
   * 
   * @return true
   */
  ...
  /**
   * Return the class that originally defined the method that is overridden by
   * the current definition, or null if no such class exists.
   * 
   * @return a ClassDocImpl representing the superclass that originally defined
   *         this method, null if this method does not override a definition in
   *         a superclass.
   */
  ...
  /**
   * Return the type containing the method that this method overrides. It may be
   * a <code>ClassDoc</code> or a <code>ParameterizedType</code>.
   */
  public com.sun.javadoc.Type overriddenType() {
    ...
    for (Type t = env.types.supertype(origin.type); t.tag == TypeTags.CLASS; t = env.types
        .supertype(t)) {
      ClassSymbol c = (ClassSymbol) t.tsym;
      for (Scope.Entry e = c.members().lookup(sym.name); e.scope != null; e = e
          .next()) {
  ...
  /**
   * Returns a string representation of this method. Includes the qualified
   * signature, the qualified method name, and any type parameters. Type
   * parameters follow the class name, as they do in the syntax for invoking
   * methods with explicit type parameters.
   */
  public String toString() {
    return sym.enclClass().getQualifiedName() + "." + typeParametersString()
        + name() + signature();
  }
}
