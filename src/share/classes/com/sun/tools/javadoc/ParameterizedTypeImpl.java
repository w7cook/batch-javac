...
/**
 * Implementation of <code>ParameterizedType</code>, which represents an
 * invocation of a generic class or interface.
 * 
 * @author Scott Seligman
 * @since 1.5
 */
public class ParameterizedTypeImpl extends AbstractTypeImpl implements
    ParameterizedType {
  ...
  /**
   * Return the class type that is a direct supertype of this one. Return null
   ...
   * parameterized type. Return an empty array if there are no interfaces.
   */
  ...
  /**
   * Return the type that contains this type as a member. Return null is this is
   * a top-level type.
   */
  ...
  static String parameterizedTypeToString(DocEnv env, ClassType cl, boolean full) {
    ...
    if (cl.getEnclosingType().tag != CLASS) { // if not an inner class...
      ...
      s.append(parameterizedTypeToString(env, encl, full)).append('.')
