...
/**
 * Represents a java class and provides access to information about the class,
 * the class' comment and tags, and the members of the class. A ClassDocImpl
 * only exists if it was processed in this run of javadoc. References to classes
 * which may or may not have been processed in this run are referred to using
 * Type (which can be converted to ClassDocImpl, if possible).
 * 
 * @see Type
 * 
 * @since 1.2
...
public class ClassDocImpl extends ProgramElementDocImpl implements ClassDoc {
  ...
  public final ClassType type; // protected->public for
                               // debugging
  ...
  /**
   * Return true if this is a ordinary class, not an enumeration, exception, an
   * error, or an interface.
   */
  @Override
  ...
  public boolean isEnum() {
    return (getFlags() & Flags.ENUM) != 0 && !env.legacyDoclet;
  }

  /**
   * Return true if this is an interface, but not an annotation type. Overridden
  ...
  /**
   * Return true if this class is included in the active set. A ClassDoc is
   * included iff either it is specified on the commandline, or if it's
   * containing package is specified on the command line, or if it is a member
   * class of an included class.
   */
  ...
  public PackageDoc containingPackage() {
    ...
    if (p.setDocPath == false) {
      FileObject docPath;
      try {
        Location location = env.fileManager
            .hasLocation(StandardLocation.SOURCE_PATH) ? StandardLocation.SOURCE_PATH
            : StandardLocation.CLASS_PATH;

        docPath = env.fileManager.getFileForInput(location, p.qualifiedName(),
            "package.html");
      } catch (IOException e) {
        docPath = null;
      }
  ...
  /**
   * Return the class name without package qualifier - but with enclosing class
   * qualifier - as a String.
   * 
   * <pre>
   * Examples:
  ...
  /**
   * Return the qualified name and any type parameters. Each parameter is a type
   * variable with optional bounds.
   */
  @Override
  ...
  /**
   * Return the class name as a string. If "full" is true the name is qualified,
   * otherwise it is qualified by its enclosing class(es) only.
   */
  ...
  /**
   * Return the class name with any type parameters as a string. Each parameter
   * is a type variable with optional bounds. If "full" is true all names are
   * qualified, otherwise they are qualified by their enclosing class(es) only.
   */
  static String classToString(DocEnv env, ClassSymbol c, boolean full) {
    ...
    if (!c.isInner()) { // if c is not an inner class
      ...
      s.append(classToString(env, encl, full)).append('.').append(c.name);
    }
  ...
  /**
   * Is this class (or any enclosing class) generic? That is, does it have type
   * parameters?
   */
  ...
  public ParamTag[] typeParamTags() {
    return (env.legacyDoclet) ? new ParamTag[0] : comment().typeParamTags();
  }

  /**
   * Return the modifier string for this class. If it's an interface exclude
   * 'abstract' keyword from the modifier string
   */
  @Override
  ...
  public int modifierSpecifier() {
    ...
    return (isInterface() || isAnnotationType()) ? modifiers
        & ~Modifier.ABSTRACT : modifiers;
  }

  /**
   * Return the superclass of this class
   * 
   * @return the ClassDocImpl for the superclass of this class, null if there is
   *         no superclass.
   */
   ...
   * superclass is represented by either a <code>ClassDoc</code> or a
  ...
  public com.sun.javadoc.Type superclassType() {
    ...
    return TypeMaker.getType(env, (sup != type) ? sup : env.syms.objectType);
  }
  ...
  /**
   * Return interfaces implemented by this class or interfaces extended by this
   * interface.
   * 
   * @return An array of ClassDocImpl representing the interfaces. Return an
   *         empty array if there are no interfaces.
   */
  ...
  /**
   * Return interfaces implemented by this class or interfaces extended by this
   * interface. Includes only directly-declared interfaces, not inherited
   ...
   * 
   * @param filter
   *          include only the included fields if filter==true
   */
   ...
   * 
   * @param filter
   *          if true, return only the included fields
   * @param enumConstants
  ...
  private FieldDoc[] fields(boolean filter, boolean enumConstants) {
    ...
    for (Scope.Entry e = tsym.members().elems; e != null; e = e.sibling) {
      if (e.sym != null && e.sym.kind == VAR) {
        ...
        boolean isEnum = ((s.flags() & Flags.ENUM) != 0) && !env.legacyDoclet;
        if (isEnum == enumConstants && (!filter || env.shouldDocument(s))) {
  ...
  /**
   * Return methods in class. This method is overridden by
   * AnnotationTypeDocImpl.
   * 
   * @param filter
   *          include only the included methods if filter==true
   * @return an array of MethodDocImpl for representing the visible methods in
   *         this class. Does not include constructors.
   */
   ...
   * 
   * @param filter
   *          include only the included constructors if filter==true
  ...
  /**
   * Adds all inner classes of this class, and their inner classes recursively,
   * to the list l.
   */
  void addAllClasses(ListBuffer<ClassDocImpl> l, boolean filtered) {
    try {
      ...
      if (!JavadocTool.isValidClassName(tsym.name.toString()))
        return;
   ...
   * 
   * @param filter
   *          include only the included inner classes if filter==true.
   * @return an array of ClassDocImpl for representing the visible classes
   *         defined in this class. Anonymous and local classes are not
   *         included.
   */
  ...
  /**
   * Find a class within the context of this class. Search order: qualified
   * name, in this class (inner), in this package, in the class imports, in the
   * package imports. Return the ClassDocImpl if found, null if not found.
   */
  ...
  public ClassDoc findClass(String className) {
    ...
    if (searchResult == null) {
      ...
      searchResult = enclosingClass == null ? null : enclosingClass
  ...
  private ClassDoc searchClass(String className) {
    ...
    for (int i = 0; i < innerClasses.length; i++) {
      ...
      // ### 'name' of the inner class, rather than the true simple name.
        ...
        ClassDoc innercd = ((ClassDocImpl) innerClasses[i])
            .searchClass(className);
        if (innercd != null) {
          return innercd;
        }
    ...
    if (tsym.sourcefile != null) {
      ...
      for (Scope.Entry e = s.lookup(names.fromString(className)); e.scope != null; e = e
          .next()) {
  ...
  private boolean hasTypeName(Type t, String name) {
    return name.equals(TypeMaker.getTypeName(t, true))
        ...
        || (qualifiedName() + "." + name)
            .equals(TypeMaker.getTypeName(t, true));
  }

  /**
   * Find a method in this class scope. Search order: this class, interfaces,
   * superclasses, outerclasses. Note that this is not necessarily what the
   * compiler would do!
   * 
   * @param methodName
   *          the unqualified name to search for.
   * @param paramTypeArray
   *          the array of Strings for method parameter types.
  ...
  public ConstructorDoc findConstructor(String constrName, String[] paramTypes) {
    Names names = tsym.name.table.names;
    for (Scope.Entry e = tsym.members().lookup(names.fromString("<init>")); e.scope != null; e = e
        .next()) {
      if (e.sym.kind == Kinds.MTH) {
        if (hasParameterTypes((MethodSymbol) e.sym, paramTypes)) {
  ...
  /**
   * Find a field in this class scope. Search order: this class, outerclasses,
   * interfaces, superclasses. IMP: If see tag is defined in an inner class,
   * which extends a super class and if outerclass and the super class have a
   * visible field in common then Java compiler cribs about the ambiguity, but
   * the following code will search in the above given search order.
   * 
   * @param fieldName
   *          the unqualified name to search for.
  ...
  private FieldDocImpl searchField(String fieldName, Set<ClassDocImpl> searched) {
    ...
    for (Scope.Entry e = tsym.members().lookup(names.fromString(fieldName)); e.scope != null; e = e
        .next()) {
   ...
   * "single-type-import declarations" in the JLS. This method is deprecated in
   * the ClassDoc interface.
   * 
   * @return an array of ClassDocImpl representing the imported classes.
   * 
   * @deprecated Import declarations are implementation details that should not
   *             be exposed here. In addition, not all imported classes are
   *             imported through single-type-import declarations.
   */
  @Deprecated
   ...
   * "type-import-on-demand declarations" in the JLS. This method is deprecated
   * in the ClassDoc interface.
   * 
   * @return an array of PackageDocImpl representing the imported packages.
   * 
   *         ###NOTE: the syntax supports importing all inner classes from a
   *         class as well.
   * @deprecated Import declarations are implementation details that should not
   *             be exposed here. In addition, this method's return type does
   *             not allow for all type-import-on-demand declarations to be
   *             returned.
   */
  @Deprecated
  public PackageDoc[] importedPackages() {
    ...
    Names names = tsym.name.table.names;
    importedPackages.append(env.getPackageDoc(env.reader
        .enterPackage(names.java_lang)));
    ...
    return importedPackages.toArray(new PackageDocImpl[importedPackages
        .length()]);
  }

  /**
   * Return the type's dimension information. Always return "", as this is not
   * an array type.
   */
  ...
  /**
   * Return true if this class implements <code>java.io.Externalizable</code>.
   */
   ...
   * 
   * @return an array of <code>MethodDocImpl</code> that represents the
   ...
   * 
   * @returns an array of <code>FieldDoc</code> for the Serializable fields of
   *          this class.
   * 
  ...
  /**
   * Return true if Serializable fields are explicitly defined with the special
   * class member <code>serialPersistentFields</code>.
   * 
  ...
  public SourcePosition position() {
    ...
    return SourcePositionImpl.make(tsym.sourcefile,
        (tree == null) ? Position.NOPOS : tree.pos, lineMap);
  }
}
