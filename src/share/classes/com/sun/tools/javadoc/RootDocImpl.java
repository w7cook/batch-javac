...
/**
 * This class holds the information from one run of javadoc. Particularly the
...
public class RootDocImpl extends DocImpl implements RootDoc {
   ...
   * 
   * @param env
   *          the documentation environment, state for this javadoc run
   * @param classes
   *          list of classes specified on the commandline
   * @param packages
   *          list of package names specified on the commandline
   * @param options
   *          list of options
   */
  public RootDocImpl(DocEnv env, List<JCClassDecl> classes,
   ...
   *          the documentation environment, state for this javadoc run
   * @param classes
   *          list of class names specified on the commandline
   * @param options
   *          list of options
   */
  ...
  private void setClasses(DocEnv env, List<JCClassDecl> classes) {
    ...
    for (JCClassDecl def : classes) {
      ...
      if (env.shouldDocument(def.sym)) {
        ...
        // System.out.println(" (env.shouldDocument() returned false)");//DEBUG
    }
  ...
  public ClassDoc[] specifiedClasses() {
    ...
    return (ClassDoc[]) classesToDocument
        .toArray(new ClassDocImpl[classesToDocument.length()]);
  }

  /**
   * Return all classes and interfaces (including those inside packages) to be
   * documented.
   */
  public ClassDoc[] classes() {
    ...
    return classesToDocument.toArray(new ClassDocImpl[classesToDocument
        .length()]);
  }
   ...
   * 
   * @param qualifiedName
   *          qualified class name (i.e. includes package name).
   * 
   * @return a ClassDocImpl holding the specified class, null if this class is
   *         not referenced.
   */
   ...
   *          package name
   * 
   * @return a PackageDoc holding the specified package, null if this package is
   *         not referenced.
   */
  ...
  /**
   * Return true if this Doc is include in the active set. RootDocImpl isn't
   * even a program entity so it is always false.
   */
  ...
  protected String documentation() {
    if (documentation == null) {
      ...
      if (overviewPath == null) {
        ...
        // read from file
        try {
          documentation = readHTMLDocumentation(overviewPath.openInputStream(),
              overviewPath);
  ...
  public SourcePosition position() {
    JavaFileObject path;
    return ((path = getOverviewPath()) == null) ? null : SourcePositionImpl
        .make(path, Position.NOPOS, null);
  }
