 ...
 * "1.2"). TagImpls with structure or which require special processing are
 * handled by subclasses (ParamTagImpl, SeeTagImpl, and ThrowsTagImpl
 * 
 * @author Robert Field
...
class TagImpl implements Tag {
   ...
   * TagImpls consisting of SeeTagImpl(s) and text containing TagImpl(s). Within
   * a comment string "This is an example of inline tags for a documentation
   * comment {@link Doc {@link Doc commentlabel} ", where inside the inner
   * braces, the first "Doc" carries exctly the same syntax as a SeeTagImpl and
   * the second "commentlabel" is label for the Html Link, will return an array
   * of TagImpl(s) with first element as TagImpl with comment text "This is an
   * example of inline tags for a documentation
