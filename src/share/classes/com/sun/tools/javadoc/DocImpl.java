 ...
 * java language constructs (class, package, method,...) which have comments and
 * have been processed by this run of javadoc. All Doc items are unique, that
...
public abstract class DocImpl implements Doc, Comparable<Object> {
   ...
   * Doc environment
   */
  protected final DocEnv env; // ### Rename this everywhere to
                              // 'docenv' ?
   ...
   */
  protected String documentation; // Accessed in PackageDocImpl,
                                  // RootDocImpl
  ...
  /**
   * So subclasses have the option to do lazy initialization of "documentation"
   * string.
   */
  ...
  /**
   * Return the text of the comment for this doc item. TagImpls have been
   * removed.
   */
   ...
   * @return an array of TagImpl containing all tags whose 'kind()' matches
   *         'tagname'.
   */
   ...
   */
  String readHTMLDocumentation(InputStream input, FileObject filename)
      throws IOException {
    ...
    if (m.matches()) {
      ...
      String key = rawDoc.matches("(?is).*<body\\b.*") ? "javadoc.End_body_missing_from_html_file"
  ...
  public int compareTo(Object obj) {
    // System.out.println("COMPARE \"" + this + "\" to \"" + obj + "\" = " +
    // key().compareTo(((DocImpl)obj).key()));
   ...
   * element)? False until overridden.
   * 
  ...
  /**
   * Is this Doc item a interface (but not an annotation type)? False until
   * overridden.
   * 
  ...
  /**
   * Is this Doc item an ordinary class (i.e. not an interface, annotation type,
   * enumeration, exception, or error)? False until overridden.
   * 
  ...
  /**
   * Is this Doc item a class (and not an interface or annotation type)? This
   * includes ordinary classes, enums, errors and exceptions. False until
   * overridden.
   * 
