...
/**
 * Implementation of <code>WildcardType</code>, which represents a wildcard
 * type.
 * 
...
public class WildcardTypeImpl extends AbstractTypeImpl implements WildcardType {
   ...
   * <i>super</i> clause. Return an empty array if no such bounds are explicitly
   * given.
   */
  ...
  /**
   * Return the string form of a wildcard type ("?") along with any "extends" or
   * "super" clause. Delimiting brackets are not included. Class names are
   * qualified if "full" is true.
   */
  static String wildcardTypeToString(DocEnv env, Type.WildcardType wildThing,
      boolean full) {
    if (env.legacyDoclet) {
  ...
  private static List<Type> getExtendsBounds(Type.WildcardType wild) {
    return wild.isSuperBound() ? List.<Type> nil() : List.of(wild.type);
  }

  private static List<Type> getSuperBounds(Type.WildcardType wild) {
    return wild.isExtendsBound() ? List.<Type> nil() : List.of(wild.type);
  }
}
