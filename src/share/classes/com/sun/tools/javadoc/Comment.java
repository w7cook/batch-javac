...
class Comment {
  ...
  Comment(final DocImpl holder, final String commentString) {
    ...
    /**
     * Separate the comment into the text part and zero to N tags. Simple state
  ...
  /**
   * Return param tags in this comment. If typeParams is true include only type
   * param tags, otherwise include only ordinary param tags.
   */
  ...
  static Tag[] getInlineTags(DocImpl holder, String inlinetext) {
    ...
    while (true) {
      ...
      if ((linkstart = inlineTagFound(holder, inlinetext, textstart)) == -1) {
        taglist.append(new TagImpl(holder, "Text", inlinetext
            .substring(textstart)));
        break;
      } else {
        ...
        while (Character.isWhitespace(inlinetext.charAt(seetextstart))) {
          if (inlinetext.length() <= seetextstart) {
            taglist.append(new TagImpl(holder, "Text", inlinetext.substring(
        ...
        if ((delimend = findInlineTagDelim(inlinetext, textstart)) == -1) {
          ...
          docenv.warning(holder,
              "tag.End_delimiter_missing_for_possible_SeeTag", inlinetext);
          return taglist.toArray(new Tag[taglist.length()]);
        } else {
          // Found closing '}' character.
          if (linkName.equals("see") || linkName.equals("link")
              || linkName.equals("linkplain")) {
            taglist.append(new SeeTagImpl(holder, "@" + linkName, inlinetext
                .substring(textstart, delimend)));
          } else {
            taglist.append(new TagImpl(holder, "@" + linkName, inlinetext
                .substring(textstart, delimend)));
          }
   ...
   * @param searchStart
   *          the index of the place to start searching at.
  ...
  private static int findInlineTagDelim(String inlineText, int searchStart) {
    ...
    if ((delimEnd = inlineText.indexOf("}", searchStart)) == -1) {
      ...
      return (nestedCloseBrace != -1) ? findInlineTagDelim(inlineText,
          nestedCloseBrace + 1) : -1;
    } else {
      return delimEnd;
    }
  }
   ...
   * and white space, if found return the index of the text following the white
   * space. else return -1.
   */
