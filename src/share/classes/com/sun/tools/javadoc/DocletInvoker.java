...
public class DocletInvoker {
  ...
  public DocletInvoker(Messager messager, String docletClassName,
      String docletPath, ClassLoader docletParentClassLoader) {
  ...
  /*
   * Returns the delegation class loader to use when creating appClassLoader
   * (used to load the doclet). The context class loader is the best choice, but
   * legacy behavior was to use the default delegation class loader (aka system
   * class loader).
   * 
   * Here we favor using the context class loader. To ensure compatibility with
   * existing apps, we revert to legacy behavior if either or both of the
   * following conditions hold:
   * 
   * 1) the doclet is loadable from the system class loader but not from the
   * context class loader,
   * 
   * 2) this.getClass() is loadable from the system class loader but not from
  ...
  /**
   * Check for doclet added options here. Zero return means option not known.
   * Positive value indicates number of arguments to option. Negative value
  ...
  /**
   * Let doclet check that all options are OK. Returning true means options are
   * OK. If method does not exist, assume true.
   */
  ...
  /**
   * Return the language version supported by this doclet. If the method does
   * not exist in the doclet, assume version 1.1.
   */
  ...
  private Object invoke(String methodName, Object returnValueIfNonExistent,
    ...
    try {
      ...
      if (returnValueIfNonExistent == null) {
        messager.error(null, "main.doclet_method_not_found", docletClassName,
            methodName);
